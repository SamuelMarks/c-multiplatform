...
typedef struct M3NetworkConfig {
    ...
    const M3Allocator *allocator; /**< Allocator used for response storage (NULL uses default). */
} M3NetworkConfig;
 ...
 * @return M3_OK on success or a failure code.
 */
M3_API int M3_CALL m3_network_init(M3NetworkClient *client, const M3NetworkConfig *config);
 ...
 * @param out_response Receives the response data (must be freed with m3_network_response_free).
 * @return M3_OK on success or a failure code.
 */
M3_API int M3_CALL m3_network_request(M3NetworkClient *client, const M3NetworkRequest *request, M3NetworkResponse *out_response);
 ...
 * @return M3_OK on success or a failure code.
 */
M3_API int M3_CALL m3_network_response_free(M3NetworkClient *client, M3NetworkResponse *response);
 ...
 */
M3_API int M3_CALL m3_network_copy_response_body(const M3NetworkResponse *response, void *dst, m3_usize dst_capacity,
    m3_usize *out_size);
