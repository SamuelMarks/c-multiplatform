...
/**
 * @file m3_progress.h
 * @brief Progress indicators and slider widgets for LibCMPC.
 */
...
typedef struct M3LinearProgressStyle {
  CMPScalar height;              /**< Track height in pixels (> 0). */
  CMPScalar min_width;           /**< Minimum width in pixels (>= 0). */
  CMPScalar corner_radius;       /**< Track corner radius in pixels (>= 0). */
  CMPColor track_color;          /**< Track background color. */
  CMPColor indicator_color;      /**< Filled indicator color. */
  CMPColor disabled_track_color; /**< Track color when disabled. */
  CMPColor disabled_indicator_color; /**< Indicator color when disabled. */
 ...
 * @return CMP_OK on success or a failure code.
 */
CMP_API int CMP_CALL m3_linear_progress_get_value(
    const M3LinearProgress *progress, CMPScalar *out_value);
 ...
 * @return CMP_OK on success or a failure code.
 */
CMP_API int CMP_CALL m3_linear_progress_set_style(
...
typedef struct M3CircularProgressStyle {
  CMPScalar diameter;            /**< Indicator diameter in pixels (> 0). */
  CMPScalar thickness;           /**< Stroke thickness in pixels (> 0). */
  CMPScalar start_angle;         /**< Start angle in radians. */
  cmp_u32 segments;              /**< Segment count for approximation (>= 3). */
  CMPColor track_color;          /**< Track background color. */
  CMPColor indicator_color;      /**< Indicator arc color. */
  CMPColor disabled_track_color; /**< Track color when disabled. */
  CMPColor disabled_indicator_color; /**< Indicator color when disabled. */
 ...
 * @return CMP_OK on success or a failure code.
 */
CMP_API int CMP_CALL m3_circular_progress_get_value(
    const M3CircularProgress *progress, CMPScalar *out_value);
 ...
 * @return CMP_OK on success or a failure code.
 */
CMP_API int CMP_CALL m3_circular_progress_set_style(
    M3CircularProgress *progress, const M3CircularProgressStyle *style);
...
typedef struct M3SliderStyle {
  ...
  CMPBool
      is_media_slider; /**< CMP_TRUE for expressive morphing media slider. */
  CMPScalar active_track_height; /**< Height of active track when dragging media
                                    slider. */
 ...
 * @return CMP_OK on success or a failure code.
 */
CMP_API int CMP_CALL m3_slider_set_value(M3Slider *slider, CMPScalar value);
 ...
 * @return CMP_OK on success or a failure code.
 */
CMP_API int CMP_CALL m3_slider_set_step(M3Slider *slider, CMPScalar step);
 ...
 * @return CMP_OK on success or a failure code.
 */
CMP_API int CMP_CALL m3_progress_test_set_fail_point(cmp_u32 fail_point);

/**
 * @brief Clear progress test fail points.
 * @return CMP_OK on success or a failure code.
 */
CMP_API int CMP_CALL m3_progress_test_clear_fail_points(void);
 ...
 */
CMP_API int CMP_CALL
m3_progress_test_set_color_alpha_fail_after(cmp_u32 call_count);
 ...
 * @return CMP_OK on success or a failure code.
 */
CMP_API int CMP_CALL m3_progress_test_validate_color(const CMPColor *color);
 ...
 * @return CMP_OK on success or a failure code.
 */
CMP_API int CMP_CALL m3_progress_test_validate_rect(const CMPRect *rect);
 ...
 * @return CMP_OK on success or a failure code.
 */
CMP_API int CMP_CALL m3_progress_test_validate_value01(CMPScalar value);
 ...
 * @return CMP_OK on success or a failure code.
 */
CMP_API int CMP_CALL m3_progress_test_linear_resolve_colors(
 ...
 * @return CMP_OK on success or a failure code.
 */
CMP_API int CMP_CALL m3_progress_test_circular_resolve_colors(
 ...
 * @return CMP_OK on success or a failure code.
 */
CMP_API int CMP_CALL m3_progress_test_slider_value_to_fraction(
 ...
 * @return CMP_OK on success or a failure code.
 */
CMP_API int CMP_CALL m3_progress_test_slider_resolve_colors(
    const M3Slider *slider, CMPColor *out_track, CMPColor *out_active,
    CMPColor *out_thumb);
 ...
 * @param notify CMP_TRUE to invoke the change callback.
 * @return CMP_OK on success or a failure code.
 */
