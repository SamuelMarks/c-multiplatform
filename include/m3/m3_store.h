 ...
 */
typedef int(M3_CALL *M3ReducerFn)(void *ctx, const M3Action *action,
...
typedef struct M3StoreConfig {
  const M3Allocator *allocator; /**< Allocator for store memory; NULL uses
                                   default allocator. */
 ...
 * @return M3_OK on success or a failure code.
 */
M3_API int M3_CALL m3_store_init(M3Store *store, const M3StoreConfig *config,
 ...
 * @return M3_OK on success or a failure code.
 */
M3_API int M3_CALL m3_store_get_state_ptr(const M3Store *store,
 ...
 * @return M3_OK on success or a failure code.
 */
M3_API int M3_CALL m3_store_get_state(const M3Store *store, void *out_state,
                                      m3_usize state_size);
 ...
 * @return M3_OK on success or a failure code.
 */
M3_API int M3_CALL m3_store_can_undo(const M3Store *store,
                                     M3Bool *out_can_undo);
 ...
 * @return M3_OK on success or a failure code.
 */
M3_API int M3_CALL m3_store_can_redo(const M3Store *store,
                                     M3Bool *out_can_redo);

/**
 * @brief Undo the most recent state transition.
 * @param store Store instance.
 * @return M3_OK on success, M3_ERR_NOT_FOUND if no undo is available, or a
 * failure code.
 */
 ...
 * @param store Store instance.
 * @return M3_OK on success, M3_ERR_NOT_FOUND if no redo is available, or a
 * failure code.
 */
 ...
 * @return M3_OK on success or a failure code.
 */
M3_API int M3_CALL m3_store_get_undo_count(const M3Store *store,
                                           m3_usize *out_count);
 ...
 * @return M3_OK on success or a failure code.
 */
M3_API int M3_CALL m3_store_get_redo_count(const M3Store *store,
                                           m3_usize *out_count);
 ...
 * @param out_state Destination buffer.
 * @param state_size Size of the destination buffer in bytes.
 * @return M3_OK on success, M3_ERR_NOT_FOUND if the index is out of range, or a
 * failure code.
 */
M3_API int M3_CALL m3_store_copy_undo_state(const M3Store *store,
 ...
 * failure code.
 */
M3_API int M3_CALL m3_store_copy_redo_state(const M3Store *store,
                                            m3_usize index_from_latest,
                                            void *out_state,
                                            m3_usize state_size);
 ...
 * @return M3_OK on success or a failure code.
 */
M3_API int M3_CALL m3_store_test_mul_overflow(m3_usize a, m3_usize b,
                                              m3_usize *out_value);
 ...
 * @return M3_OK on success or a failure code.
 */
M3_API int M3_CALL m3_store_test_history_push(m3_u8 *buffer, m3_usize capacity,
                                              m3_usize state_size,
                                              m3_usize *io_count,
                                              const void *state);
 ...
 * @return M3_OK on success or a failure code.
 */
M3_API int M3_CALL m3_store_test_history_pop(const m3_u8 *buffer,
                                             m3_usize capacity,
                                             m3_usize state_size,
                                             m3_usize *io_count,
                                             void *out_state);
 ...
 * @return M3_OK on success or a failure code.
 */
M3_API int M3_CALL m3_store_test_copy_history(
    const m3_u8 *buffer, m3_usize capacity, m3_usize state_size, m3_usize count,
