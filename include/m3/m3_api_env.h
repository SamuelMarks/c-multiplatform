...
typedef struct M3IOVTable {
     ...
     */
    int (M3_CALL *read_file)(void *io, const char *utf8_path, void *buffer, m3_usize buffer_size, m3_usize *out_read);
    /**
     ...
     * @return M3_OK on success or a failure code.
     */
    int (M3_CALL *read_file_alloc)(void *io, const char *utf8_path, const M3Allocator *allocator, void **out_data, m3_usize *out_size);
    /**
     ...
     * @return M3_OK on success or a failure code.
     */
    int (M3_CALL *write_file)(void *io, const char *utf8_path, const void *data, m3_usize size, M3Bool overwrite);
    /**
     ...
     * @return M3_OK on success or a failure code.
     */
    int (M3_CALL *file_exists)(void *io, const char *utf8_path, M3Bool *out_exists);
    /**
     * @brief Delete a file.
     * @param io IO backend instance.
     * @param utf8_path File path in UTF-8.
     ...
     * @return M3_OK on success or a failure code.
     */
    int (M3_CALL *stat_file)(void *io, const char *utf8_path, M3FileInfo *out_info);
} M3IOVTable;
...
typedef struct M3SensorsVTable {
     ...
     * @return M3_OK on success or a failure code.
     */
    int (M3_CALL *read)(void *sensors, m3_u32 type, M3SensorReading *out_reading, M3Bool *out_has_reading);
} M3SensorsVTable;
...
typedef struct M3CameraVTable {
     ...
     * @return M3_OK on success or a failure code.
     */
    int (M3_CALL *read_frame)(void *camera, M3CameraFrame *out_frame, M3Bool *out_has_frame);
} M3CameraVTable;
...
typedef struct M3NetworkVTable {
     ...
     * @return M3_OK on success or a failure code.
     */
    int (M3_CALL *request)(void *net, const M3NetworkRequest *request, const M3Allocator *allocator, M3NetworkResponse *out_response);
    /**
     ...
     * @return M3_OK on success or a failure code.
     */
    int (M3_CALL *free_response)(void *net, const M3Allocator *allocator, M3NetworkResponse *response);
} M3NetworkVTable;
...
typedef struct M3TasksVTable {
     ...
     * @return M3_OK on success or a failure code.
     */
    int (M3_CALL *thread_create)(void *tasks, M3ThreadFn entry, void *user, M3Handle *out_thread);
    /**
     * @brief Join a thread.
     * @param tasks Task backend instance.
     ...
     * @return M3_OK on success or a failure code.
     */
    int (M3_CALL *task_post_delayed)(void *tasks, M3TaskFn fn, void *user, m3_u32 delay_ms);
} M3TasksVTable;
