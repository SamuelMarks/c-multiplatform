...
#define M3_DATE_PICKER_INVALID_INDEX ((cmp_usize) ~(cmp_usize)0)
...
typedef struct M3DateRange {
  CMPDate start;     /**< Range start date. */
  CMPDate end;       /**< Range end date. */
  CMPBool has_start; /**< CMP_TRUE when start is set. */
  CMPBool has_end;   /**< CMP_TRUE when end is set. */
} M3DateRange;
...
typedef struct M3DatePickerCell {
  CMPDate date;   /**< Cell date. */
  cmp_u32 flags;  /**< Cell flags (CMP_DATE_CELL_FLAG_*). */
  CMPRect bounds; /**< Cell bounds in pixels. */
} M3DatePickerCell;
...
typedef struct M3DatePickerStyle {
  CMPLayoutEdges padding;       /**< Padding around the picker grid. */
  CMPScalar cell_width;         /**< Cell width in pixels (> 0). */
  CMPScalar cell_height;        /**< Cell height in pixels (> 0). */
  CMPScalar cell_spacing_x;     /**< Horizontal spacing between cells (>= 0). */
  CMPScalar cell_spacing_y;     /**< Vertical spacing between cells (>= 0). */
  CMPScalar header_height;      /**< Header height in pixels (>= 0). */
  CMPScalar weekday_height;     /**< Weekday row height in pixels (>= 0). */
  CMPScalar cell_corner_radius; /**< Cell corner radius in pixels (>= 0). */
  CMPColor background_color;    /**< Background color. */
  CMPColor cell_color;          /**< Default cell color. */
  CMPColor cell_range_color;    /**< In-range cell color. */
  CMPColor cell_selected_color; /**< Selected cell color. */
  CMPColor cell_disabled_color; /**< Disabled/out-of-range cell color. */
} M3DatePickerStyle;
 ...
 * @return CMP_OK on success or a failure code.
 */
CMP_API int CMP_CALL m3_date_picker_style_init(M3DatePickerStyle *style);
 ...
 * @return CMP_OK on success or a failure code.
 */
CMP_API int CMP_CALL m3_date_picker_next_month(M3DatePicker *picker);
 ...
 * @return CMP_OK on success or a failure code.
 */
CMP_API int CMP_CALL m3_date_picker_prev_month(M3DatePicker *picker);
 ...
 * @return CMP_OK on success or a failure code.
 */
CMP_API int CMP_CALL m3_date_picker_set_constraints(
 ...
 * @return CMP_OK on success or a failure code.
 */
CMP_API int CMP_CALL m3_date_picker_update(M3DatePicker *picker);
 ...
 * @return CMP_OK on success or a failure code.
 */
CMP_API int CMP_CALL m3_date_picker_test_validate_color(const CMPColor *color);
/**
...
m3_date_picker_test_validate_edges(const CMPLayoutEdges *edges);
/**
 ...
 */
CMP_API int CMP_CALL
m3_date_picker_test_validate_measure_spec(CMPMeasureSpec spec);
/**
 * @brief Test hook for validating rectangles.
 * @param rect Rectangle to validate.
 * @return CMP_OK on success or a failure code.
 */
CMP_API int CMP_CALL m3_date_picker_test_validate_rect(const CMPRect *rect);
/**
 * @brief Test hook for validating dates.
 * @param date Date to validate.
 * @return CMP_OK on success or a failure code.
 */
CMP_API int CMP_CALL m3_date_picker_test_validate_date(const CMPDate *date);
/**
 ...
 * @param out_in_range Receives CMP_TRUE if the date is in range.
 * @return CMP_OK on success or a failure code.
 */
 ...
 * @return CMP_OK on success or a failure code.
 */
CMP_API int CMP_CALL m3_date_picker_test_compute_cell_bounds(
    const M3DatePicker *picker, cmp_u32 row, cmp_u32 col, CMPRect *out_bounds);
/**
 ...
 * @return CMP_OK on success or a failure code.
 */
CMP_API int CMP_CALL m3_date_picker_test_update_grid(M3DatePicker *picker);
#endif
