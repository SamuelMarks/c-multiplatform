/**
 * @file ui_file_uploader_base.h
 * @brief Base component logic for file uploaders and dropzones.
 *
 * This header defines structures, state definitions, and functions required
 * for implementing a drag-and-drop enabled file uploader UI component.
 */
...
enum ui_file_uploader_state {
  ...
  UI_FILE_UPLOADER_STATE_COMPLETE =
      3,                           /**< Files have been read successfully. */
...
struct ui_file_uploader_base {
  ...
  struct ui_drag_list dropzone_list; /**< Internal drag list for hooking into
                                        drag and drop system. */

  /* CVA Integration */
  /** \brief Callback for when the file list changes. */
  ui_error_t (*on_change_cb)(union ui_signal_payload new_value,
                             void *user_data);
  void *on_change_user_data; /**< Opaque user data for the change callback. */

  /** \brief Callback for when the uploader is interacted with. */
  ui_error_t (*on_touched_cb)(void *user_data);
  void *on_touched_user_data; /**< Opaque user data for the touched callback. */

  ui_bool_t is_disabled; /**< 1 (`UI_TRUE`) if the uploader is disabled, 0
                            (`UI_FALSE`) otherwise. */
};
 ...
 * @param out_cva Pointer to output the initialized control value accessor.
 * @return `UI_ERROR_NONE` on success, or an appropriate error code.
 */
extern C_MULTIPLATFORM_EXPORT ui_error_t ui_file_uploader_init(
    struct ui_file_uploader_base *uploader, int max_files, int x, int y,
    int width, int height, struct ui_control_value_accessor *out_cva);

/**
 * @brief Destroys a file uploader component, freeing all resources.
 *
 * @param uploader Pointer to the uploader structure to destroy.
 * @return `UI_ERROR_NONE` on success, or an appropriate error code.
 */
extern C_MULTIPLATFORM_EXPORT ui_error_t
ui_file_uploader_destroy(struct ui_file_uploader_base *uploader);
 ...
 * reached.
 */
extern C_MULTIPLATFORM_EXPORT ui_error_t ui_file_uploader_drop_file(
    struct ui_file_uploader_base *uploader, const char *file_path);
