...
typedef struct CupertinoBlurEffect {
    ...
    CMPColor base_color;   /**< Base solid color underneath the blur (for fallbacks or reduced transparency). */
} CupertinoBlurEffect;

/**
 * @brief Retrieves the blur configuration for a specific style, adapting to light/dark mode.
 ...
 */
CMP_API int CMP_CALL cupertino_blur_get_effect(CupertinoBlurStyle style_id, CMPBool is_dark, CupertinoBlurEffect *out_effect);
