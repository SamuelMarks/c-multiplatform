 ...
 */
typedef enum ui_error (*ui_virtual_scroll_create_node_fn)(
    size_t index, struct ui_dom_node **out_node, void *user_data);

/**
 * @brief Callback to update an existing DOM node for a new item.
 */
typedef enum ui_error (*ui_virtual_scroll_update_node_fn)(
    size_t index, struct ui_dom_node *node, void *user_data);
