...
static __inline int cmp_theme_measure_button(const cmp_ui_node_t *node,
...
static __inline int cmp_theme_measure_text_input(const cmp_ui_node_t *node,
                                                 float *width, float *height) {
  const cmp_theme_vtable_t *vt = NULL;
  int rc;
  rc = cmp_resolve_vtable(node, &vt);
...
static __inline int cmp_theme_measure_slider(const cmp_ui_node_t *node,
                                             float *width, float *height) {
  const cmp_theme_vtable_t *vt = NULL;
  int rc;
  rc = cmp_resolve_vtable(node, &vt);
...
static __inline int cmp_theme_measure_toggle(const cmp_ui_node_t *node,
                                             float *width, float *height) {
  const cmp_theme_vtable_t *vt = NULL;
  int rc;
  rc = cmp_resolve_vtable(node, &vt);
...
static __inline int cmp_theme_measure_checkbox(const cmp_ui_node_t *node,
                                               float *width, float *height) {
  const cmp_theme_vtable_t *vt = NULL;
  int rc;
  rc = cmp_resolve_vtable(node, &vt);
...
static __inline int cmp_theme_measure_radio(const cmp_ui_node_t *node,
                                            float *width, float *height) {
  const cmp_theme_vtable_t *vt = NULL;
  int rc;
  rc = cmp_resolve_vtable(node, &vt);
...
static __inline int cmp_theme_measure_progress(const cmp_ui_node_t *node,
                                               float *width, float *height) {
  const cmp_theme_vtable_t *vt = NULL;
  int rc;
  rc = cmp_resolve_vtable(node, &vt);
...
static __inline int cmp_theme_measure_dropdown(const cmp_ui_node_t *node,
                                               float *width, float *height) {
  const cmp_theme_vtable_t *vt = NULL;
  int rc;
  rc = cmp_resolve_vtable(node, &vt);
...
static __inline int cmp_theme_layout_nav_bar(cmp_ui_node_t *node) {
  const cmp_theme_vtable_t *vt = NULL;
  int rc;
  rc = cmp_resolve_vtable(node, &vt);
  if (rc != 0) {
...
static __inline int cmp_theme_layout_tab_bar(cmp_ui_node_t *node) {
  const cmp_theme_vtable_t *vt = NULL;
  int rc;
  rc = cmp_resolve_vtable(node, &vt);
  if (rc != 0) {
...
static __inline int cmp_theme_layout_dialog_content(cmp_ui_node_t *node) {
  const cmp_theme_vtable_t *vt = NULL;
  int rc;
  rc = cmp_resolve_vtable(node, &vt);
  if (rc != 0) {
...
static __inline int cmp_theme_layout_sidebar(cmp_ui_node_t *node) {
  const cmp_theme_vtable_t *vt = NULL;
  int rc;
  rc = cmp_resolve_vtable(node, &vt);
  if (rc != 0) {
...
static __inline int cmp_theme_draw_button(const cmp_ui_node_t *node) {
  const cmp_theme_vtable_t *vt = NULL;
  int rc;
  rc = cmp_resolve_vtable(node, &vt);
  if (rc != 0) {
...
static __inline int cmp_theme_draw_text_input(const cmp_ui_node_t *node) {
  const cmp_theme_vtable_t *vt = NULL;
  int rc;
  rc = cmp_resolve_vtable(node, &vt);
  if (rc != 0) {
...
static __inline int cmp_theme_draw_slider(const cmp_ui_node_t *node) {
  const cmp_theme_vtable_t *vt = NULL;
  int rc;
  rc = cmp_resolve_vtable(node, &vt);
  if (rc != 0) {
...
static __inline int cmp_theme_draw_toggle(const cmp_ui_node_t *node) {
  const cmp_theme_vtable_t *vt = NULL;
  int rc;
  rc = cmp_resolve_vtable(node, &vt);
  if (rc != 0) {
...
static __inline int cmp_theme_draw_checkbox(const cmp_ui_node_t *node) {
  const cmp_theme_vtable_t *vt = NULL;
  int rc;
  rc = cmp_resolve_vtable(node, &vt);
  if (rc != 0) {
...
static __inline int cmp_theme_draw_radio(const cmp_ui_node_t *node) {
  const cmp_theme_vtable_t *vt = NULL;
  int rc;
  rc = cmp_resolve_vtable(node, &vt);
  if (rc != 0) {
...
static __inline int cmp_theme_draw_progress(const cmp_ui_node_t *node) {
  const cmp_theme_vtable_t *vt = NULL;
  int rc;
  rc = cmp_resolve_vtable(node, &vt);
  if (rc != 0) {
...
static __inline int cmp_theme_draw_card(const cmp_ui_node_t *node) {
  const cmp_theme_vtable_t *vt = NULL;
  int rc;
  rc = cmp_resolve_vtable(node, &vt);
  if (rc != 0) {
...
static __inline int cmp_theme_draw_tooltip(const cmp_ui_node_t *node) {
  const cmp_theme_vtable_t *vt = NULL;
  int rc;
  rc = cmp_resolve_vtable(node, &vt);
  if (rc != 0) {
...
static __inline int cmp_theme_draw_menu(const cmp_ui_node_t *node) {
  const cmp_theme_vtable_t *vt = NULL;
  int rc;
  rc = cmp_resolve_vtable(node, &vt);
  if (rc != 0) {
...
static __inline int cmp_theme_draw_focus_ring(const cmp_ui_node_t *node) {
  const cmp_theme_vtable_t *vt = NULL;
  int rc;
  rc = cmp_resolve_vtable(node, &vt);
  if (rc != 0) {
...
static __inline int cmp_theme_get_spring_config(const cmp_ui_node_t *node,
...
static __inline int cmp_theme_get_ripple_config(const cmp_ui_node_t *node,
                                                float *duration,
                                                float *opacity) {
  const cmp_theme_vtable_t *vt = NULL;
  int rc;
...
static __inline int cmp_theme_get_state_layer_opacity(const cmp_ui_node_t *node,
                                                      int state,
                                                      float *opacity) {
  const cmp_theme_vtable_t *vt = NULL;
  int rc;
...
static __inline int cmp_theme_get_transition_duration(const cmp_ui_node_t *node,
                                                      float *duration) {
  const cmp_theme_vtable_t *vt = NULL;
  int rc;
  rc = cmp_resolve_vtable(node, &vt);
