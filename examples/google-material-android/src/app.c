...
static int build_header(cmp_ui_node_t *parent, const char *title) {
  ...
  (void)cmp_ui_box_create(&btn_theme);
  btn_theme->type = 3;
...
static int build_buttons_screen(cmp_ui_node_t *main_box) {
  ...
  (void)cmp_ui_box_create(&btn1);
  btn1->type = 3;
  btn1->layout->width = -1.0f; /* auto stretch */
  /* flex layout adjusted */
  btn1->layout->height = 2.5f * REM;
...
static int build_inputs_screen(cmp_ui_node_t *main_box) {
  ...
  /* flex layout adjusted */
  ti->layout->height = 3.5f * REM; /* M3 text field height */
  ti->layout->margin[2] = 1.0f * REM;

  (void)cmp_ui_node_add_child(main_box, cb);
  (void)cmp_ui_node_add_child(main_box, tg);
  (void)cmp_ui_node_add_child(main_box, sl);
  (void)cmp_ui_node_add_child(main_box, ti);
  if (rc != 0) {
    return rc;
  }
  return rc;
...
static int build_ui(void) {
  ...
  build_header(main_box, "Google Material Catalog");
...
int app_init(void) {
  ...
  config.title = "Google Material Android Component Gallery";
  config.width = 800;
  config.height = 600;
  build_ui();
