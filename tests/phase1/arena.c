...
int main(void)
{
    ...
    M3_TEST_EXPECT(m3_arena_init(&arena, &bad_alloc, 64), M3_ERR_INVALID_ARGUMENT);
    ...
    M3_TEST_EXPECT(m3_arena_alloc(&align_overflow_arena, 8, 8, &ptr), M3_ERR_OVERFLOW);
    ...
    block->offset = max_size;
    M3_TEST_EXPECT(m3_arena_get_stats(&stats_used_arena, &stats), M3_ERR_OVERFLOW);
    ...
    M3_TEST_EXPECT(m3_arena_init(&fail_arena, &test_allocator, 16), M3_ERR_OUT_OF_MEMORY);
