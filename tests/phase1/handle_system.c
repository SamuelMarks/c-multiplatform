...
static const M3ObjectVTable test_vtable = {test_retain, test_release,
...
static const M3ObjectVTable test_vtable_no_destroy = {test_retain, test_release,
                                                      NULL, test_get_type_id};

static int init_test_object(TestObject *obj, M3HandleSystem *sys,
                            m3_u32 type_id) {
  int rc;
...
static int M3_CALL fail_realloc(void *ctx, void *ptr, m3_usize size,
                                void **out_ptr) {
  M3_UNUSED(ctx);
  M3_UNUSED(ptr);
  M3_UNUSED(size);
...
static int M3_CALL ok_realloc(void *ctx, void *ptr, m3_usize size,
                              void **out_ptr) {
  void *mem;
...
int main(void) {
  ...
  M3_TEST_EXPECT(m3_object_header_init(NULL, 1, 0, &test_vtable),
                 M3_ERR_INVALID_ARGUMENT);
  M3_TEST_EXPECT(m3_object_header_init(&header, 1, 0, NULL),
                 M3_ERR_INVALID_ARGUMENT);
  M3_TEST_EXPECT(m3_object_header_init(&header, 1, 0, &test_vtable_no_release),
                 M3_ERR_INVALID_ARGUMENT);
  ...
  M3_TEST_EXPECT(m3_object_get_type_id(NULL, &type_id),
                 M3_ERR_INVALID_ARGUMENT);
  M3_TEST_EXPECT(m3_object_get_type_id(&obj.header, NULL),
                 M3_ERR_INVALID_ARGUMENT);
  M3_TEST_OK(m3_object_get_type_id(&obj.header, &type_id));
  M3_TEST_ASSERT(type_id == obj.header.type_id);

  M3_TEST_EXPECT(m3_handle_system_default_create(0, NULL, &sys),
                 M3_ERR_INVALID_ARGUMENT);
  M3_TEST_EXPECT(m3_handle_system_default_create(1, NULL, NULL),
                 M3_ERR_INVALID_ARGUMENT);
  M3_TEST_EXPECT(m3_handle_system_default_create(1, &bad_alloc, &sys),
                 M3_ERR_INVALID_ARGUMENT);
  M3_TEST_EXPECT(m3_handle_system_default_destroy(NULL),
                 M3_ERR_INVALID_ARGUMENT);

#ifdef M3_TESTING
  M3_TEST_OK(m3_core_test_set_default_allocator_fail(M3_TRUE));
  M3_TEST_EXPECT(m3_handle_system_default_create(1, NULL, &sys),
                 M3_ERR_UNKNOWN);
  ...
  M3_TEST_EXPECT(m3_handle_system_default_create(1, &fail_allocator, &fail_sys),
                 M3_ERR_OUT_OF_MEMORY);

  max_usize = (m3_usize) ~(m3_usize)0;
  M3_TEST_EXPECT(
      m3_handle_system_default_create(max_usize, NULL, &overflow_sys),
      M3_ERR_OVERFLOW);
  ...
  M3_TEST_EXPECT(sys.vtable->unregister_object(NULL, handle),
                 M3_ERR_INVALID_ARGUMENT);
  ...
  if ((m3_usize)max_u32 < max_usize) {
    M3_TEST_EXPECT(sys.vtable->init(&impl, (m3_usize)max_u32 + 1),
                   M3_ERR_RANGE);
  }
  ...
  handle.id = 999;
  handle.generation = 1;
  M3_TEST_EXPECT(sys.vtable->resolve(sys.ctx, handle, &resolved),
                 M3_ERR_NOT_FOUND);
  M3_TEST_EXPECT(sys.vtable->unregister_object(sys.ctx, handle),
                 M3_ERR_NOT_FOUND);
  ...
  M3_TEST_EXPECT(sys.vtable->unregister_object(sys.ctx, handle),
                 M3_ERR_UNKNOWN);
  M3_TEST_EXPECT(sys.vtable->register_object(sys.ctx, &obj.header),
                 M3_ERR_UNKNOWN);
  M3_TEST_OK(m3_object_test_set_handle_is_valid_fail(M3_FALSE));
#endif
  obj.header.vtable = &test_vtable_no_release;
  M3_TEST_EXPECT(sys.vtable->register_object(NULL, &obj.header),
                 M3_ERR_INVALID_ARGUMENT);
  ...
  M3_TEST_EXPECT(sys.vtable->resolve(NULL, handle, &resolved),
                 M3_ERR_INVALID_ARGUMENT);
  M3_TEST_EXPECT(sys.vtable->resolve(sys.ctx, handle, NULL),
                 M3_ERR_INVALID_ARGUMENT);
  ...
  M3_TEST_EXPECT(sys.vtable->unregister_object(sys.ctx, obj2_handle),
                 M3_ERR_NOT_FOUND);
  ...
  M3_TEST_EXPECT(sys_small.vtable->register_object(sys_small.ctx, &obj2.header),
                 M3_ERR_BUSY);
  ...
  free_fail_allocator.free = free_fail_on_call;
  M3_TEST_OK(
      m3_handle_system_default_create(1, &free_fail_allocator, &free_fail_sys));
