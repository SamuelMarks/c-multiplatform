...
int main(void)
{
    ...
    const char four_byte[] = { (char)0xF0, (char)0x9F, (char)0x98, (char)0x80, '\0' };
    const char mixed[] = { 'A', (char)0xE2, (char)0x82, (char)0xAC, (char)0xF0, (char)0x9F, (char)0x98, (char)0x80, '\0' };
    ...
    const char invalid_too_large[] = { (char)0xF4, (char)0x90, (char)0x80, (char)0x80, '\0' };
    const char invalid_four_boundary[] = { (char)0xF0, (char)0x80, (char)0x80, (char)0x80, '\0' };
    const char invalid_high_start[] = { (char)0xF5, (char)0x80, (char)0x80, (char)0x80, '\0' };
    const char invalid_bad_b2_4[] = { (char)0xF0, (char)0x90, (char)0x28, (char)0xBC, '\0' };
    const char invalid_continuation[] = { (char)0xE2, (char)0x28, (char)0xA1, '\0' };
    ...
    advance = 0;
    M3_TEST_EXPECT(m3_utf8_test_decode(NULL, 1, &codepoint, &advance), M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(m3_utf8_test_decode((const m3_u8 *)ascii, 1, NULL, &advance), M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(m3_utf8_test_decode((const m3_u8 *)ascii, 1, &codepoint, NULL), M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(m3_utf8_test_decode((const m3_u8 *)ascii, 0, &codepoint, &advance), M3_ERR_NOT_FOUND);
    M3_TEST_EXPECT(m3_utf8_test_decode(invalid_trunc2, 1, &codepoint, &advance), M3_ERR_CORRUPT);
    M3_TEST_EXPECT(m3_utf8_test_decode(invalid_e0_boundary, 3, &codepoint, &advance), M3_ERR_CORRUPT);

#ifdef M3_TESTING
    M3_TEST_OK(m3_utf8_test_set_relaxed_checks(M3_TRUE));
    M3_TEST_EXPECT(m3_utf8_test_decode(invalid_surrogate_relaxed, 3, &codepoint, &advance), M3_ERR_CORRUPT);
    M3_TEST_EXPECT(m3_utf8_test_decode(invalid_too_large_relaxed, 4, &codepoint, &advance), M3_ERR_CORRUPT);
    ...
    M3_TEST_EXPECT(m3_utf8_iter_init(NULL, ascii, strlen(ascii)), M3_ERR_INVALID_ARGUMENT);
