...
static int M3_CALL test_sink_write(void *ctx, M3LogLevel level, const char *tag,
...
int main(void) {
  ...
  M3_TEST_EXPECT(m3_log_write(M3_LOG_LEVEL_INFO, "core", NULL),
  ...
  M3_TEST_OK(
      m3_log_test_set_mutex_failures(M3_FALSE, M3_TRUE, M3_FALSE, M3_FALSE));
  sink.ctx = &state;
  sink.write = test_sink_write;
  ...
  M3_TEST_EXPECT(m3_log_write_n(M3_LOG_LEVEL_INFO, "core", NULL, 1),
                 M3_ERR_INVALID_ARGUMENT);
  ...
  M3_TEST_EXPECT(m3_log_write_n(M3_LOG_LEVEL_INFO, "toolong", "x", 1),
                 M3_ERR_OVERFLOW);
  M3_TEST_OK(m3_log_test_set_cstr_limit(1));
  M3_TEST_EXPECT(m3_log_write(M3_LOG_LEVEL_INFO, "core", "ab"),
                 M3_ERR_OVERFLOW);
  M3_TEST_EXPECT(m3_log_write_n(M3_LOG_LEVEL_TRACE, NULL, "x", 1),
                 M3_ERR_OVERFLOW);
  ...
  M3_TEST_OK(
      m3_log_test_set_mutex_failures(M3_FALSE, M3_TRUE, M3_FALSE, M3_FALSE));
  M3_TEST_EXPECT(m3_log_write_n(M3_LOG_LEVEL_INFO, "core", "x", 1),
                 M3_ERR_UNKNOWN);
  M3_TEST_OK(
      m3_log_test_set_mutex_failures(M3_FALSE, M3_FALSE, M3_FALSE, M3_FALSE));

  M3_TEST_OK(
      m3_log_test_set_mutex_failures(M3_FALSE, M3_FALSE, M3_TRUE, M3_FALSE));
  M3_TEST_EXPECT(m3_log_write_n(M3_LOG_LEVEL_INFO, "core", "x", 1),
                 M3_ERR_UNKNOWN);
  M3_TEST_OK(
      m3_log_test_set_mutex_failures(M3_FALSE, M3_FALSE, M3_FALSE, M3_FALSE));
  ...
  state.fail = 1;
  M3_TEST_EXPECT(m3_log_write_n(M3_LOG_LEVEL_ERROR, "tag", "msg", 3),
                 M3_ERR_IO);
  state.fail = 0;
