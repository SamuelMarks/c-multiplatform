...
int main(void) {
  ...
  if (ui_audio_sink_coreaudio_get_backend(NULL) != UI_ERROR_INVALID_ARGUMENT) {
    fprintf(stderr, "Expected UI_ERROR_INVALID_ARGUMENT for NULL backend\n");
    return 1;
  }

  if (ui_audio_sink_coreaudio_get_backend(&backend) != UI_ERROR_UNKNOWN) {
    fprintf(stderr, "Expected UI_ERROR_UNKNOWN for CoreAudio stub\n");
    return 1;
  }
  ...
  if (ui_audio_sink_get_default_backend(&backend) != UI_ERROR_UNKNOWN) {
    fprintf(stderr, "Expected UI_ERROR_UNKNOWN for default CoreAudio stub\n");
    return 1;
  }
#endif

  printf("test_ui_audio_coreaudio passed\n");
  return 0;
}
