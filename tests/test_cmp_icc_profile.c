...
TEST test_icc_profile_parse(void) {
    ...
    char mock_jpg[] = { 0xFF, 0xD8, 0xFF, 0xE2, 0x00, 0x00, 'I', 'C', 'C', '_', 'P', 'R', 'O', 'F', 'I', 'L', 'E' };
    ASSERT_EQ(CMP_SUCCESS, cmp_icc_profile_parse(mock_jpg, sizeof(mock_jpg), &prof));
...
SUITE(suite_icc_profile) {
    RUN_TEST(test_icc_profile_parse);
}
