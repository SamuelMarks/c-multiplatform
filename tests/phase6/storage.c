...
static int test_storage_helpers(void) {
  ...
  M3_TEST_EXPECT(m3_storage_test_mul_overflow(1u, 2u, NULL),
                 M3_ERR_INVALID_ARGUMENT);
  M3_TEST_EXPECT(m3_storage_test_add_overflow(1u, 2u, NULL),
                 M3_ERR_INVALID_ARGUMENT);
  M3_TEST_EXPECT(m3_storage_test_u32_from_usize(1u, NULL),
                 M3_ERR_INVALID_ARGUMENT);
  ...
  M3_TEST_EXPECT(m3_storage_test_mul_overflow(max_value, 2u, &value),
                 M3_ERR_OVERFLOW);
  M3_TEST_EXPECT(m3_storage_test_add_overflow(max_value, 1u, &value),
                 M3_ERR_OVERFLOW);
  ...
  if (max_u32 < (m3_usize) ~(m3_usize)0) {
    M3_TEST_EXPECT(m3_storage_test_u32_from_usize(max_u32 + 1u, &value32),
                   M3_ERR_RANGE);
  }
  return 0;
}
...
static int test_storage_config_init(void) {
  ...
  M3_TEST_ASSERT(config.entry_capacity ==
                 (m3_usize)M3_STORAGE_DEFAULT_CAPACITY);
  return 0;
}
...
static int test_storage_put_errors(void) {
  ...
  M3_TEST_EXPECT(m3_storage_put(NULL, "a", 1u, "b", 1u, M3_FALSE),
                 M3_ERR_INVALID_ARGUMENT);

  memset(&storage, 0, sizeof(storage));
  M3_TEST_EXPECT(m3_storage_put(&storage, "a", 1u, "b", 1u, M3_FALSE),
                 M3_ERR_STATE);
  ...
  M3_TEST_EXPECT(m3_storage_put(&storage, "a", 1u, "b", 1u, 2),
                 M3_ERR_INVALID_ARGUMENT);
  M3_TEST_EXPECT(m3_storage_put(&storage, "a", 0u, "b", 1u, M3_TRUE),
                 M3_ERR_INVALID_ARGUMENT);
  M3_TEST_EXPECT(m3_storage_put(&storage, "a", 1u, NULL, 1u, M3_TRUE),
                 M3_ERR_INVALID_ARGUMENT);

  max_value = (m3_usize) ~(m3_usize)0;
  M3_TEST_EXPECT(m3_storage_put(&storage, "a", max_value, "b", 1u, M3_TRUE),
                 M3_ERR_OVERFLOW);
  ...
  test_alloc.fail_alloc_on_call = 1;
  M3_TEST_EXPECT(m3_storage_put(&storage, "alpha", 5u, "two", 3u, M3_TRUE),
  ...
  M3_TEST_EXPECT(m3_storage_put(&storage, "beta", 4u, "two", 3u, M3_FALSE),
                 M3_ERR_INVALID_ARGUMENT);
  ...
  test_alloc.free_calls = 0;
  test_alloc.fail_free_on_call = 1;
  M3_TEST_EXPECT(m3_storage_put(&storage, "alpha", 5u, NULL, 0u, M3_TRUE),
                 M3_ERR_IO);
  test_alloc.fail_free_on_call = 0;
...
static int test_storage_access_errors(void) {
  ...
  M3_TEST_EXPECT(
      m3_storage_get(NULL, "a", 1u, buffer, sizeof(buffer), &value_size),
      M3_ERR_INVALID_ARGUMENT);
  M3_TEST_EXPECT(
      m3_storage_get(&storage, "a", 1u, buffer, sizeof(buffer), &value_size),
      M3_ERR_STATE);
  M3_TEST_EXPECT(
      m3_storage_get(&storage, "a", 1u, buffer, sizeof(buffer), NULL),
      M3_ERR_INVALID_ARGUMENT);

  M3_TEST_EXPECT(m3_storage_contains(NULL, "a", 1u, &exists),
                 M3_ERR_INVALID_ARGUMENT);
  ...
  M3_TEST_EXPECT(
      m3_storage_get(&storage, NULL, 1u, buffer, sizeof(buffer), &value_size),
      M3_ERR_INVALID_ARGUMENT);
  M3_TEST_EXPECT(m3_storage_get(&storage, "a", 1u, NULL, 1u, &value_size),
                 M3_ERR_INVALID_ARGUMENT);
  M3_TEST_EXPECT(m3_storage_contains(&storage, NULL, 1u, &exists),
                 M3_ERR_INVALID_ARGUMENT);
  M3_TEST_EXPECT(m3_storage_contains(&storage, "a", 1u, NULL),
                 M3_ERR_INVALID_ARGUMENT);
  M3_TEST_EXPECT(m3_storage_remove(&storage, NULL, 1u),
                 M3_ERR_INVALID_ARGUMENT);
...
static int test_storage_put_get_remove(void) {
  ...
  M3_TEST_OK(m3_storage_get(&storage, "alpha", 5u, buffer, sizeof(buffer),
                            &value_size));
  ...
  M3_TEST_EXPECT(m3_storage_get(&storage, "missing", 7u, buffer, sizeof(buffer),
                                &value_size),
                 M3_ERR_NOT_FOUND);
  M3_TEST_ASSERT(value_size == 0u);

  M3_TEST_EXPECT(m3_storage_get(&storage, "alpha", 5u, buffer, 2u, &value_size),
                 M3_ERR_RANGE);
  M3_TEST_ASSERT(value_size == 3u);
...
static int test_storage_overwrite_and_grow(void) {
  ...
  M3_TEST_EXPECT(m3_storage_put(&storage, "dup", 3u, "two", 3u, M3_FALSE),
                 M3_ERR_BUSY);

  M3_TEST_OK(m3_storage_put(&storage, "dup", 3u, "two", 3u, M3_TRUE));
  M3_TEST_OK(
      m3_storage_get(&storage, "dup", 3u, buffer, sizeof(buffer), &value_size));
  M3_TEST_ASSERT(value_size == 3u);
  ...
  M3_TEST_EXPECT(m3_storage_put(&storage, "fail", 4u, "y", 1u, M3_FALSE),
                 M3_ERR_OUT_OF_MEMORY);
  test_alloc.fail_realloc_on_call = 0;

  max_value = (m3_usize) ~(m3_usize)0;
  storage.entry_capacity = max_value / 2u + 1u;
  M3_TEST_EXPECT(m3_storage_test_grow(&storage, storage.entry_capacity + 1u),
                 M3_ERR_OVERFLOW);
  storage.entry_capacity = 2u;
...
static int test_storage_alloc_failures(void) {
  ...
  test_alloc.alloc_calls = 0;
  test_alloc.fail_alloc_on_call = 1;
  M3_TEST_EXPECT(m3_storage_put(&storage, "key", 3u, NULL, 0u, M3_FALSE),
                 M3_ERR_OUT_OF_MEMORY);
  test_alloc.fail_alloc_on_call = 0;
  ...
  M3_TEST_EXPECT(m3_storage_put(&storage, "key", 3u, "val", 3u, M3_FALSE),
                 M3_ERR_OUT_OF_MEMORY);
  test_alloc.fail_alloc_on_call = 0;
...
static int test_storage_save_load_roundtrip(void) {
  ...
  M3_TEST_OK(
      m3_storage_get(&storage, "a", 1u, value, sizeof(value), &value_size));
  M3_TEST_ASSERT(value_size == 1u);
  M3_TEST_ASSERT(value[0] == '1');

  M3_TEST_OK(
      m3_storage_get(&storage, "b", 1u, value, sizeof(value), &value_size));
  M3_TEST_ASSERT(value_size == 1u);
...
static int test_storage_save_errors(void) {
  ...
  M3_TEST_EXPECT(m3_storage_save(&storage, NULL, "mem"),
                 M3_ERR_INVALID_ARGUMENT);
  ...
  io.vtable = NULL;
  M3_TEST_EXPECT(m3_storage_save(&storage, &io, "mem"),
                 M3_ERR_INVALID_ARGUMENT);
  io.vtable = &g_test_io_vtable;
...
static int test_storage_load_errors(void) {
  ...
  M3_TEST_EXPECT(m3_storage_load(&storage, NULL, "mem"),
                 M3_ERR_INVALID_ARGUMENT);
  ...
  io.vtable = NULL;
  M3_TEST_EXPECT(m3_storage_load(&storage, &io, "mem"),
                 M3_ERR_INVALID_ARGUMENT);
  io.vtable = &g_test_io_vtable;
...
int main(void) {
  ...
  M3_TEST_ASSERT(test_storage_init_custom_allocator() == 0);
