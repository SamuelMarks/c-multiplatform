...
static int test_camera_read(void *camera, M3CameraFrame *out_frame,
...
static const M3EnvVTable g_test_env_vtable = {NULL, NULL, test_env_get_camera,
                                              NULL, NULL, NULL};

static const M3EnvVTable g_test_env_vtable_no_camera = {NULL, NULL, NULL,
                                                        NULL, NULL, NULL};

static int test_env_reset(TestEnvState *env_state,
                          TestCameraState *camera_state,
                          const M3CameraVTable *vtable) {
  if (env_state == NULL) {
    return M3_ERR_INVALID_ARGUMENT;
  }
...
static int test_session_reset(M3CameraSession *session,
                              TestCameraState *camera_state,
                              const M3CameraVTable *vtable, M3Bool opened,
                              M3Bool streaming) {
  if (session == NULL) {
    return M3_ERR_INVALID_ARGUMENT;
  }
...
static int test_camera_init_errors(void) {
  ...
  env_state.camera.vtable = &vtable_missing;
  memset(&session, 0, sizeof(session));
  M3_TEST_EXPECT(m3_camera_test_set_skip_vtable_check(2),
...
static int test_camera_shutdown_errors(void) {
  ...
  M3_TEST_OK(test_camera_state_reset(&cam_state));
  M3_TEST_OK(test_session_reset(&session, &cam_state, &g_test_camera_vtable,
                                M3_TRUE, M3_FALSE));
  ...
  M3_TEST_OK(test_session_reset(&session, &cam_state, &vtable_missing, M3_TRUE,
                                M3_FALSE));
...
static int test_camera_read_errors(void) {
  ...
  M3_TEST_EXPECT(m3_camera_read_frame(NULL, &frame, &has_frame),
                 M3_ERR_INVALID_ARGUMENT);
  M3_TEST_EXPECT(m3_camera_read_frame(&session, NULL, &has_frame),
                 M3_ERR_INVALID_ARGUMENT);
  M3_TEST_EXPECT(m3_camera_read_frame(&session, &frame, NULL),
                 M3_ERR_INVALID_ARGUMENT);

  memset(&session, 0, sizeof(session));
  M3_TEST_EXPECT(m3_camera_read_frame(&session, &frame, &has_frame),
                 M3_ERR_STATE);
...
static int test_camera_copy_frame(void) {
  ...
  M3_TEST_EXPECT(m3_camera_copy_frame(NULL, buffer, sizeof(buffer), &out_size),
                 M3_ERR_INVALID_ARGUMENT);
  M3_TEST_EXPECT(m3_camera_copy_frame(&frame, buffer, sizeof(buffer), NULL),
                 M3_ERR_INVALID_ARGUMENT);
  ...
  frame.data = NULL;
  M3_TEST_EXPECT(
      m3_camera_copy_frame(&frame, buffer, sizeof(buffer), &out_size),
      M3_ERR_INVALID_ARGUMENT);

  frame.data = buffer;
  M3_TEST_EXPECT(m3_camera_copy_frame(&frame, NULL, sizeof(buffer), &out_size),
                 M3_ERR_INVALID_ARGUMENT);

  M3_TEST_EXPECT(m3_camera_copy_frame(&frame, buffer, 2u, &out_size),
                 M3_ERR_RANGE);
  ...
  buffer[3] = 6u;
  M3_TEST_OK(
      m3_camera_copy_frame(&frame, buffer + 4, sizeof(buffer) - 4u, &out_size));
