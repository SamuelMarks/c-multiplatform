...
static int test_network_request(void *net, const M3NetworkRequest *request,
...
static int test_network_free_response(void *net, const M3Allocator *allocator,
...
static const M3EnvVTable g_test_env_vtable = {
    NULL, NULL, NULL, test_env_get_network, NULL, NULL};

static const M3EnvVTable g_test_env_vtable_no_network = {NULL, NULL, NULL,
                                                         NULL, NULL, NULL};

static int test_env_reset(TestEnvState *env_state, TestNetworkState *net_state,
                          const M3NetworkVTable *vtable) {
  if (env_state == NULL) {
    return M3_ERR_INVALID_ARGUMENT;
  }
...
static int test_client_reset(M3NetworkClient *client,
...
static int test_network_shutdown_errors(void) {
  ...
  allocator.realloc = test_realloc;
  allocator.free = test_free;
  M3_TEST_OK(test_client_reset(&client, &net_state, &g_test_network_vtable,
                               &allocator, M3_TRUE));
...
static int test_network_request_errors(void) {
  ...
  M3_TEST_EXPECT(m3_network_request(NULL, &request, &response),
                 M3_ERR_INVALID_ARGUMENT);
  M3_TEST_EXPECT(m3_network_request(&client, NULL, &response),
                 M3_ERR_INVALID_ARGUMENT);
  M3_TEST_EXPECT(m3_network_request(&client, &request, NULL),
                 M3_ERR_INVALID_ARGUMENT);

  M3_TEST_EXPECT(m3_network_request(&client, &request, &response),
                 M3_ERR_STATE);
...
static int test_network_request_success(void) {
  ...
  M3_TEST_ASSERT(net_state.last_request.body_size ==
                 (m3_usize)sizeof(request_body));
  ...
  M3_TEST_OK(
      m3_network_copy_response_body(&response, copy, sizeof(copy), &out_size));
...
static int test_network_response_free_errors(void) {
  ...
  M3_TEST_EXPECT(m3_network_response_free(NULL, &response),
                 M3_ERR_INVALID_ARGUMENT);
  M3_TEST_EXPECT(m3_network_response_free(&client, NULL),
                 M3_ERR_INVALID_ARGUMENT);
  ...
  client.network.ctx = NULL;
  M3_TEST_EXPECT(m3_network_response_free(&client, &response),
                 M3_ERR_INVALID_ARGUMENT);
  client.network.ctx = &net_state;
...
static int test_network_copy_response_body(void) {
  ...
  M3_TEST_EXPECT(
      m3_network_copy_response_body(NULL, buffer, sizeof(buffer), &out_size),
      M3_ERR_INVALID_ARGUMENT);
  M3_TEST_EXPECT(
      m3_network_copy_response_body(&response, buffer, sizeof(buffer), NULL),
      M3_ERR_INVALID_ARGUMENT);

  memset(&response, 0, sizeof(response));
  response.body_size = 4u;
  response.body = NULL;
  M3_TEST_EXPECT(m3_network_copy_response_body(&response, buffer,
                                               sizeof(buffer), &out_size),
                 M3_ERR_INVALID_ARGUMENT);

  response.body = buffer;
  M3_TEST_EXPECT(
      m3_network_copy_response_body(&response, NULL, sizeof(buffer), &out_size),
      M3_ERR_INVALID_ARGUMENT);
  M3_TEST_EXPECT(
      m3_network_copy_response_body(&response, buffer, 2u, &out_size),
      M3_ERR_RANGE);
  ...
  buffer[2] = 13u;
  buffer[3] = 14u;
  M3_TEST_OK(m3_network_copy_response_body(&response, buffer + 4,
