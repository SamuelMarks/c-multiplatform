...
int main(void) {
  ...
  CMP_TEST_OK(cmp_win32_backend_test_validate_config(&config));
  {
    ...
    CMP_TEST_EXPECT(cmp_win32_backend_test_validate_config(&config),
                    CMP_ERR_INVALID_ARGUMENT);
  ...
  CMP_TEST_EXPECT(cmp_win32_backend_create(NULL, NULL),
                  CMP_ERR_INVALID_ARGUMENT);
  CMP_TEST_OK(cmp_win32_backend_test_set_config_init_fail(CMP_TRUE));
  CMP_TEST_EXPECT(cmp_win32_backend_create(NULL, &backend), CMP_ERR_IO);
  {
    int rc;
