...
int main(void) {
  ...
  M3_TEST_EXPECT(m3_win32_backend_test_validate_config(NULL),
  ...
  M3_TEST_EXPECT(m3_win32_backend_create(&config, &backend),
                 M3_ERR_INVALID_ARGUMENT);
  ...
  if (!available) {
    ...
    M3_TEST_EXPECT(m3_win32_backend_destroy((M3Win32Backend *)1),
                   M3_ERR_UNSUPPORTED);

    M3_TEST_EXPECT(m3_win32_backend_get_ws(NULL, &ws), M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(m3_win32_backend_get_ws((M3Win32Backend *)1, NULL),
                   M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(m3_win32_backend_get_ws((M3Win32Backend *)1, &ws),
                   M3_ERR_UNSUPPORTED);

    M3_TEST_EXPECT(m3_win32_backend_get_gfx(NULL, &gfx),
                   M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(m3_win32_backend_get_gfx((M3Win32Backend *)1, NULL),
                   M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(m3_win32_backend_get_gfx((M3Win32Backend *)1, &gfx),
                   M3_ERR_UNSUPPORTED);

    M3_TEST_EXPECT(m3_win32_backend_get_env(NULL, &env),
                   M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(m3_win32_backend_get_env((M3Win32Backend *)1, NULL),
                   M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(m3_win32_backend_get_env((M3Win32Backend *)1, &env),
                   M3_ERR_UNSUPPORTED);
...
#if defined(M3_WIN32_AVAILABLE)
  {
    ...
    if (gfx.text_vtable != NULL) {
      ...
      M3_TEST_EXPECT(
          ...
          M3_ERR_INVALID_ARGUMENT);
      M3_TEST_EXPECT(gfx.text_vtable->create_font(gfx.ctx, "Arial", 0, 400,
                                                  M3_FALSE, &font),
                     M3_ERR_RANGE);
      M3_TEST_EXPECT(gfx.text_vtable->create_font(gfx.ctx, "Arial", 12, 50,
                                                  M3_FALSE, &font),
                     M3_ERR_RANGE);
      M3_TEST_OK(gfx.text_vtable->create_font(gfx.ctx, "Arial", 12, 400,
                                              M3_FALSE, &font));
