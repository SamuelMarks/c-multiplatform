...
int main(void) {
  ...
  M3_TEST_EXPECT(m3_sdl3_backend_test_validate_config(NULL),
  ...
  M3_TEST_EXPECT(m3_sdl3_backend_create(&config, &backend),
                 M3_ERR_INVALID_ARGUMENT);
  ...
  if (!available) {
    ...
    M3_TEST_EXPECT(m3_sdl3_backend_destroy((M3SDL3Backend *)1),
                   M3_ERR_UNSUPPORTED);

    M3_TEST_EXPECT(m3_sdl3_backend_get_ws(NULL, &ws), M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(m3_sdl3_backend_get_ws((M3SDL3Backend *)1, NULL),
                   M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(m3_sdl3_backend_get_ws((M3SDL3Backend *)1, &ws),
                   M3_ERR_UNSUPPORTED);

    M3_TEST_EXPECT(m3_sdl3_backend_get_gfx(NULL, &gfx),
                   M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(m3_sdl3_backend_get_gfx((M3SDL3Backend *)1, NULL),
                   M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(m3_sdl3_backend_get_gfx((M3SDL3Backend *)1, &gfx),
                   M3_ERR_UNSUPPORTED);

    M3_TEST_EXPECT(m3_sdl3_backend_get_env(NULL, &env),
                   M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(m3_sdl3_backend_get_env((M3SDL3Backend *)1, NULL),
                   M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(m3_sdl3_backend_get_env((M3SDL3Backend *)1, &env),
                   M3_ERR_UNSUPPORTED);
...
#if defined(M3_SDL3_AVAILABLE)
  {
    ...
    if (gfx.text_vtable != NULL) {
      ...
      M3_TEST_EXPECT(
          ...
          M3_ERR_INVALID_ARGUMENT);
      M3_TEST_EXPECT(gfx.text_vtable->create_font(gfx.ctx, "missing.ttf", 0,
                                                  400, M3_FALSE, &font),
                     M3_ERR_RANGE);
      M3_TEST_EXPECT(gfx.text_vtable->create_font(gfx.ctx, "missing.ttf", 12,
                                                  50, M3_FALSE, &font),
                     M3_ERR_RANGE);
      ...
      if (text_rc == M3_OK) {
        ...
        M3_TEST_OK(gfx.text_vtable->measure_text(gfx.ctx, font, "", 0, &width,
                                                 &height, &baseline));
        M3_TEST_OK(gfx.text_vtable->destroy_font(gfx.ctx, font));
      } else {
        M3_TEST_ASSERT(text_rc == M3_ERR_NOT_FOUND ||
                       text_rc == M3_ERR_UNKNOWN);
      }
#else
    ...
    if (rc == M3_OK) {
      M3_TEST_OK(gfx.vtable->begin_frame(gfx.ctx, window, window_config.width,
                                         window_config.height, 1.0f));

      memset(pixels, 255, sizeof(pixels));
      texture.id = 0u;
      texture.generation = 0u;
      M3_TEST_OK(gfx.vtable->create_texture(gfx.ctx, 2, 2, M3_TEX_FORMAT_RGBA8,
      ...
      M3_TEST_OK(gfx.vtable->update_texture(
      ...
      M3_TEST_OK(gfx.vtable->draw_texture(gfx.ctx, texture, &src_rect,
                                          &dst_rect, 1.0f));
