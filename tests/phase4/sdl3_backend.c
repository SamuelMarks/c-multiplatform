...
int main(void) {
  ...
  CMPSDL3Backend *backend;
  CMPAllocator default_alloc;
  CMPBool available;

  CMP_TEST_EXPECT(cmp_sdl3_backend_config_init(NULL), CMP_ERR_INVALID_ARGUMENT);
  CMP_TEST_OK(cmp_sdl3_backend_config_init(&config));
  CMP_TEST_ASSERT(config.handle_capacity != 0);
  CMP_TEST_ASSERT(config.renderer_flags != 0);
  ...
  CMP_TEST_OK(cmp_sdl3_backend_test_validate_config(&config));
  {
    ...
    CMP_TEST_EXPECT(cmp_sdl3_backend_test_validate_config(&config),
                    CMP_ERR_INVALID_ARGUMENT);
  ...
  CMP_TEST_EXPECT(cmp_sdl3_backend_create(NULL, NULL),
                  CMP_ERR_INVALID_ARGUMENT);
  CMP_TEST_OK(cmp_sdl3_backend_test_set_config_init_fail(CMP_TRUE));
  CMP_TEST_EXPECT(cmp_sdl3_backend_create(NULL, &backend), CMP_ERR_IO);
  {
    int rc;
    ...
    if (!available) {
      ...
      CMP_TEST_OK(cmp_sdl3_backend_destroy(backend));
    }
  }
...
#if defined(CMP_SDL3_AVAILABLE)
  {
    ...
    CMP_TEST_OK(cmp_sdl3_backend_get_ws(backend, &ws));
    CMP_TEST_OK(cmp_sdl3_backend_get_gfx(backend, &gfx));
    CMP_TEST_OK(cmp_sdl3_backend_get_env(backend, &env));
    ...
    window_config.utf8_title = "LibCMPC SDL3";
