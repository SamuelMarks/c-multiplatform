...
int main(void)
{
    ...
    backend = NULL;
    M3_TEST_EXPECT(m3_android_backend_create(NULL, NULL), M3_ERR_INVALID_ARGUMENT);
    {
        int rc;
    ...
    M3_TEST_EXPECT(m3_android_backend_create(&config, &backend), M3_ERR_INVALID_ARGUMENT);
    ...
    if (!available) {
        ...
        M3_TEST_OK(m3_android_backend_config_init(&config));
        backend = NULL;
        M3_TEST_EXPECT(m3_android_backend_create(&config, &backend), M3_ERR_UNSUPPORTED);
        M3_TEST_ASSERT(backend == NULL);

        M3_TEST_EXPECT(m3_android_backend_destroy(NULL), M3_ERR_INVALID_ARGUMENT);
        M3_TEST_EXPECT(m3_android_backend_destroy((M3AndroidBackend *)1), M3_ERR_UNSUPPORTED);

        M3_TEST_EXPECT(m3_android_backend_get_ws(NULL, &ws), M3_ERR_INVALID_ARGUMENT);
        M3_TEST_EXPECT(m3_android_backend_get_ws((M3AndroidBackend *)1, NULL), M3_ERR_INVALID_ARGUMENT);
        M3_TEST_EXPECT(m3_android_backend_get_ws((M3AndroidBackend *)1, &ws), M3_ERR_UNSUPPORTED);

        M3_TEST_EXPECT(m3_android_backend_get_gfx(NULL, &gfx), M3_ERR_INVALID_ARGUMENT);
        M3_TEST_EXPECT(m3_android_backend_get_gfx((M3AndroidBackend *)1, NULL), M3_ERR_INVALID_ARGUMENT);
        M3_TEST_EXPECT(m3_android_backend_get_gfx((M3AndroidBackend *)1, &gfx), M3_ERR_UNSUPPORTED);

        M3_TEST_EXPECT(m3_android_backend_get_env(NULL, &env), M3_ERR_INVALID_ARGUMENT);
        M3_TEST_EXPECT(m3_android_backend_get_env((M3AndroidBackend *)1, NULL), M3_ERR_INVALID_ARGUMENT);
        M3_TEST_EXPECT(m3_android_backend_get_env((M3AndroidBackend *)1, &env), M3_ERR_UNSUPPORTED);
