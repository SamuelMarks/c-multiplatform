...
int main(void) {
  ...
  CMP_TEST_EXPECT(cmp_android_backend_create(NULL, NULL),
                  CMP_ERR_INVALID_ARGUMENT);
  CMP_TEST_OK(cmp_android_backend_test_set_config_init_fail(CMP_TRUE));
  CMP_TEST_EXPECT(cmp_android_backend_create(NULL, &backend), CMP_ERR_IO);
  {
    int rc;
  ...
  if (!available) {
    ...
    CMP_TEST_EXPECT(cmp_android_backend_set_predictive_back(NULL, NULL),
                    CMP_ERR_INVALID_ARGUMENT);
    CMP_TEST_EXPECT(cmp_android_backend_get_predictive_back(NULL, &predictive),
                    CMP_ERR_INVALID_ARGUMENT);
    CMP_TEST_EXPECT(
