...
int main(void) {
  ...
  CMP_TEST_EXPECT(cmp_cocoa_backend_config_init(NULL), CMP_ERR_INVALID_ARGUMENT);
  ...
  CMP_TEST_EXPECT(cmp_cocoa_backend_is_available(NULL), CMP_ERR_INVALID_ARGUMENT);
  CMP_TEST_OK(cmp_cocoa_backend_is_available(&available));

  backend = NULL;
  CMP_TEST_EXPECT(cmp_cocoa_backend_create(NULL, NULL), CMP_ERR_INVALID_ARGUMENT);
  {
    int rc;
  ...
  if (!available) {
    ...
    CMP_TEST_EXPECT(cmp_cocoa_backend_get_ws(NULL, &ws), CMP_ERR_INVALID_ARGUMENT);
