...
int main(void) {
  ...
  M3_TEST_EXPECT(m3_cocoa_backend_test_validate_config(NULL),
                 M3_ERR_INVALID_ARGUMENT);
  M3_TEST_OK(m3_get_default_allocator(&default_alloc));
  M3_TEST_OK(m3_cocoa_backend_config_init(&config));
  config.allocator = &default_alloc;
  M3_TEST_OK(m3_cocoa_backend_test_validate_config(&config));
  ...
  M3_TEST_EXPECT(m3_cocoa_backend_create(&config, &backend),
                 M3_ERR_INVALID_ARGUMENT);
  ...
  if (!available) {
    ...
    M3_TEST_EXPECT(m3_cocoa_backend_destroy((M3CocoaBackend *)1),
                   M3_ERR_UNSUPPORTED);

    M3_TEST_EXPECT(m3_cocoa_backend_get_ws(NULL, &ws), M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(m3_cocoa_backend_get_ws((M3CocoaBackend *)1, NULL),
                   M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(m3_cocoa_backend_get_ws((M3CocoaBackend *)1, &ws),
                   M3_ERR_UNSUPPORTED);

    M3_TEST_EXPECT(m3_cocoa_backend_get_gfx(NULL, &gfx),
                   M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(m3_cocoa_backend_get_gfx((M3CocoaBackend *)1, NULL),
                   M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(m3_cocoa_backend_get_gfx((M3CocoaBackend *)1, &gfx),
                   M3_ERR_UNSUPPORTED);

    M3_TEST_EXPECT(m3_cocoa_backend_get_env(NULL, &env),
                   M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(m3_cocoa_backend_get_env((M3CocoaBackend *)1, NULL),
                   M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(m3_cocoa_backend_get_env((M3CocoaBackend *)1, &env),
                   M3_ERR_UNSUPPORTED);
