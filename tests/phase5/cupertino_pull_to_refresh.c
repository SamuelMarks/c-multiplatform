...
static int test_cupertino_pull_to_refresh(void) {
    ...
    CMP_TEST_EXPECT(cupertino_refresh_init(NULL, &child), CMP_ERR_INVALID_ARGUMENT);
    CMP_TEST_OK(cupertino_refresh_init(&ptr, &child));

    CMP_TEST_EXPECT(cupertino_refresh_set_refreshing(NULL, CMP_TRUE), CMP_ERR_INVALID_ARGUMENT);
    ...
    CMP_TEST_EXPECT(cupertino_refresh_set_callback(NULL, NULL, NULL), CMP_ERR_INVALID_ARGUMENT);
    CMP_TEST_OK(cupertino_refresh_set_callback(&ptr, NULL, NULL));

    CMP_TEST_EXPECT(cupertino_refresh_layout(NULL, bounds), CMP_ERR_INVALID_ARGUMENT);
...
int main(void) {
    if (test_cupertino_pull_to_refresh() != 0) return 1;
    return 0;
}
