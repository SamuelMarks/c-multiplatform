    ...
    M3Handle *out_font)
{
    ...
    if (backend->fail_create_at > 0 && backend->create_calls == backend->fail_create_at) {
        return M3_ERR_IO;
    }
...
static int test_text_destroy_font(void *text, M3Handle font)
{
    ...
    if (backend->fail_destroy_at > 0 && backend->destroy_calls == backend->fail_destroy_at) {
        return M3_ERR_IO;
    }
...
static void test_setup_text_backend(TestDialogBackend *backend, M3TextBackend *out_backend, const M3TextVTable *vtable)
{
    out_backend->ctx = backend;
    out_backend->vtable = vtable;
}

static void test_setup_gfx_backend(TestDialogBackend *backend, M3Gfx *out_gfx, const M3GfxVTable *vtable,
...
static int test_alert_action(void *ctx, struct M3AlertDialog *dialog, m3_u32 action_id)
{
...
static int test_fullscreen_action(void *ctx, struct M3FullScreenDialog *dialog)
{
    TestActionState *state;
...
static void test_set_alert_families(M3AlertDialogStyle *style, const char *family)
{
...
static void test_set_fullscreen_families(M3FullScreenDialogStyle *style, const char *family)
{
    style->title_style.utf8_family = family;
    style->body_style.utf8_family = family;
    style->action_style.utf8_family = family;
...
static void test_set_snackbar_families(M3SnackbarStyle *style, const char *family)
{
...
static int test_dialog_helpers(void)
{
    ...
    M3_TEST_EXPECT(m3_dialog_test_point_in_rect(NULL, 0.0f, 0.0f, NULL), M3_ERR_INVALID_ARGUMENT);
    ...
    M3_TEST_EXPECT(m3_dialog_test_validate_text_style(NULL, M3_TRUE), M3_ERR_INVALID_ARGUMENT);
    M3_TEST_OK(m3_text_style_init(&text_style));
    text_style.utf8_family = NULL;
    M3_TEST_EXPECT(m3_dialog_test_validate_text_style(&text_style, M3_TRUE), M3_ERR_INVALID_ARGUMENT);
    text_style.utf8_family = "Test";
    text_style.size_px = 0;
    M3_TEST_EXPECT(m3_dialog_test_validate_text_style(&text_style, M3_TRUE), M3_ERR_RANGE);
    ...
    spec.mode = 99u;
    spec.size = 0.0f;
    M3_TEST_EXPECT(m3_dialog_test_validate_measure_spec(spec), M3_ERR_INVALID_ARGUMENT);
    ...
    backend.ctx = NULL;
    backend.vtable = NULL;
    M3_TEST_EXPECT(m3_dialog_test_validate_backend(NULL), M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(m3_dialog_test_validate_backend(&backend), M3_ERR_INVALID_ARGUMENT);
...
{
    {
        ...
        M3_TEST_EXPECT(m3_alert_dialog_init(NULL, &text_backend, &style, NULL, 0u, NULL, 0u), M3_ERR_INVALID_ARGUMENT);
        M3_TEST_EXPECT(m3_alert_dialog_init(&dialog, NULL, &style, NULL, 0u, NULL, 0u), M3_ERR_INVALID_ARGUMENT);
        M3_TEST_EXPECT(m3_alert_dialog_init(&dialog, &text_backend, NULL, NULL, 0u, NULL, 0u), M3_ERR_INVALID_ARGUMENT);
        M3_TEST_EXPECT(m3_alert_dialog_init(&dialog, &text_backend, &style, NULL, 1u, NULL, 0u), M3_ERR_INVALID_ARGUMENT);
        M3_TEST_EXPECT(m3_alert_dialog_init(&dialog, &text_backend, &style, NULL, 0u, NULL, 1u), M3_ERR_INVALID_ARGUMENT);

        text_backend.vtable = NULL;
        M3_TEST_EXPECT(m3_alert_dialog_init(&dialog, &text_backend, &style, NULL, 0u, NULL, 0u), M3_ERR_INVALID_ARGUMENT);
        test_setup_text_backend(&backend, &text_backend, &g_test_text_vtable_no_create);
        M3_TEST_EXPECT(m3_alert_dialog_init(&dialog, &text_backend, &style, NULL, 0u, NULL, 0u), M3_ERR_UNSUPPORTED);
        test_setup_text_backend(&backend, &text_backend, &g_test_text_vtable);
    }

    {
        ...
        M3_TEST_EXPECT(m3_alert_dialog_init(&dialog, &text_backend, &style, NULL, 0u, NULL, 0u), M3_ERR_RANGE);
...

    {
        ...
        test_backend_init(&backend);
        test_setup_text_backend(&backend, &text_backend, &g_test_text_vtable_no_measure);
        family = "Test";
        title = "T";
        ...
        M3_TEST_OK(m3_alert_dialog_style_init(&style));
        test_set_alert_families(&style, family);
        M3_TEST_OK(m3_alert_dialog_init(&dialog, &text_backend, &style, title, title_len, NULL, 0u));

        width_spec.mode = M3_MEASURE_UNSPECIFIED;
        width_spec.size = 0.0f;
        height_spec = width_spec;
        M3_TEST_EXPECT(dialog.widget.vtable->measure(dialog.widget.ctx, width_spec, height_spec, &size), M3_ERR_UNSUPPORTED);
    }

    {
        ...
        M3_TEST_OK(m3_alert_dialog_style_init(&style));
        test_set_alert_families(&style, family);
        M3_TEST_OK(m3_alert_dialog_init(&dialog, &text_backend, &style, title, title_len, body, body_len));

        M3_TEST_EXPECT(m3_alert_dialog_set_title(&dialog, NULL, 1u), M3_ERR_INVALID_ARGUMENT);
        M3_TEST_EXPECT(m3_alert_dialog_set_body(&dialog, NULL, 1u), M3_ERR_INVALID_ARGUMENT);
        M3_TEST_EXPECT(m3_alert_dialog_set_actions(&dialog, NULL, 1u, NULL, 0u), M3_ERR_INVALID_ARGUMENT);
        M3_TEST_EXPECT(m3_alert_dialog_set_actions(&dialog, confirm, confirm_len, NULL, 1u), M3_ERR_INVALID_ARGUMENT);

        M3_TEST_OK(m3_alert_dialog_set_actions(&dialog, confirm, confirm_len, dismiss, dismiss_len));

        width_spec.mode = 99u;
        width_spec.size = 10.0f;
        height_spec = width_spec;
        M3_TEST_EXPECT(dialog.widget.vtable->measure(dialog.widget.ctx, width_spec, height_spec, &size), M3_ERR_INVALID_ARGUMENT);

        width_spec.mode = M3_MEASURE_EXACTLY;
        width_spec.size = -1.0f;
        height_spec = width_spec;
        M3_TEST_EXPECT(dialog.widget.vtable->measure(dialog.widget.ctx, width_spec, height_spec, &size), M3_ERR_RANGE);

        width_spec.mode = M3_MEASURE_UNSPECIFIED;
        width_spec.size = 0.0f;
        height_spec = width_spec;
        M3_TEST_EXPECT(dialog.widget.vtable->measure(NULL, width_spec, height_spec, &size), M3_ERR_INVALID_ARGUMENT);
        M3_TEST_EXPECT(dialog.widget.vtable->measure(dialog.widget.ctx, width_spec, height_spec, NULL), M3_ERR_INVALID_ARGUMENT);
        M3_TEST_OK(dialog.widget.vtable->measure(dialog.widget.ctx, width_spec, height_spec, &size));
        ...
        bounds.width = -10.0f;
        bounds.height = 20.0f;
        M3_TEST_EXPECT(dialog.widget.vtable->layout(dialog.widget.ctx, bounds), M3_ERR_RANGE);
        M3_TEST_EXPECT(dialog.widget.vtable->layout(NULL, bounds), M3_ERR_INVALID_ARGUMENT);

        bounds.width = 300.0f;
        bounds.height = 200.0f;
        M3_TEST_OK(m3_alert_dialog_set_actions(&dialog, confirm, confirm_len, NULL, 0u));
        ...
        M3_TEST_OK(m3_alert_dialog_set_actions(&dialog, NULL, 0u, dismiss, dismiss_len));
        M3_TEST_OK(dialog.widget.vtable->layout(dialog.widget.ctx, bounds));
        M3_TEST_ASSERT(dialog.dismiss_bounds.width > 0.0f);
        M3_TEST_ASSERT(dialog.confirm_bounds.width == 0.0f);
        ...
        test_setup_gfx_backend(&backend, &gfx, &g_test_gfx_vtable_no_draw, &g_test_text_vtable);
        ctx.gfx = &gfx;
        ctx.clip = bounds;
        ctx.dpi_scale = 1.0f;
        M3_TEST_EXPECT(dialog.widget.vtable->paint(dialog.widget.ctx, &ctx), M3_ERR_UNSUPPORTED);

        test_setup_gfx_backend(&backend, &gfx, &g_test_gfx_vtable, &g_test_text_vtable_no_draw);
        M3_TEST_OK(m3_alert_dialog_set_actions(&dialog, confirm, confirm_len, dismiss, dismiss_len));
        M3_TEST_EXPECT(dialog.widget.vtable->paint(dialog.widget.ctx, &ctx), M3_ERR_UNSUPPORTED);
        ...
        test_setup_gfx_backend(&backend, &gfx, &g_test_gfx_vtable, &g_test_text_vtable);
        ctx.gfx = &gfx;
        ...
        M3_TEST_EXPECT(dialog.widget.vtable->paint(dialog.widget.ctx, NULL), M3_ERR_INVALID_ARGUMENT);
        ctx.gfx = NULL;
        M3_TEST_EXPECT(dialog.widget.vtable->paint(dialog.widget.ctx, &ctx), M3_ERR_INVALID_ARGUMENT);
        ctx.gfx = &gfx;
        gfx.vtable = NULL;
        M3_TEST_EXPECT(dialog.widget.vtable->paint(dialog.widget.ctx, &ctx), M3_ERR_INVALID_ARGUMENT);
        test_setup_gfx_backend(&backend, &gfx, &g_test_gfx_vtable, &g_test_text_vtable);
        ...
        M3_TEST_OK(dialog.widget.vtable->event(dialog.widget.ctx, &event, &handled));
        M3_TEST_ASSERT(handled == M3_TRUE);

        M3_TEST_EXPECT(dialog.widget.vtable->event(dialog.widget.ctx, &event, &handled), M3_ERR_STATE);

        memset(&state, 0, sizeof(state));
        M3_TEST_OK(m3_alert_dialog_set_on_action(&dialog, test_alert_action, &state));
        ...
        state.fail_next = 1;
        event.type = M3_INPUT_POINTER_UP;
        M3_TEST_EXPECT(dialog.widget.vtable->event(dialog.widget.ctx, &event, &handled), M3_ERR_IO);
        ...
        M3_TEST_EXPECT(dialog.widget.vtable->event(dialog.widget.ctx, NULL, &handled), M3_ERR_INVALID_ARGUMENT);
        M3_TEST_EXPECT(dialog.widget.vtable->event(NULL, &event, &handled), M3_ERR_INVALID_ARGUMENT);
        M3_TEST_EXPECT(dialog.widget.vtable->event(dialog.widget.ctx, &event, NULL), M3_ERR_INVALID_ARGUMENT);

        {
            M3Semantics semantics;
            M3_TEST_EXPECT(dialog.widget.vtable->get_semantics(NULL, &semantics), M3_ERR_INVALID_ARGUMENT);
            M3_TEST_EXPECT(dialog.widget.vtable->get_semantics(dialog.widget.ctx, NULL), M3_ERR_INVALID_ARGUMENT);
            M3_TEST_OK(dialog.widget.vtable->get_semantics(dialog.widget.ctx, &semantics));
            M3_TEST_ASSERT(semantics.utf8_label == dialog.utf8_title);
        }

        M3_TEST_EXPECT(dialog.widget.vtable->destroy(NULL), M3_ERR_INVALID_ARGUMENT);
...

    {
        ...
        M3_TEST_OK(m3_alert_dialog_style_init(&style));
        test_set_alert_families(&style, family);
        M3_TEST_OK(m3_alert_dialog_init(&dialog, &text_backend, &style, NULL, 0u, NULL, 0u));
...

    {
        ...
        M3_TEST_EXPECT(dialog.widget.vtable->destroy(dialog.widget.ctx), M3_ERR_INVALID_ARGUMENT);
    }
...

    {
        ...
        M3_TEST_EXPECT(m3_alert_dialog_set_style(NULL, &style), M3_ERR_INVALID_ARGUMENT);
        M3_TEST_EXPECT(m3_alert_dialog_set_style(&dialog, NULL), M3_ERR_INVALID_ARGUMENT);
    }
...
{
    {
        ...
        M3_TEST_EXPECT(m3_fullscreen_dialog_style_init(NULL), M3_ERR_INVALID_ARGUMENT);
        M3_TEST_OK(m3_fullscreen_dialog_style_init(&style));
        test_set_fullscreen_families(&style, family);

        M3_TEST_EXPECT(m3_fullscreen_dialog_init(NULL, &text_backend, &style, NULL, 0u, NULL, 0u), M3_ERR_INVALID_ARGUMENT);
        M3_TEST_EXPECT(m3_fullscreen_dialog_init(&dialog, NULL, &style, NULL, 0u, NULL, 0u), M3_ERR_INVALID_ARGUMENT);
        M3_TEST_EXPECT(m3_fullscreen_dialog_init(&dialog, &text_backend, NULL, NULL, 0u, NULL, 0u), M3_ERR_INVALID_ARGUMENT);
        M3_TEST_EXPECT(m3_fullscreen_dialog_init(&dialog, &text_backend, &style, NULL, 1u, NULL, 0u), M3_ERR_INVALID_ARGUMENT);
        M3_TEST_EXPECT(m3_fullscreen_dialog_init(&dialog, &text_backend, &style, NULL, 0u, NULL, 1u), M3_ERR_INVALID_ARGUMENT);

        text_backend.vtable = NULL;
        M3_TEST_EXPECT(m3_fullscreen_dialog_init(&dialog, &text_backend, &style, NULL, 0u, NULL, 0u), M3_ERR_INVALID_ARGUMENT);
    }

    {
        ...
        test_set_fullscreen_families(&style, family);
        style.corner_radius = -1.0f;
        M3_TEST_EXPECT(m3_fullscreen_dialog_init(&dialog, &text_backend, &style, NULL, 0u, NULL, 0u), M3_ERR_RANGE);
...

    {
        ...
        M3_TEST_OK(m3_fullscreen_dialog_style_init(&style));
        test_set_fullscreen_families(&style, family);
        M3_TEST_OK(m3_fullscreen_dialog_init(&dialog, &text_backend, &style, title, title_len, body, body_len));

        M3_TEST_EXPECT(m3_fullscreen_dialog_set_title(&dialog, NULL, 1u), M3_ERR_INVALID_ARGUMENT);
        M3_TEST_EXPECT(m3_fullscreen_dialog_set_body(&dialog, NULL, 1u), M3_ERR_INVALID_ARGUMENT);
        M3_TEST_EXPECT(m3_fullscreen_dialog_set_action(&dialog, NULL, 1u), M3_ERR_INVALID_ARGUMENT);
        ...
        memset(&state, 0, sizeof(state));
        M3_TEST_OK(m3_fullscreen_dialog_set_on_action(&dialog, test_fullscreen_action, &state));
...

    {
        ...
        M3_TEST_OK(m3_fullscreen_dialog_style_init(&style));
        test_set_fullscreen_families(&style, family);
        M3_TEST_OK(m3_fullscreen_dialog_init(&dialog, &text_backend, &style, title, title_len, NULL, 0u));
        ...
        M3_TEST_EXPECT(m3_fullscreen_dialog_set_style(NULL, &style), M3_ERR_INVALID_ARGUMENT);
        M3_TEST_EXPECT(m3_fullscreen_dialog_set_style(&dialog, NULL), M3_ERR_INVALID_ARGUMENT);
    }
...
{
    {
        ...
        M3_TEST_EXPECT(m3_snackbar_init(NULL, &text_backend, &style, NULL, 0u), M3_ERR_INVALID_ARGUMENT);
        M3_TEST_EXPECT(m3_snackbar_init(&snackbar, NULL, &style, NULL, 0u), M3_ERR_INVALID_ARGUMENT);
        M3_TEST_EXPECT(m3_snackbar_init(&snackbar, &text_backend, NULL, NULL, 0u), M3_ERR_INVALID_ARGUMENT);
        M3_TEST_EXPECT(m3_snackbar_init(&snackbar, &text_backend, &style, NULL, 1u), M3_ERR_INVALID_ARGUMENT);

        text_backend.vtable = NULL;
        M3_TEST_EXPECT(m3_snackbar_init(&snackbar, &text_backend, &style, NULL, 0u), M3_ERR_INVALID_ARGUMENT);
    }

    {
        ...
        test_set_snackbar_families(&style, family);
        style.corner_radius = -1.0f;
        M3_TEST_EXPECT(m3_snackbar_init(&snackbar, &text_backend, &style, NULL, 0u), M3_ERR_RANGE);
...

    {
        ...
        M3_TEST_OK(m3_snackbar_style_init(&style));
        test_set_snackbar_families(&style, family);
        M3_TEST_OK(m3_snackbar_init(&snackbar, &text_backend, &style, message, message_len));

        M3_TEST_EXPECT(m3_snackbar_set_message(&snackbar, NULL, 1u), M3_ERR_INVALID_ARGUMENT);
        M3_TEST_EXPECT(m3_snackbar_set_action(&snackbar, NULL, 1u), M3_ERR_INVALID_ARGUMENT);
        ...
        width_spec.size = 10.0f;
        height_spec = width_spec;
        M3_TEST_EXPECT(snackbar.widget.vtable->measure(snackbar.widget.ctx, width_spec, height_spec, &size), M3_ERR_INVALID_ARGUMENT);

        width_spec.mode = M3_MEASURE_EXACTLY;
        width_spec.size = -1.0f;
        height_spec = width_spec;
        M3_TEST_EXPECT(snackbar.widget.vtable->measure(snackbar.widget.ctx, width_spec, height_spec, &size), M3_ERR_RANGE);

        width_spec.mode = M3_MEASURE_UNSPECIFIED;
        width_spec.size = 0.0f;
        height_spec = width_spec;
        M3_TEST_EXPECT(snackbar.widget.vtable->measure(NULL, width_spec, height_spec, &size), M3_ERR_INVALID_ARGUMENT);
        M3_TEST_EXPECT(snackbar.widget.vtable->measure(snackbar.widget.ctx, width_spec, height_spec, NULL), M3_ERR_INVALID_ARGUMENT);
        M3_TEST_OK(snackbar.widget.vtable->measure(snackbar.widget.ctx, width_spec, height_spec, &size));
        ...
        bounds.width = -10.0f;
        bounds.height = 20.0f;
        M3_TEST_EXPECT(snackbar.widget.vtable->layout(snackbar.widget.ctx, bounds), M3_ERR_RANGE);
        M3_TEST_EXPECT(snackbar.widget.vtable->layout(NULL, bounds), M3_ERR_INVALID_ARGUMENT);
        ...
        ctx.clip = bounds;
        ctx.dpi_scale = 1.0f;
        M3_TEST_EXPECT(snackbar.widget.vtable->paint(snackbar.widget.ctx, NULL), M3_ERR_INVALID_ARGUMENT);
        ctx.gfx = NULL;
        M3_TEST_EXPECT(snackbar.widget.vtable->paint(snackbar.widget.ctx, &ctx), M3_ERR_INVALID_ARGUMENT);
        ctx.gfx = &gfx;
        gfx.vtable = NULL;
        M3_TEST_EXPECT(snackbar.widget.vtable->paint(snackbar.widget.ctx, &ctx), M3_ERR_INVALID_ARGUMENT);
        test_setup_gfx_backend(&backend, &gfx, &g_test_gfx_vtable_no_draw, &g_test_text_vtable);
        M3_TEST_EXPECT(snackbar.widget.vtable->paint(snackbar.widget.ctx, &ctx), M3_ERR_UNSUPPORTED);
        ...
        M3_TEST_OK(snackbar.widget.vtable->event(snackbar.widget.ctx, &event, &handled));
        M3_TEST_ASSERT(handled == M3_TRUE);

        M3_TEST_EXPECT(snackbar.widget.vtable->event(snackbar.widget.ctx, &event, &handled), M3_ERR_STATE);

        memset(&state, 0, sizeof(state));
        M3_TEST_OK(m3_snackbar_set_on_action(&snackbar, test_snackbar_action, &state));
        ...
        state.fail_next = 1;
        event.type = M3_INPUT_POINTER_UP;
        M3_TEST_EXPECT(snackbar.widget.vtable->event(snackbar.widget.ctx, &event, &handled), M3_ERR_IO);
        ...
        {
            M3Semantics semantics;
            M3_TEST_EXPECT(snackbar.widget.vtable->get_semantics(NULL, &semantics), M3_ERR_INVALID_ARGUMENT);
            M3_TEST_EXPECT(snackbar.widget.vtable->get_semantics(snackbar.widget.ctx, NULL), M3_ERR_INVALID_ARGUMENT);
            M3_TEST_OK(snackbar.widget.vtable->get_semantics(snackbar.widget.ctx, &semantics));
            M3_TEST_ASSERT(semantics.utf8_label == snackbar.utf8_message);
        }

        M3_TEST_EXPECT(snackbar.widget.vtable->event(NULL, &event, &handled), M3_ERR_INVALID_ARGUMENT);
        M3_TEST_EXPECT(snackbar.widget.vtable->event(snackbar.widget.ctx, NULL, &handled), M3_ERR_INVALID_ARGUMENT);
        M3_TEST_EXPECT(snackbar.widget.vtable->event(snackbar.widget.ctx, &event, NULL), M3_ERR_INVALID_ARGUMENT);

        M3_TEST_EXPECT(snackbar.widget.vtable->destroy(NULL), M3_ERR_INVALID_ARGUMENT);

        backend.fail_destroy = M3_ERR_IO;
        M3_TEST_EXPECT(snackbar.widget.vtable->destroy(snackbar.widget.ctx), M3_ERR_IO);
    }

    {
        ...
        M3_TEST_EXPECT(m3_snackbar_set_style(NULL, &style), M3_ERR_INVALID_ARGUMENT);
        M3_TEST_EXPECT(m3_snackbar_set_style(&snackbar, NULL), M3_ERR_INVALID_ARGUMENT);
    }
