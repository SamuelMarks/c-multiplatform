...
static int test_cupertino_navigation_bar(void) {
  ...
  CMPRect safe_area = {0.0f, 44.0f, 0.0f,
                       0.0f}; /* iPhone Notch/Dynamic Island top inset */
  CupertinoButton left_btn;

  text_backend.ctx = NULL;
  text_backend.vtable = &mock_text_vtable;

  CMP_TEST_EXPECT(cupertino_navigation_bar_style_init(NULL),
                  CMP_ERR_INVALID_ARGUMENT);
  CMP_TEST_OK(cupertino_navigation_bar_style_init(&style));
  CMP_TEST_EXPECT(style.variant, CUPERTINO_NAV_BAR_STANDARD);

  CMP_TEST_EXPECT(cupertino_navigation_bar_init(NULL, &text_backend),
                  CMP_ERR_INVALID_ARGUMENT);
  CMP_TEST_EXPECT(cupertino_navigation_bar_init(&bar, NULL),
                  CMP_ERR_INVALID_ARGUMENT);
  CMP_TEST_OK(cupertino_navigation_bar_init(&bar, &text_backend));

  CMP_TEST_EXPECT(cupertino_navigation_bar_set_title(NULL, "Title", 5),
                  CMP_ERR_INVALID_ARGUMENT);
  CMP_TEST_OK(cupertino_navigation_bar_set_title(&bar, "Title", 5));

  /* Layout Standard */
  CMP_TEST_EXPECT(cupertino_navigation_bar_layout(NULL, bounds, safe_area),
                  CMP_ERR_INVALID_ARGUMENT);
  ...
  CMP_TEST_OK(cupertino_navigation_bar_layout(&bar, bounds, safe_area));
  CMP_TEST_ASSERT(bar.bounds.height ==
                  140.0f); /* 44 top inset + 44 base + 52 extension */

  /* Layout Large Title fully collapsed */
  bar.scroll_offset = 52.0f;
  CMP_TEST_OK(cupertino_navigation_bar_layout(&bar, bounds, safe_area));
  CMP_TEST_ASSERT(bar.bounds.height ==
                  88.0f); /* 44 top inset + 44 base + 0 extension */
...
int main(void) {
  if (test_cupertino_navigation_bar() != 0)
    return 1;
  return 0;
}
