...
static int test_cupertino_tab_bar(void) {
  ...
  CMPRect safe_area = {0.0f, 0.0f, 0.0f,
                       34.0f}; /* iPhone Home Indicator bottom inset */
  ...
  CMP_TEST_EXPECT(cupertino_tab_bar_init(NULL, &text_backend),
  ...
  CMP_TEST_EXPECT(cupertino_tab_bar_set_items(NULL, items, 2),
                  CMP_ERR_INVALID_ARGUMENT);
  CMP_TEST_EXPECT(
      cupertino_tab_bar_set_items(&bar, items, CUPERTINO_TAB_BAR_MAX_ITEMS + 1),
      CMP_ERR_INVALID_ARGUMENT);
  ...
  /* Layout */
  CMP_TEST_EXPECT(cupertino_tab_bar_layout(NULL, bounds, safe_area),
                  CMP_ERR_INVALID_ARGUMENT);
