...
static int test_cupertino_stepper(void) {
    ...
    CMP_TEST_EXPECT(cupertino_stepper_set_value(NULL, 5.0), CMP_ERR_INVALID_ARGUMENT);
    CMP_TEST_EXPECT(cupertino_stepper_set_value(&stepper, 5.0), CMP_OK);
    CMP_TEST_EXPECT((int)stepper.value, 5);

    CMP_TEST_EXPECT(cupertino_stepper_paint(NULL, &ctx), CMP_ERR_INVALID_ARGUMENT);
    CMP_TEST_EXPECT(cupertino_stepper_paint(&stepper, NULL), CMP_ERR_INVALID_ARGUMENT);
