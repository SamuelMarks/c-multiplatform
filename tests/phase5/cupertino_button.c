...
static int test_cupertino_button(void) {
    ...
    CMP_TEST_EXPECT(cupertino_button_init(NULL, &text_backend), CMP_ERR_INVALID_ARGUMENT);
    CMP_TEST_EXPECT(cupertino_button_init(&button, NULL), CMP_ERR_INVALID_ARGUMENT);
    CMP_TEST_OK(cupertino_button_init(&button, &text_backend));

    CMP_TEST_EXPECT(cupertino_button_set_label(NULL, "Test", 4), CMP_ERR_INVALID_ARGUMENT);
    CMP_TEST_OK(cupertino_button_set_label(&button, "Test", 4));

    CMP_TEST_EXPECT(cupertino_button_set_style(NULL, &style), CMP_ERR_INVALID_ARGUMENT);
    CMP_TEST_EXPECT(cupertino_button_set_style(&button, NULL), CMP_ERR_INVALID_ARGUMENT);
    CMP_TEST_OK(cupertino_button_set_style(&button, &style));

    CMP_TEST_EXPECT(cupertino_button_measure(NULL, unspec, unspec, &size), CMP_ERR_INVALID_ARGUMENT);
    CMP_TEST_EXPECT(cupertino_button_measure(&button, unspec, unspec, NULL), CMP_ERR_INVALID_ARGUMENT);
    ...
    CMP_TEST_ASSERT(size.height == 44.0f); /* 16 + 0 padding, bumped to 44 hit target */
    ...
    /* Test layout */
    CMP_TEST_EXPECT(cupertino_button_layout(NULL, button.bounds), CMP_ERR_INVALID_ARGUMENT);
    {
...
int main(void) {
    if (test_cupertino_button() != 0) return 1;
    return 0;
}
