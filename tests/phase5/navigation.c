...
static int test_validation_helpers(void)
{
    ...
    M3_TEST_EXPECT(m3_navigation_test_validate_color(NULL), M3_ERR_INVALID_ARGUMENT);
    color.r = -0.1f;
    color.g = 0.0f;
    color.b = 0.0f;
    color.a = 0.0f;
    ...
    M3_TEST_EXPECT(m3_navigation_test_validate_edges(NULL), M3_ERR_INVALID_ARGUMENT);
    edges.left = -1.0f;
    edges.right = 0.0f;
    edges.top = 0.0f;
    edges.bottom = 0.0f;
    ...
    M3_TEST_EXPECT(m3_navigation_test_validate_text_style(NULL, M3_TRUE), M3_ERR_INVALID_ARGUMENT);
    M3_TEST_OK(m3_text_style_init(&text_style));
    text_style.utf8_family = NULL;
    M3_TEST_EXPECT(m3_navigation_test_validate_text_style(&text_style, M3_TRUE), M3_ERR_INVALID_ARGUMENT);
    M3_TEST_OK(m3_navigation_test_validate_text_style(&text_style, M3_FALSE));
    text_style.utf8_family = "Test";
    text_style.size_px = 0;
    M3_TEST_EXPECT(m3_navigation_test_validate_text_style(&text_style, M3_TRUE), M3_ERR_RANGE);
    ...
    M3_TEST_EXPECT(m3_navigation_test_validate_style(NULL, M3_TRUE), M3_ERR_INVALID_ARGUMENT);
    M3_TEST_OK(m3_navigation_style_init(&style));
    style.text_style.utf8_family = "Sans";
    base_style = style;
    style.mode = 99u;
    M3_TEST_EXPECT(m3_navigation_test_validate_style(&style, M3_TRUE), M3_ERR_RANGE);
    style = base_style;
    ...
    M3_TEST_EXPECT(m3_navigation_test_validate_style(&style, M3_TRUE), M3_ERR_INVALID_ARGUMENT);
    style = base_style;
    ...
    M3_TEST_EXPECT(m3_navigation_test_validate_items(NULL, 1), M3_ERR_INVALID_ARGUMENT);
    M3_TEST_OK(m3_navigation_test_validate_items(NULL, 0));
    item.utf8_label = NULL;
    item.utf8_len = 1;
    M3_TEST_EXPECT(m3_navigation_test_validate_items(&item, 1), M3_ERR_INVALID_ARGUMENT);
    item.utf8_len = 0;
    M3_TEST_OK(m3_navigation_test_validate_items(&item, 1));

    spec.mode = 99u;
    spec.size = 0.0f;
    M3_TEST_EXPECT(m3_navigation_test_validate_measure_spec(spec), M3_ERR_INVALID_ARGUMENT);
    ...
    M3_TEST_EXPECT(m3_navigation_test_validate_rect(NULL), M3_ERR_INVALID_ARGUMENT);
    rect.x = 0.0f;
    rect.y = 0.0f;
    rect.width = -1.0f;
    rect.height = 1.0f;
    ...
    M3_TEST_EXPECT(m3_navigation_test_resolve_mode(NULL, 0.0f, &mode), M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(m3_navigation_test_resolve_mode(&base_style, -1.0f, &mode), M3_ERR_RANGE);
    ...
    M3_TEST_EXPECT(m3_navigation_test_measure_content(NULL, M3_NAV_MODE_BAR, 0, &width, &height), M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(m3_navigation_test_measure_content(&base_style, M3_NAV_MODE_BAR, 0, NULL, &height), M3_ERR_INVALID_ARGUMENT);
    base_style.item_spacing = -1.0f;
    M3_TEST_EXPECT(m3_navigation_test_measure_content(&base_style, M3_NAV_MODE_BAR, 0, &width, &height), M3_ERR_RANGE);
    base_style = style;
    M3_TEST_EXPECT(m3_navigation_test_measure_content(&base_style, 99u, 0, &width, &height), M3_ERR_RANGE);
    base_style.bar_height = -1.0f;
    M3_TEST_EXPECT(m3_navigation_test_measure_content(&base_style, M3_NAV_MODE_BAR, 0, &width, &height), M3_ERR_RANGE);
    base_style = style;
    M3_TEST_OK(m3_navigation_test_measure_content(&base_style, M3_NAV_MODE_BAR, 0, &width, &height));
    M3_TEST_OK(m3_navigation_test_measure_content(&base_style, M3_NAV_MODE_BAR, 2, &width, &height));
    M3_TEST_OK(m3_navigation_test_measure_content(&base_style, M3_NAV_MODE_RAIL, 2, &width, &height));
...
static int test_init_validation(void)
{
    ...
    M3_TEST_EXPECT(m3_navigation_init(&nav, &bad_backend, &style, items, 2, 0), M3_ERR_INVALID_ARGUMENT);
    ...
    M3_TEST_EXPECT(m3_navigation_init(&nav, &backend, &style, items, 2, 0), M3_ERR_UNKNOWN);
    M3_TEST_OK(m3_navigation_test_clear_fail_points());
#endif
    ...
    M3_TEST_EXPECT(m3_navigation_init(&nav, &backend, &style, items, 2, 0), M3_ERR_IO);
...
static int test_setters_and_getters(void)
{
    ...
    M3_TEST_EXPECT(nav.widget.vtable->get_semantics(NULL, &semantics), M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(nav.widget.vtable->get_semantics(nav.widget.ctx, NULL), M3_ERR_INVALID_ARGUMENT);
    ...
    M3_TEST_EXPECT(m3_navigation_set_style(NULL, &style), M3_ERR_INVALID_ARGUMENT);
    ...
    M3_TEST_ASSERT(nav.selected_index == M3_NAV_INVALID_INDEX);
    {
        ...
        bad_items[0].utf8_label = NULL;
        bad_items[0].utf8_len = 1;
        M3_TEST_EXPECT(m3_navigation_set_items(&nav, bad_items, 1), M3_ERR_INVALID_ARGUMENT);
    }
...
static int test_measure_layout_mode(void)
{
    ...
    size_spec.size = 0.0f;
    M3_TEST_EXPECT(nav.widget.vtable->measure(NULL, size_spec, size_spec, &size), M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(nav.widget.vtable->measure(&nav, size_spec, size_spec, NULL), M3_ERR_INVALID_ARGUMENT);
    ...
    M3_TEST_EXPECT(nav.widget.vtable->measure(&nav, size_spec, size_spec, &size), M3_ERR_RANGE);
    nav.style.item_height = style.item_height;

#ifdef M3_TESTING
    M3_TEST_OK(m3_navigation_test_set_fail_point(M3_NAV_TEST_FAIL_RESOLVE_MODE));
    M3_TEST_EXPECT(nav.widget.vtable->measure(&nav, size_spec, size_spec, &size), M3_ERR_UNKNOWN);
    M3_TEST_OK(m3_navigation_test_clear_fail_points());
#endif
    ...
    M3_TEST_EXPECT(nav.widget.vtable->layout(NULL, bounds), M3_ERR_INVALID_ARGUMENT);
...
static int test_compute_layout_and_hit_test(void)
{
    ...
    M3_TEST_EXPECT(m3_navigation_test_compute_layout(NULL, &layout), M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(m3_navigation_test_compute_layout(&nav, NULL), M3_ERR_INVALID_ARGUMENT);

    nav.style.item_height = -1.0f;
    M3_TEST_EXPECT(m3_navigation_test_compute_layout(&nav, &layout), M3_ERR_RANGE);
    nav.style.item_height = style.item_height;
...
#ifdef M3_TESTING
    M3_TEST_OK(m3_navigation_test_set_fail_point(M3_NAV_TEST_FAIL_RESOLVE_MODE));
    M3_TEST_EXPECT(m3_navigation_test_compute_layout(&nav, &layout), M3_ERR_UNKNOWN);
    M3_TEST_OK(m3_navigation_test_clear_fail_points());
#endif
...
#ifdef M3_TESTING
    M3_TEST_OK(m3_navigation_test_set_fail_point(M3_NAV_TEST_FAIL_LAYOUT_ITEM_WIDTH_NEGATIVE));
    M3_TEST_EXPECT(m3_navigation_test_compute_layout(&nav, &layout), M3_ERR_RANGE);
    M3_TEST_OK(m3_navigation_test_clear_fail_points());
#endif
...
#ifdef M3_TESTING
    M3_TEST_OK(m3_navigation_test_set_fail_point(M3_NAV_TEST_FAIL_LAYOUT_MODE_INVALID));
    M3_TEST_EXPECT(m3_navigation_test_compute_layout(&nav, &layout), M3_ERR_RANGE);
    M3_TEST_OK(m3_navigation_test_clear_fail_points());
#endif
    ...
    M3_TEST_EXPECT(m3_navigation_test_hit_test(NULL, &layout, 0, 0, &index), M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(m3_navigation_test_hit_test(&nav, NULL, 0, 0, &index), M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(m3_navigation_test_hit_test(&nav, &layout, 0, 0, NULL), M3_ERR_INVALID_ARGUMENT);
    ...
    layout.spacing = 0.0f;
    M3_TEST_EXPECT(m3_navigation_test_hit_test(&nav, &layout, 5, 5, &index), M3_ERR_RANGE);
    layout.item_width = 10.0f;

#ifdef M3_TESTING
    M3_TEST_OK(m3_navigation_test_set_fail_point(M3_NAV_TEST_FAIL_HIT_TEST_POS_NEGATIVE));
    M3_TEST_OK(m3_navigation_test_hit_test(&nav, &layout, 5, 5, &index));
    M3_TEST_OK(m3_navigation_test_clear_fail_points());
#endif
...
static int test_event_selection(void)
{
    ...
    M3_TEST_EXPECT(nav.widget.vtable->event(NULL, &event, &handled), M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(nav.widget.vtable->event(nav.widget.ctx, NULL, &handled), M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(nav.widget.vtable->event(nav.widget.ctx, &event, NULL), M3_ERR_INVALID_ARGUMENT);
    ...
    nav.style.item_height = -1.0f;
    M3_TEST_EXPECT(nav.widget.vtable->event(&nav, &event, &handled), M3_ERR_RANGE);
    nav.style.item_height = style.item_height;
...
static int test_paint(void)
{
    ...
    M3_TEST_EXPECT(nav.widget.vtable->paint(NULL, &paint_ctx), M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(nav.widget.vtable->paint(nav.widget.ctx, NULL), M3_ERR_INVALID_ARGUMENT);
...
#ifdef M3_TESTING
    M3_TEST_OK(m3_navigation_test_set_fail_point(M3_NAV_TEST_FAIL_INDICATOR_RECT_NEGATIVE));
    M3_TEST_EXPECT(nav.widget.vtable->paint(&nav, &paint_ctx), M3_ERR_RANGE);
    M3_TEST_OK(m3_navigation_test_clear_fail_points());

    M3_TEST_OK(m3_navigation_test_set_fail_point(M3_NAV_TEST_FAIL_ITEM_RECT_NEGATIVE));
    M3_TEST_EXPECT(nav.widget.vtable->paint(&nav, &paint_ctx), M3_ERR_RANGE);
    M3_TEST_OK(m3_navigation_test_clear_fail_points());
#endif
