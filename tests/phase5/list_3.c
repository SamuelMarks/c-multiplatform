...
int main(void) {
  ...
  CMP_TEST_EXPECT(item.widget.vtable->paint(NULL, &pctx), CMP_ERR_INVALID_ARGUMENT);
  CMP_TEST_EXPECT(item.widget.vtable->paint(&item, NULL), CMP_ERR_INVALID_ARGUMENT);
