...
static int test_cupertino_action_sheet(void) {
    ...
    CMP_TEST_EXPECT(cupertino_action_sheet_style_init(NULL), CMP_ERR_INVALID_ARGUMENT);
    CMP_TEST_OK(cupertino_action_sheet_style_init(&style));

    CMP_TEST_EXPECT(cupertino_action_sheet_init(NULL, &text_backend), CMP_ERR_INVALID_ARGUMENT);
    CMP_TEST_EXPECT(cupertino_action_sheet_init(&sheet, NULL), CMP_ERR_INVALID_ARGUMENT);
    CMP_TEST_OK(cupertino_action_sheet_init(&sheet, &text_backend));

    CMP_TEST_EXPECT(cupertino_action_sheet_set_text(NULL, "T", 1, "M", 1), CMP_ERR_INVALID_ARGUMENT);
    CMP_TEST_OK(cupertino_action_sheet_set_text(&sheet, "Title", 5, "Message", 7));
    ...
    CMP_TEST_EXPECT(cupertino_action_sheet_add_action(NULL, &action1), CMP_ERR_INVALID_ARGUMENT);
    CMP_TEST_EXPECT(cupertino_action_sheet_add_action(&sheet, NULL), CMP_ERR_INVALID_ARGUMENT);
    ...
    /* Layout */
    CMP_TEST_EXPECT(cupertino_action_sheet_layout(NULL, bounds, safe_area), CMP_ERR_INVALID_ARGUMENT);
...
int main(void) {
    if (test_cupertino_action_sheet() != 0) return 1;
    return 0;
}
