...
int main(void) {
  ...
  M3_TEST_EXPECT(m3_text_test_validate_style(&alt_style),
  ...
  M3_TEST_EXPECT(m3_text_test_validate_backend(&alt_backend),
                 M3_ERR_INVALID_ARGUMENT);
  ...
  M3_TEST_EXPECT(m3_text_test_validate_measure_spec(width_spec),
                 M3_ERR_INVALID_ARGUMENT);
  ...
  M3_TEST_EXPECT(m3_text_backend_from_gfx(NULL, &text_backend),
                 M3_ERR_INVALID_ARGUMENT);
  gfx.text_vtable = NULL;
  M3_TEST_EXPECT(m3_text_backend_from_gfx(&gfx, &text_backend),
                 M3_ERR_UNSUPPORTED);
  gfx.text_vtable = &g_test_text_vtable;
  M3_TEST_OK(m3_text_backend_from_gfx(&gfx, &text_backend));

  M3_TEST_EXPECT(m3_text_font_create(NULL, &style, &font),
                 M3_ERR_INVALID_ARGUMENT);
  M3_TEST_EXPECT(m3_text_font_create(&text_backend, NULL, &font),
                 M3_ERR_INVALID_ARGUMENT);
  M3_TEST_EXPECT(m3_text_font_create(&text_backend, &style, NULL),
                 M3_ERR_INVALID_ARGUMENT);
  memset(&invalid_backend, 0, sizeof(invalid_backend));
  M3_TEST_EXPECT(m3_text_font_create(&invalid_backend, &style, &font),
                 M3_ERR_INVALID_ARGUMENT);

  text_backend.vtable = &g_test_text_vtable_no_create;
  M3_TEST_EXPECT(m3_text_font_create(&text_backend, &style, &font),
                 M3_ERR_UNSUPPORTED);
  ...
  M3_TEST_EXPECT(m3_text_font_destroy(&text_backend, saved_font),
                 M3_ERR_UNSUPPORTED);
  text_backend.vtable = &g_test_text_vtable;
  font.id = 0u;
  font.generation = 0u;
  ...
  M3_TEST_EXPECT(m3_text_measure_utf8(NULL, font, "abc", 3, &metrics),
                 M3_ERR_INVALID_ARGUMENT);
  M3_TEST_EXPECT(
      m3_text_measure_utf8(&invalid_backend, font, "abc", 3, &metrics),
      M3_ERR_INVALID_ARGUMENT);
  M3_TEST_EXPECT(m3_text_measure_utf8(&text_backend, font, NULL, 3, &metrics),
                 M3_ERR_INVALID_ARGUMENT);
  M3_TEST_EXPECT(m3_text_measure_utf8(&text_backend, font, "abc", 3, NULL),
                 M3_ERR_INVALID_ARGUMENT);

  text_backend.vtable = &g_test_text_vtable_no_measure;
  M3_TEST_EXPECT(m3_text_measure_utf8(&text_backend, font, "abc", 3, &metrics),
                 M3_ERR_UNSUPPORTED);
  text_backend.vtable = &g_test_text_vtable;
  ...
  M3_TEST_EXPECT(m3_text_measure_cstr(NULL, font, "abc", &metrics),
                 M3_ERR_INVALID_ARGUMENT);
  M3_TEST_EXPECT(m3_text_measure_cstr(&text_backend, font, NULL, &metrics),
                 M3_ERR_INVALID_ARGUMENT);
  M3_TEST_OK(m3_text_test_set_cstr_limit(2));
  M3_TEST_EXPECT(m3_text_measure_cstr(&text_backend, font, "abcd", &metrics),
                 M3_ERR_OVERFLOW);
  ...
  M3_TEST_EXPECT(m3_text_test_cstrlen(NULL, &cstr_len),
                 M3_ERR_INVALID_ARGUMENT);
  ...
  M3_TEST_EXPECT(m3_text_widget_init(NULL, &text_backend, &style, "Hi", 2),
                 M3_ERR_INVALID_ARGUMENT);
  M3_TEST_EXPECT(m3_text_widget_init(&widget, NULL, &style, "Hi", 2),
                 M3_ERR_INVALID_ARGUMENT);
  M3_TEST_EXPECT(m3_text_widget_init(&widget, &text_backend, NULL, "Hi", 2),
                 M3_ERR_INVALID_ARGUMENT);
  M3_TEST_EXPECT(m3_text_widget_init(&widget, &text_backend, &style, NULL, 2),
                 M3_ERR_INVALID_ARGUMENT);
  M3_TEST_EXPECT(
      m3_text_widget_init(&widget, &invalid_backend, &style, "Hi", 2),
      M3_ERR_INVALID_ARGUMENT);

  text_backend.vtable = &g_test_text_vtable_no_create;
  M3_TEST_EXPECT(m3_text_widget_init(&widget, &text_backend, &style, "Hi", 2),
                 M3_ERR_UNSUPPORTED);
  ...
  alt_style.size_px = 0;
  M3_TEST_EXPECT(
      m3_text_widget_init(&widget, &text_backend, &alt_style, "Hi", 2),
      M3_ERR_RANGE);
  ...
  M3_TEST_OK(widget.widget.vtable->measure(widget.widget.ctx, width_spec,
                                           height_spec, &size));
  ...
  M3_TEST_EXPECT(m3_text_widget_set_text(NULL, "Hi", 2),
                 M3_ERR_INVALID_ARGUMENT);
  M3_TEST_EXPECT(m3_text_widget_set_text(&widget, NULL, 2),
                 M3_ERR_INVALID_ARGUMENT);
  width_spec.mode = M3_MEASURE_UNSPECIFIED;
  width_spec.size = 0.0f;
  ...
  width_spec.mode = 42u;
  width_spec.size = 10.0f;
  M3_TEST_EXPECT(widget.widget.vtable->measure(widget.widget.ctx, width_spec,
                                               height_spec, &size),
                 M3_ERR_INVALID_ARGUMENT);
  width_spec.mode = M3_MEASURE_EXACTLY;
  width_spec.size = 10.0f;
  ...
  bounds.width = -1.0f;
  M3_TEST_EXPECT(widget.widget.vtable->layout(widget.widget.ctx, bounds),
                 M3_ERR_RANGE);
  bounds.width = 100.0f;
  M3_TEST_EXPECT(widget.widget.vtable->layout(NULL, bounds),
                 M3_ERR_INVALID_ARGUMENT);
  M3_TEST_OK(widget.widget.vtable->layout(widget.widget.ctx, bounds));

  M3_TEST_EXPECT(widget.widget.vtable->paint(NULL, &paint_ctx),
                 M3_ERR_INVALID_ARGUMENT);
  M3_TEST_EXPECT(widget.widget.vtable->paint(widget.widget.ctx, NULL),
                 M3_ERR_INVALID_ARGUMENT);
  paint_ctx.gfx = NULL;
  M3_TEST_EXPECT(widget.widget.vtable->paint(widget.widget.ctx, &paint_ctx),
                 M3_ERR_INVALID_ARGUMENT);
  paint_ctx.gfx = &gfx;
  ...
  M3_TEST_EXPECT(widget.widget.vtable->event(NULL, &event, &handled),
                 M3_ERR_INVALID_ARGUMENT);
  M3_TEST_EXPECT(widget.widget.vtable->event(widget.widget.ctx, NULL, &handled),
                 M3_ERR_INVALID_ARGUMENT);
  M3_TEST_EXPECT(widget.widget.vtable->event(widget.widget.ctx, &event, NULL),
                 M3_ERR_INVALID_ARGUMENT);
  M3_TEST_OK(widget.widget.vtable->event(widget.widget.ctx, &event, &handled));
  M3_TEST_ASSERT(handled == M3_FALSE);

  M3_TEST_EXPECT(widget.widget.vtable->get_semantics(NULL, &semantics),
                 M3_ERR_INVALID_ARGUMENT);
  M3_TEST_EXPECT(widget.widget.vtable->get_semantics(widget.widget.ctx, NULL),
                 M3_ERR_INVALID_ARGUMENT);
  ...
  M3_TEST_EXPECT(m3_text_widget_set_style(NULL, &style),
                 M3_ERR_INVALID_ARGUMENT);
  M3_TEST_EXPECT(m3_text_widget_set_style(&widget, NULL),
                 M3_ERR_INVALID_ARGUMENT);
  backend.fail_create = 1;
  ...
  M3_TEST_EXPECT(m3_text_widget_get_metrics(NULL, &metrics),
                 M3_ERR_INVALID_ARGUMENT);
  M3_TEST_EXPECT(m3_text_widget_get_metrics(&widget, NULL),
                 M3_ERR_INVALID_ARGUMENT);
  backend.fail_measure = 1;
