...
static int test_cupertino_watch_button(void) {
  ...
  CMP_TEST_EXPECT(cupertino_watch_button_init(NULL, &text_backend),
                  CMP_ERR_INVALID_ARGUMENT);
  CMP_TEST_EXPECT(cupertino_watch_button_init(&button, NULL),
                  CMP_ERR_INVALID_ARGUMENT);
  CMP_TEST_EXPECT(cupertino_watch_button_init(&button, &text_backend), CMP_OK);

  CMP_TEST_EXPECT(cupertino_watch_button_set_title(NULL, "Title"),
                  CMP_ERR_INVALID_ARGUMENT);
  CMP_TEST_EXPECT(cupertino_watch_button_set_title(&button, "Title"), CMP_OK);

  CMP_TEST_EXPECT(cupertino_watch_button_paint(NULL, &ctx),
                  CMP_ERR_INVALID_ARGUMENT);
  CMP_TEST_EXPECT(cupertino_watch_button_paint(&button, NULL),
                  CMP_ERR_INVALID_ARGUMENT);
