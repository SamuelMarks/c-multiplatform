...
static int test_checkbox_on_change(void *ctx, M3Checkbox *checkbox, M3Bool checked)
{
...
int main(void)
{
    ...
    M3_TEST_ASSERT(m3_near(checkbox_style.size, M3_CHECKBOX_DEFAULT_SIZE, 0.001f));

    M3_TEST_EXPECT(m3_checkbox_init(NULL, &checkbox_style, M3_FALSE), M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(m3_checkbox_init(&checkbox, NULL, M3_FALSE), M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(m3_checkbox_init(&checkbox, &checkbox_style, 2), M3_ERR_INVALID_ARGUMENT);

    other_checkbox_style = checkbox_style;
    other_checkbox_style.size = 0.0f;
    M3_TEST_EXPECT(m3_checkbox_init(&checkbox, &other_checkbox_style, M3_FALSE), M3_ERR_RANGE);
    ...
    M3_TEST_EXPECT(m3_checkbox_get_checked(NULL, &state), M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(m3_checkbox_get_checked(&checkbox, NULL), M3_ERR_INVALID_ARGUMENT);

    M3_TEST_EXPECT(m3_checkbox_set_label(NULL, "A", 1), M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(m3_checkbox_set_label(&checkbox, NULL, 1), M3_ERR_INVALID_ARGUMENT);
    ...
    counter.fail = 0;
    M3_TEST_OK(m3_checkbox_set_on_change(&checkbox, test_checkbox_on_change, &counter));
    ...
    M3_TEST_EXPECT(checkbox.widget.vtable->measure(checkbox.widget.ctx, width_spec, height_spec, &size), M3_ERR_RANGE);

    width_spec.mode = M3_MEASURE_UNSPECIFIED;
    width_spec.size = 0.0f;
    height_spec.mode = M3_MEASURE_UNSPECIFIED;
    height_spec.size = 0.0f;
    M3_TEST_OK(checkbox.widget.vtable->measure(checkbox.widget.ctx, width_spec, height_spec, &size));
    ...
    bounds.width = -1.0f;
    bounds.height = 10.0f;
    M3_TEST_EXPECT(checkbox.widget.vtable->layout(checkbox.widget.ctx, bounds), M3_ERR_RANGE);
    ...
    M3_TEST_EXPECT(checkbox.widget.vtable->paint(NULL, &paint_ctx), M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(checkbox.widget.vtable->paint(checkbox.widget.ctx, NULL), M3_ERR_INVALID_ARGUMENT);

    gfx.vtable = NULL;
    M3_TEST_EXPECT(checkbox.widget.vtable->paint(checkbox.widget.ctx, &paint_ctx), M3_ERR_INVALID_ARGUMENT);
    gfx.vtable = &g_test_vtable_no_rect;
    M3_TEST_EXPECT(checkbox.widget.vtable->paint(checkbox.widget.ctx, &paint_ctx), M3_ERR_UNSUPPORTED);
    ...
    M3_TEST_ASSERT(m3_color_near(backend.last_line_color, checkbox_style.checked.mark, 0.001f));
    ...
    M3_TEST_EXPECT(checkbox.widget.vtable->event(NULL, &event, &handled), M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(checkbox.widget.vtable->event(checkbox.widget.ctx, NULL, &handled), M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(checkbox.widget.vtable->event(checkbox.widget.ctx, &event, NULL), M3_ERR_INVALID_ARGUMENT);
    ...
    checkbox.pressed = M3_TRUE;
    M3_TEST_OK(init_pointer_event(&event, M3_INPUT_POINTER_UP, 5, 6));
    M3_TEST_EXPECT(checkbox.widget.vtable->event(checkbox.widget.ctx, &event, &handled), M3_ERR_IO);
    M3_TEST_ASSERT(checkbox.checked == M3_TRUE);
    counter.fail = 0;

    M3_TEST_EXPECT(checkbox.widget.vtable->get_semantics(NULL, &semantics), M3_ERR_INVALID_ARGUMENT);
    ...
    M3_TEST_ASSERT(m3_near(switch_style.track_width, M3_SWITCH_DEFAULT_TRACK_WIDTH, 0.001f));

    M3_TEST_EXPECT(m3_switch_init(NULL, &switch_style, M3_FALSE), M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(m3_switch_init(&sw, NULL, M3_FALSE), M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(m3_switch_init(&sw, &switch_style, 2), M3_ERR_INVALID_ARGUMENT);

    other_switch_style = switch_style;
    other_switch_style.track_width = 10.0f;
    other_switch_style.track_height = 20.0f;
    M3_TEST_EXPECT(m3_switch_init(&sw, &other_switch_style, M3_FALSE), M3_ERR_RANGE);
    ...
    M3_TEST_EXPECT(m3_switch_set_on_change(NULL, test_switch_on_change, NULL), M3_ERR_INVALID_ARGUMENT);
    counter.calls = 0;
    counter.fail = 0;
    ...
    width_spec.mode = M3_MEASURE_AT_MOST;
    width_spec.size = -1.0f;
    M3_TEST_EXPECT(sw.widget.vtable->measure(sw.widget.ctx, width_spec, height_spec, &size), M3_ERR_RANGE);

    width_spec.mode = M3_MEASURE_UNSPECIFIED;
    width_spec.size = 0.0f;
    height_spec.mode = M3_MEASURE_UNSPECIFIED;
    height_spec.size = 0.0f;
    M3_TEST_OK(sw.widget.vtable->measure(sw.widget.ctx, width_spec, height_spec, &size));
    ...
    M3_TEST_EXPECT(sw.widget.vtable->event(sw.widget.ctx, NULL, &handled), M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(sw.widget.vtable->event(sw.widget.ctx, &event, NULL), M3_ERR_INVALID_ARGUMENT);
    ...
    sw.pressed = M3_TRUE;
    M3_TEST_OK(init_pointer_event(&event, M3_INPUT_POINTER_UP, 5, 6));
    M3_TEST_EXPECT(sw.widget.vtable->event(sw.widget.ctx, &event, &handled), M3_ERR_IO);
    ...
    M3_TEST_EXPECT(m3_radio_init(NULL, &radio_style, M3_FALSE), M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(m3_radio_init(&radio, NULL, M3_FALSE), M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(m3_radio_init(&radio, &radio_style, 2), M3_ERR_INVALID_ARGUMENT);

    other_radio_style = radio_style;
    other_radio_style.dot_radius = 20.0f;
    M3_TEST_EXPECT(m3_radio_init(&radio, &other_radio_style, M3_FALSE), M3_ERR_RANGE);
    ...
    M3_TEST_EXPECT(m3_radio_set_on_change(NULL, test_radio_on_change, NULL), M3_ERR_INVALID_ARGUMENT);
    counter.calls = 0;
    counter.fail = 0;
    ...
    width_spec.mode = M3_MEASURE_AT_MOST;
    width_spec.size = -1.0f;
    M3_TEST_EXPECT(radio.widget.vtable->measure(radio.widget.ctx, width_spec, height_spec, &size), M3_ERR_RANGE);

    width_spec.mode = M3_MEASURE_UNSPECIFIED;
    width_spec.size = 0.0f;
    height_spec.mode = M3_MEASURE_UNSPECIFIED;
    height_spec.size = 0.0f;
    M3_TEST_OK(radio.widget.vtable->measure(radio.widget.ctx, width_spec, height_spec, &size));
    ...
    bounds.width = -1.0f;
    bounds.height = 10.0f;
    M3_TEST_EXPECT(radio.widget.vtable->layout(radio.widget.ctx, bounds), M3_ERR_RANGE);
    ...
    M3_TEST_EXPECT(radio.widget.vtable->event(radio.widget.ctx, &event, NULL), M3_ERR_INVALID_ARGUMENT);
    ...
    radio.pressed = M3_TRUE;
    M3_TEST_OK(init_pointer_event(&event, M3_INPUT_POINTER_UP, 5, 6));
    M3_TEST_EXPECT(radio.widget.vtable->event(radio.widget.ctx, &event, &handled), M3_ERR_IO);
