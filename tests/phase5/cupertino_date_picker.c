...
static int test_cupertino_date_picker(void) {
  ...
  CMP_TEST_EXPECT(cupertino_date_picker_init(NULL, &text_backend),
                  CMP_ERR_INVALID_ARGUMENT);
  CMP_TEST_EXPECT(cupertino_date_picker_init(&picker, NULL),
                  CMP_ERR_INVALID_ARGUMENT);
  CMP_TEST_EXPECT(cupertino_date_picker_init(&picker, &text_backend), CMP_OK);

  CMP_TEST_EXPECT(cupertino_date_picker_set_datetime(NULL, 2026, 3, 8, 12, 0),
                  CMP_ERR_INVALID_ARGUMENT);
  CMP_TEST_EXPECT(
      cupertino_date_picker_set_datetime(&picker, 2026, 3, 8, 12, 0), CMP_OK);

  CMP_TEST_EXPECT(cupertino_date_picker_paint(NULL, &ctx),
                  CMP_ERR_INVALID_ARGUMENT);
  CMP_TEST_EXPECT(cupertino_date_picker_paint(&picker, NULL),
                  CMP_ERR_INVALID_ARGUMENT);
