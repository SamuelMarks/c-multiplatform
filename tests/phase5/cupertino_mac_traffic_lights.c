...
static int test_cupertino_mac_traffic_lights(void) {
    ...
    CMP_TEST_EXPECT(cupertino_mac_traffic_lights_init(NULL), CMP_ERR_INVALID_ARGUMENT);
    CMP_TEST_EXPECT(cupertino_mac_traffic_lights_init(&lights), CMP_OK);

    CMP_TEST_EXPECT(cupertino_mac_traffic_lights_paint(NULL, &ctx), CMP_ERR_INVALID_ARGUMENT);
    CMP_TEST_EXPECT(cupertino_mac_traffic_lights_paint(&lights, NULL), CMP_ERR_INVALID_ARGUMENT);
