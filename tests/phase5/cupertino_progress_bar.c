...
static int test_cupertino_progress_bar(void) {
    ...
    CMP_TEST_EXPECT(cupertino_progress_bar_set_progress(NULL, 0.5f, CMP_FALSE), CMP_ERR_INVALID_ARGUMENT);
    CMP_TEST_EXPECT(cupertino_progress_bar_set_progress(&bar, 0.5f, CMP_FALSE), CMP_OK);
    CMP_TEST_EXPECT((int)(bar.progress * 10.0f), 5);

    CMP_TEST_EXPECT(cupertino_progress_bar_update(NULL, 0.1), CMP_ERR_INVALID_ARGUMENT);
    CMP_TEST_EXPECT(cupertino_progress_bar_update(&bar, 0.1), CMP_OK);

    CMP_TEST_EXPECT(cupertino_progress_bar_paint(NULL, &ctx), CMP_ERR_INVALID_ARGUMENT);
    CMP_TEST_EXPECT(cupertino_progress_bar_paint(&bar, NULL), CMP_ERR_INVALID_ARGUMENT);
