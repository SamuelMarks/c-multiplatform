...
static int test_cupertino_alert(void) {
    ...
    CMP_TEST_EXPECT(cupertino_alert_init(NULL, &text_backend), CMP_ERR_INVALID_ARGUMENT);
    ...
    CMP_TEST_EXPECT(cupertino_alert_set_content(NULL, "Title", "Message"), CMP_ERR_INVALID_ARGUMENT);
    CMP_TEST_EXPECT(cupertino_alert_set_content(&alert, "Title", "Message"), CMP_OK);

    CMP_TEST_EXPECT(cupertino_alert_add_action(NULL, "OK", CMP_FALSE), CMP_ERR_INVALID_ARGUMENT);
    CMP_TEST_EXPECT(cupertino_alert_add_action(&alert, NULL, CMP_FALSE), CMP_ERR_INVALID_ARGUMENT);
    ...
    CMP_TEST_EXPECT(cupertino_alert_paint(&alert, NULL), CMP_ERR_INVALID_ARGUMENT);
