...
static int test_widget_measure(void *widget, CMPMeasureSpec width,
  ...
  if (width.mode != CMP_MEASURE_UNSPECIFIED &&
      width.mode != CMP_MEASURE_EXACTLY && width.mode != CMP_MEASURE_AT_MOST) {
    return CMP_ERR_INVALID_ARGUMENT;
  }
  ...
  if (height.mode != CMP_MEASURE_UNSPECIFIED && height.size < 0.0f) {
    return CMP_ERR_RANGE;
  }
...
static int test_widget_event(void *widget, const CMPInputEvent *event,
  ...
  *out_handled = state->handle_events ? CMP_TRUE : CMP_FALSE;
  return CMP_OK;
}
...
static const CMPWidgetVTable g_test_widget_vtable_no_measure = {
    NULL,
    test_widget_layout,
    test_widget_paint,
    test_widget_event,
    ...
    test_widget_destroy};
static const CMPWidgetVTable g_test_widget_vtable_no_layout = {
    test_widget_measure,       NULL,
    test_widget_paint,         test_widget_event,
    test_widget_get_semantics, test_widget_destroy};
static const CMPWidgetVTable g_test_widget_vtable_no_paint = {
    test_widget_measure, test_widget_layout,        NULL,
    test_widget_event,   test_widget_get_semantics, test_widget_destroy};
static const CMPWidgetVTable g_test_widget_vtable_no_event = {
    test_widget_measure,       test_widget_layout, test_widget_paint, NULL,
    test_widget_get_semantics, test_widget_destroy};

static int test_widget_init(TestScaffoldWidget *state, CMPScalar width,
                            CMPScalar height) {
  if (state == NULL) {
    return CMP_ERR_INVALID_ARGUMENT;
  }
...
static int test_scaffold_helpers(void) {
  ...
  CMP_TEST_OK(m3_scaffold_test_compute_fab_target(&style, 40.0f, &offset));
  ...
  widget.widget.flags = CMP_WIDGET_FLAG_HIDDEN;
  CMP_TEST_OK(m3_scaffold_test_widget_is_visible(&widget.widget, &visible));
  CMP_TEST_ASSERT(visible == CMP_FALSE);
  ...
  CMP_TEST_OK(m3_scaffold_test_event_get_position(&event, &visible, &x, &y));
  CMP_TEST_ASSERT(visible == CMP_TRUE);
  CMP_TEST_ASSERT(x == 5.0f);
  CMP_TEST_ASSERT(y == 6.0f);
  memset(&event, 0, sizeof(event));
  ...
  CMP_TEST_OK(m3_scaffold_test_event_get_position(&event, &visible, &x, &y));
  CMP_TEST_ASSERT(visible == CMP_TRUE);
  CMP_TEST_ASSERT(x == 7.0f);
  CMP_TEST_ASSERT(y == 8.0f);
  memset(&event, 0, sizeof(event));
  event.type = CMP_INPUT_KEY_DOWN;
  CMP_TEST_OK(m3_scaffold_test_event_get_position(&event, &visible, &x, &y));
  CMP_TEST_ASSERT(visible == CMP_FALSE);
...
static int test_scaffold_style_init(void) {
  ...
  CMP_TEST_EXPECT(m3_scaffold_style_init(NULL), CMP_ERR_INVALID_ARGUMENT);
  CMP_TEST_OK(m3_scaffold_style_init(&style));
  CMP_TEST_ASSERT(style.padding.left == M3_SCAFFOLD_DEFAULT_PADDING_X);
  CMP_TEST_ASSERT(style.padding.right == M3_SCAFFOLD_DEFAULT_PADDING_X);
  CMP_TEST_ASSERT(style.padding.top == M3_SCAFFOLD_DEFAULT_PADDING_Y);
  CMP_TEST_ASSERT(style.padding.bottom == M3_SCAFFOLD_DEFAULT_PADDING_Y);
...
static int test_scaffold_init_and_setters(void) {
  ...
  CMP_TEST_OK(m3_scaffold_style_init(&style));
  CMP_TEST_OK(m3_scaffold_set_style(&scaffold, &style));
  ...
  safe_area.right = 0.0f;
  safe_area.bottom = 0.0f;
  CMP_TEST_OK(m3_scaffold_get_safe_area(&scaffold, &safe_area));
  CMP_TEST_ASSERT(safe_area.left == 1.0f);
  CMP_TEST_ASSERT(safe_area.top == 2.0f);
  CMP_TEST_ASSERT(safe_area.right == 3.0f);
...
static int test_scaffold_measure(void) {
  ...
  CMP_TEST_OK(test_widget_init(&body, 200.0f, 100.0f));
  CMP_TEST_OK(test_widget_init(&top_bar, 300.0f, 50.0f));
  CMP_TEST_OK(test_widget_init(&bottom_bar, 280.0f, 40.0f));
  CMP_TEST_OK(test_widget_init(&fab, 50.0f, 50.0f));
...
static int test_scaffold_layout(void) {
  ...
  CMP_TEST_OK(test_widget_init(&body, 100.0f, 100.0f));
  CMP_TEST_OK(test_widget_init(&top_bar, 300.0f, 56.0f));
  CMP_TEST_OK(test_widget_init(&bottom_bar, 300.0f, 64.0f));
  CMP_TEST_OK(test_widget_init(&fab, 56.0f, 56.0f));
  ...
  CMP_TEST_ASSERT(last.x == 10.0f);
  CMP_TEST_ASSERT(last.y == 20.0f);
  CMP_TEST_ASSERT(last.width == 340.0f);
  CMP_TEST_ASSERT(last.height == 56.0f);

  last = bottom_bar.last_bounds;
  CMP_TEST_ASSERT(last.x == 10.0f);
  CMP_TEST_ASSERT(last.y == 546.0f);
  CMP_TEST_ASSERT(last.width == 340.0f);
  CMP_TEST_ASSERT(last.height == 64.0f);

  last = body.last_bounds;
  CMP_TEST_ASSERT(last.x == 22.0f);
  CMP_TEST_ASSERT(last.y == 84.0f);
  CMP_TEST_ASSERT(last.width == 316.0f);
  CMP_TEST_ASSERT(last.height == 454.0f);

  last = snackbar.last_bounds;
  CMP_TEST_ASSERT(last.width == 200.0f);
  CMP_TEST_ASSERT(last.height == 40.0f);
  CMP_TEST_ASSERT(last.x == 80.0f);
  CMP_TEST_ASSERT(last.y == 494.0f);

  last = fab.last_bounds;
  CMP_TEST_ASSERT(last.width == 56.0f);
  CMP_TEST_ASSERT(last.height == 56.0f);
  CMP_TEST_ASSERT(last.x == 278.0f);
  CMP_TEST_ASSERT(last.y == 430.0f);
...
static int test_scaffold_paint(void) {
  ...
  CMP_TEST_OK(m3_scaffold_style_init(&style));
  CMP_TEST_OK(test_widget_init(&body, 100.0f, 100.0f));
  CMP_TEST_OK(test_widget_init(&top_bar, 100.0f, 10.0f));
  CMP_TEST_OK(test_widget_init(&bottom_bar, 100.0f, 10.0f));
  ...
  memset(&ctx, 0, sizeof(ctx));
  CMP_TEST_OK(scaffold.widget.vtable->paint(scaffold.widget.ctx, &ctx));
  CMP_TEST_ASSERT(body.paint_calls == 1);
  CMP_TEST_ASSERT(top_bar.paint_calls == 1);
  CMP_TEST_ASSERT(bottom_bar.paint_calls == 1);
  CMP_TEST_ASSERT(snackbar.paint_calls == 1);
  CMP_TEST_ASSERT(fab.paint_calls == 1);
...

  {
    ...
    body.widget.flags = CMP_WIDGET_FLAG_HIDDEN;
    CMP_TEST_OK(scaffold.widget.vtable->paint(scaffold.widget.ctx, &ctx));
    CMP_TEST_ASSERT(body.paint_calls == prev_calls);
...
static int test_scaffold_event(void) {
  ...
  CMP_TEST_OK(m3_scaffold_style_init(&style));
  CMP_TEST_OK(test_widget_init(&body, 100.0f, 100.0f));
  CMP_TEST_OK(test_widget_init(&top_bar, 100.0f, 20.0f));
  CMP_TEST_OK(test_widget_init(&bottom_bar, 100.0f, 20.0f));
  CMP_TEST_OK(test_widget_init(&fab, 10.0f, 10.0f));
  CMP_TEST_OK(test_widget_init(&snackbar, 50.0f, 10.0f));
  ...
  CMP_TEST_OK(
      scaffold.widget.vtable->event(scaffold.widget.ctx, &event, &handled));
  CMP_TEST_ASSERT(handled == CMP_TRUE);
  CMP_TEST_ASSERT(body.event_calls == 1);
  ...
  scaffold.widget.flags = CMP_WIDGET_FLAG_HIDDEN;
  CMP_TEST_OK(
      scaffold.widget.vtable->event(scaffold.widget.ctx, &event, &handled));
  CMP_TEST_ASSERT(handled == CMP_FALSE);
  ...
  scaffold.widget.flags = CMP_WIDGET_FLAG_DISABLED;
  CMP_TEST_OK(
      scaffold.widget.vtable->event(scaffold.widget.ctx, &event, &handled));
  CMP_TEST_ASSERT(handled == CMP_FALSE);
  scaffold.widget.flags = 0u;
...
static int test_scaffold_semantics_destroy(void) {
  ...
  scaffold.widget.flags = CMP_WIDGET_FLAG_DISABLED | CMP_WIDGET_FLAG_FOCUSABLE;
  CMP_TEST_OK(
  ...
  CMP_TEST_ASSERT(scaffold.body == NULL);
  CMP_TEST_ASSERT(scaffold.top_bar == NULL);
  CMP_TEST_ASSERT(scaffold.bottom_bar == NULL);
  CMP_TEST_ASSERT(scaffold.fab == NULL);
  CMP_TEST_ASSERT(scaffold.snackbar == NULL);
  return 0;
}
...
static int test_scaffold_step(void) {
  ...
  CMP_TEST_OK(m3_scaffold_step(&scaffold, 0.1f, &changed));
  CMP_TEST_ASSERT(changed == CMP_TRUE);
  CMP_TEST_ASSERT(scaffold.fab_offset > 0.0f);
...
int main(void) {
  ...
  CMP_TEST_ASSERT(test_scaffold_init_and_setters() == 0);
  CMP_TEST_ASSERT(test_scaffold_measure() == 0);
  CMP_TEST_ASSERT(test_scaffold_layout() == 0);
  CMP_TEST_ASSERT(test_scaffold_paint() == 0);
