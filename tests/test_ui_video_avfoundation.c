...
int main(void) {
  ...
  if (ui_video_decoder_avfoundation_get_backend(NULL) !=
      UI_ERROR_INVALID_ARGUMENT) {
    fprintf(stderr, "Expected UI_ERROR_INVALID_ARGUMENT for NULL backend\n");
    return 1;
  }

  if (ui_video_decoder_avfoundation_get_backend(&backend) != UI_ERROR_UNKNOWN) {
  ...
  if (ui_video_decoder_get_default_backend(&backend) != UI_ERROR_UNKNOWN) {
    fprintf(stderr,
            "Expected UI_ERROR_UNKNOWN for default AVFoundation stub\n");
    return 1;
  }
#endif
