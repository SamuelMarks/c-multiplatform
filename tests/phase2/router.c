...
int main(void)
{
    ...
    M3_TEST_EXPECT(m3_router_test_cstrlen(NULL, &max_value), M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(m3_router_test_cstrlen("x", NULL), M3_ERR_INVALID_ARGUMENT);
    M3_TEST_OK(m3_router_test_slice_set(NULL, "x", 1));
    temp_slice.data = "bar";
    temp_slice.length = 3;
    M3_TEST_EXPECT(m3_router_test_slice_equals(NULL, "bar", 3, &match), M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(m3_router_test_slice_equals(&temp_slice, NULL, 3, &match), M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(m3_router_test_slice_equals(&temp_slice, "bar", 3, NULL), M3_ERR_INVALID_ARGUMENT);
    ...
    M3_TEST_EXPECT(m3_uri_query_find(NULL, "foo", &value, &found), M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(m3_uri_query_find(&uri, NULL, &value, &found), M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(m3_uri_query_find(&uri, "foo", NULL, &found), M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(m3_uri_query_find(&uri, "foo", &value, NULL), M3_ERR_INVALID_ARGUMENT);
    ...
    M3_TEST_EXPECT(m3_route_test_match(NULL, "/a", NULL, 0, NULL, &match), M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(m3_route_test_match("/a", NULL, NULL, 0, NULL, &match), M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(m3_route_test_match("/a", "/a", NULL, 0, &param_count, &match), M3_ERR_INVALID_ARGUMENT);
    ...
    M3_TEST_OK(m3_route_test_match("/users/:id", "/users/42", params, 2, &param_count, &match));
    M3_TEST_ASSERT(match == M3_TRUE);
    ...
    M3_TEST_EXPECT(m3_router_test_copy_path(NULL, "/", 1, NULL), M3_ERR_INVALID_ARGUMENT);
    router_copy.allocator = m3_alloc;
    M3_TEST_EXPECT(m3_router_test_copy_path(&router_copy, "/", max_value, &path_copy), M3_ERR_OVERFLOW);
    router_copy.allocator = null_alloc;
    M3_TEST_EXPECT(m3_router_test_copy_path(&router_copy, "/", 1, &path_copy), M3_ERR_OUT_OF_MEMORY);
    ...
    M3_TEST_EXPECT(m3_router_navigate(NULL, "/", &component), M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(m3_router_navigate(&router, NULL, &component), M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(m3_router_get_current(&router, NULL, NULL), M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(m3_router_navigate_uri(NULL, "app://example.com", &component), M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(m3_router_navigate_uri(&router, NULL, &component), M3_ERR_INVALID_ARGUMENT);
    ...
    M3_TEST_EXPECT(m3_router_get_current(NULL, &current_path, &component), M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(m3_router_clear(NULL), M3_ERR_INVALID_ARGUMENT);

    M3_TEST_EXPECT(m3_router_navigate(&router, "/missing", &component), M3_ERR_NOT_FOUND);
    ...
    M3_TEST_OK(m3_router_navigate_uri(&router, "app://example.com/users/42?x=1", &component));
    ...
    M3_TEST_EXPECT(m3_router_navigate(&router, "/users/99", &component), M3_ERR_UNKNOWN);
    user_ctx.build_fail = 0;

    alloc.fail_alloc_on = alloc.alloc_calls + 1;
    M3_TEST_EXPECT(m3_router_navigate(&router, "/users/77", &component), M3_ERR_OUT_OF_MEMORY);
    alloc.fail_alloc_on = 0;

    M3_TEST_OK(m3_router_navigate(&router, "/users/77", &component));
    M3_TEST_OK(m3_router_navigate(&router, "/files/a/b", &component));
    M3_TEST_EXPECT(m3_router_navigate(&router, "/users/88", &component), M3_ERR_OVERFLOW);
    ...
    M3_TEST_EXPECT(m3_router_get_current(&router, &current_path, NULL), M3_ERR_NOT_FOUND);
    ...
    M3_TEST_OK(m3_router_test_set_cstr_limit(3));
    M3_TEST_EXPECT(m3_router_navigate(&router, "abcd", &component), M3_ERR_OVERFLOW);
    M3_TEST_EXPECT(m3_router_navigate_uri(&router, "abcd", &component), M3_ERR_OVERFLOW);
    M3_TEST_EXPECT(m3_route_test_match("abcd", "/a", NULL, 0, NULL, &match), M3_ERR_OVERFLOW);
    M3_TEST_EXPECT(m3_route_test_match("a", "/abcd", NULL, 0, NULL, &match), M3_ERR_OVERFLOW);
