...
int main(void) {
  ...
  CMP_TEST_EXPECT(cmp_spring_set_tolerance(NULL, 0.1f, 0.1f),
                 CMP_ERR_INVALID_ARGUMENT);
  CMP_TEST_EXPECT(cmp_spring_set_tolerance(&spring, -0.1f, 0.1f), CMP_ERR_RANGE);
  CMP_TEST_EXPECT(cmp_spring_set_tolerance(&spring, 0.1f, -0.1f), CMP_ERR_RANGE);
  CMP_TEST_OK(cmp_spring_set_tolerance(&spring, 0.001f, 0.001f));

  CMP_TEST_EXPECT(cmp_spring_step(NULL, 0.1f, &finished),
                 CMP_ERR_INVALID_ARGUMENT);
  CMP_TEST_EXPECT(cmp_spring_step(&spring, 0.1f, NULL), CMP_ERR_INVALID_ARGUMENT);
  ...
  CMP_TEST_EXPECT(cmp_anim_controller_step(&controller, 0.1f, &value, NULL),
                 CMP_ERR_INVALID_ARGUMENT);
  CMP_TEST_EXPECT(cmp_anim_controller_step(&controller, 0.1f, &value, &finished),
                 CMP_ERR_STATE);
  CMP_TEST_EXPECT(cmp_anim_controller_step(&controller, -0.1f, &value, &finished),
                 CMP_ERR_RANGE);
  ...
  CMP_TEST_EXPECT(
      ...
      CMP_ERR_INVALID_ARGUMENT);
  CMP_TEST_EXPECT(cmp_anim_controller_start_spring(&controller, 0.0f, 1.0f, 0.0f,
                                                 8.0f, 1.0f),
                 CMP_ERR_RANGE);
  ...
  CMP_TEST_OK(cmp_spring_init(&controller.spring, 1.0f, 1.0f, 60.0f, 8.0f, 1.0f));
  controller.running = CMP_TRUE;
