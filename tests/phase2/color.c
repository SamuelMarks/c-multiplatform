...
int main(void)
{
    ...
    M3_TEST_EXPECT(m3_color_test_argb_to_xyz(0, NULL, &y, &z), M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(m3_color_test_argb_to_xyz(0, &x, NULL, &z), M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(m3_color_test_argb_to_xyz(0, &x, &y, NULL), M3_ERR_INVALID_ARGUMENT);

    M3_TEST_EXPECT(m3_color_test_xyz_to_argb(0.0f, 0.0f, 0.0f, NULL, &in_gamut), M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(m3_color_test_xyz_to_argb(0.0f, 0.0f, 0.0f, &argb2, NULL), M3_ERR_INVALID_ARGUMENT);

    M3_TEST_EXPECT(m3_color_test_xyz_to_lab(0.0f, 0.0f, 0.0f, NULL, &a_lab, &b_lab), M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(m3_color_test_xyz_to_lab(0.0f, 0.0f, 0.0f, &l, NULL, &b_lab), M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(m3_color_test_xyz_to_lab(0.0f, 0.0f, 0.0f, &l, &a_lab, NULL), M3_ERR_INVALID_ARGUMENT);

    M3_TEST_EXPECT(m3_color_test_lab_to_xyz(0.0f, 0.0f, 0.0f, NULL, &y, &z), M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(m3_color_test_lab_to_xyz(0.0f, 0.0f, 0.0f, &x, NULL, &z), M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(m3_color_test_lab_to_xyz(0.0f, 0.0f, 0.0f, &x, &y, NULL), M3_ERR_INVALID_ARGUMENT);

    M3_TEST_EXPECT(m3_color_test_lch_to_argb(0.0f, 0.0f, 0.0f, NULL, &in_gamut), M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(m3_color_test_lch_to_argb(0.0f, 0.0f, 0.0f, &argb2, NULL), M3_ERR_INVALID_ARGUMENT);
    ...
    M3_TEST_EXPECT(m3_color_test_set_tonal_palette_init_fail_call(-1), M3_ERR_RANGE);
    M3_TEST_EXPECT(m3_color_test_set_tonal_palette_tone_fail_call(-1), M3_ERR_RANGE);

    M3_TEST_EXPECT(m3_color_argb_from_rgba(0, 0, 0, 0, NULL), M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(m3_color_rgba_from_argb(0, NULL, &g, &b, &a), M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(m3_color_rgba_from_argb(0, &r, NULL, &b, &a), M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(m3_color_rgba_from_argb(0, &r, &g, NULL, &a), M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(m3_color_rgba_from_argb(0, &r, &g, &b, NULL), M3_ERR_INVALID_ARGUMENT);
    ...
    M3_TEST_EXPECT(m3_color_test_xyz_to_argb(0.0f, 0.0f, 0.0f, &argb2, &in_gamut), M3_ERR_UNKNOWN);
    ...
    M3_TEST_EXPECT(m3_color_test_lch_to_argb(0.0f, 10.0f, 50.0f, &argb2, &in_gamut), M3_ERR_UNKNOWN);
    ...
    M3_TEST_ASSERT(m3_color_near_u8((m3_u8)(hct2.tone + 0.5f), (m3_u8)(hct.tone + 0.5f), 2));
    ...
    M3_TEST_EXPECT(m3_tonal_palette_init(NULL, 0.0f, 1.0f), M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(m3_tonal_palette_init(&palette, 0.0f, -1.0f), M3_ERR_RANGE);
    M3_TEST_OK(m3_tonal_palette_init(&palette, -10.0f, 20.0f));
    M3_TEST_EXPECT(m3_tonal_palette_tone_argb(NULL, 50.0f, &argb2), M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(m3_tonal_palette_tone_argb(&palette, 50.0f, NULL), M3_ERR_INVALID_ARGUMENT);
    M3_TEST_EXPECT(m3_tonal_palette_tone_argb(&palette, -1.0f, &argb2), M3_ERR_RANGE);
    M3_TEST_EXPECT(m3_tonal_palette_tone_argb(&palette, 101.0f, &argb2), M3_ERR_RANGE);
    M3_TEST_OK(m3_tonal_palette_tone_argb(&palette, 50.0f, &argb2));

    M3_TEST_EXPECT(m3_scheme_generate(0xFF000000u, M3_FALSE, NULL), M3_ERR_INVALID_ARGUMENT);

    M3_TEST_OK(m3_color_test_set_argb_to_xyz_fail(M3_TRUE));
    M3_TEST_EXPECT(m3_scheme_generate(0xFF3366FFu, M3_FALSE, &scheme_light), M3_ERR_UNKNOWN);
    M3_TEST_OK(m3_color_test_set_argb_to_xyz_fail(M3_FALSE));
    ...
    for (i = 1; i <= 23; ++i) {
        ...
        M3_TEST_EXPECT(m3_scheme_generate(0xFF3366FFu, M3_TRUE, &scheme_dark), M3_ERR_UNKNOWN);
    }
    for (i = 1; i <= 23; ++i) {
        M3_TEST_OK(m3_color_test_set_tonal_palette_tone_fail_call(i));
        M3_TEST_EXPECT(m3_scheme_generate(0xFF3366FFu, M3_FALSE, &scheme_light), M3_ERR_UNKNOWN);
