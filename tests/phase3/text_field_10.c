...
static int test_draw_rect(void *ctx, const CMPRect *bounds, CMPColor color,
...
int main(void) {
  ...
  CMP_TEST_EXPECT(field.widget.vtable->measure(NULL, mw, mh, &size),
                  CMP_ERR_INVALID_ARGUMENT);
  CMP_TEST_EXPECT(field.widget.vtable->measure(&field, mw, mh, NULL),
                  CMP_ERR_INVALID_ARGUMENT);
  CMP_TEST_EXPECT(field.widget.vtable->measure(&field, bad_spec, mh, &size),
                  CMP_ERR_INVALID_ARGUMENT);
  CMP_TEST_EXPECT(field.widget.vtable->measure(&field, mw, bad_spec, &size),
                  CMP_ERR_INVALID_ARGUMENT);
  ...
  /* The fail points for style init are for text_field_init, validate_style
   * doesn't use them */
  ...
  CMP_TEST_EXPECT(field.widget.vtable->measure(&field, mw, mh, &size),
                  CMP_ERR_IO);
  ...
  /* Make text metrics valid so it skips the fail for text metrics, wait, if I
   * set it to valid, it skips update_text_metrics! But then it goes to label
   * metrics. If I set fail point 4u, it will fail in label_metrics! Because
   * label_metrics ALSO checks 4u! */
  ...
  cmp_text_field_test_set_fail_point(
      4u); /* label measure uses same fail point */
  CMP_TEST_EXPECT(field.widget.vtable->measure(&field, mw, mh, &size),
                  CMP_ERR_IO);
  cmp_text_field_test_set_fail_point(0u);
  ...
  cmp_text_field_test_set_fail_point(
      4u); /* placeholder measure uses same fail point */
  CMP_TEST_EXPECT(field.widget.vtable->measure(&field, mw, mh, &size),
                  CMP_ERR_IO);
  cmp_text_field_test_set_fail_point(0u);
  ...
  CMP_TEST_EXPECT(field.widget.vtable->layout(NULL, field.bounds),
                  CMP_ERR_INVALID_ARGUMENT);
  CMPRect bad_bounds = field.bounds;
  bad_bounds.width = -1.0f;
  CMP_TEST_EXPECT(field.widget.vtable->layout(&field, bad_bounds),
                  CMP_ERR_RANGE);
  ...
  field.text_font.id = 1;
  field.text_font.generation = 1;
  /* We also need field.text_metrics.width and others to not be 0 so placeholder
   * is painted? No, placeholder metrics update is done REGARDLESS of whether
   * it's painted! */
  cmp_text_field_test_set_fail_point(4u); /* text measure */
  CMP_TEST_EXPECT(field.widget.vtable->paint(&field, &pctx), CMP_ERR_IO);
  cmp_text_field_test_set_fail_point(0u);
