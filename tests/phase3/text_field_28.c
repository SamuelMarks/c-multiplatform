...
int main(void) {
    ...
    CMP_TEST_EXPECT(cmp_text_field_test_color_set(&color, 1.0f, 1.0f, 1.0f, 1.0f), CMP_ERR_IO);
    
    cmp_text_field_test_set_overflow_fail_after(1);
    cmp_usize val;
    CMP_TEST_EXPECT(cmp_text_field_test_add_overflow(1, 1, &val), CMP_ERR_OVERFLOW);
