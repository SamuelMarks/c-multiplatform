...
static int test_gfx_begin_frame(void *gfx, M3Handle window, m3_i32 width, m3_i32 height, M3Scalar dpi_scale)
{
    TestBackend *backend;
...
static int test_gfx_draw_line(void *gfx, M3Scalar x0, M3Scalar y0, M3Scalar x1, M3Scalar y1, M3Color color, M3Scalar thickness)
{
    TestBackend *backend;
...
static int test_gfx_create_texture(void *gfx, m3_i32 width, m3_i32 height, m3_u32 format, const void *pixels, m3_usize size,
...
static int test_gfx_update_texture(void *gfx, M3Handle texture, m3_i32 x, m3_i32 y, m3_i32 width, m3_i32 height,
...
static int test_gfx_draw_texture(void *gfx, M3Handle texture, const M3Rect *src, const M3Rect *dst, M3Scalar opacity)
{
    TestBackend *backend;
...
static const M3GfxVTable g_test_gfx_vtable = {
    ...
    test_gfx_push_clip,
    test_gfx_pop_clip,
    test_gfx_set_transform,
    test_gfx_create_texture,
    test_gfx_update_texture,
    test_gfx_destroy_texture,
...
static int test_widget_paint(void *ctx, M3PaintContext *paint_ctx)
{
    ...
    if (ctx == NULL || paint_ctx == NULL || paint_ctx->gfx == NULL || paint_ctx->gfx->vtable == NULL) {
        return M3_ERR_INVALID_ARGUMENT;
    }
    ...
    switch (scenario->mode) {
        case SCENARIO_OK:
            ...
            rc = paint_ctx->gfx->vtable->begin_frame(paint_ctx->gfx->ctx, handle, 100, 200, paint_ctx->dpi_scale);
            if (rc != M3_OK) {
                return rc;
            }
            ...
            rc = paint_ctx->gfx->vtable->draw_rect(paint_ctx->gfx->ctx, &rect, color, 2.0f);
            if (rc != M3_OK) {
                return rc;
            }
            rc = paint_ctx->gfx->vtable->draw_line(paint_ctx->gfx->ctx, 0.0f, 1.0f, 2.0f, 3.0f, color, 1.0f);
            if (rc != M3_OK) {
                return rc;
            }
            ...
            rc = paint_ctx->gfx->vtable->draw_texture(paint_ctx->gfx->ctx, handle, &src, &dst, 1.0f);
            if (rc != M3_OK) {
                return rc;
            }
            ...
            rc = paint_ctx->gfx->text_vtable->draw_text(paint_ctx->gfx->ctx, handle, "Hi", 2, 10.0f, 12.0f, color);
            if (rc != M3_OK) {
                return rc;
            }
        ...
        case SCENARIO_DRAW_RECT_ONLY:
            return paint_ctx->gfx->vtable->draw_rect(paint_ctx->gfx->ctx, &rect, color, 0.0f);
        case SCENARIO_BAD_BEGIN_FRAME:
            return paint_ctx->gfx->vtable->begin_frame(paint_ctx->gfx->ctx, handle, 0, 10, 1.0f);
        case SCENARIO_BAD_RECT:
            rect.width = -1.0f;
            return paint_ctx->gfx->vtable->draw_rect(paint_ctx->gfx->ctx, &rect, color, 0.0f);
        case SCENARIO_BAD_RECT_NULL:
            return paint_ctx->gfx->vtable->draw_rect(paint_ctx->gfx->ctx, NULL, color, 0.0f);
        case SCENARIO_BAD_CORNER:
            return paint_ctx->gfx->vtable->draw_rect(paint_ctx->gfx->ctx, &rect, color, -1.0f);
        case SCENARIO_BAD_LINE:
            return paint_ctx->gfx->vtable->draw_line(paint_ctx->gfx->ctx, 0.0f, 0.0f, 1.0f, 1.0f, color, -1.0f);
        ...
        case SCENARIO_BAD_TEXTURE_OPACITY:
            return paint_ctx->gfx->vtable->draw_texture(paint_ctx->gfx->ctx, handle, &src, &dst, 2.0f);
        case SCENARIO_BAD_TEXTURE_RECT:
            src.width = -1.0f;
            return paint_ctx->gfx->vtable->draw_texture(paint_ctx->gfx->ctx, handle, &src, &dst, 1.0f);
        case SCENARIO_BAD_TEXT:
            if (paint_ctx->gfx->text_vtable == NULL) {
                return M3_ERR_INVALID_ARGUMENT;
            }
            return paint_ctx->gfx->text_vtable->draw_text(paint_ctx->gfx->ctx, handle, NULL, 4, 0.0f, 0.0f, color);
        case SCENARIO_CREATE_TEXTURE_INVALID:
            return paint_ctx->gfx->vtable->create_texture(paint_ctx->gfx->ctx, 1, 1, M3_TEX_FORMAT_RGBA8, NULL, 0, NULL);
        case SCENARIO_CREATE_TEXTURE_UNSUPPORTED:
            return paint_ctx->gfx->vtable->create_texture(paint_ctx->gfx->ctx, 1, 1, M3_TEX_FORMAT_RGBA8, NULL, 0, &out_handle);
        case SCENARIO_UPDATE_TEXTURE_INVALID:
            return paint_ctx->gfx->vtable->update_texture(NULL, handle, 0, 0, 1, 1, NULL, 0);
        case SCENARIO_UPDATE_TEXTURE_UNSUPPORTED:
            return paint_ctx->gfx->vtable->update_texture(paint_ctx->gfx->ctx, handle, 0, 0, 1, 1, NULL, 0);
        ...
        case SCENARIO_CREATE_FONT_INVALID:
            ...
            return paint_ctx->gfx->text_vtable->create_font(paint_ctx->gfx->ctx, "Test", 12, 400, M3_FALSE, NULL);
        case SCENARIO_CREATE_FONT_UNSUPPORTED:
            if (paint_ctx->gfx->text_vtable == NULL) {
                return M3_ERR_INVALID_ARGUMENT;
            }
            return paint_ctx->gfx->text_vtable->create_font(paint_ctx->gfx->ctx, "Test", 12, 400, M3_FALSE, &out_handle);
        ...
        case SCENARIO_DESTROY_FONT_UNSUPPORTED:
            ...
            return paint_ctx->gfx->text_vtable->destroy_font(paint_ctx->gfx->ctx, handle);
        case SCENARIO_MEASURE_TEXT_INVALID:
            if (paint_ctx->gfx->text_vtable == NULL) {
                return M3_ERR_INVALID_ARGUMENT;
            }
            return paint_ctx->gfx->text_vtable->measure_text(paint_ctx->gfx->ctx, handle, "Hi", 2, NULL, &out_height, &out_baseline);
        case SCENARIO_MEASURE_TEXT_UNSUPPORTED:
            if (paint_ctx->gfx->text_vtable == NULL) {
                return M3_ERR_INVALID_ARGUMENT;
            }
            return paint_ctx->gfx->text_vtable->measure_text(paint_ctx->gfx->ctx, handle, "Hi", 2, &out_width, &out_height,
                &out_baseline);
        case SCENARIO_RECORD_ERRORS: {
            ...
            if (paint_ctx->gfx->vtable->begin_frame(NULL, handle, 1, 1, 1.0f) != M3_ERR_INVALID_ARGUMENT) {
                ok = 0;
            }
            if (paint_ctx->gfx->vtable->begin_frame(paint_ctx->gfx->ctx, handle, 1, 1, 0.0f) != M3_ERR_RANGE) {
                ok = 0;
            }
            if (paint_ctx->gfx->vtable->begin_frame(&fake, handle, 1, 1, 1.0f) != M3_ERR_STATE) {
                ok = 0;
            }

            if (paint_ctx->gfx->vtable->end_frame(NULL, handle) != M3_ERR_INVALID_ARGUMENT) {
                ok = 0;
            }
            ...
            if (paint_ctx->gfx->vtable->draw_rect(NULL, &rect, color, 1.0f) != M3_ERR_INVALID_ARGUMENT) {
                ok = 0;
            }
            if (paint_ctx->gfx->vtable->draw_rect(&fake, &rect, color, 1.0f) != M3_ERR_STATE) {
                ok = 0;
            }

            if (paint_ctx->gfx->vtable->draw_line(NULL, 0.0f, 0.0f, 1.0f, 1.0f, color, 1.0f) != M3_ERR_INVALID_ARGUMENT) {
                ok = 0;
            }
            if (paint_ctx->gfx->vtable->draw_line(&fake, 0.0f, 0.0f, 1.0f, 1.0f, color, 1.0f) != M3_ERR_STATE) {
                ok = 0;
            }

            if (paint_ctx->gfx->vtable->push_clip(NULL, &rect) != M3_ERR_INVALID_ARGUMENT) {
                ok = 0;
            }
            ...
            if (paint_ctx->gfx->vtable->set_transform(NULL, &transform) != M3_ERR_INVALID_ARGUMENT) {
                ok = 0;
            }
            if (paint_ctx->gfx->vtable->set_transform(paint_ctx->gfx->ctx, NULL) != M3_ERR_INVALID_ARGUMENT) {
                ok = 0;
            }
            if (paint_ctx->gfx->vtable->set_transform(&fake, &transform) != M3_ERR_STATE) {
                ok = 0;
            }

            if (paint_ctx->gfx->vtable->draw_texture(NULL, handle, &src, &dst, 1.0f) != M3_ERR_INVALID_ARGUMENT) {
                ok = 0;
            }
            if (paint_ctx->gfx->vtable->draw_texture(&fake, handle, &src, &dst, 1.0f) != M3_ERR_STATE) {
                ok = 0;
            }
            bad_dst = dst;
            bad_dst.width = -1.0f;
            if (paint_ctx->gfx->vtable->draw_texture(paint_ctx->gfx->ctx, handle, &src, &bad_dst, 1.0f) != M3_ERR_RANGE) {
                ok = 0;
            }

            if (paint_ctx->gfx->text_vtable != NULL) {
                if (paint_ctx->gfx->text_vtable->draw_text(NULL, handle, "Hi", 2, 0.0f, 0.0f, color)
                    != M3_ERR_INVALID_ARGUMENT) {
                    ok = 0;
                }
                if (paint_ctx->gfx->text_vtable->draw_text(&fake, handle, "Hi", 2, 0.0f, 0.0f, color) != M3_ERR_STATE) {
                    ok = 0;
                }
            }
...
{
    {
...
#endif
        M3_TEST_EXPECT(m3_render_test_add_overflow(1, 1, NULL), M3_ERR_INVALID_ARGUMENT);
        M3_TEST_EXPECT(m3_render_test_mul_overflow(1, 1, NULL), M3_ERR_INVALID_ARGUMENT);
        M3_TEST_EXPECT(m3_render_test_list_reserve(NULL, 1), M3_ERR_INVALID_ARGUMENT);
        ...
        bad_alloc.free = NULL;
        M3_TEST_EXPECT(m3_render_list_init(&list, &bad_alloc, 0), M3_ERR_INVALID_ARGUMENT);
        ...
        M3_TEST_EXPECT(m3_render_list_init(&list, &alloc_iface, 4), M3_ERR_OUT_OF_MEMORY);
        alloc.fail_alloc_on = 0;

        memset(&list, 0, sizeof(list));
        max_size = (m3_usize)~(m3_usize)0;
        M3_TEST_EXPECT(m3_render_list_init(&list, &alloc_iface, max_size), M3_ERR_OVERFLOW);
...

    {
        ...
        gfx.vtable = &gfx_vtable;
        gfx.text_vtable = &text_vtable;
        M3_TEST_EXPECT(m3_render_list_execute(&list, &gfx), M3_ERR_INVALID_ARGUMENT);
        M3_TEST_OK(m3_render_list_shutdown(&list));
        ...
        M3_TEST_EXPECT(m3_render_list_execute(&list, NULL), M3_ERR_INVALID_ARGUMENT);
    }

    {
        ...
        M3_TEST_EXPECT(m3_render_node_init(NULL, &widget, &bounds), M3_ERR_INVALID_ARGUMENT);
        M3_TEST_EXPECT(m3_render_node_init(&node, NULL, &bounds), M3_ERR_INVALID_ARGUMENT);
        M3_TEST_EXPECT(m3_render_node_init(&node, &widget, NULL), M3_ERR_INVALID_ARGUMENT);
        bounds.width = -1.0f;
        M3_TEST_EXPECT(m3_render_node_init(&node, &widget, &bounds), M3_ERR_RANGE);
        bounds.width = 10.0f;

        M3_TEST_EXPECT(m3_render_node_set_bounds(NULL, &bounds), M3_ERR_INVALID_ARGUMENT);
        bounds.height = -1.0f;
        M3_TEST_EXPECT(m3_render_node_set_bounds(&node, &bounds), M3_ERR_RANGE);
        bounds.height = 10.0f;

        M3_TEST_EXPECT(m3_render_node_set_children(NULL, children, 1), M3_ERR_INVALID_ARGUMENT);
        M3_TEST_EXPECT(m3_render_node_set_children(&node, NULL, 1), M3_ERR_INVALID_ARGUMENT);
        children[0] = NULL;
        M3_TEST_EXPECT(m3_render_node_set_children(&node, children, 1), M3_ERR_INVALID_ARGUMENT);
        ...
        node.child_count = 1;
        node.children = NULL;
        M3_TEST_EXPECT(m3_render_test_validate_node(&node), M3_ERR_INVALID_ARGUMENT);
        children[0] = NULL;
        ...
        M3_TEST_ASSERT(list.commands[list.count - 1].type == M3_RENDER_CMD_POP_CLIP);
        M3_TEST_OK(m3_render_list_shutdown(&list));
        ...
        M3_TEST_OK(m3_render_list_init(&list, NULL, 2));
        widget.vtable = NULL;
        M3_TEST_EXPECT(m3_render_build(&node, &list, 1.0f), M3_ERR_INVALID_ARGUMENT);
        M3_TEST_OK(m3_render_list_shutdown(&list));
        widget.vtable = &vtable;
    ...
    if (run_paint_scenario(SCENARIO_CREATE_TEXTURE_INVALID, M3_ERR_INVALID_ARGUMENT)) {
        return 1;
    }
    if (run_paint_scenario(SCENARIO_CREATE_TEXTURE_UNSUPPORTED, M3_ERR_UNSUPPORTED)) {
        return 1;
    }
    if (run_paint_scenario(SCENARIO_UPDATE_TEXTURE_INVALID, M3_ERR_INVALID_ARGUMENT)) {
        return 1;
    }
    if (run_paint_scenario(SCENARIO_UPDATE_TEXTURE_UNSUPPORTED, M3_ERR_UNSUPPORTED)) {
        return 1;
    }
    if (run_paint_scenario(SCENARIO_DESTROY_TEXTURE_INVALID, M3_ERR_INVALID_ARGUMENT)) {
        return 1;
    }
    if (run_paint_scenario(SCENARIO_DESTROY_TEXTURE_UNSUPPORTED, M3_ERR_UNSUPPORTED)) {
        return 1;
    }
    if (run_paint_scenario(SCENARIO_CREATE_FONT_INVALID, M3_ERR_INVALID_ARGUMENT)) {
        return 1;
    }
    if (run_paint_scenario(SCENARIO_CREATE_FONT_UNSUPPORTED, M3_ERR_UNSUPPORTED)) {
        return 1;
    }
    if (run_paint_scenario(SCENARIO_DESTROY_FONT_INVALID, M3_ERR_INVALID_ARGUMENT)) {
        return 1;
    }
    if (run_paint_scenario(SCENARIO_DESTROY_FONT_UNSUPPORTED, M3_ERR_UNSUPPORTED)) {
        return 1;
    }
    if (run_paint_scenario(SCENARIO_MEASURE_TEXT_INVALID, M3_ERR_INVALID_ARGUMENT)) {
        return 1;
    }
    if (run_paint_scenario(SCENARIO_MEASURE_TEXT_UNSUPPORTED, M3_ERR_UNSUPPORTED)) {
        return 1;
    }
