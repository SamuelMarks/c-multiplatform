...
static void init_text_event(M3InputEvent *event, const char *text)
{
    ...
    for (i = 0; text[i] != '\0' && i < (m3_usize)(sizeof(event->data.text.utf8) - 1); ++i) {
...
static void setup_widget(M3Widget *widget, M3WidgetVTable *vtable, TestWidget *state, m3_u32 flags, m3_u32 id)
{
    widget->ctx = state;
...
{
    {
        ...
        M3_TEST_EXPECT(m3_event_dispatcher_get_focus(NULL, &focused), M3_ERR_INVALID_ARGUMENT);
        M3_TEST_EXPECT(m3_event_dispatcher_get_focus(&dispatcher, NULL), M3_ERR_INVALID_ARGUMENT);
        M3_TEST_EXPECT(m3_event_dispatcher_set_focus(NULL, &widget), M3_ERR_INVALID_ARGUMENT);
        M3_TEST_EXPECT(m3_event_dispatcher_set_focus(&dispatcher, NULL), M3_ERR_INVALID_ARGUMENT);
        M3_TEST_EXPECT(m3_event_dispatcher_clear_focus(NULL), M3_ERR_INVALID_ARGUMENT);
        ...
        M3_TEST_EXPECT(m3_event_dispatcher_set_focus(&dispatcher, &widget), M3_ERR_INVALID_ARGUMENT);
...

    {
        ...
        M3_TEST_EXPECT(m3_event_dispatch(NULL, &root, &event, &target, &handled), M3_ERR_INVALID_ARGUMENT);
        M3_TEST_EXPECT(m3_event_dispatch(&dispatcher, NULL, &event, &target, &handled), M3_ERR_INVALID_ARGUMENT);
        M3_TEST_EXPECT(m3_event_dispatch(&dispatcher, &root, NULL, &target, &handled), M3_ERR_INVALID_ARGUMENT);
        M3_TEST_EXPECT(m3_event_dispatch(&dispatcher, &root, &event, &target, NULL), M3_ERR_INVALID_ARGUMENT);
