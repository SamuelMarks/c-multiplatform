...
int main(void) {
    ...
    tstyle.size_px = 0;
    CMP_TEST_EXPECT(cmp_text_field_test_validate_text_style(&tstyle, CMP_FALSE), CMP_ERR_RANGE);
    ...
    CMP_TEST_EXPECT(cmp_text_field_test_validate_rect(NULL), CMP_ERR_INVALID_ARGUMENT);
    ...
    CMP_TEST_EXPECT(cmp_text_field_test_add_overflow(val, 1, &val), CMP_ERR_OVERFLOW);
    
    CMP_TEST_EXPECT(cmp_text_field_test_reserve(NULL, 10), CMP_ERR_INVALID_ARGUMENT);
    CMP_TEST_OK(cmp_text_field_test_reserve(&field, 10));
    
    CMP_TEST_EXPECT(cmp_text_field_test_validate_utf8(NULL, 1), CMP_ERR_INVALID_ARGUMENT);
    CMP_TEST_OK(cmp_text_field_test_validate_utf8("a", 1));
    
    cmp_text_field_test_set_offset_skip_early(CMP_TRUE);
    CMP_TEST_EXPECT(cmp_text_field_test_validate_offset(NULL, 1, 0), CMP_ERR_INVALID_ARGUMENT);
    cmp_text_field_test_set_offset_skip_early(CMP_FALSE);
    CMP_TEST_EXPECT(cmp_text_field_test_validate_offset("a", 1, 2), CMP_ERR_RANGE);
    ...
    CMP_TEST_EXPECT(cmp_text_field_test_prev_offset(NULL, 1, 1, &val), CMP_ERR_INVALID_ARGUMENT);
    CMP_TEST_EXPECT(cmp_text_field_test_next_offset(NULL, 1, 0, &val), CMP_ERR_INVALID_ARGUMENT);
    ...
    CMP_TEST_EXPECT(cmp_text_field_test_validate_offset("ab", 2, 1), CMP_ERR_RANGE);
    
    CMP_TEST_EXPECT(cmp_text_field_test_set_text_internal(NULL, "a", 1, CMP_FALSE), CMP_ERR_INVALID_ARGUMENT);
    
    CMP_TEST_EXPECT(cmp_text_field_test_delete_range(NULL, 0, 1, CMP_FALSE), CMP_ERR_INVALID_ARGUMENT);
    ...
    CMPTextField field2;
    CMP_TEST_EXPECT(cmp_text_field_init(&field2, &text_backend, &style, NULL, NULL, 0), CMP_ERR_IO);
    
    cmp_text_field_test_set_fail_point(9u); 
    CMPColor c1, c2, c3, c4, c5, c6, c7, c8;
    CMP_TEST_EXPECT(cmp_text_field_test_resolve_colors(&field, &c1, &c2, &c3, &c4, &c5, &c6, &c7, &c8), CMP_ERR_IO);
    
    cmp_text_field_test_set_color_fail_after(1u);
    CMP_TEST_EXPECT(cmp_text_field_test_resolve_colors(&field, &c1, &c2, &c3, &c4, &c5, &c6, &c7, &c8), CMP_ERR_RANGE);
    ...
    CMP_TEST_EXPECT(cmp_text_field_test_resolve_colors(&field, &c1, &c2, &c3, &c4, &c5, &c6, &c7, &c8), CMP_OK);
    cmp_text_field_test_set_color_fail_after(1u);
    CMP_TEST_EXPECT(cmp_text_field_test_resolve_colors(&field, &c1, &c2, &c3, &c4, &c5, &c6, &c7, &c8), CMP_ERR_RANGE);
    cmp_text_field_test_set_color_fail_after(2u);
    CMP_TEST_EXPECT(cmp_text_field_test_resolve_colors(&field, &c1, &c2, &c3, &c4, &c5, &c6, &c7, &c8), CMP_ERR_RANGE);
    ...
    CMPScalar width;
    CMP_TEST_EXPECT(cmp_text_field_test_measure_prefix(NULL, 0, &width), CMP_ERR_INVALID_ARGUMENT);
    
    CMP_TEST_EXPECT(cmp_text_field_test_offset_for_x(NULL, 0.0f, &val), CMP_ERR_INVALID_ARGUMENT);
