...
typedef __int64 docgen_int64_t;
#else
#undef NUM_FORMAT
#define NUM_FORMAT "%ld"
typedef long docgen_int64_t;
#endif
...
#endif
static int
doc_printf_file(FILE *f, const char *fmt, ...) {
  int ret;
  va_list args;
  va_start(args, fmt);
#if defined(_MSC_VER)
...
static int CMP_CALL svg_begin_frame(void *gfx, CMPHandle window, cmp_i32 width,
  ...
  doc_printf_file(
      ...
      "\">\n",
      (docgen_int64_t)width, (docgen_int64_t)height, (docgen_int64_t)width,
      (docgen_int64_t)height);
  return CMP_OK;
}
...
static void print_color(FILE *f, CMPColor c) {
  ...
  doc_printf_file(f, "rgb(" NUM_FORMAT "," NUM_FORMAT "," NUM_FORMAT ")",
                  (docgen_int64_t)r, (docgen_int64_t)g, (docgen_int64_t)b);
}
...
static int CMP_CALL svg_draw_line(void *gfx, CMPScalar x0, CMPScalar y0,
  ...
  doc_printf_file(ctx->file,
                  "  <line x1=\"%f\" y1=\"%f\" x2=\"%f\" y2=\"%f\" stroke=\"",
                  x0, y0, x1, y1);
  print_color(ctx->file, color);
  doc_printf_file(ctx->file,
                  "\" stroke-width=\"%f\" stroke-opacity=\"%f\" />\n",
...
static int CMP_CALL svg_draw_path(void *gfx, const CMPPath *path,
  ...
  for (i = 0; i < path->count; ++i) {
    ...
    if (cmd->type == CMP_PATH_CMD_MOVE_TO) {
      doc_printf_file(ctx->file, "M %f %f ", cmd->data.move_to.x,
                      cmd->data.move_to.y);
    } else if (cmd->type == CMP_PATH_CMD_LINE_TO) {
      doc_printf_file(ctx->file, "L %f %f ", cmd->data.line_to.x,
                      cmd->data.line_to.y);
    } else if (cmd->type == CMP_PATH_CMD_QUAD_TO) {
      doc_printf_file(ctx->file, "Q %f %f %f %f ", cmd->data.quad_to.cx,
                      cmd->data.quad_to.cy, cmd->data.quad_to.x,
                      cmd->data.quad_to.y);
...
#if defined(_MSC_VER)
  sprintf_s(relative_filename, sizeof(relative_filename),
            "assets/%s_%s_%s_%s.svg", category, name, g_platform, g_theme);
#else
...
#if defined(_MSC_VER)
  sprintf_s(json_filename, sizeof(json_filename), "docs/doc_%s_%s.json",
            g_platform, g_theme);
#else
