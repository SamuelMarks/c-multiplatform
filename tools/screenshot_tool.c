...
int main(int argc, char **argv) {
  ...
  int i;
  for (i=0; i<10; i++) do { CMPInputEvent ev; CMPBool has; ws.vtable->poll_event(ws.ctx, &ev, &has); } while(0);
  ...
  if (strcmp(widget_name, "button") == 0) {
    M3ButtonStyle style; m3_button_style_init_filled(&style);
    M3Button w; m3_button_init(&w, &text_backend, &style, "Button", 6);
    CMPRect bounds = {50, 50, 100, 40};
    
    w.widget.vtable->layout(w.widget.ctx, bounds);
    CMPPaintContext pctx;
    pctx.gfx = &gfx;
    pctx.clip.x = 0; pctx.clip.y = 0; pctx.clip.width = 200; pctx.clip.height = 200;
    pctx.dpi_scale = 1.0f;
    w.widget.vtable->paint(w.widget.ctx, &pctx);
  } else if (strcmp(widget_name, "card") == 0) {
    M3CardStyle style; m3_card_style_init_elevated(&style);
    M3Card w; m3_card_init(&w, &style);
    ...
    M3CheckboxStyle style; m3_checkbox_style_init(&style);
    M3Checkbox w; m3_checkbox_init(&w, &style, CMP_TRUE);
    ...
    M3SwitchStyle style; m3_switch_style_init(&style);
    M3Switch w; m3_switch_init(&w, &style, CMP_TRUE);
    ...
    M3SliderStyle style; m3_slider_style_init(&style);
    M3Slider w; m3_slider_init(&w, &style, 0, 100, 50);
    ...
    M3LinearProgressStyle style; m3_linear_progress_style_init(&style);
    M3LinearProgress w; m3_linear_progress_init(&w, &style, 0.5f);
    ...
    M3ChipStyle style; m3_chip_style_init_filter(&style);
    M3Chip w; m3_chip_init(&w, &text_backend, &style, "Chip", 4);
    CMPRect bounds = {60, 80, 80, 40};
    ...
    M3TextFieldStyle style; m3_text_field_style_init(&style, CMP_TRUE);
    M3TextField w; m3_text_field_init(&w, &style, allocator, text_backend);
    ...
    M3TabRowStyle style; m3_tab_row_style_init(&style);
    M3TabItem items[2];
    memset(items, 0, sizeof(items));
    items[0].utf8_label = "Tab 1"; items[0].utf8_len = 5;
    items[1].utf8_label = "Tab 2"; items[1].utf8_len = 5;
    M3TabRow w; m3_tab_row_init(&w, &text_backend, &style, items, 2, 0);
