...
- [x] 1. **Research & Feasibility**: Evaluate libuv, libev, or native OS APIs (epoll/kqueue/IOCP) for the foundational asynchronous event loop.
- [x] 2. **API Definition**: Define the `cmp_modality_t` enum in `cmp_core.h`.
- [x] 3. **Config Struct**: Update `cmp_app_config_t` to include the execution modality enum.
- [x] 4. **Core Loop Refactor**: Abstract the existing `while(running)` UI loop into a `cmp_run_loop()` function pointer or interface.
- [x] 5. **Sync Single-Thread Impl**: Implement the `CMP_MODALITY_SYNC_SINGLE` backend (classic blocking poll loop).
- [x] 6. **Task Queue Core**: Create a lock-free (or mutex-guarded) thread-safe FIFO queue for tasks/messages.
- [x] 7. **Thread Pool API**: Define `cmp_thread_pool_init`, `_push`, `_destroy` in `cmp_tasks.h`.
- [x] 8. **Worker Thread Impl**: Implement POSIX/Windows threads for the worker pool.
- [x] 9. **Sync Multi-Thread Impl**: Wire up `CMP_MODALITY_SYNC_MULTI` to initialize the thread pool alongside the sync UI loop.
- [x] 10. **Event Loop Abstraction**: Define an interface for `cmp_event_loop_t` (timers, file descriptors, UI events).
- [x] 11. **libuv Integration (Optional/Core)**: Integrate a high-performance event loop library (or write a custom abstraction layer).
- [x] 12. **Async API Design**: Create non-blocking APIs for file I/O (e.g., `cmp_fs_read_async(path, callback)`).
- [x] 13. **Async Network API**: Create non-blocking APIs for networking (e.g., `cmp_net_fetch(url, callback)`).
- [x] 14. **Timer Integration**: Map `cmp_set_timeout` and `cmp_set_interval` to the underlying event loop.
- [x] 15. **UI Event to Async**: Ensure native OS UI events (mouse, keyboard, paint) are pushed into the async event loop safely.
- [x] 16. **Async Single-Thread Impl**: Implement `CMP_MODALITY_ASYNC_SINGLE` to run the integrated event loop (the "Node.js" model).
- [x] 17. **Cross-Thread Wakeup**: Implement a mechanism (e.g., `eventfd` or loopback socket) for worker threads to wake up the main async event loop.
- [x] 18. **Async Multi-Thread Impl**: Combine the async event loop with the thread pool for `CMP_MODALITY_ASYNC_MULTI`.
- [x] 19. **Message Structure**: Define a standard `cmp_msg_t` payload structure for message passing.
- [x] 20. **Message Bus API**: Define `cmp_msg_subscribe` and `cmp_msg_publish`.
- [x] 21. **In-Process Messaging**: Implement the message bus using shared memory and queues for inter-thread communication.
- [x] 22. **Actor Model Prototyping**: Create an API for spawning "Actors" (stateful message handlers).
- [x] 23. **Actor Supervision**: Implement basic supervision trees for Actors (restart on failure).
- [x] 24. **Greenthread Research**: Evaluate C coroutine libraries (e.g., libco, boost::context ported to C, or ucontext.h).
- [x] 25. **Fiber Context Switch**: Implement assembly or OS-level context switching for greenthreads.
- [x] 26. **Greenthread Scheduler**: Build a round-robin scheduler to multiplex greenthreads over OS threads.
- [x] 27. **Yielding API**: Implement `cmp_yield()` to pause a greenthread and return control to the scheduler.
- [x] 28. **Async I/O to Yielding**: Hook async I/O callbacks to resume yielding greenthreads (making async look synchronous).
- [x] 29. **Greenthread Modality Impl**: Integrate the scheduler into `CMP_MODALITY_GREENTHREADS`.
- [x] 30. **Multiprocess IPC Research**: Evaluate named pipes, Unix domain sockets, and shared memory for IPC.
- [x] 31. **Process Forking/Spawning**: Implement `cmp_process_spawn()` abstracting `fork()` or `CreateProcess()`.
- [x] 32. **IPC Message Serialization**: Write a lightweight binary serializer for `cmp_msg_t` to send over IPC.
- [x] 33. **IPC Channel Impl**: Implement the communication channels between the main UI process and worker processes.
- [x] 34. **Multiprocess Modality Impl**: Wire up `CMP_MODALITY_MULTIPROCESS_ACTOR` to transparently route messages over IPC.
- [x] 35. **Shared Memory Allocator**: Implement a specific allocator for sharing large data (e.g., images, buffers) between processes to avoid copy overhead.
- [x] 36. **Synchronization Primitives**: Abstract Mutexes, Semaphores, and Condition Variables across all supported platforms.
- [x] 37. **Atomic Operations**: Provide a wrapper for C11 stdatomic.h or compiler intrinsics.
- [x] 38. **Memory Ownership Semantics**: Document and enforce rules for who owns memory passed between threads/processes/actors.
- [x] 39. **Deadlock Detection**: (Optional Debug Tool) Add instrumentation to detect cyclic dependencies in mutex locks.
- [x] 40. **Modality Switching (Runtime vs Startup)**: Enforce that modality is strictly a startup configuration and cannot be changed on the fly.
- [x] 41. **Test Suite - Sync**: Write extensive unit tests asserting behavior in synchronous modes.
- [x] 42. **Test Suite - Async**: Write tests ensuring callbacks fire correctly in the async event loop.
- [x] 43. **Test Suite - Threading**: Write stress tests for the thread pool to catch race conditions.
- [x] 44. **Test Suite - IPC**: Write tests validating message serialization and multiprocess communication.
- [x] 45. **Example App - Node.js Clone**: Create a sample app demonstrating the asynchronous event loop serving HTTP requests while rendering UI.
- [x] 46. **Example App - Heavy Compute**: Create a sample app demonstrating multithreading (e.g., real-time raytracing or fractal generation) keeping the UI smooth.
- [x] 47. **Example App - Resilient Actors**: Create a multiprocess example where background workers intentionally crash and are revived by the main process.
- [x] 48. **Documentation - Modalities**: Write detailed guides in `docs/` explaining when to use each modality.
- [x] 49. **Documentation - API Ref**: Update API reference for all new concurrency, task, and messaging functions.
- [x] 50. **Final Review & Profiling**: Audit the event loop and thread pool overhead to ensure performance regressions are not introduced in simple modalities.