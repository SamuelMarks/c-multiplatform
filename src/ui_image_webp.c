...
static enum ui_error webp_supports_format(enum ui_image_format format,
                                          int *out_supported) {
  *out_supported = (format == UI_IMAGE_FORMAT_WEBP) ? 1 : 0;
  return UI_ERROR_NONE;
}

static enum ui_error webp_decode_memory(const void *data, size_t size,
...
static enum ui_error webp_free_image(struct ui_image *image) {
  if (!image)
    return UI_ERROR_INVALID_ARGUMENT;
