...
static enum ui_error gles_fallback_begin_frame(void *ctx, int width,
                                               int height) {
  ...
  if (gctx->backend->clear) {
    gctx->backend->clear(gctx->backend, clear_color);
  }
  return UI_ERROR_NONE;
}

static enum ui_error gles_fallback_end_frame(void *ctx) {
  struct gles_fallback_context *gctx = (struct gles_fallback_context *)ctx;
  if (gctx && gctx->backend && gctx->backend->flush) {
    gctx->backend->flush(gctx->backend);
  }
  return UI_ERROR_NONE;
}

static enum ui_error gles_fallback_draw_rect(void *ctx, const struct ui_rect *r,
...
static enum ui_error gles_fallback_draw_text(void *ctx, const char *text,
...
static enum ui_error gles_fallback_draw_image(void *ctx,
...
static enum ui_error gles_fallback_draw_path(void *ctx, const struct ui_path *p,
                                             const struct ui_color *c) {
  (void)ctx;
  (void)p;
  (void)c;
...
static enum ui_error gles_fallback_push_clip(void *ctx,
                                             const struct ui_rect *r) {
  (void)ctx;
  (void)r;
  /* Stub for now */
...
static enum ui_error gles_fallback_pop_clip(void *ctx) {
  (void)ctx;
  /* Stub for now */
  return UI_ERROR_NONE;
}

static enum ui_error gles_fallback_set_blend_mode(void *ctx,
...
static enum ui_error
gles_fallback_set_shadow(void *ctx, const struct ui_css_shadow *shadow) {
  (void)ctx;
  (void)shadow;
  /* Stub for now */
  return UI_ERROR_NONE;
...
static enum ui_error gles_fallback_read_pixels(void *ctx,
...
static enum ui_error gles_fallback_destroy(void *ctx) {
  struct gles_fallback_context *gctx = (struct gles_fallback_context *)ctx;
  if (gctx) {
    if (gctx->backend) {
      ...
      if (gctx->backend->destroy) {
        gctx->backend->destroy(gctx->backend);
      }
      UI_FREE(gctx->backend);
    }
    UI_FREE(gctx);
  }
  return UI_ERROR_NONE;
...
enum ui_error ui_renderer_gles_fallback_init(struct ui_renderer *renderer) {
  ...
  gctx = (struct gles_fallback_context *)UI_MALLOC(
