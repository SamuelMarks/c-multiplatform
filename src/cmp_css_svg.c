...
int cmp_prop_svg_geom_init(cmp_prop_svg_geom_t *geom) {
  ...
  if (cmp_prop_size_init_auto(&geom->cx) != 0)
    return -1;
  if (cmp_prop_size_init_auto(&geom->cy) != 0)
    return -1;
  if (cmp_prop_size_init_auto(&geom->r) != 0)
    return -1;
  if (cmp_prop_size_init_auto(&geom->rx) != 0)
    return -1;
  if (cmp_prop_size_init_auto(&geom->ry) != 0)
    return -1;
  if (cmp_prop_size_init_auto(&geom->x) != 0)
    return -1;
  if (cmp_prop_size_init_auto(&geom->y) != 0)
    return -1;
  return 0;
}
