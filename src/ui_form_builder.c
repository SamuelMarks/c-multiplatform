...
enum ui_error ui_form_builder_create(struct ui_arena *arena,
  ...
  builder = (ui_form_builder_t *)UI_MALLOC(sizeof(*builder));
  if (!builder)
...
static enum ui_error dummy_equality(union ui_signal_payload a,
...
static enum ui_error dummy_destructor(union ui_signal_payload p) {
  (void)p;
  return UI_ERROR_NONE;
}
