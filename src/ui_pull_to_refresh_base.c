...
static enum ui_error update_dom_state(struct ui_pull_to_refresh_base *ptr) {
...
#endif
    ui_dom_node_set_attribute(ptr->component->shadow_root, "data-progress",
                              buf);

    switch (ptr->state) {
    case UI_PULL_TO_REFRESH_RESTING:
      ui_dom_node_set_attribute(ptr->component->shadow_root, "data-state",
                                "resting");
      break;
...
ui_pull_to_refresh_base_create(struct ui_pull_to_refresh_base **out_ptr) {
  ...
  ptr = (struct ui_pull_to_refresh_base *)UI_MALLOC(
...
ui_pull_to_refresh_base_process_event(struct ui_pull_to_refresh_base *ptr,
  ...
  (void)ui_gesture_recognizer_process_event(ptr->gesture_recognizer, event,
                                            timestamp_ms, &ge);
...
ui_pull_to_refresh_base_set_spinner(struct ui_pull_to_refresh_base *ptr,
  ...
  if (spinner_comp) {
    ui_dom_node_append_child(ptr->component->shadow_root,
