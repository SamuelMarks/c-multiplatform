...
static int m3_segmented_test_color_should_fail(CMPBool *out_fail) {
  ...
  *out_fail =
      (g_m3_segmented_test_color_fail_after == 0u) ? CMP_TRUE : CMP_FALSE;
  return CMP_OK;
}

static int m3_segmented_test_fail_point_match(cmp_u32 point,
                                              CMPBool *out_match) {
  if (out_match == NULL) {
    return CMP_ERR_INVALID_ARGUMENT;
  }
...
static int m3_tab_row_validate_style(const M3TabRowStyle *style,
  ...
  if (style->indicator_anim_easing != CMP_ANIM_EASE_LINEAR &&
      ...
      style->indicator_anim_easing != CMP_ANIM_EASE_EMPHASIZED_DECELERATE &&
      style->indicator_anim_easing != CMP_ANIM_EASE_EMPHASIZED_ACCELERATE) {
    return CMP_ERR_RANGE;
  }
...
static int m3_tab_row_measure_max_text(const M3TabRow *row,
                                       CMPScalar *out_width,
...
static int m3_tab_row_compute_layout(const M3TabRow *row,
  ...
  if (mode == M3_TAB_MODE_FIXED) {
    if (row->item_count > 0) {
      ...
      tab_width =
          (available_width - total_spacing) / (CMPScalar)row->item_count;
#ifdef CMP_TESTING
      rc = m3_tab_row_test_fail_point_match(
...
static int m3_tab_row_indicator_target(const M3TabRow *row,
  ...
  if (layout->mode == M3_TAB_MODE_FIXED) {
    pos =
        (layout->tab_width + layout->spacing) * (CMPScalar)row->selected_index;
...
static int m3_tab_row_sync_indicator(M3TabRow *row,
  ...
  if (animate == CMP_TRUE) {
    ...
    rc = cmp_anim_controller_start_timing(&row->indicator_width_anim,
...
static int m3_tab_row_widget_paint(void *widget, CMPPaintContext *ctx) {
  ...
  for (i = 0; i < row->item_count; ++i) {
    ...
    rc = cmp_text_draw_utf8_gfx(
        ctx->gfx, row->font, row->items[i].utf8_label, row->items[i].utf8_len,
        row->style.is_rtl ? 1 : 0, text_x, text_y, text_color);
    if (rc != CMP_OK) {
      return rc;
    }
  }
...
int CMP_CALL m3_tab_row_set_on_select(M3TabRow *row,
                                      CMPTabRowOnSelect on_select, void *ctx) {
  if (row == NULL) {
    return CMP_ERR_INVALID_ARGUMENT;
  }
...
int CMP_CALL m3_tab_row_step(M3TabRow *row, CMPScalar dt,
                             CMPBool *out_changed) {
  CMPScalar value;
...
static int m3_segmented_validate_text_style(const CMPTextStyle *style,
                                            CMPBool require_family) {
  int rc = CMP_OK;
...
static int m3_segmented_widget_paint(void *widget, CMPPaintContext *ctx) {
  ...
  for (i = 0; i < buttons->item_count; ++i) {
    ...
    rc = cmp_text_draw_utf8_gfx(
        ctx->gfx, buttons->font, buttons->items[i].utf8_label,
...
int CMP_CALL m3_segmented_buttons_set_selected_index(
...
int CMP_CALL m3_tab_row_test_compute_layout(const M3TabRow *row,
  ...
  *out_content_width = layout.content_width;
  *out_tab_width = layout.tab_width;
  *out_tab_height = layout.tab_height;
  *out_mode = layout.mode;
  return CMP_OK;
}
...
int CMP_CALL m3_segmented_test_compute_layout(const M3SegmentedButtons *buttons,
  ...
  *out_content_width = layout.content_width;
  *out_segment_width = layout.segment_width;
  *out_segment_height = layout.segment_height;
  *out_spacing = layout.spacing;
  return CMP_OK;
}
