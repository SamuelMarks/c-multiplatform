...
int cmp_fluent_reveal_create(cmp_fluent_reveal_t **out_reveal) {
  ...
  if (CMP_MALLOC(sizeof(struct cmp_fluent_reveal), (void **)&ctx) != CMP_SUCCESS)
...
int cmp_acrylic_noise_create(int width, int height, cmp_acrylic_noise_t **out_noise) {
  ...
  if (!out_noise || width <= 0 || height <= 0)
    return CMP_ERROR_INVALID_ARG;
  if (CMP_MALLOC(sizeof(struct cmp_acrylic_noise), (void **)&ctx) != CMP_SUCCESS)
    return CMP_ERROR_OOM;
  memset(ctx, 0, sizeof(struct cmp_acrylic_noise));
  ctx->width = width;
  ctx->height = height;
  if (CMP_MALLOC((size_t)(width * height), (void **)&ctx->pixels) != CMP_SUCCESS) {
    CMP_FREE(ctx);
    return CMP_ERROR_OOM;
  }
