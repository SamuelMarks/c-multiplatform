...
static int m3_list_validate_style(const M3ListStyle *style)
{
    ...
    if (style->orientation != M3_LIST_ORIENTATION_VERTICAL && style->orientation != M3_LIST_ORIENTATION_HORIZONTAL) {
        return M3_ERR_RANGE;
    }
...
static int m3_grid_validate_style(const M3GridStyle *style)
{
    ...
    if (style->scroll_axis != M3_GRID_SCROLL_VERTICAL && style->scroll_axis != M3_GRID_SCROLL_HORIZONTAL) {
        return M3_ERR_RANGE;
    }
    if (style->span == 0) {
        return M3_ERR_RANGE;
...
static int m3_list_compute_content_extent(m3_usize item_count, M3Scalar item_extent, M3Scalar spacing,
    M3Scalar padding_start, M3Scalar padding_end, M3Scalar *out_extent)
{
    if (out_extent == NULL) {
        return M3_ERR_INVALID_ARGUMENT;
    }
    if (item_extent < 0.0f || spacing < 0.0f || padding_start < 0.0f || padding_end < 0.0f) {
        return M3_ERR_RANGE;
    }
...
static int m3_list_compute_visible_range(m3_usize item_count, M3Scalar item_extent, M3Scalar spacing, M3Scalar padding_start,
    M3Scalar scroll, M3Scalar viewport, m3_usize overscan, m3_usize *out_first, m3_usize *out_last, m3_usize *out_count)
{
    M3Scalar stride;
    M3Scalar start;
    M3Scalar end;
...
static int m3_grid_compute_visible_range(const M3GridView *view, m3_usize *out_first, m3_usize *out_last,
    m3_usize *out_count)
{
    M3Scalar item_main;
    ...
    if (view == NULL || out_first == NULL || out_last == NULL || out_count == NULL) {
        return M3_ERR_INVALID_ARGUMENT;
    }
...
static int m3_list_compute_item_bounds(const M3ListView *view, m3_usize index, M3Rect *out_bounds)
{
    M3Scalar x;
    M3Scalar y;
    M3Scalar width;
    ...
    if (view->style.orientation == M3_LIST_ORIENTATION_VERTICAL) {
        width = view->bounds.width - view->style.padding.left - view->style.padding.right;
        height = view->style.item_extent;
        x = view->bounds.x + view->style.padding.left;
        y = view->bounds.y + view->style.padding.top - view->scroll_offset + (M3Scalar)index * stride;
    } else {
        width = view->style.item_extent;
        height = view->bounds.height - view->style.padding.top - view->style.padding.bottom;
        x = view->bounds.x + view->style.padding.left - view->scroll_offset + (M3Scalar)index * stride;
...
static int m3_grid_compute_item_bounds(const M3GridView *view, m3_usize index, M3Rect *out_bounds)
{
    m3_usize row;
    m3_usize col;
    M3Scalar x;
    ...
    if (view->style.scroll_axis == M3_GRID_SCROLL_VERTICAL) {
        ...
        x = view->bounds.x + view->style.padding.left + (M3Scalar)col * (view->style.item_width + view->style.spacing_x);
        y = view->bounds.y + view->style.padding.top + (M3Scalar)row * (view->style.item_height + view->style.spacing_y)
            - view->scroll_offset;
    } else {
        row = index % view->style.span;
        col = index / view->style.span;
...
static int m3_list_view_update_metrics(M3ListView *view)
{
    ...
    rc = m3_list_compute_content_extent(view->item_count, view->style.item_extent, view->style.spacing, padding_start,
...
static int m3_grid_view_update_metrics(M3GridView *view)
{
    ...
    if (spacing_main < 0.0f || item_main < 0.0f || padding_start < 0.0f || padding_end < 0.0f) {
        return M3_ERR_RANGE;
    }
    ...
    if (line_count == 0) {
        ...
        view->content_extent = padding_start + padding_end + item_main * (M3Scalar)line_count
...
static int m3_list_reserve_slots(M3ListSlot **slots, M3RenderNode ***visible_nodes, m3_usize *slot_capacity,
    const M3Allocator *allocator, m3_usize capacity)
{
    ...
    if (slots == NULL || visible_nodes == NULL || slot_capacity == NULL || allocator == NULL) {
        return M3_ERR_INVALID_ARGUMENT;
    }
    ...
    rc = m3_list_mul_overflow(capacity, (m3_usize)sizeof(M3ListSlot), &slot_bytes);
    if (rc != M3_OK) {
        return rc;
    }
...
static int m3_list_widget_measure(void *widget, M3MeasureSpec width, M3MeasureSpec height, M3Size *out_size)
{
    M3ListView *view;
...
static int m3_list_widget_paint(void *widget, M3PaintContext *ctx)
{
    ...
    return ctx->gfx->vtable->draw_rect(ctx->gfx->ctx, &view->bounds, view->style.background_color, 0.0f);
}

static int m3_list_widget_event(void *widget, const M3InputEvent *event, M3Bool *out_handled)
{
    M3ListView *view;
    M3Scalar delta;
    int rc;
...
static int m3_list_widget_get_semantics(void *widget, M3Semantics *out_semantics)
{
    M3ListView *view;
...
static const M3WidgetVTable g_m3_list_widget_vtable = {
    ...
    m3_list_widget_layout,
    m3_list_widget_paint,
    m3_list_widget_event,
    m3_list_widget_get_semantics,
    m3_list_widget_destroy
};

static int m3_grid_widget_measure(void *widget, M3MeasureSpec width, M3MeasureSpec height, M3Size *out_size)
{
    M3GridView *view;
    M3Scalar content_width;
    M3Scalar content_height;
...
static int m3_grid_widget_event(void *widget, const M3InputEvent *event, M3Bool *out_handled)
{
    M3GridView *view;
    M3Scalar delta;
    int rc;
...
static int m3_grid_widget_get_semantics(void *widget, M3Semantics *out_semantics)
{
    M3GridView *view;
...
static const M3WidgetVTable g_m3_grid_widget_vtable = {
    m3_grid_widget_measure,
    m3_grid_widget_layout,
    m3_grid_widget_paint,
    m3_grid_widget_event,
    m3_grid_widget_get_semantics,
    m3_grid_widget_destroy
};
...
int M3_CALL m3_list_view_init(M3ListView *view, const M3ListStyle *style, const M3Allocator *allocator,
...
int M3_CALL m3_list_view_set_bind(M3ListView *view, M3ListBindFn bind, void *ctx)
{
    if (view == NULL) {
        return M3_ERR_INVALID_ARGUMENT;
    }
...
int M3_CALL m3_list_view_get_scroll(const M3ListView *view, M3Scalar *out_offset)
{
...
int M3_CALL m3_list_view_get_content_extent(const M3ListView *view, M3Scalar *out_extent)
{
    M3ListView temp;
    int rc;
...
int M3_CALL m3_list_view_get_required_slots(const M3ListView *view, m3_usize *out_required)
{
    M3ListView temp;
    M3Scalar padding_start;
    M3Scalar viewport;
    ...
    rc = m3_list_compute_visible_range(temp.item_count, temp.style.item_extent, temp.style.spacing, padding_start,
        temp.scroll_offset, viewport, temp.style.overscan, &first, &last, &count);
    if (rc != M3_OK) {
        return rc;
    }
...
int M3_CALL m3_list_view_reserve(M3ListView *view, m3_usize capacity)
{
    ...
    rc = m3_list_reserve_slots(&view->slots, &view->visible_nodes, &view->slot_capacity, &view->allocator, capacity);
    if (rc != M3_OK) {
        return rc;
    }
...
int M3_CALL m3_list_view_update(M3ListView *view)
{
    ...
    rc = m3_list_compute_visible_range(view->item_count, view->style.item_extent, view->style.spacing, padding_start,
        view->scroll_offset, viewport, view->style.overscan, &first, &last, &count);
    if (rc != M3_OK) {
        return rc;
    }
    ...
    if (count == 0) {
        if (view->visible_nodes != NULL && view->slot_capacity > 0) {
            memset(view->visible_nodes, 0, (size_t)(view->slot_capacity * sizeof(M3RenderNode *)));
        }
        return M3_OK;
    }
...
int M3_CALL m3_list_view_get_visible(const M3ListView *view, M3RenderNode ***out_nodes, m3_usize *out_count)
{
...
int M3_CALL m3_grid_view_init(M3GridView *view, const M3GridStyle *style, const M3Allocator *allocator,
    m3_usize item_count, m3_usize slot_capacity)
{
    M3Allocator alloc;
    int rc;
...
int M3_CALL m3_grid_view_set_bind(M3GridView *view, M3ListBindFn bind, void *ctx)
{
    if (view == NULL) {
        return M3_ERR_INVALID_ARGUMENT;
    }
...
int M3_CALL m3_grid_view_get_scroll(const M3GridView *view, M3Scalar *out_offset)
{
    if (view == NULL || out_offset == NULL) {
        return M3_ERR_INVALID_ARGUMENT;
    }
...
int M3_CALL m3_grid_view_get_content_extent(const M3GridView *view, M3Scalar *out_extent)
{
    M3GridView temp;
    int rc;
...
int M3_CALL m3_grid_view_get_required_slots(const M3GridView *view, m3_usize *out_required)
{
    M3GridView temp;
    m3_usize first;
    m3_usize last;
...
int M3_CALL m3_grid_view_get_visible(const M3GridView *view, M3RenderNode ***out_nodes, m3_usize *out_count)
{
    if (view == NULL || out_nodes == NULL || out_count == NULL) {
        return M3_ERR_INVALID_ARGUMENT;
    }
