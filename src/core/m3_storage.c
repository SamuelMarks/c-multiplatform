...
#define M3_STORAGE_RETURN_IF_ERROR(rc)                                         \
  do {                                                                         \
    if ((rc) != M3_OK) {                                                       \
      return (rc);                                                             \
    }                                                                          \
...
static int m3_storage_mul_overflow(m3_usize a, m3_usize b,
                                   m3_usize *out_value) {
  m3_usize max_value;
...
static int m3_storage_add_overflow(m3_usize a, m3_usize b,
                                   m3_usize *out_value) {
  m3_usize max_value;
...
static int m3_storage_read_u32_le(const m3_u8 *src, m3_u32 *out_value) {
  *out_value = (m3_u32)src[0] | ((m3_u32)src[1] << 8) | ((m3_u32)src[2] << 16) |
               ((m3_u32)src[3] << 24);
  return M3_OK;
}

static int m3_storage_find_entry(const M3Storage *storage, const char *key,
...
static int m3_storage_grow(M3Storage *storage, m3_usize min_capacity) {
  ...
  rc = m3_storage_mul_overflow(new_capacity, (m3_usize)sizeof(M3StorageEntry),
                               &alloc_size);
  if (rc != M3_OK) {
    return rc;
  }
  ...
  if (clear_count > 0) {
    memset((M3StorageEntry *)new_entries + storage->entry_capacity, 0,
           (size_t)(clear_count * sizeof(M3StorageEntry)));
  }
...
int M3_CALL m3_storage_init(M3Storage *storage, const M3StorageConfig *config) {
  ...
  rc = m3_storage_mul_overflow(config->entry_capacity,
                               (m3_usize)sizeof(M3StorageEntry), &alloc_size);
  if (rc != M3_OK) {
    return rc;
  }
...
int M3_CALL m3_storage_put(M3Storage *storage, const char *utf8_key,
                           m3_usize key_len, const void *value,
...
int M3_CALL m3_storage_get(const M3Storage *storage, const char *utf8_key,
                           m3_usize key_len, void *out_value,
...
int M3_CALL m3_storage_contains(const M3Storage *storage, const char *utf8_key,
...
int M3_CALL m3_storage_remove(M3Storage *storage, const char *utf8_key,
                              m3_usize key_len) {
  m3_usize index;
  m3_usize tail_count;
  M3Bool found;
  ...
  if (tail_count > 0) {
    memmove(&storage->entries[index], &storage->entries[index + 1u],
            (size_t)(tail_count * sizeof(M3StorageEntry)));
  }
...
int M3_CALL m3_storage_clear(M3Storage *storage) {
  ...
  memset(storage->entries, 0,
         (size_t)(storage->entry_capacity * sizeof(M3StorageEntry)));
  storage->entry_count = 0;
  return rc;
}
...
int M3_CALL m3_storage_save(const M3Storage *storage, M3IO *io,
...
int M3_CALL m3_storage_load(M3Storage *storage, M3IO *io,
                            const char *utf8_path) {
  ...
  for (i = 0; i < (m3_usize)entry_count_u32; ++i) {
    ...
    if (offset + (m3_usize)key_len_u32 + (m3_usize)value_len_u32 >
        buffer_size) {
      rc = M3_ERR_CORRUPT;
      break;
    }

    rc = m3_storage_put(&temp_storage, (const char *)(buffer + offset),
                        (m3_usize)key_len_u32, buffer + offset + key_len_u32,
                        (m3_usize)value_len_u32, M3_FALSE);
    if (rc != M3_OK) {
      break;
    }
...
#ifdef M3_TESTING
int M3_CALL m3_storage_test_mul_overflow(m3_usize a, m3_usize b,
                                         m3_usize *out_value) {
  return m3_storage_mul_overflow(a, b, out_value);
}

int M3_CALL m3_storage_test_add_overflow(m3_usize a, m3_usize b,
                                         m3_usize *out_value) {
