...
static int m3_dialog_validate_text_style(const M3TextStyle *style,
...
static int m3_dialog_measure_optional_text(const M3TextBackend *backend,
                                           M3Handle font, const char *utf8,
                                           m3_usize len,
...
static M3Bool m3_dialog_point_in_rect(const M3Rect *rect, M3Scalar x,
                                      M3Scalar y) {
  if (rect == NULL) {
    return M3_FALSE;
  }
...
#ifdef M3_TESTING
int M3_CALL m3_dialog_test_point_in_rect(const M3Rect *rect, M3Scalar x,
...
int M3_CALL m3_dialog_test_validate_text_style(const M3TextStyle *style,
                                               M3Bool require_family) {
...
static int m3_dialog_destroy_fonts(const M3TextBackend *backend,
                                   M3Handle *fonts, m3_usize count) {
  int rc;
  int destroy_rc;
  m3_usize i;
...
static int m3_alert_dialog_compute_actions(const M3AlertDialog *dialog,
                                           M3Scalar *out_total_width,
                                           M3Scalar *out_height,
                                           M3Scalar *out_confirm_width,
                                           M3Scalar *out_dismiss_width) {
...
static int m3_alert_dialog_layout_actions(M3AlertDialog *dialog) {
  ...
  M3Scalar confirm_width;
  M3Scalar dismiss_width; /* GCOVR_EXCL_LINE */
  M3Scalar x;
  int rc;
...
static int m3_alert_dialog_widget_measure(void *widget, M3MeasureSpec width,
...
static int m3_alert_dialog_draw_action_text(
    const M3AlertDialog *dialog, const M3Rect *bounds,
    const M3TextMetrics *metrics, /* GCOVR_EXCL_LINE */
...
static int m3_alert_dialog_widget_paint(void *widget, M3PaintContext *ctx) {
  ...
  if (dialog->style.shadow_enabled == M3_TRUE) {
    rc = m3_shadow_paint(&dialog->style.shadow, ctx->gfx, &dialog->bounds,
                         shadow_clip);
    if (rc != M3_OK) {
      return rc; /* GCOVR_EXCL_LINE */
    }
...
static int m3_alert_dialog_widget_event(void *widget, const M3InputEvent *event,
                                        M3Bool *out_handled) {
  M3AlertDialog *dialog;
  M3Scalar x;
  M3Scalar y;
...
static int m3_fullscreen_dialog_draw_action_text(
    const M3FullScreenDialog *dialog, const M3Rect *bounds,
    const M3TextMetrics *metrics, const char *utf8, m3_usize len,
    M3PaintContext *ctx) {
  M3Scalar text_x;
  M3Scalar text_y;
  M3Scalar available_width;
...
static int m3_fullscreen_dialog_widget_paint(void *widget,
  ...
  if (dialog->style.shadow_enabled == M3_TRUE) {
    rc = m3_shadow_paint(&dialog->style.shadow, ctx->gfx,
...
static int
m3_snackbar_compute_action(const M3Snackbar *snackbar, M3Scalar *out_width,
...
static int m3_snackbar_widget_measure(void *widget, M3MeasureSpec width,
...
static int m3_snackbar_draw_action_text(
    const M3Snackbar *snackbar, const M3Rect *bounds,      /* GCOVR_EXCL_LINE */
    const M3TextMetrics *metrics,                          /* GCOVR_EXCL_LINE */
    const char *utf8, m3_usize len, M3PaintContext *ctx) { /* GCOVR_EXCL_LINE */
...
static int m3_snackbar_widget_event(void *widget, const M3InputEvent *event,
                                    M3Bool *out_handled) {
  M3Snackbar *snackbar;
  M3Scalar x;
  M3Scalar y;
...
int M3_CALL m3_alert_dialog_style_init(M3AlertDialogStyle *style) {
  ...
  rc = m3_shadow_set(&style->shadow, 0.0f, 8.0f, 16.0f, 0.0f,
...
int M3_CALL m3_alert_dialog_set_title(M3AlertDialog *dialog,
...
int M3_CALL m3_alert_dialog_set_body(M3AlertDialog *dialog,
                                     const char *utf8_body, m3_usize body_len) {
  if (dialog == NULL) {
    return M3_ERR_INVALID_ARGUMENT;
  }
...
int M3_CALL m3_alert_dialog_set_actions(M3AlertDialog *dialog,
...
int M3_CALL m3_alert_dialog_set_style(
    M3AlertDialog *dialog,
  ...
  rc = m3_text_font_create(&dialog->text_backend, &style->title_style,
                           &new_title);
  if (rc != M3_OK) {
    return rc;
  }
  rc =
      m3_text_font_create(&dialog->text_backend, &style->body_style, &new_body);
  if (rc != M3_OK) {
  ...
  rc = m3_text_font_create(&dialog->text_backend, &style->action_style,
                           &new_action);
  if (rc != M3_OK) {
...
int M3_CALL m3_alert_dialog_set_on_action(M3AlertDialog *dialog,
                                          M3AlertDialogOnAction on_action,
                                          void *ctx) {
  if (dialog == NULL) {
    return M3_ERR_INVALID_ARGUMENT;
  }
...
int M3_CALL m3_fullscreen_dialog_style_init(M3FullScreenDialogStyle *style) {
  ...
  shadow_color.b = 0.0f;
  shadow_color.a = 0.24f;
  rc = m3_shadow_set(&style->shadow, 0.0f, 6.0f, 12.0f, 0.0f,
...
int M3_CALL m3_fullscreen_dialog_init(
    M3FullScreenDialog *dialog,
    const M3TextBackend *backend, /* GCOVR_EXCL_LINE */
    const M3FullScreenDialogStyle *style, const char *utf8_title,
    m3_usize title_len, const char *utf8_body, m3_usize body_len) {
  M3Handle fonts[2];
  int rc; /* GCOVR_EXCL_LINE */
  int cleanup_rc;
...
int M3_CALL m3_fullscreen_dialog_set_title(M3FullScreenDialog *dialog,
                                           const char *utf8_title,
                                           m3_usize title_len) {
  if (dialog == NULL) {
    return M3_ERR_INVALID_ARGUMENT;
...
int M3_CALL m3_fullscreen_dialog_set_body(M3FullScreenDialog *dialog,
...
int M3_CALL m3_fullscreen_dialog_set_action(
...
int M3_CALL m3_fullscreen_dialog_set_style(
...
int M3_CALL m3_fullscreen_dialog_set_on_action(
    M3FullScreenDialog *dialog, M3FullScreenDialogOnAction on_action,
    void *ctx) {
  if (dialog == NULL) {
    return M3_ERR_INVALID_ARGUMENT;
  }
...
int M3_CALL m3_snackbar_init(M3Snackbar *snackbar, const M3TextBackend *backend,
  ...
  rc = m3_text_font_create(backend, &style->message_style,
                           &snackbar->message_font);
  if (rc != M3_OK) {
    return rc; /* GCOVR_EXCL_LINE */
  }
  rc = m3_text_font_create(backend, &style->action_style,
...
int M3_CALL m3_snackbar_set_message(M3Snackbar *snackbar,
...
int M3_CALL m3_snackbar_set_action(M3Snackbar *snackbar,
...
int M3_CALL m3_snackbar_set_style(M3Snackbar *snackbar,
                                  const M3SnackbarStyle *style) {
  ...
  rc = m3_text_font_create(&snackbar->text_backend, &style->message_style,
                           &new_message);
  if (rc != M3_OK) {
    return rc;
  }
  rc = m3_text_font_create(&snackbar->text_backend, &style->action_style,
                           &new_action);
  if (rc != M3_OK) {
    cleanup_rc =
...
int M3_CALL m3_snackbar_set_on_action(M3Snackbar *snackbar,
                                      M3SnackbarOnAction on_action, void *ctx) {
  if (snackbar == NULL) {
    return M3_ERR_INVALID_ARGUMENT; /* GCOVR_EXCL_LINE */
  }
