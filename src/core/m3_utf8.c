...
static int m3_utf8_decode(const m3_u8 *data, m3_usize length, m3_u32 *out_codepoint, m3_usize *out_advance)
{
    m3_u8 b0;
    m3_u8 b1;
    m3_u8 b2;
    ...
    if (b0 <= 0xF4u) {
        ...
        b2 = data[2];
        b3 = data[3];
        if (b1 < 0x80u || b1 > 0xBFu || b2 < 0x80u || b2 > 0xBFu || b3 < 0x80u || b3 > 0xBFu) {
            return M3_ERR_CORRUPT;
        }
#ifdef M3_TESTING
        if (!g_m3_utf8_relaxed_checks) {
...
int M3_CALL m3_utf8_validate(const char *data, m3_usize length, M3Bool *out_valid)
{
    const m3_u8 *bytes;
    m3_usize offset;
    m3_usize advance;
...
int M3_CALL m3_utf8_iter_init(M3Utf8Iter *iter, const char *data, m3_usize length)
{
...
int M3_CALL m3_utf8_iter_next(M3Utf8Iter *iter, m3_u32 *out_codepoint)
{
    ...
    rc = m3_utf8_decode(iter->data + iter->offset, iter->length - iter->offset, out_codepoint, &advance);
    if (rc != M3_OK) {
        return rc;
    }
...
int M3_CALL m3_utf8_test_decode(const m3_u8 *data, m3_usize length, m3_u32 *out_codepoint, m3_usize *out_advance)
{
