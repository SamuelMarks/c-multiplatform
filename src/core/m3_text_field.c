...
static int m3_text_field_color_set(M3Color *color, M3Scalar r, M3Scalar g, M3Scalar b, M3Scalar a)
{
    if (color == NULL) {
        return M3_ERR_INVALID_ARGUMENT;
    }
...
static int m3_text_field_validate_text_style(const M3TextStyle *style, M3Bool require_family)
{
    int rc;
...
static int m3_text_field_validate_style(const M3TextFieldStyle *style, M3Bool require_text_family,
    M3Bool require_label_family)
{
    ...
    rc = m3_text_field_validate_text_style(&style->text_style, require_text_family);
    if (rc != M3_OK) {
        return rc;
    }
    rc = m3_text_field_validate_text_style(&style->label_style, require_label_family);
    if (rc != M3_OK) {
        return rc;
    }
...
static int m3_text_field_add_overflow(m3_usize a, m3_usize b, m3_usize *out_value)
{
    m3_usize max_value;
...
static int m3_text_field_reserve(M3TextField *field, m3_usize required)
{
...
#ifdef M3_TESTING
        if (!m3_text_field_test_fail_point_match(M3_TEXT_FIELD_TEST_FAIL_RESERVE_BYPASS)) {
            return M3_OK;
        }
#else
        return M3_OK;
    ...
    rc = field->allocator.realloc(field->allocator.ctx, field->utf8, new_capacity, &mem);
#ifdef M3_TESTING
    if (m3_text_field_test_fail_point_match(M3_TEXT_FIELD_TEST_FAIL_RESERVE_REALLOC)) {
        rc = M3_ERR_IO;
    }
#endif
    if (rc != M3_OK) {
...
#ifdef M3_TESTING
    if (m3_text_field_test_fail_point_match(M3_TEXT_FIELD_TEST_FAIL_UTF8_VALIDATE)) {
        rc = M3_ERR_IO;
    }
#endif
    if (rc != M3_OK) {
...
static int m3_text_field_validate_offset(const char *utf8, m3_usize utf8_len, m3_usize offset)
{
    M3Utf8Iter iter;
    m3_u32 codepoint;
...
#ifdef M3_TESTING
    if (m3_text_field_test_fail_point_match(M3_TEXT_FIELD_TEST_FAIL_UTF8_ITER_INIT)) {
        rc = M3_ERR_IO;
    }
#endif
    if (rc != M3_OK) {
...
#ifdef M3_TESTING
        if (m3_text_field_test_fail_point_match(M3_TEXT_FIELD_TEST_FAIL_UTF8_ITER_NEXT)) {
            rc = M3_ERR_IO;
        }
        if (m3_text_field_test_fail_point_match(M3_TEXT_FIELD_TEST_FAIL_UTF8_ITER_NOT_FOUND)) {
            rc = M3_ERR_NOT_FOUND;
        }
#endif
...
static int m3_text_field_prev_offset(const char *utf8, m3_usize utf8_len, m3_usize offset, m3_usize *out_offset)
{
    M3Utf8Iter iter;
    m3_u32 codepoint;
    m3_usize last_offset;
...
static int m3_text_field_next_offset(const char *utf8, m3_usize utf8_len, m3_usize offset, m3_usize *out_offset)
{
    M3Utf8Iter iter;
    m3_u32 codepoint;
    int rc;
...
static int m3_text_field_update_text_metrics(M3TextField *field)
{
    ...
    rc = m3_text_measure_utf8(&field->text_backend, field->text_font, field->utf8, field->utf8_len, &field->text_metrics);
#ifdef M3_TESTING
    if (m3_text_field_test_fail_point_match(M3_TEXT_FIELD_TEST_FAIL_TEXT_MEASURE)) {
        rc = M3_ERR_IO;
    }
#endif
    if (rc != M3_OK) {
...
static int m3_text_field_update_label_metrics(M3TextField *field)
{
    ...
    rc = m3_text_measure_utf8(&field->text_backend, field->label_font, field->utf8_label, field->label_len, &field->label_metrics);
#ifdef M3_TESTING
    if (m3_text_field_test_fail_point_match(M3_TEXT_FIELD_TEST_FAIL_TEXT_MEASURE)) {
        rc = M3_ERR_IO;
    }
...
static int m3_text_field_update_placeholder_metrics(M3TextField *field)
{
    ...
    rc = m3_text_measure_utf8(&field->text_backend, field->text_font, field->utf8_placeholder, field->placeholder_len,
...
static int m3_text_field_update_font_metrics(M3TextField *field)
{
    ...
    rc = m3_text_font_metrics(&field->text_backend, field->text_font, &field->text_font_metrics);
#ifdef M3_TESTING
...
static int m3_text_field_sync_label(M3TextField *field)
{
    ...
    if (field->utf8_label == NULL || field->label_len == 0u) {
        ...
        rc = m3_anim_controller_start_timing(&field->label_anim, 0.0f, 0.0f, 0.0f, M3_ANIM_EASE_OUT);
#ifdef M3_TESTING
    ...
    rc = m3_anim_controller_start_timing(&field->label_anim, field->label_value, target, field->style.label_anim_duration,
        M3_ANIM_EASE_OUT);
#ifdef M3_TESTING
    if (m3_text_field_test_fail_point_match(M3_TEXT_FIELD_TEST_FAIL_ANIM_START)) {
        rc = M3_ERR_IO;
...
static int m3_text_field_set_text_internal(M3TextField *field, const char *utf8_text, m3_usize utf8_len, M3Bool notify)
{
    m3_usize required;
    int rc;
    ...
    if (notify == M3_TRUE && field->on_change != NULL) {
        rc = field->on_change(field->on_change_ctx, field, field->utf8_len ? field->utf8 : NULL, field->utf8_len);
        if (rc != M3_OK) {
            return rc;
        }
    }
...
static int m3_text_field_delete_range(M3TextField *field, m3_usize start, m3_usize end, M3Bool notify)
{
    m3_usize tail_len;
    int rc;
...
static int m3_text_field_measure_prefix(M3TextField *field, m3_usize offset, M3Scalar *out_width)
{
...
#ifdef M3_TESTING
    if (m3_text_field_test_fail_point_match(M3_TEXT_FIELD_TEST_FAIL_MEASURE_PREFIX)) {
        return M3_ERR_IO;
    }
#endif
    ...
    rc = m3_text_measure_utf8(&field->text_backend, field->text_font, field->utf8, offset, &metrics);
#ifdef M3_TESTING
    if (m3_text_field_test_fail_point_match(M3_TEXT_FIELD_TEST_FAIL_TEXT_MEASURE)) {
        rc = M3_ERR_IO;
    }
...
static int m3_text_field_offset_for_x(M3TextField *field, M3Scalar x, m3_usize *out_offset)
{
    M3Utf8Iter iter;
    m3_u32 codepoint;
...
static int m3_text_field_resolve_colors(const M3TextField *field, M3Color *out_container, M3Color *out_outline,
    M3Color *out_text, M3Color *out_label, M3Color *out_placeholder, M3Color *out_cursor, M3Color *out_selection,
    M3Color *out_handle)
{
    int rc;

    if (field == NULL || out_container == NULL || out_outline == NULL || out_text == NULL || out_label == NULL
        || out_placeholder == NULL || out_cursor == NULL || out_selection == NULL || out_handle == NULL) {
        return M3_ERR_INVALID_ARGUMENT;
    }
#ifdef M3_TESTING
    if (m3_text_field_test_fail_point_match(M3_TEXT_FIELD_TEST_FAIL_RESOLVE_COLORS)) {
        return M3_ERR_IO;
    }
#endif
    ...
    if (field->widget.flags & M3_WIDGET_FLAG_DISABLED) {
        ...
        *out_outline = (field->focused == M3_TRUE) ? field->style.focused_outline_color : field->style.outline_color;
...
static int m3_text_field_widget_measure(void *widget, M3MeasureSpec width, M3MeasureSpec height, M3Size *out_size)
{
    M3TextField *field;
    ...
    rc = m3_text_field_validate_style(&field->style, M3_TRUE, (field->utf8_label != NULL && field->label_len > 0u) ? M3_TRUE : M3_FALSE);
    if (rc != M3_OK) {
        return rc;
    }
...
static int m3_text_field_widget_paint(void *widget, M3PaintContext *ctx)
{
    ...
    rc = m3_text_field_resolve_colors(field, &container, &outline, &text_color, &label_color, &placeholder_color, &cursor_color,
    ...
    if (container.a > 0.0f) {
        rc = ctx->gfx->vtable->draw_rect(ctx->gfx->ctx, &inner, container, inner_corner);
        if (rc != M3_OK) {
            return rc;
        }
    }
    ...
    if (field->selection_start != field->selection_end && selection_color.a > 0.0f) {
        ...
        rc = ctx->gfx->vtable->draw_rect(ctx->gfx->ctx, &selection_rect, selection_color, 0.0f);
        if (rc != M3_OK) {
            return rc;
        }
    }
    ...
    if (field->utf8_len > 0u) {
        rc = ctx->gfx->text_vtable->draw_text(ctx->gfx->ctx, field->text_font, field->utf8, field->utf8_len, text_x,
            text_top + text_baseline, text_color);
        if (rc != M3_OK) {
            return rc;
        }
    } else if ((field->utf8_placeholder != NULL && field->placeholder_len > 0u)
        && (field->utf8_label == NULL || field->label_len == 0u || field->label_value > 0.0f)) {
        rc = ctx->gfx->text_vtable->draw_text(ctx->gfx->ctx, field->text_font, field->utf8_placeholder, field->placeholder_len,
            text_x, text_top + text_baseline, placeholder_color);
        if (rc != M3_OK) {
            return rc;
        }
    }

    if (field->utf8_label != NULL && field->label_len > 0u && field->label_font.id != 0u) {
        ...
        label_y = label_rest_y + (label_float_y - label_rest_y) * field->label_value;
        rc = ctx->gfx->text_vtable->draw_text(ctx->gfx->ctx, field->label_font, field->utf8_label, field->label_len, label_x,
            label_y, label_color);
        if (rc != M3_OK) {
            return rc;
        }
    ...
    if (field->focused == M3_TRUE && (field->widget.flags & M3_WIDGET_FLAG_DISABLED) == 0 && field->cursor_visible == M3_TRUE
        && field->style.cursor_width > 0.0f && cursor_color.a > 0.0f) {
        ...
        rc = ctx->gfx->vtable->draw_rect(ctx->gfx->ctx, &selection_rect, cursor_color, 0.0f);
        if (rc != M3_OK) {
            return rc;
        }
    }
    ...
    if (field->selection_start != field->selection_end && handle_color.a > 0.0f) {
        ...
        if (handle_radius > 0.0f && handle_height > 0.0f) {
            ...
            handle_rect.y = text_top + field->text_font_metrics.height - handle_height * 0.5f;
            rc = ctx->gfx->vtable->draw_rect(ctx->gfx->ctx, &handle_rect, handle_color, handle_radius);
            if (rc != M3_OK) {
                return rc;
            }
...
static int m3_text_field_widget_event(void *widget, const M3InputEvent *event, M3Bool *out_handled)
{
    M3TextField *field;
    m3_usize offset;
    m3_usize other;
    ...
    *out_handled = M3_FALSE;
    ...
    case M3_INPUT_POINTER_DOWN:
        rc = m3_text_field_offset_for_x(field, (M3Scalar)event->data.pointer.x - field->bounds.x - field->style.padding_x, &offset);
        if (rc != M3_OK) {
            return rc;
        }
        field->cursor = offset;
    ...
    case M3_INPUT_TEXT:
        ...
        rc = m3_text_field_insert_utf8(field, event->data.text.utf8, (m3_usize)event->data.text.length);
        if (rc != M3_OK) {
            return rc;
        }
        field->focused = M3_TRUE;
    ...
    case M3_INPUT_TEXT_UTF8:
        ...
        rc = m3_text_field_insert_utf8(field, event->data.text_utf8.utf8, event->data.text_utf8.length);
        if (rc != M3_OK) {
            return rc;
        }
        field->focused = M3_TRUE;
    ...
    case M3_INPUT_KEY_DOWN:
        if (event->data.key.key_code == 8u || event->data.key.key_code == 127u || event->data.key.key_code == 46u) {
            rc = (event->data.key.key_code == 46u) ? m3_text_field_delete_forward(field) : m3_text_field_backspace(field);
            if (rc != M3_OK) {
                return rc;
            }
            field->focused = M3_TRUE;
        ...
        if (event->data.key.key_code == 37u || event->data.key.key_code == 0x25u || event->data.key.key_code == 1073741904u) {
            rc = m3_text_field_prev_offset(field->utf8, field->utf8_len, field->cursor, &other);
            if (rc != M3_OK) {
                return rc;
            }
            field->cursor = other;
        ...
        if (event->data.key.key_code == 39u || event->data.key.key_code == 0x27u || event->data.key.key_code == 1073741903u) {
            rc = m3_text_field_next_offset(field->utf8, field->utf8_len, field->cursor, &other);
            if (rc != M3_OK) {
                return rc;
            }
            field->cursor = other;
        ...
        if (event->data.key.key_code == 36u || event->data.key.key_code == 0x24u || event->data.key.key_code == 1073741898u) {
            field->cursor = 0u;
        ...
        if (event->data.key.key_code == 35u || event->data.key.key_code == 0x23u || event->data.key.key_code == 1073741901u) {
...
static int m3_text_field_widget_get_semantics(void *widget, M3Semantics *out_semantics)
{
    M3TextField *field;
...
static int m3_text_field_widget_destroy(void *widget)
{
    ...
    if (field->owns_fonts == M3_TRUE) {
        if (field->text_font.id != 0u || field->text_font.generation != 0u) {
            if (field->text_backend.vtable != NULL && field->text_backend.vtable->destroy_font != NULL) {
                rc2 = field->text_backend.vtable->destroy_font(field->text_backend.ctx, field->text_font);
        ...
        if (field->label_font.id != 0u || field->label_font.generation != 0u) {
            if (field->text_backend.vtable != NULL && field->text_backend.vtable->destroy_font != NULL) {
                rc2 = field->text_backend.vtable->destroy_font(field->text_backend.ctx, field->label_font);
                if (rc2 != M3_OK && rc == M3_OK) {
                    rc = rc2;
                }
            } else if (rc == M3_OK) {
...
static const M3WidgetVTable g_m3_text_field_widget_vtable = {
    ...
    m3_text_field_widget_event,
    m3_text_field_widget_get_semantics,
    m3_text_field_widget_destroy
};
...
int M3_CALL m3_text_field_style_init(M3TextFieldStyle *style)
{
...
#ifdef M3_TESTING
    if (m3_text_field_test_fail_point_match(M3_TEXT_FIELD_TEST_FAIL_TEXT_STYLE_INIT)) {
        rc = M3_ERR_IO;
    }
#endif
    if (rc != M3_OK) {
...
#ifdef M3_TESTING
    if (m3_text_field_test_fail_point_match(M3_TEXT_FIELD_TEST_FAIL_LABEL_STYLE_INIT)) {
        rc = M3_ERR_IO;
    }
#endif
    if (rc != M3_OK) {
    ...
    rc = m3_text_field_color_set(&style->focused_outline_color, 0.2f, 0.4f, 0.9f, 1.0f);
    if (rc != M3_OK) {
        return rc;
    }
    rc = m3_text_field_color_set(&style->disabled_container_color, 0.95f, 0.95f, 0.95f, 1.0f);
    if (rc != M3_OK) {
        return rc;
    }
    rc = m3_text_field_color_set(&style->disabled_outline_color, 0.0f, 0.0f, 0.0f, 0.12f);
    if (rc != M3_OK) {
        return rc;
    }
    rc = m3_text_field_color_set(&style->disabled_text_color, 0.0f, 0.0f, 0.0f, 0.38f);
    if (rc != M3_OK) {
        return rc;
    }
    rc = m3_text_field_color_set(&style->disabled_label_color, 0.0f, 0.0f, 0.0f, 0.38f);
    if (rc != M3_OK) {
        return rc;
    }
    rc = m3_text_field_color_set(&style->placeholder_color, 0.0f, 0.0f, 0.0f, 0.6f);
    if (rc != M3_OK) {
        return rc;
    }
    rc = m3_text_field_color_set(&style->disabled_placeholder_color, 0.0f, 0.0f, 0.0f, 0.38f);
    if (rc != M3_OK) {
        return rc;
    }
...
int M3_CALL m3_text_field_init(M3TextField *field, const M3TextBackend *backend, const M3TextFieldStyle *style,
...
int M3_CALL m3_text_field_set_text(M3TextField *field, const char *utf8_text, m3_usize utf8_len)
{
    return m3_text_field_set_text_internal(field, utf8_text, utf8_len, M3_TRUE);
}

int M3_CALL m3_text_field_get_text(const M3TextField *field, const char **out_utf8, m3_usize *out_len)
{
...
int M3_CALL m3_text_field_insert_utf8(M3TextField *field, const char *utf8_text, m3_usize utf8_len)
{
    m3_usize insert_at;
    m3_usize tail_len;
    m3_usize new_len;
    ...
    if (field->selection_start != field->selection_end) {
        rc = m3_text_field_delete_range(field, field->selection_start, field->selection_end, M3_FALSE);
        if (rc != M3_OK) {
            return rc;
        }
    }
    ...
    if (tail_len > 0u && utf8_len > 0u) {
        memmove(field->utf8 + insert_at + utf8_len, field->utf8 + insert_at, tail_len);
    }
    if (utf8_len > 0u && utf8_text != NULL) {
...
int M3_CALL m3_text_field_delete_selection(M3TextField *field)
{
    ...
    return m3_text_field_delete_range(field, field->selection_start, field->selection_end, M3_TRUE);
}
...
int M3_CALL m3_text_field_backspace(M3TextField *field)
{
    ...
    rc = m3_text_field_prev_offset(field->utf8, field->utf8_len, field->cursor, &prev);
    if (rc != M3_OK) {
        return rc;
    }
...
int M3_CALL m3_text_field_delete_forward(M3TextField *field)
{
    ...
    rc = m3_text_field_next_offset(field->utf8, field->utf8_len, field->cursor, &next);
    if (rc != M3_OK) {
        return rc;
    }
...
int M3_CALL m3_text_field_set_label(M3TextField *field, const char *utf8_label, m3_usize label_len)
{
    int rc;
    ...
    if ((utf8_label != NULL && label_len > 0u) && (field->label_font.id == 0u && field->label_font.generation == 0u)) {
        if (field->style.label_style.utf8_family == NULL) {
            return M3_ERR_INVALID_ARGUMENT;
        }
        rc = m3_text_font_create(&field->text_backend, &field->style.label_style, &field->label_font);
        if (rc != M3_OK) {
            return rc;
        }
    }
...
int M3_CALL m3_text_field_set_placeholder(M3TextField *field, const char *utf8_placeholder, m3_usize placeholder_len)
{
    int rc;
...
int M3_CALL m3_text_field_set_style(M3TextField *field, const M3TextFieldStyle *style)
{
    ...
    need_label = (field->utf8_label != NULL && field->label_len > 0u) ? M3_TRUE : M3_FALSE;
    ...
    rc = m3_text_font_create(&field->text_backend, &style->text_style, &new_text_font);
    if (rc != M3_OK) {
        return rc;
    }

    if (need_label == M3_TRUE) {
        rc = m3_text_font_create(&field->text_backend, &style->label_style, &new_label_font);
        if (rc != M3_OK) {
...
int M3_CALL m3_text_field_set_selection(M3TextField *field, m3_usize start, m3_usize end)
{
    m3_usize tmp;
    int rc;
...
int M3_CALL m3_text_field_get_selection(const M3TextField *field, m3_usize *out_start, m3_usize *out_end)
{
...
int M3_CALL m3_text_field_get_cursor(const M3TextField *field, m3_usize *out_cursor)
{
...
int M3_CALL m3_text_field_step(M3TextField *field, M3Scalar dt, M3Bool *out_changed)
{
    M3Scalar value;
    M3Bool finished;
    M3Bool changed;
    ...
    if (field->focused == M3_TRUE && field->style.cursor_blink_period > 0.0f) {
        ...
        field->cursor_visible = (field->cursor_timer < field->style.cursor_blink_period * 0.5f) ? M3_TRUE : M3_FALSE;
...
int M3_CALL m3_text_field_set_on_change(M3TextField *field, M3TextFieldOnChange on_change, void *ctx)
{
    if (field == NULL) {
        return M3_ERR_INVALID_ARGUMENT;
    }
...
int M3_CALL m3_text_field_test_validate_style(const M3TextFieldStyle *style, M3Bool require_text_family,
    M3Bool require_label_family)
{
    return m3_text_field_validate_style(style, require_text_family, require_label_family);
}
...
int M3_CALL m3_text_field_test_resolve_colors(const M3TextField *field, M3Color *out_container, M3Color *out_outline,
    ...
    M3Color *out_handle)
{
    return m3_text_field_resolve_colors(field, out_container, out_outline, out_text, out_label, out_placeholder, out_cursor,
        out_selection, out_handle);
}
#endif
