...
int M3_CALL m3_ripple_start(M3Ripple *ripple, M3Scalar center_x, M3Scalar center_y, M3Scalar max_radius,
...
int M3_CALL m3_ripple_step(M3Ripple *ripple, M3Scalar dt, M3Bool *out_finished)
{
    M3Bool running;
    M3Bool finished;
    M3Scalar value;
    ...
    if (ripple->state != M3_RIPPLE_STATE_EXPANDING && ripple->state != M3_RIPPLE_STATE_FADING) {
        return M3_ERR_INVALID_ARGUMENT;
    }
...
int M3_CALL m3_ripple_is_active(const M3Ripple *ripple, M3Bool *out_active)
{
    ...
    if (ripple->state != M3_RIPPLE_STATE_IDLE && ripple->state != M3_RIPPLE_STATE_EXPANDING
        && ripple->state != M3_RIPPLE_STATE_FADING) {
        return M3_ERR_INVALID_ARGUMENT;
    }
...
int M3_CALL m3_ripple_compute_max_radius(const M3Rect *bounds, M3Scalar center_x, M3Scalar center_y,
    M3Scalar *out_radius)
{
    M3Scalar max_radius;
    M3Scalar dx;
    ...
    dx = center_x - x0;
    dy = center_y - y0;
    max_radius = (M3Scalar)sqrt((double)dx * (double)dx + (double)dy * (double)dy);
...
int M3_CALL m3_ripple_paint(const M3Ripple *ripple, M3Gfx *gfx, const M3Rect *clip, M3Scalar corner_radius)
{
    M3Rect rect;
    M3Color color;
    int rc;
    ...
    if (ripple->state == M3_RIPPLE_STATE_IDLE || ripple->opacity <= 0.0f || ripple->radius <= 0.0f) {
        return M3_OK;
    }
    if (ripple->state != M3_RIPPLE_STATE_EXPANDING && ripple->state != M3_RIPPLE_STATE_FADING) {
        return M3_ERR_INVALID_ARGUMENT;
...
int M3_CALL m3_shadow_set(M3Shadow *shadow, M3Scalar offset_x, M3Scalar offset_y, M3Scalar blur_radius,
...
int M3_CALL m3_shadow_paint(const M3Shadow *shadow, M3Gfx *gfx, const M3Rect *rect, const M3Rect *clip)
{
    M3Rect layer_rect;
    M3Color layer_color;
    ...
    if (shadow->blur_radius < 0.0f || shadow->spread < 0.0f || shadow->corner_radius < 0.0f) {
        return M3_ERR_RANGE;
    }
    if (shadow->layers == 0) {
        return M3_ERR_RANGE;
    ...
    for (i = 0; i < shadow->layers; ++i) {
        ...
        rc = gfx->vtable->draw_rect(gfx->ctx, &layer_rect, layer_color, layer_corner);
        if (rc != M3_OK) {
            draw_rc = rc;
            break;
        }
