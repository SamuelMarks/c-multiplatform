...
static int m3_render_list_reserve(M3RenderList *list, m3_usize additional)
{
    ...
    rc = m3_render_mul_overflow(new_capacity, (m3_usize)sizeof(M3RenderCmd), &bytes);
    if (rc != M3_OK) {
        return rc;
    }

    rc = list->allocator.realloc(list->allocator.ctx, list->commands, bytes, &mem);
    if (rc != M3_OK) {
        return rc;
    }
...
static int m3_render_record_begin_frame(void *gfx, M3Handle window, m3_i32 width, m3_i32 height, M3Scalar dpi_scale)
{
...
static int m3_render_record_draw_rect(void *gfx, const M3Rect *rect, M3Color color, M3Scalar corner_radius)
{
    M3RenderRecorder *recorder;
    M3RenderCmd cmd;
    int rc;
...
static int m3_render_record_draw_line(void *gfx, M3Scalar x0, M3Scalar y0, M3Scalar x1, M3Scalar y1, M3Color color, M3Scalar thickness)
{
    M3RenderRecorder *recorder;
    M3RenderCmd cmd;
...
static int m3_render_record_create_texture(void *gfx, m3_i32 width, m3_i32 height, m3_u32 format, const void *pixels,
...
static int m3_render_record_update_texture(void *gfx, M3Handle texture, m3_i32 x, m3_i32 y, m3_i32 width, m3_i32 height,
...
static int m3_render_record_draw_texture(void *gfx, M3Handle texture, const M3Rect *src, const M3Rect *dst, M3Scalar opacity)
{
    M3RenderRecorder *recorder;
    M3RenderCmd cmd;
    int rc;
...
static int m3_render_record_draw_text(void *text, M3Handle font, const char *utf8, m3_usize utf8_len, M3Scalar x, M3Scalar y, M3Color color)
{
    M3RenderRecorder *recorder;
    M3RenderCmd cmd;
...
static int m3_render_record_create_font(void *text, const char *utf8_family, m3_i32 size_px, m3_i32 weight, M3Bool italic,
    M3Handle *out_font)
{
...
static int m3_render_record_measure_text(void *text, M3Handle font, const char *utf8, m3_usize utf8_len, M3Scalar *out_width,
...
static const M3TextVTable g_m3_render_record_text_vtable = {
    m3_render_record_create_font,
    m3_render_record_destroy_font,
    m3_render_record_measure_text,
    m3_render_record_draw_text
};
...
static int m3_render_validate_node(const M3RenderNode *node)
{
    ...
    if (node->widget == NULL || node->widget->vtable == NULL || node->widget->vtable->paint == NULL) {
        return M3_ERR_INVALID_ARGUMENT;
    }
...
static int m3_render_build_node(const M3RenderNode *node, const M3Rect *parent_clip, M3Scalar dpi_scale, M3RenderRecorder *recorder)
{
    M3Rect clip;
...
int M3_CALL m3_render_list_init(M3RenderList *list, const M3Allocator *allocator, m3_usize initial_capacity)
{
    M3Allocator chosen;
    m3_usize bytes;
    void *mem;
    ...
    rc = m3_render_mul_overflow(initial_capacity, (m3_usize)sizeof(M3RenderCmd), &bytes);
    if (rc != M3_OK) {
        return rc;
    }
    ...
    for (i = 0; i < list->count; ++i) {
        ...
        switch (cmd->type) {
            case M3_RENDER_CMD_BEGIN_FRAME:
                ...
                rc = gfx->vtable->begin_frame(gfx->ctx, cmd->data.begin_frame.window, cmd->data.begin_frame.width,
            ...
            case M3_RENDER_CMD_DRAW_RECT:
                ...
                rc = gfx->vtable->draw_rect(gfx->ctx, &cmd->data.draw_rect.rect, cmd->data.draw_rect.color,
            ...
            case M3_RENDER_CMD_DRAW_LINE:
                ...
                rc = gfx->vtable->draw_line(gfx->ctx, cmd->data.draw_line.x0, cmd->data.draw_line.y0,
                    cmd->data.draw_line.x1, cmd->data.draw_line.y1, cmd->data.draw_line.color, cmd->data.draw_line.thickness);
                break;
            ...
            case M3_RENDER_CMD_SET_TRANSFORM:
                ...
                rc = gfx->vtable->set_transform(gfx->ctx, &cmd->data.set_transform.transform);
                break;
            case M3_RENDER_CMD_DRAW_TEXTURE:
                if (gfx->vtable->draw_texture == NULL) {
                    return M3_ERR_UNSUPPORTED;
                }
                rc = gfx->vtable->draw_texture(gfx->ctx, cmd->data.draw_texture.texture, &cmd->data.draw_texture.src,
            ...
            case M3_RENDER_CMD_DRAW_TEXT:
                ...
                rc = gfx->text_vtable->draw_text(gfx->ctx, cmd->data.draw_text.font, cmd->data.draw_text.utf8,
                    cmd->data.draw_text.utf8_len, cmd->data.draw_text.x, cmd->data.draw_text.y, cmd->data.draw_text.color);
                break;
            default:
                return M3_ERR_INVALID_ARGUMENT;
        }
...
int M3_CALL m3_render_node_init(M3RenderNode *node, M3Widget *widget, const M3Rect *bounds)
{
    int rc;
...
int M3_CALL m3_render_node_set_bounds(M3RenderNode *node, const M3Rect *bounds)
{
    int rc;
...
int M3_CALL m3_render_node_set_children(M3RenderNode *node, M3RenderNode **children, m3_usize count)
{
    m3_usize i;
...
int M3_CALL m3_render_build(const M3RenderNode *root, M3RenderList *list, M3Scalar dpi_scale)
{
    M3RenderRecorder recorder;
...
#ifdef M3_TESTING
int M3_CALL m3_render_test_add_overflow(m3_usize a, m3_usize b, m3_usize *out_value)
{
    return m3_render_add_overflow(a, b, out_value);
}

int M3_CALL m3_render_test_mul_overflow(m3_usize a, m3_usize b, m3_usize *out_value)
{
