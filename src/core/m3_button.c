...
static int m3_button_color_set(M3Color *color, M3Scalar r, M3Scalar g,
...
static int m3_button_color_with_alpha(const M3Color *base, M3Scalar alpha,
                                      M3Color *out_color) {
  int rc;
...
static int m3_button_validate_text_style(const M3TextStyle *style,
...
static int m3_button_metrics_update(M3Button *button) {
  ...
  rc = m3_text_measure_utf8(&button->text_backend, button->font,
...
int M3_CALL m3_button_style_init_fab(M3ButtonStyle *style) {
  ...
  rc = m3_shadow_set(&style->shadow, 0.0f, 4.0f, 10.0f, 0.0f,
...
static int m3_button_widget_measure(void *widget, M3MeasureSpec width,
...
static int m3_button_widget_event(void *widget, const M3InputEvent *event,
                                  M3Bool *out_handled) {
  M3Button *button;
  M3Scalar center_x;
  M3Scalar center_y;
  ...
  *out_handled = M3_FALSE;
  ...
  case M3_INPUT_POINTER_DOWN:
    ...
    rc = m3_ripple_compute_max_radius(&button->bounds, center_x, center_y,
                                      &max_radius);
#ifdef M3_TESTING
    ...
    rc = m3_ripple_start(&button->ripple, center_x, center_y, max_radius,
...
int M3_CALL m3_button_init(M3Button *button, const M3TextBackend *backend,
...
int M3_CALL m3_button_set_label(M3Button *button, const char *utf8_label,
                                m3_usize utf8_len) {
  if (button == NULL) {
    return M3_ERR_INVALID_ARGUMENT;
  }
...
int M3_CALL m3_button_set_style(M3Button *button, const M3ButtonStyle *style) {
  ...
  rc =
      m3_text_font_create(&button->text_backend, &style->text_style, &new_font);
  if (rc != M3_OK) {
    return rc;
  }
...
int M3_CALL m3_button_set_on_click(M3Button *button, M3ButtonOnClick on_click,
                                   void *ctx) {
  if (button == NULL) {
    return M3_ERR_INVALID_ARGUMENT;
  }
