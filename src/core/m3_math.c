...
int M3_CALL m3_rect_intersect(const M3Rect *a, const M3Rect *b, M3Rect *out_rect, M3Bool *out_has_intersection)
{
    M3Scalar ax2;
    M3Scalar ay2;
    M3Scalar bx2;
    ...
    if (a == NULL || b == NULL || out_rect == NULL || out_has_intersection == NULL) {
...
int M3_CALL m3_rect_contains_point(const M3Rect *rect, M3Scalar x, M3Scalar y, M3Bool *out_contains)
{
...
int M3_CALL m3_mat3_transform_point(const M3Mat3 *m, M3Scalar x, M3Scalar y, M3Scalar *out_x, M3Scalar *out_y)
{
    M3Scalar tx;
    M3Scalar ty;
    M3Scalar tw;
