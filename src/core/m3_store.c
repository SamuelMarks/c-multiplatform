...
static int m3_store_history_push(m3_u8 *buffer, m3_usize capacity,
                                 m3_usize state_size, m3_usize *io_count,
                                 const void *state) {
  m3_usize offset;
  m3_usize copy_size;
...
static int m3_store_history_pop(const m3_u8 *buffer, m3_usize capacity,
                                m3_usize state_size, m3_usize *io_count,
                                void *out_state) {
  m3_usize offset;
...
static int m3_store_copy_history(const m3_u8 *buffer, m3_usize capacity,
                                 m3_usize state_size, m3_usize count,
...
int M3_CALL m3_store_init(M3Store *store, const M3StoreConfig *config,
  ...
  if (store->undo_capacity > 0) {
    rc = m3_store_mul_overflow(store->state_size, store->undo_capacity,
                               &history_bytes);
    if (rc != M3_OK) {
      memset(store, 0, sizeof(*store));
      return rc;
...
int M3_CALL m3_store_dispatch(M3Store *store, const M3Action *action) {
  ...
  rc = m3_store_history_push(store->undo_buffer, store->undo_capacity,
...
int M3_CALL m3_store_get_state_ptr(const M3Store *store, const void **out_state,
                                   m3_usize *out_size) {
...
int M3_CALL m3_store_get_state(const M3Store *store, void *out_state,
                               m3_usize state_size) {
...
int M3_CALL m3_store_undo(M3Store *store) {
  ...
  rc = m3_store_history_push(store->redo_buffer, store->redo_capacity,
  ...
  rc =
      m3_store_history_pop(store->undo_buffer, store->undo_capacity,
...
int M3_CALL m3_store_redo(M3Store *store) {
  ...
  rc =
      m3_store_history_pop(store->redo_buffer, store->redo_capacity,
...
int M3_CALL m3_store_copy_undo_state(const M3Store *store,
                                     m3_usize index_from_latest,
                                     void *out_state, m3_usize state_size) {
  if (store == NULL || out_state == NULL) {
    return M3_ERR_INVALID_ARGUMENT;
  }
  if (store->state == NULL) {
  ...
  return m3_store_copy_history(store->undo_buffer, store->undo_capacity,
                               store->state_size, store->undo_count,
                               index_from_latest, out_state);
}

int M3_CALL m3_store_copy_redo_state(const M3Store *store,
                                     m3_usize index_from_latest,
                                     void *out_state, m3_usize state_size) {
  if (store == NULL || out_state == NULL) {
    return M3_ERR_INVALID_ARGUMENT;
  ...
  return m3_store_copy_history(store->redo_buffer, store->redo_capacity,
                               store->state_size, store->redo_count,
                               index_from_latest, out_state);
}

#ifdef M3_TESTING
int M3_CALL m3_store_test_mul_overflow(m3_usize a, m3_usize b,
                                       m3_usize *out_value) {
  return m3_store_mul_overflow(a, b, out_value);
}

int M3_CALL m3_store_test_history_push(m3_u8 *buffer, m3_usize capacity,
                                       m3_usize state_size, m3_usize *io_count,
                                       const void *state) {
  return m3_store_history_push(buffer, capacity, state_size, io_count, state);
}

int M3_CALL m3_store_test_history_pop(const m3_u8 *buffer, m3_usize capacity,
                                      m3_usize state_size, m3_usize *io_count,
                                      void *out_state) {
  return m3_store_history_pop(buffer, capacity, state_size, io_count,
                              out_state);
}

int M3_CALL m3_store_test_copy_history(const m3_u8 *buffer, m3_usize capacity,
                                       m3_usize state_size, m3_usize count,
                                       m3_usize index_from_latest,
                                       void *out_state) {
  return m3_store_copy_history(buffer, capacity, state_size, count,
                               index_from_latest, out_state);
}
#endif
