...
int M3_CALL m3_network_init(M3NetworkClient *client, const M3NetworkConfig *config)
{
    M3Network network;
    ...
    if (client->network.vtable != NULL || client->network.ctx != NULL || client->ready != M3_FALSE) {
        return M3_ERR_STATE;
    }
    if (config->env == NULL || config->env->vtable == NULL || config->env->vtable->get_network == NULL) {
        return M3_ERR_INVALID_ARGUMENT;
    }
...
int M3_CALL m3_network_request(M3NetworkClient *client, const M3NetworkRequest *request, M3NetworkResponse *out_response)
{
    int rc;
    ...
    if (client->allocator.alloc == NULL || client->allocator.realloc == NULL || client->allocator.free == NULL) {
        return M3_ERR_INVALID_ARGUMENT;
    }
    ...
    rc = client->network.vtable->request(client->network.ctx, request, &client->allocator, out_response);
    if (rc != M3_OK) {
        return rc;
    }
...
int M3_CALL m3_network_response_free(M3NetworkClient *client, M3NetworkResponse *response)
{
    int rc;
    ...
    rc = client->network.vtable->free_response(client->network.ctx, &client->allocator, response);
    if (rc != M3_OK) {
        return rc;
    }
...
int M3_CALL m3_network_copy_response_body(const M3NetworkResponse *response, void *dst, m3_usize dst_capacity,
    m3_usize *out_size)
{
