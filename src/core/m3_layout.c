...
static int m3_layout_apply_spec(M3Scalar size, M3LayoutMeasureSpec spec, M3Scalar *out_size)
{
    if (out_size == NULL) {
        return M3_ERR_INVALID_ARGUMENT;
    }
...
static M3Scalar m3_layout_resolve_available(M3Scalar style_size, M3LayoutMeasureSpec spec)
{
    M3Scalar available;
...
static int m3_layout_measure_node(M3LayoutNode *node, M3LayoutMeasureSpec width, M3LayoutMeasureSpec height);

static int m3_layout_measure_leaf(M3LayoutNode *node, M3LayoutMeasureSpec width, M3LayoutMeasureSpec height, M3Size *out_size)
{
    ...
    if (node->measure != NULL) {
        rc = node->measure(node->measure_ctx, measure_width, measure_height, &measured);
        if (rc != M3_OK) {
            return rc;
        }
    } else {
...
static int m3_layout_measure_row(M3LayoutNode *node, M3LayoutMeasureSpec width, M3LayoutMeasureSpec height, M3Size *out_size)
{
    ...
    padding_lr = node->style.padding.left + node->style.padding.right;
    ...
    if (available_width >= 0.0f) {
        inner_width_limit = m3_layout_clamp_non_negative(available_width - padding_lr);
    } else {
        inner_width_limit = -1.0f;
    }

    if (available_height >= 0.0f) {
        inner_height_limit = m3_layout_clamp_non_negative(available_height - padding_tb);
    } else {
    ...
    wrap_enabled = (node->style.wrap == M3_LAYOUT_WRAP_YES && inner_width_limit >= 0.0f) ? M3_TRUE : M3_FALSE;
    ...
    for (i = 0; i < node->child_count; ++i) {
        ...
        rc = m3_layout_measure_node(child, child_width_spec, child_height_spec);
        if (rc != M3_OK) {
            return rc;
        }
        ...
        if (wrap_enabled && line_main > 0.0f && line_main + child_main > inner_width_limit) {
...
static int m3_layout_measure_column(M3LayoutNode *node, M3LayoutMeasureSpec width, M3LayoutMeasureSpec height, M3Size *out_size)
{
    M3LayoutMeasureSpec child_width_spec;
    M3LayoutMeasureSpec child_height_spec;
    M3Scalar padding_lr;
    ...
    wrap_enabled = (node->style.wrap == M3_LAYOUT_WRAP_YES && inner_height_limit >= 0.0f) ? M3_TRUE : M3_FALSE;
    ...
    for (i = 0; i < node->child_count; ++i) {
        ...
        if (wrap_enabled && line_main > 0.0f && line_main + child_main > inner_height_limit) {
            if (line_main > content_main_max) {
                content_main_max = line_main;
            }
            content_cross_sum += line_cross;
...
static int m3_layout_measure_node(M3LayoutNode *node, M3LayoutMeasureSpec width, M3LayoutMeasureSpec height)
{
    ...
    if (node->child_count == 0) {
        ...
        if (node->style.direction == M3_LAYOUT_DIRECTION_ROW) {
            rc = m3_layout_measure_row(node, width, height, &measured);
        } else {
            rc = m3_layout_measure_column(node, width, height, &measured);
        }
    }
...
static int m3_layout_layout_children_row(M3LayoutNode *node, M3Scalar origin_x, M3Scalar origin_y, M3Scalar width, M3Scalar height);
static int m3_layout_layout_children_column(M3LayoutNode *node, M3Scalar origin_x, M3Scalar origin_y, M3Scalar width, M3Scalar height);

static int m3_layout_layout_node(M3LayoutNode *node, M3Scalar origin_x, M3Scalar origin_y, M3Scalar width, M3Scalar height)
{
    int rc;
    ...
    if (node->style.direction == M3_LAYOUT_DIRECTION_ROW) {
        rc = m3_layout_layout_children_row(node, origin_x, origin_y, width, height);
    } else if (node->style.direction == M3_LAYOUT_DIRECTION_COLUMN) {
        rc = m3_layout_layout_children_column(node, origin_x, origin_y, width, height);
    } else {
...
static int m3_layout_layout_children_row(M3LayoutNode *node, M3Scalar origin_x, M3Scalar origin_y, M3Scalar width, M3Scalar height)
{
    ...
    padding_left = node->style.padding.left;
    padding_top = node->style.padding.top;
    padding_right = node->style.padding.right;
    ...
    wrap_enabled = (node->style.wrap == M3_LAYOUT_WRAP_YES && inner_width >= 0.0f) ? M3_TRUE : M3_FALSE;
    ...
    while (index < node->child_count) {
        ...
        while (end < node->child_count) {
            ...
            if (wrap_enabled && line_count > 0 && line_main + child_main > inner_width) {
                break;
            }
        ...
        for (i = 0; i < line_count; ++i) {
            ...
            child_x = origin_x + padding_left + cursor_main;
            child_y = origin_y + padding_top + cursor_cross + cross_offset;

            rc = m3_layout_layout_node(child, child_x, child_y, child_main, child_cross);
            if (rc != M3_OK) {
                return rc;
            }
...
static int m3_layout_layout_children_column(M3LayoutNode *node, M3Scalar origin_x, M3Scalar origin_y, M3Scalar width, M3Scalar height)
{
    ...
    wrap_enabled = (node->style.wrap == M3_LAYOUT_WRAP_YES && inner_height >= 0.0f) ? M3_TRUE : M3_FALSE;
    ...
    while (index < node->child_count) {
        ...
        while (end < node->child_count) {
            ...
            if (wrap_enabled && line_count > 0 && line_main + child_main > inner_height) {
                break;
            }
        ...
        for (i = 0; i < line_count; ++i) {
            ...
            child_x = origin_x + padding_left + cursor_cross + cross_offset;
            child_y = origin_y + padding_top + cursor_main;

            rc = m3_layout_layout_node(child, child_x, child_y, child_cross, child_main);
            if (rc != M3_OK) {
                return rc;
            }
...
int M3_CALL m3_layout_measure_spec_init(M3LayoutMeasureSpec *spec, m3_u32 mode, M3Scalar size)
{
    if (spec == NULL) {
        return M3_ERR_INVALID_ARGUMENT;
    }
...
int M3_CALL m3_layout_edges_set(M3LayoutEdges *edges, M3Scalar left, M3Scalar top, M3Scalar right, M3Scalar bottom)
{
    if (edges == NULL) {
        return M3_ERR_INVALID_ARGUMENT;
    }
...
int M3_CALL m3_layout_node_init(M3LayoutNode *node, const M3LayoutStyle *style)
{
    int rc;
...
int M3_CALL m3_layout_node_set_children(M3LayoutNode *node, M3LayoutNode **children, m3_usize count)
{
    m3_usize i;
...
int M3_CALL m3_layout_node_set_measure(M3LayoutNode *node, M3LayoutMeasureFn measure, void *ctx)
{
    if (node == NULL) {
        return M3_ERR_INVALID_ARGUMENT;
    }
...
int M3_CALL m3_layout_node_get_measured(const M3LayoutNode *node, M3Size *out_size)
{
    if (node == NULL || out_size == NULL) {
        return M3_ERR_INVALID_ARGUMENT;
    }
...
int M3_CALL m3_layout_node_get_layout(const M3LayoutNode *node, M3Rect *out_rect)
{
...
int M3_CALL m3_layout_compute(M3LayoutNode *root, M3LayoutMeasureSpec width, M3LayoutMeasureSpec height)
{
    int rc;
    ...
    rc = m3_layout_measure_node(root, width, height);
    if (rc != M3_OK) {
        return rc;
    }

    rc = m3_layout_layout_node(root, 0.0f, 0.0f, root->measured.width, root->measured.height);
    if (rc != M3_OK) {
        return rc;
    }
...
#ifdef M3_TESTING
int M3_CALL m3_layout_test_validate_measure_spec(const M3LayoutMeasureSpec *spec)
{
    return m3_layout_validate_measure_spec(spec);
}
...
int M3_CALL m3_layout_test_apply_spec(M3Scalar size, M3LayoutMeasureSpec spec, M3Scalar *out_size)
{
    return m3_layout_apply_spec(size, spec, out_size);
}

int M3_CALL m3_layout_test_measure_leaf(M3LayoutNode *node, M3LayoutMeasureSpec width, M3LayoutMeasureSpec height, M3Size *out_size)
{
    return m3_layout_measure_leaf(node, width, height, out_size);
}

int M3_CALL m3_layout_test_measure_row(M3LayoutNode *node, M3LayoutMeasureSpec width, M3LayoutMeasureSpec height, M3Size *out_size)
{
    return m3_layout_measure_row(node, width, height, out_size);
}
...
{
    return m3_layout_measure_node(node, width, height);
}

int M3_CALL m3_layout_test_layout_node(M3LayoutNode *node, M3Scalar x, M3Scalar y, M3Scalar width, M3Scalar height)
{
    return m3_layout_layout_node(node, x, y, width, height);
}

int M3_CALL m3_layout_test_layout_children_row(M3LayoutNode *node, M3Scalar x, M3Scalar y, M3Scalar width, M3Scalar height)
{
    return m3_layout_layout_children_row(node, x, y, width, height);
}

int M3_CALL m3_layout_test_layout_children_column(M3LayoutNode *node, M3Scalar x, M3Scalar y, M3Scalar width, M3Scalar height)
{
    return m3_layout_layout_children_column(node, x, y, width, height);
}
#endif
