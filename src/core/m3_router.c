...
static void m3_router_slice_set(M3UriSlice *slice, const char *data, m3_usize length)
{
    if (slice == NULL) {
        return;
    }
...
int M3_CALL m3_uri_parse(const char *uri, M3Uri *out_uri)
{
    ...
    if (scheme_end > 0) {
        ...
        m3_router_slice_set(&out_uri->authority, uri + authority_start, authority_end - authority_start);
    ...
    if (path_end > path_start) {
        m3_router_slice_set(&out_uri->path, uri + path_start, path_end - path_start);
    }
    if (query_start < len) {
        m3_router_slice_set(&out_uri->query, uri + query_start, query_end - query_start);
    }
    if (fragment_start < len) {
        m3_router_slice_set(&out_uri->fragment, uri + fragment_start, len - fragment_start);
    }

    return M3_OK;
}

static int m3_router_slice_equals(const M3UriSlice *slice, const char *key, m3_usize key_len, M3Bool *out_equal)
{
...
int M3_CALL m3_uri_query_find(const M3Uri *uri, const char *key, M3UriSlice *out_value, M3Bool *out_found)
{
    m3_usize key_len;
    m3_usize start;
    m3_usize end;
    ...
    while (start <= uri->query.length) {
        ...
        if (equal == M3_TRUE) {
            if (eq_pos < end && uri->query.data[eq_pos] == '=') {
                m3_router_slice_set(out_value, uri->query.data + eq_pos + 1, end - eq_pos - 1);
            } else {
...
static void m3_route_next_segment(const char *str, m3_usize len, m3_usize *index, const char **out_seg, m3_usize *out_len)
{
    m3_usize i;
    m3_usize start;
...
static M3Bool m3_route_has_more_segments(const char *str, m3_usize len, m3_usize index)
{
    m3_usize i;
...
static int m3_route_match_len(const char *pattern, m3_usize pattern_len, const char *path, m3_usize path_len, M3RouteParam *params, m3_usize max_params, m3_usize *out_param_count, M3Bool *out_match)
{
    const char *pat_str;
    const char *path_str;
    m3_usize pat_len;
...
static int m3_router_find_route(const M3Router *router, const char *path, m3_usize path_len, const M3Route **out_route)
{
    m3_usize i;
    M3Bool match;
    ...
    for (i = 0; i < router->route_count; ++i) {
        ...
        match = M3_FALSE;
        rc = m3_route_match_len(route->pattern, pattern_len, path, path_len, NULL, 0, NULL, &match);
        if (rc != M3_OK) {
            return rc;
        }
        if (match == M3_TRUE) {
...
static int m3_router_copy_path(M3Router *router, const char *path, m3_usize path_len, char **out_copy)
{
    m3_usize alloc_size;
    void *mem;
    int rc;
...
static int m3_router_navigate_path_len(M3Router *router, const char *path, m3_usize path_len, void **out_component)
{
    const M3Route *route;
    char *path_copy;
    void *component;
...
int M3_CALL m3_router_init(M3Router *router, const M3RouterConfig *config)
{
    ...
    if (config->stack_capacity > 0) {
        if (config->stack_capacity > m3_router_usize_max_value() / sizeof(M3RouteEntry)) {
            return M3_ERR_OVERFLOW;
        }
    }
    ...
    if (config->routes != NULL) {
        for (i = 0; i < config->route_count; ++i) {
            if (config->routes[i].pattern == NULL || config->routes[i].build == NULL) {
                return M3_ERR_INVALID_ARGUMENT;
            }
...
int M3_CALL m3_router_navigate(M3Router *router, const char *path, void **out_component)
{
    m3_usize path_len;
    int rc;
...
int M3_CALL m3_router_navigate_uri(M3Router *router, const char *uri, void **out_component)
{
    M3Uri parsed;
    const char *path;
    m3_usize path_len;
...
int M3_CALL m3_router_get_current(const M3Router *router, const char **out_path, void **out_component)
{
...
int M3_CALL m3_router_clear(M3Router *router)
{
    ...
    while (router->stack_size > 0) {
        entry_rc = m3_router_release_entry(router, &router->stack[router->stack_size - 1]);
        router->stack_size -= 1;
...
int M3_CALL m3_route_test_match(const char *pattern, const char *path, M3RouteParam *params, m3_usize max_params, m3_usize *out_param_count, M3Bool *out_match)
{
    ...
    return m3_route_match_len(pattern, pattern_len, path, path_len, params, max_params, out_param_count, out_match);
}
...
int M3_CALL m3_router_test_slice_set(M3UriSlice *slice, const char *data, m3_usize length)
{
    m3_router_slice_set(slice, data, length);
    return M3_OK;
}

int M3_CALL m3_router_test_slice_equals(const M3UriSlice *slice, const char *key, m3_usize key_len, M3Bool *out_equal)
{
...
int M3_CALL m3_router_test_next_segment(const char *str, m3_usize len, m3_usize *index, const char **out_seg, m3_usize *out_len)
{
    m3_route_next_segment(str, len, index, out_seg, out_len);
    return M3_OK;
}

int M3_CALL m3_router_test_has_more_segments(const char *str, m3_usize len, m3_usize index, M3Bool *out_has_more)
{
...
int M3_CALL m3_router_test_match_len(const char *pattern, m3_usize pattern_len, const char *path, m3_usize path_len, M3RouteParam *params,
    m3_usize max_params, m3_usize *out_param_count, M3Bool *out_match)
{
    return m3_route_match_len(pattern, pattern_len, path, path_len, params, max_params, out_param_count, out_match);
}
...
int M3_CALL m3_router_test_copy_path(M3Router *router, const char *path, m3_usize path_len, char **out_copy)
{
    return m3_router_copy_path(router, path, path_len, out_copy);
}

int M3_CALL m3_router_test_navigate_path_len(M3Router *router, const char *path, m3_usize path_len, void **out_component)
{
    return m3_router_navigate_path_len(router, path, path_len, out_component);
}
#endif
