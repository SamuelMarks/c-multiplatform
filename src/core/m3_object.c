...
static int m3_handle_system_resolve_impl(M3HandleSystemImpl *impl, M3Handle handle, void **out_obj)
{
    M3HandleSlot *slot;
    M3Bool valid;
    m3_u32 index;
...
static int M3_CALL m3_handle_system_default_init(void *sys, m3_usize capacity)
{
    ...
    if (impl->allocator.alloc == NULL || impl->allocator.realloc == NULL || impl->allocator.free == NULL) {
        return M3_ERR_INVALID_ARGUMENT;
    }

    rc = impl->allocator.alloc(impl->allocator.ctx, sizeof(M3HandleSlot) * capacity, (void **)&slots);
    if (rc != M3_OK) {
        return rc;
    }
...
static int M3_CALL m3_handle_system_default_register_object(void *sys, M3ObjectHeader *obj)
{
    M3HandleSystemImpl *impl;
    M3HandleSlot *slot;
    M3Bool valid;
    ...
    if (obj->vtable == NULL || obj->vtable->retain == NULL || obj->vtable->release == NULL || obj->vtable->destroy == NULL) {
        return M3_ERR_INVALID_ARGUMENT;
    }
...
static int M3_CALL m3_handle_system_default_unregister_object(void *sys, M3Handle handle)
{
    M3HandleSystemImpl *impl;
    M3HandleSlot *slot;
    M3ObjectHeader *obj;
...
static int M3_CALL m3_handle_system_default_resolve(void *sys, M3Handle handle, void **out_obj)
{
    return m3_handle_system_resolve_impl((M3HandleSystemImpl *)sys, handle, out_obj);
}
...
static int M3_CALL m3_handle_system_default_retain(void *sys, M3Handle handle)
{
    ...
    rc = m3_handle_system_resolve_impl((M3HandleSystemImpl *)sys, handle, &resolved);
    if (rc != M3_OK) {
        return rc;
    }
...
static int M3_CALL m3_handle_system_default_release(void *sys, M3Handle handle)
{
    M3ObjectHeader *obj;
    void *resolved;
    int rc;
...
int M3_CALL m3_object_header_init(M3ObjectHeader *obj, m3_u32 type_id, m3_u32 flags, const M3ObjectVTable *vtable)
{
    if (obj == NULL || vtable == NULL) {
        return M3_ERR_INVALID_ARGUMENT;
    }

    if (vtable->retain == NULL || vtable->release == NULL || vtable->destroy == NULL) {
        return M3_ERR_INVALID_ARGUMENT;
    }
...
int M3_CALL m3_object_get_type_id(const M3ObjectHeader *obj, m3_u32 *out_type_id)
{
...
int M3_CALL m3_handle_system_default_create(m3_usize capacity, const M3Allocator *allocator, M3HandleSystem *out_sys)
{
    ...
    rc = allocator->alloc(allocator->ctx, sizeof(M3HandleSystemImpl), (void **)&impl);
    if (rc != M3_OK) {
        return rc;
    }
