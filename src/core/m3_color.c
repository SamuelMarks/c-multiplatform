...
static int m3_color_argb_to_xyz(m3_u32 argb, M3Scalar *out_x, M3Scalar *out_y,
                                M3Scalar *out_z) {
  m3_u8 r8;
  m3_u8 g8;
  m3_u8 b8;
...
static int m3_color_xyz_to_argb(M3Scalar x, M3Scalar y, M3Scalar z,
                                m3_u32 *out_argb, M3Bool *out_in_gamut) {
  M3Scalar r_lin;
  M3Scalar g_lin;
  M3Scalar b_lin;
  M3Scalar r;
  ...
  *out_argb = 0xFF000000u | ((m3_u32)(r * 255.0f + 0.5f) << 16) |
              ((m3_u32)(g * 255.0f + 0.5f) << 8) | (m3_u32)(b * 255.0f + 0.5f);
...
static int m3_color_xyz_to_lab(M3Scalar x, M3Scalar y, M3Scalar z,
...
static int m3_color_lab_to_xyz(M3Scalar l, M3Scalar a, M3Scalar b,
...
static int m3_color_lch_to_argb(M3Scalar hue, M3Scalar chroma, M3Scalar tone,
                                m3_u32 *out_argb, M3Bool *out_in_gamut) {
  M3Scalar rad;
  M3Scalar a;
  M3Scalar b;
...
int M3_CALL m3_color_argb_from_rgba(m3_u8 r, m3_u8 g, m3_u8 b, m3_u8 a,
                                    m3_u32 *out_argb) {
  if (out_argb == NULL) {
    return M3_ERR_INVALID_ARGUMENT;
  }
  ...
  *out_argb =
      ((m3_u32)a << 24) | ((m3_u32)r << 16) | ((m3_u32)g << 8) | (m3_u32)b;
  return M3_OK;
}

int M3_CALL m3_color_rgba_from_argb(m3_u32 argb, m3_u8 *out_r, m3_u8 *out_g,
                                    m3_u8 *out_b, m3_u8 *out_a) {
...
int M3_CALL m3_tonal_palette_init(M3TonalPalette *palette, M3Scalar hue,
                                  M3Scalar chroma) {
  if (palette == NULL) {
    return M3_ERR_INVALID_ARGUMENT;
  }
...
int M3_CALL m3_tonal_palette_tone_argb(const M3TonalPalette *palette,
...
int M3_CALL m3_scheme_generate(m3_u32 source_argb, M3Bool dark,
                               M3Scheme *out_scheme) {
...
int M3_CALL m3_color_test_argb_to_xyz(m3_u32 argb, M3Scalar *out_x,
                                      M3Scalar *out_y, M3Scalar *out_z) {
  return m3_color_argb_to_xyz(argb, out_x, out_y, out_z);
}

int M3_CALL m3_color_test_xyz_to_argb(M3Scalar x, M3Scalar y, M3Scalar z,
                                      m3_u32 *out_argb, M3Bool *out_in_gamut) {
  return m3_color_xyz_to_argb(x, y, z, out_argb, out_in_gamut);
}

int M3_CALL m3_color_test_xyz_to_lab(M3Scalar x, M3Scalar y, M3Scalar z,
                                     M3Scalar *out_l, M3Scalar *out_a,
                                     M3Scalar *out_b) {
  return m3_color_xyz_to_lab(x, y, z, out_l, out_a, out_b);
}

int M3_CALL m3_color_test_lab_to_xyz(M3Scalar l, M3Scalar a, M3Scalar b,
                                     M3Scalar *out_x, M3Scalar *out_y,
                                     M3Scalar *out_z) {
  return m3_color_lab_to_xyz(l, a, b, out_x, out_y, out_z);
}

int M3_CALL m3_color_test_lch_to_argb(M3Scalar hue, M3Scalar chroma,
                                      M3Scalar tone, m3_u32 *out_argb,
                                      M3Bool *out_in_gamut) {
