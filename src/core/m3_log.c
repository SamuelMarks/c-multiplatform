...
static M3LogState g_log_state = { M3_FALSE, { NULL, NULL }, { NULL, NULL, NULL, NULL } };
...
static int M3_CALL m3_log_default_sink(void *ctx, M3LogLevel level, const char *tag, const char *message, m3_usize length)
{
    FILE *stream;
...
int M3_CALL m3_log_write(M3LogLevel level, const char *tag, const char *message)
{
    m3_usize length;
    int rc;
...
int M3_CALL m3_log_write_n(M3LogLevel level, const char *tag, const char *message, m3_usize length)
{
    int rc;
    int write_rc;
    ...
    write_rc = g_log_state.sink.write(g_log_state.sink.ctx, level, tag, message, length);
...
#ifdef M3_TESTING
int M3_CALL m3_log_test_set_mutex_failures(M3Bool init_fail, M3Bool lock_fail, M3Bool unlock_fail, M3Bool shutdown_fail)
{
...
int M3_CALL m3_log_test_mutex_lock(void)
{
    return m3_log_mutex_lock();
}

int M3_CALL m3_log_test_mutex_unlock(void)
{
    return m3_log_mutex_unlock();
}

int M3_CALL m3_log_test_mutex_shutdown(void)
{
    return m3_log_mutex_shutdown();
}
#endif
