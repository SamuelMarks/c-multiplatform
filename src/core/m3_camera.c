...
#define M3_CAMERA_VTABLE_COMPLETE(vtable)                                      \
  (M3_CAMERA_VTABLE_HAS_OPEN(vtable) && (vtable)->close != NULL &&             \
...
int M3_CALL m3_camera_init(M3CameraSession *session,
                           const M3CameraSessionConfig *config) {
  M3Camera camera;
  int rc;
...
int M3_CALL m3_camera_read_frame(M3CameraSession *session,
                                 M3CameraFrame *out_frame,
...
int M3_CALL m3_camera_copy_frame(const M3CameraFrame *frame, void *dst,
