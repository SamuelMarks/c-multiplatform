...
int M3_CALL m3_anim_timing_init(M3AnimTiming *timing, M3Scalar from,
                                M3Scalar to, M3Scalar duration, m3_u32 easing) {
  M3Scalar eased;
  int rc;
...
int M3_CALL m3_anim_timing_step(M3AnimTiming *timing, M3Scalar dt,
                                M3Scalar *out_value, M3Bool *out_finished) {
...
int M3_CALL m3_spring_init(M3Spring *spring, M3Scalar position, M3Scalar target,
                           M3Scalar stiffness, M3Scalar damping,
                           M3Scalar mass) {
  if (spring == NULL) {
    return M3_ERR_INVALID_ARGUMENT;
  }
...
int M3_CALL m3_spring_set_tolerance(M3Spring *spring, M3Scalar tolerance,
...
int M3_CALL m3_spring_step(M3Spring *spring, M3Scalar dt,
                           M3Bool *out_finished) {
  ...
  displacement = spring->position - spring->target;
  if (m3_anim_abs(displacement) <= spring->tolerance &&
      m3_anim_abs(spring->velocity) <= spring->rest_velocity) {
...
int M3_CALL m3_anim_controller_start_timing(M3AnimController *controller,
                                            M3Scalar from, M3Scalar to,
                                            M3Scalar duration, m3_u32 easing) {
  int rc;
...
int M3_CALL m3_anim_controller_start_spring(M3AnimController *controller,
                                            M3Scalar position, M3Scalar target,
                                            M3Scalar stiffness,
  ...
  rc = m3_spring_init(&controller->spring, position, target, stiffness, damping,
                      mass);
  if (rc != M3_OK) {
    return rc;
  }
...
int M3_CALL m3_anim_controller_set_spring_target(M3AnimController *controller,
                                                 M3Scalar target) {
  if (controller == NULL) {
    return M3_ERR_INVALID_ARGUMENT;
  }
...
int M3_CALL m3_anim_controller_step(M3AnimController *controller, M3Scalar dt,
                                    M3Scalar *out_value, M3Bool *out_finished) {
  M3Scalar value;
  M3Bool finished;
  int rc;
...
int M3_CALL m3_anim_controller_get_value(const M3AnimController *controller,
                                         M3Scalar *out_value) {
...
int M3_CALL m3_anim_controller_is_running(const M3AnimController *controller,
                                          M3Bool *out_running) {
...
#ifdef M3_TESTING
int M3_CALL m3_anim_test_apply_ease(m3_u32 easing, M3Scalar t,
                                    M3Scalar *out_value) {
