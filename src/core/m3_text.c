...
static int m3_text_metrics_update(M3TextWidget *widget)
{
    ...
    rc = m3_text_measure_utf8(&widget->backend, widget->font, widget->utf8, widget->utf8_len, &widget->metrics);
    if (rc != M3_OK) {
        return rc;
    }
...
static int m3_text_widget_measure(void *widget, M3MeasureSpec width, M3MeasureSpec height, M3Size *out_size)
{
    M3TextWidget *text;
    ...
    if (height.mode == M3_MEASURE_EXACTLY) {
        ...
        out_size->height = (metrics.height > height.size) ? height.size : metrics.height;
    } else {
...
static int m3_text_widget_paint(void *widget, M3PaintContext *ctx)
{
    ...
    return ctx->gfx->text_vtable->draw_text(ctx->gfx->ctx, text->font, text->utf8, text->utf8_len, x, y, text->style.color);
}

static int m3_text_widget_event(void *widget, const M3InputEvent *event, M3Bool *out_handled)
{
...
static int m3_text_widget_get_semantics(void *widget, M3Semantics *out_semantics)
{
    M3TextWidget *text;
...
static int m3_text_widget_destroy(void *widget)
{
    ...
    text = (M3TextWidget *)widget;
    rc = M3_OK;
    if (text->owns_font == M3_TRUE && (text->font.id != 0u || text->font.generation != 0u)) {
        if (text->backend.vtable != NULL && text->backend.vtable->destroy_font != NULL) {
...
static const M3WidgetVTable g_m3_text_widget_vtable = {
    m3_text_widget_measure,
    m3_text_widget_layout,
    m3_text_widget_paint,
    m3_text_widget_event,
    m3_text_widget_get_semantics,
    m3_text_widget_destroy
};
...
int M3_CALL m3_text_backend_from_gfx(const M3Gfx *gfx, M3TextBackend *out_backend)
{
...
int M3_CALL m3_text_font_create(const M3TextBackend *backend, const M3TextStyle *style, M3Handle *out_font)
{
    int rc;
    ...
    return backend->vtable->create_font(backend->ctx, style->utf8_family, style->size_px, style->weight, style->italic, out_font);
}
...
int M3_CALL m3_text_measure_utf8(const M3TextBackend *backend, M3Handle font, const char *utf8, m3_usize utf8_len,
    M3TextMetrics *out_metrics)
{
    ...
    rc = backend->vtable->measure_text(backend->ctx, font, utf8, utf8_len, &width, &height, &baseline);
    if (rc != M3_OK) {
        return rc;
    }
...
int M3_CALL m3_text_measure_cstr(const M3TextBackend *backend, M3Handle font, const char *utf8, M3TextMetrics *out_metrics)
{
    m3_usize len;
    int rc;
...
int M3_CALL m3_text_font_metrics(const M3TextBackend *backend, M3Handle font, M3TextMetrics *out_metrics)
{
    return m3_text_measure_utf8(backend, font, NULL, 0, out_metrics);
}

int M3_CALL m3_text_widget_init(M3TextWidget *widget, const M3TextBackend *backend, const M3TextStyle *style,
    const char *utf8, m3_usize utf8_len)
{
    ...
    if (backend->vtable->create_font == NULL || backend->vtable->measure_text == NULL || backend->vtable->draw_text == NULL
...
int M3_CALL m3_text_widget_set_text(M3TextWidget *widget, const char *utf8, m3_usize utf8_len)
{
    if (widget == NULL) {
        return M3_ERR_INVALID_ARGUMENT;
    }
...
int M3_CALL m3_text_widget_set_style(M3TextWidget *widget, const M3TextStyle *style)
{
    M3Handle new_font;
    int rc;
...
int M3_CALL m3_text_widget_get_metrics(M3TextWidget *widget, M3TextMetrics *out_metrics)
{
    int rc;
