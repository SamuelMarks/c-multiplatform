...
static int m3_event_hit_test(const M3RenderNode *node, M3Scalar x, M3Scalar y, M3Widget **out_widget)
{
    M3Bool contains;
    m3_usize i;
    int rc;
...
static int m3_event_dispatch_to_widget(M3Widget *widget, const M3InputEvent *event, M3Bool *out_handled)
{
    M3Bool handled;
...
int M3_CALL m3_event_dispatcher_get_focus(const M3EventDispatcher *dispatcher, M3Widget **out_widget)
{
...
int M3_CALL m3_event_dispatcher_set_focus(M3EventDispatcher *dispatcher, M3Widget *widget)
{
...
int M3_CALL m3_event_dispatcher_get_focus_visible(const M3EventDispatcher *dispatcher, M3Bool *out_visible)
{
...
int M3_CALL m3_event_dispatcher_set_focus_visible(M3EventDispatcher *dispatcher, M3Bool visible)
{
    if (dispatcher == NULL) {
        return M3_ERR_INVALID_ARGUMENT;
    }
...
int M3_CALL m3_event_dispatch(M3EventDispatcher *dispatcher, const M3RenderNode *root, const M3InputEvent *event,
    M3Widget **out_target, M3Bool *out_handled)
{
    ...
    if (dispatcher == NULL || root == NULL || event == NULL || out_handled == NULL) {
        return M3_ERR_INVALID_ARGUMENT;
    }
    if (!dispatcher->initialized) {
        return M3_ERR_STATE;
    ...
    *out_handled = M3_FALSE;
    ...
    switch (event->type) {
        ...
        case M3_INPUT_POINTER_SCROLL:
            rc = m3_event_hit_test(root, (M3Scalar)event->data.pointer.x, (M3Scalar)event->data.pointer.y, &target);
            if (rc != M3_OK) {
                return rc;
            }
            if (target == NULL) {
