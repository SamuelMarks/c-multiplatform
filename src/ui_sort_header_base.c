...
ui_sort_header_base_create(struct ui_sort_header_base **out_sort_header) {
  ...
  manager = (struct ui_sort_header_base *)UI_MALLOC(
...
static enum ui_error find_state_index(struct ui_sort_header_base *sort_header,
...
static enum ui_error remove_state_at(struct ui_sort_header_base *sort_header,
                                     int index) {
  size_t i;
  if (index < 0 || (size_t)index >= sort_header->count) {
...
insert_or_update_state(struct ui_sort_header_base *sort_header, void *id,
  ...
  if (direction == UI_SORT_NONE) {
    (void)find_state_index(sort_header, id, &existing_index);
    if (existing_index >= 0) {
  ...
  if (sort_header->count >= sort_header->capacity) {
    ...
    struct ui_sort_state *new_states = (struct ui_sort_state *)UI_REALLOC(
...
ui_sort_header_base_toggle(struct ui_sort_header_base *sort_header, void *id) {
  ...
  (void)find_state_index(sort_header, id, &index);
  if (index >= 0) {
