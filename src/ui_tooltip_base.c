...
enum ui_error ui_tooltip_base_create(struct ui_tooltip_base **out_tooltip,
  ...
  tooltip = (struct ui_tooltip_base *)UI_MALLOC(sizeof(struct ui_tooltip_base));
  if (!tooltip)
...
enum ui_error ui_tooltip_base_destroy(struct ui_tooltip_base *tooltip) {
  if (!tooltip)
    return UI_ERROR_NONE;
  if (tooltip->text)
    UI_FREE(tooltip->text);
  if (tooltip->overlay_component)
    ui_component_destroy(tooltip->overlay_component);
  /* Note: active_overlay lifecycle is managed by overlay_director unmount */
  UI_FREE(tooltip);
  return UI_ERROR_NONE;
}

enum ui_error ui_tooltip_base_set_text(struct ui_tooltip_base *tooltip,
                                       const char *text) {
  size_t len;
  if (!tooltip)
    return UI_ERROR_INVALID_ARGUMENT;
  ...
  if (text) {
    len = strlen(text);
    tooltip->text = (char *)UI_MALLOC(len + 1);
    if (!tooltip->text)
      return UI_ERROR_OUT_OF_MEMORY;
#if defined(_MSC_VER)
...
static enum ui_error transition_state(struct ui_tooltip_base *tooltip,
...
enum ui_error ui_tooltip_base_handle_event(struct ui_tooltip_base *tooltip,
  ...
  case UI_EVENT_MOUSE_MOVE:
    /* In a real system, we'd check intersection with the trigger rect.
       ...
       events. */
    if (tooltip->state == UI_TOOLTIP_STATE_IDLE) {
      transition_state(tooltip, UI_TOOLTIP_STATE_HOVER_DELAY,
                       current_time_secs);
    }
    break;
  ...
  case UI_EVENT_TOUCH_START:
    if (tooltip->state == UI_TOOLTIP_STATE_IDLE) {
      transition_state(tooltip, UI_TOOLTIP_STATE_TOUCH_HOLD_DELAY,
                       current_time_secs);
    }
    break;
  ...
  case UI_EVENT_PEN_DOWN: /* Re-using PEN_DOWN as focus for primitive mock */
    if (tooltip->state == UI_TOOLTIP_STATE_IDLE) {
      transition_state(tooltip, UI_TOOLTIP_STATE_FOCUS_DELAY,
                       current_time_secs);
    }
    break;
...
enum ui_error ui_tooltip_base_tick(struct ui_tooltip_base *tooltip,
                                   double current_time_secs) {
  double elapsed;
  if (!tooltip)
    return UI_ERROR_INVALID_ARGUMENT;
...
enum ui_error ui_tooltip_base_is_visible(const struct ui_tooltip_base *tooltip,
                                         int *out_is_visible) {
...
enum ui_error ui_tooltip_base_hide(struct ui_tooltip_base *tooltip) {
  if (!tooltip)
    return UI_ERROR_INVALID_ARGUMENT;
...
/** \brief ui_error */
enum ui_error
ui_tooltip_base_render(struct ui_tooltip_base *tooltip,
  ...
  {
    int is_visible = 0;
    ui_tooltip_base_is_visible(tooltip, &is_visible);
    if (!is_visible) {
...
enum ui_error ui_tooltip_base_bind_open(struct ui_tooltip_base *widget,
                                        struct ui_signal *open_signal) {
  if (!widget) {
    return UI_ERROR_INVALID_ARGUMENT;
  }
