...
cupertino_action_sheet_style_init(CupertinoActionSheetStyle *style) {
  ...
  cupertino_typography_get_style(&typo, CUPERTINO_TYPOGRAPHY_FOOTNOTE,
                                 &style->title_style);
  style->title_style.weight = 600;
  style->title_style.color.r = 0.5f;
  style->title_style.color.g = 0.5f;
  style->title_style.color.b = 0.5f;

  cupertino_typography_get_style(&typo, CUPERTINO_TYPOGRAPHY_FOOTNOTE,
                                 &style->message_style);
  style->message_style.color.r = 0.5f;
  style->message_style.color.g = 0.5f;
  style->message_style.color.b = 0.5f;

  cupertino_typography_get_style(&typo, CUPERTINO_TYPOGRAPHY_TITLE_3,
                                 &style->action_style);
...
static void draw_text_centered(CMPPaintContext *ctx,
                               const CMPTextBackend *backend, const char *text,
                               cmp_usize len, CMPRect box, CMPTextStyle style) {
  CMPHandle font = {0};
  ...
  if (cmp_text_font_create(backend, &style, &font) == CMP_OK) {
    ...
    cmp_text_draw_utf8_gfx(ctx->gfx, font, text, len, 0, text_x, text_y,
                           style.color);
...
CMP_API int CMP_CALL cupertino_action_sheet_paint(
  ...
  if (sheet == NULL || ctx == NULL || ctx->gfx == NULL ||
      ctx->gfx->vtable == NULL) {
    return CMP_ERR_INVALID_ARGUMENT;
  }
  ...
  bottom_inset = sheet->safe_area_insets.height > 0.0f
                     ? sheet->safe_area_insets.height
                     : CUPERTINO_SHEET_MARGIN;
  ...
  sheet_y_offset = sheet->bounds.height -
                   (content_height + bottom_inset) * sheet->animation_progress;

  /* Draw full screen dim background */
  dim_bg.r = 0.0f;
  dim_bg.g = 0.0f;
  dim_bg.b = 0.0f;
  dim_bg.a = 0.4f * sheet->animation_progress;
  ...
  cupertino_blur_get_effect(sheet->style.background_blur,
                            sheet->style.is_dark_mode, &blur_effect);
  ...
  main_group_rect.height =
      header_height + (standard_action_count * CUPERTINO_ACTION_HEIGHT);

  if (main_group_rect.height > 0.0f && ctx->gfx->vtable->draw_path) {
    cmp_path_init(&path, &alloc, 32);
    cupertino_shape_append_squircle(
        &path, main_group_rect, CUPERTINO_SHEET_CORNER_RADIUS,
        CUPERTINO_SHEET_CORNER_RADIUS, CUPERTINO_SHEET_CORNER_RADIUS,
        CUPERTINO_SHEET_CORNER_RADIUS);
    ...
    if (header_height > 0.0f) {
      CMPRect header_box = {main_group_rect.x, current_y, main_group_rect.width,
                            header_height};
      /* Simplification: Just draw title centered in header block */
      if (sheet->title_utf8) {
        draw_text_centered(ctx, &sheet->text_backend, sheet->title_utf8,
                           sheet->title_len, header_box,
                           sheet->style.title_style);
      }
    ...
    for (i = 0; i < sheet->action_count; i++) {
      if ((int)i != cancel_index) {
        CMPRect action_box = {main_group_rect.x, current_y,
                              main_group_rect.width, CUPERTINO_ACTION_HEIGHT};
        ...
        if (current_y > main_group_rect.y && ctx->gfx->vtable->draw_line) {
          ctx->gfx->vtable->draw_line(ctx->gfx->ctx, action_box.x, action_box.y,
                                      action_box.x + action_box.width,
                                      action_box.y, border_color, 1.0f);
        }

        draw_text_centered(ctx, &sheet->text_backend,
                           sheet->actions[i].title_utf8,
                           sheet->actions[i].title_len, action_box, act_style);
  ...
  if (cancel_index >= 0) {
    ...
    cancel_group_rect.y =
        main_group_rect.y + main_group_rect.height + CUPERTINO_SHEET_MARGIN;
    cancel_group_rect.width =
        sheet->bounds.width - (CUPERTINO_SHEET_MARGIN * 2.0f);
    cancel_group_rect.height = CUPERTINO_ACTION_HEIGHT;

    if (ctx->gfx->vtable->draw_path) {
      cmp_path_init(&path, &alloc, 32);
      cupertino_shape_append_squircle(
          &path, cancel_group_rect, CUPERTINO_SHEET_CORNER_RADIUS,
          CUPERTINO_SHEET_CORNER_RADIUS, CUPERTINO_SHEET_CORNER_RADIUS,
          CUPERTINO_SHEET_CORNER_RADIUS);
      ...
      draw_text_centered(ctx, &sheet->text_backend,
                         sheet->actions[cancel_index].title_utf8,
                         sheet->actions[cancel_index].title_len,
                         cancel_group_rect, cancel_style);
    }
  }
