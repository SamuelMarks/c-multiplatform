...
CMP_API int CMP_CALL cupertino_menu_style_init(CupertinoMenuStyle *style, CupertinoMenuVariant variant) {
    ...
    style->text_color.r = 0.0f; style->text_color.g = 0.0f; style->text_color.b = 0.0f; style->text_color.a = 1.0f;

    cupertino_typography_scale_init(&typo);

    if (variant == CUPERTINO_MENU_MAC_OS) {
        style->background_blur = CUPERTINO_BLUR_STYLE_NORMAL; /* macOS BehindWindow approximation */
        cupertino_typography_get_style(&typo, CUPERTINO_TYPOGRAPHY_BODY, &style->label_style);
    ...
    style->shortcut_style.color.r = 0.5f; style->shortcut_style.color.g = 0.5f; style->shortcut_style.color.b = 0.5f;

    return CMP_OK;
}

CMP_API int CMP_CALL cupertino_menu_init(CupertinoMenu *menu, const CMPTextBackend *text_backend, CupertinoMenuVariant variant) {
...
CMP_API int CMP_CALL cupertino_menu_set_items(CupertinoMenu *menu, CupertinoMenuItem *items, cmp_u32 count) {
    if (menu == NULL) {
        return CMP_ERR_INVALID_ARGUMENT;
    }
...
CMP_API int CMP_CALL cupertino_menu_measure(CupertinoMenu *menu, CMPSize *out_size) {
    cmp_u32 i;
    ...
    item_height = (menu->style.variant == CUPERTINO_MENU_MAC_OS) ? CUPERTINO_MENU_ITEM_HEIGHT_MAC : CUPERTINO_MENU_ITEM_HEIGHT_IOS;

    for (i = 0; i < menu->item_count; i++) {
        if (menu->items[i].style == CUPERTINO_MENU_ITEM_SEPARATOR) {
            current_h += 9.0f; /* standard separator block height */
        } else {
            /* Quick estimate, skipping actual text measurement for simplicity in this port */
...
CMP_API int CMP_CALL cupertino_menu_layout(CupertinoMenu *menu, CMPRect anchor_rect, CMPRect screen_bounds) {
    CMPSize size;
    int rc;
    ...
    if (menu->style.variant == CUPERTINO_MENU_MAC_OS) {
        /* macOS usually spawns right at the cursor (or directly below a menu bar item) */
    ...
    if (menu->bounds.y + menu->bounds.height > screen_bounds.height) {
        menu->bounds.y = anchor_rect.y - menu->bounds.height - 8.0f; /* flip above */
...
CMP_API int CMP_CALL cupertino_menu_paint(const CupertinoMenu *menu, CMPPaintContext *ctx) {
    ...
    if (menu == NULL || ctx == NULL || ctx->gfx == NULL || ctx->gfx->vtable == NULL) {
        return CMP_ERR_INVALID_ARGUMENT;
    }
    ...
    corner_radius = (menu->style.variant == CUPERTINO_MENU_MAC_OS) ? CUPERTINO_MENU_CORNER_RADIUS_MAC : CUPERTINO_MENU_CORNER_RADIUS_IOS;
    item_height = (menu->style.variant == CUPERTINO_MENU_MAC_OS) ? CUPERTINO_MENU_ITEM_HEIGHT_MAC : CUPERTINO_MENU_ITEM_HEIGHT_IOS;

    rc = cupertino_blur_get_effect(menu->style.background_blur, menu->style.is_dark_mode, &blur_effect);
    
    if (ctx->gfx->vtable->draw_path) {
        cmp_path_init(&path, &alloc, 32);
        cupertino_shape_append_squircle(&path, menu->bounds, corner_radius, corner_radius, corner_radius, corner_radius);
        ...
        border_color.g = border_color.r; border_color.b = border_color.r;
        border_color.a = 0.5f;

        /* Standard graphics doesn't support stroke_path directly, so we rely on background being handled. */
    ...
    for (i = 0; i < menu->item_count; i++) {
        ...
        if (item->style == CUPERTINO_MENU_ITEM_SEPARATOR) {
            CMPColor sep_color;
            sep_color.r = 0.5f; sep_color.g = 0.5f; sep_color.b = 0.5f; sep_color.a = 0.3f;
            ...
            if (ctx->gfx->vtable->draw_line) {
                ...
                ctx->gfx->vtable->draw_line(ctx->gfx->ctx, lx, sy, rx, sy, sep_color, 1.0f);
            }
            current_y += 9.0f;
            continue;
        }
        ...
        if ((int)i == menu->hovered_index && !item->is_disabled && ctx->gfx->vtable->draw_rect) {
            CMPColor sel_color = menu->style.tint_color;
            CMPRect sel_rect = {menu->bounds.x + 4.0f, current_y, menu->bounds.width - 8.0f, item_height};
            CMPScalar sel_radius = (menu->style.variant == CUPERTINO_MENU_MAC_OS) ? 4.0f : 0.0f;
            
            if (menu->style.variant == CUPERTINO_MENU_IOS) {
                sel_color.a = 0.15f; /* iOS is usually a grey/tinted wash */
            }
            
            ctx->gfx->vtable->draw_rect(ctx->gfx->ctx, &sel_rect, sel_color, sel_radius);
        }
        ...
        if (item->title_utf8 && item->title_len > 0) {
            ...
            } else if ((int)i == menu->hovered_index && menu->style.variant == CUPERTINO_MENU_MAC_OS) {
                /* macOS text turns white when selected/hovered */
                applied_style.color.r = 1.0f; applied_style.color.g = 1.0f; applied_style.color.b = 1.0f; applied_style.color.a = 1.0f;
            ...
            if (cmp_text_font_create(&menu->text_backend, &applied_style, &font) == CMP_OK) {
                cmp_text_measure_utf8(&menu->text_backend, font, item->title_utf8, item->title_len, 0, &metrics);
                
                text_x = menu->bounds.x + 14.0f;
                text_y = current_y + (item_height - metrics.height) / 2.0f + metrics.baseline;
                
                cmp_text_draw_utf8_gfx(ctx->gfx, font, item->title_utf8, item->title_len, 0, text_x, text_y, applied_style.color);
