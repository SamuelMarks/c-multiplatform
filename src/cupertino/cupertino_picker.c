...
CMP_API int CMP_CALL cupertino_picker_set_selected(CupertinoPicker *picker,
  ...
  if (animated) {
    cmp_anim_controller_start_spring(&picker->scroll_anim,
                                     picker->scroll_offset, target, 300.0f,
                                     25.0f, 1.0f);
  } else {
...
CMP_API int CMP_CALL cupertino_picker_update(CupertinoPicker *picker,
  ...
  if (picker->scroll_anim.running) {
    cmp_anim_controller_step(&picker->scroll_anim, (CMPScalar)delta_time,
                             &out_val, &finished);
...
CMP_API int CMP_CALL cupertino_picker_paint(const CupertinoPicker *picker,
  ...
  if (picker->is_dark_mode) {
    ...
    bg_color.b = 0.0f;
    bg_color.a =
        0.0f; /* Usually transparent or slightly translucent over content */
    selection_bg_color.r = 0.2f;
    selection_bg_color.g = 0.2f;
    selection_bg_color.b = 0.2f;
    selection_bg_color.a = 0.5f;
    text_color.r = 1.0f;
    text_color.g = 1.0f;
    text_color.b = 1.0f;
    text_color.a = 1.0f;
    ...
    bg_color.b = 1.0f;
    bg_color.a = 0.0f;
    selection_bg_color.r = 0.9f;
    selection_bg_color.g = 0.9f;
    selection_bg_color.b = 0.9f;
    selection_bg_color.a = 0.5f;
    text_color.r = 0.0f;
    text_color.g = 0.0f;
    text_color.b = 0.0f;
  ...
  if (ctx->gfx->vtable->draw_rect) {
    ...
    ctx->gfx->vtable->draw_rect(ctx->gfx->ctx, &sel_rect, selection_bg_color,
                                8.0f);
  }
  ...
  for (i = 0; i < (int)picker->item_count; i++) {
    ...
    if (alpha > 0.0f && item_center_y >= picker->bounds.y &&
        item_center_y <= picker->bounds.y + picker->bounds.height) {
