...
CMP_API int CMP_CALL cupertino_refresh_style_init(CupertinoRefreshStyle *style) {
    ...
    cupertino_color_get_system(CUPERTINO_COLOR_GRAY, CMP_FALSE, &style->tint_color);
    style->trigger_offset = 60.0f; /* Standard iOS pull distance */

    return CMP_OK;
}

CMP_API int CMP_CALL cupertino_refresh_init(CupertinoRefreshControl *ptr, CMPWidget *child) {
    if (ptr == NULL) {
        return CMP_ERR_INVALID_ARGUMENT;
    }
...
CMP_API int CMP_CALL cupertino_refresh_set_refreshing(CupertinoRefreshControl *ptr, CMPBool is_refreshing) {
    if (ptr == NULL) {
        return CMP_ERR_INVALID_ARGUMENT;
    }
...
CMP_API int CMP_CALL cupertino_refresh_set_callback(CupertinoRefreshControl *ptr, CupertinoRefreshCallback cb, void *ctx) {
    if (ptr == NULL) {
        return CMP_ERR_INVALID_ARGUMENT;
    }
    ptr->on_refresh = cb;
...
CMP_API int CMP_CALL cupertino_refresh_layout(CupertinoRefreshControl *ptr, CMPRect bounds) {
    if (ptr == NULL) {
        return CMP_ERR_INVALID_ARGUMENT;
    }
    ptr->bounds = bounds;

    /* In a real scenario, this would intercept scroll events from the child to update current_pull.
       For rendering purposes, we just layout the child in the remaining space. */
...
CMP_API int CMP_CALL cupertino_refresh_paint(const CupertinoRefreshControl *ptr, CMPPaintContext *ctx) {
    CMPColor spinner_color;
    CMPRect spinner_rect;
    CMPScalar spinner_size = 28.0f;

    if (ptr == NULL || ctx == NULL || ctx->gfx == NULL || ctx->gfx->vtable == NULL) {
        return CMP_ERR_INVALID_ARGUMENT;
    }

    /* Paint the child first so the spinner appears "behind" or "above" depending on the list style,
       usually standard iOS has the spinner in the background if the list is opaque, but let's draw it here. */
    ...
    if (spinner_color.a > 0.0f && ctx->gfx->vtable->draw_rect) {
        CMPScalar y_pos = ptr->bounds.y + (ptr->is_refreshing ? (ptr->style.trigger_offset / 2.0f) : (ptr->current_pull / 2.0f));
        ...
        ctx->gfx->vtable->draw_rect(ctx->gfx->ctx, &spinner_rect, spinner_color, spinner_size / 2.0f);
    }
