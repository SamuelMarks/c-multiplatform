...
cupertino_detent_sheet_set_bounds(CupertinoDetentSheet *sheet, CMPRect bounds) {
  ...
  if (!sheet->slide_anim.running) {
    if (sheet->current_detent == 0)
      sheet->slide_anim.value = sheet->detent_hidden;
    else if (sheet->current_detent == 1)
      sheet->slide_anim.value = sheet->detent_medium;
    else if (sheet->current_detent == 2)
      sheet->slide_anim.value = sheet->detent_large;
  }
...
CMP_API int CMP_CALL cupertino_detent_sheet_set_detent(
  ...
  if (detent < 0 || detent > 2)
    return CMP_ERR_INVALID_ARGUMENT;

  sheet->current_detent = detent;

  if (detent == 0)
    target = sheet->detent_hidden;
  else if (detent == 1)
    target = sheet->detent_medium;
  else
    target = sheet->detent_large;

  if (animated) {
    /* Standard iOS sheet spring */
    cmp_anim_controller_start_spring(&sheet->slide_anim,
                                     sheet->slide_anim.value, target, 350.0f,
                                     30.0f, 1.0f);
  } else {
...
CMP_API int CMP_CALL cupertino_detent_sheet_update(CupertinoDetentSheet *sheet,
  ...
  cmp_anim_controller_step(&sheet->slide_anim, (CMPScalar)delta_time, &out_val,
                           &finished);
  return CMP_OK;
}
...
CMP_API int CMP_CALL cupertino_detent_sheet_paint(
  ...
  if (sheet->is_dark_mode) {
    ...
    bg_color.b = 0.118f;
    bg_color.a = 1.0f;
    grabber_color.r = 0.3f;
    grabber_color.g = 0.3f;
    grabber_color.b = 0.3f;
    grabber_color.a = 1.0f;
  } else {
    bg_color.r = 1.0f;
    bg_color.g = 1.0f;
    bg_color.b = 1.0f;
    bg_color.a = 1.0f;
    grabber_color.r = 0.8f;
    grabber_color.g = 0.8f;
    grabber_color.b = 0.8f;
    grabber_color.a = 1.0f;
  }
  ...
  if (scrim_color.a > 0.01f && ctx->gfx->vtable->draw_rect) {
    ctx->gfx->vtable->draw_rect(ctx->gfx->ctx, &sheet->screen_bounds,
                                scrim_color, 0.0f);
  }
  ...
  sheet_rect.height = sheet->screen_bounds.height - current_y +
                      corner_radius; /* Overshoot to hide bottom corners */

  if (ctx->gfx->vtable->draw_rect) {
    ctx->gfx->vtable->draw_rect(ctx->gfx->ctx, &sheet_rect, bg_color,
                                corner_radius);
  }
  ...
  if (ctx->gfx->vtable->draw_rect) {
    ...
    grabber_rect.x =
        sheet_rect.x + (sheet_rect.width / 2.0f) - (grabber_rect.width / 2.0f);
    grabber_rect.y = sheet_rect.y + 8.0f;
    ctx->gfx->vtable->draw_rect(ctx->gfx->ctx, &grabber_rect, grabber_color,
                                grabber_rect.height / 2.0f);
  }
