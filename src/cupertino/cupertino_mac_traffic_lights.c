...
CMP_API int CMP_CALL cupertino_mac_traffic_lights_paint(
  ...
  if (lights == NULL || ctx == NULL || ctx->gfx == NULL ||
      ctx->gfx->vtable == NULL) {
    return CMP_ERR_INVALID_ARGUMENT;
  }

  /* Red, Yellow, Green */
  colors_active[0].r = 1.0f;
  colors_active[0].g = 0.36f;
  colors_active[0].b = 0.33f;
  colors_active[0].a = 1.0f;
  colors_active[1].r = 1.0f;
  colors_active[1].g = 0.75f;
  colors_active[1].b = 0.18f;
  colors_active[1].a = 1.0f;
  colors_active[2].r = 0.16f;
  colors_active[2].g = 0.78f;
  colors_active[2].b = 0.25f;
  colors_active[2].a = 1.0f;

  /* Darker states when pressed */
  colors_pressed[0].r = 0.75f;
  colors_pressed[0].g = 0.25f;
  colors_pressed[0].b = 0.25f;
  colors_pressed[0].a = 1.0f;
  colors_pressed[1].r = 0.75f;
  colors_pressed[1].g = 0.55f;
  colors_pressed[1].b = 0.10f;
  colors_pressed[1].a = 1.0f;
  colors_pressed[2].r = 0.10f;
  colors_pressed[2].g = 0.55f;
  colors_pressed[2].b = 0.15f;
  colors_pressed[2].a = 1.0f;

  /* Inactive window state (grayed out) */
  if (lights->is_dark_mode) {
    colors_inactive[0].r = 0.3f;
    colors_inactive[0].g = 0.3f;
    colors_inactive[0].b = 0.3f;
    colors_inactive[0].a = 1.0f;
    colors_inactive[1] = colors_inactive[0];
    colors_inactive[2] = colors_inactive[0];
    outline_color.r = 0.1f;
    outline_color.g = 0.1f;
    outline_color.b = 0.1f;
    ...
    colors_inactive[0].r = 0.85f;
    colors_inactive[0].g = 0.85f;
    colors_inactive[0].b = 0.85f;
    colors_inactive[0].a = 1.0f;
    colors_inactive[1] = colors_inactive[0];
    colors_inactive[2] = colors_inactive[0];
    outline_color.r = 0.8f;
    outline_color.g = 0.8f;
    outline_color.b = 0.8f;
    outline_color.a = 0.5f;
    icon_color.r = 0.0f;
    icon_color.g = 0.0f;
    icon_color.b = 0.0f;
  ...
  for (i = 0; i < 3; i++) {
    ...
    if (lights->is_hovered &&
        (lights->is_window_active || lights->is_hovered)) {
      ...
      if (ctx->gfx->vtable->draw_line) {
        if (i == 0) {
          /* Close (X) */
          CMPScalar s = 2.5f;
          ctx->gfx->vtable->draw_line(ctx->gfx->ctx, cx - s, cy - s, cx + s,
                                      cy + s, icon_color, 1.2f);
          ctx->gfx->vtable->draw_line(ctx->gfx->ctx, cx + s, cy - s, cx - s,
                                      cy + s, icon_color, 1.2f);
        } else if (i == 1) {
          /* Minimize (-) */
          CMPScalar s = 3.5f;
          ctx->gfx->vtable->draw_line(ctx->gfx->ctx, cx - s, cy, cx + s, cy,
                                      icon_color, 1.5f);
        } else if (i == 2) {
          ...
          ctx->gfx->vtable->draw_line(ctx->gfx->ctx, cx - s, cy, cx + s, cy,
                                      icon_color, 1.2f);
          ctx->gfx->vtable->draw_line(ctx->gfx->ctx, cx, cy - s, cx, cy + s,
                                      icon_color, 1.2f);
        }
      }
    }
