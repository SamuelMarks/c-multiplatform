...
CMP_API int CMP_CALL cupertino_text_field_paint(const CupertinoTextField *field, CMPPaintContext *ctx) {
    ...
    if (field == NULL || ctx == NULL || ctx->gfx == NULL || ctx->gfx->vtable == NULL) {
        return CMP_ERR_INVALID_ARGUMENT;
    }
    ...
    if (field->is_dark_mode) {
        ...
        border_color.r = 0.2f; border_color.g = 0.2f; border_color.b = 0.2f; border_color.a = 1.0f;
        text_color.r = 1.0f; text_color.g = 1.0f; text_color.b = 1.0f; text_color.a = 1.0f;
        placeholder_color.r = 0.5f; placeholder_color.g = 0.5f; placeholder_color.b = 0.55f; placeholder_color.a = 1.0f;
    } else {
        ...
        placeholder_color.r = 0.78f; placeholder_color.g = 0.78f; placeholder_color.b = 0.8f; placeholder_color.a = 1.0f;
    }

    /* System Blue for cursor and focus rings */
    cupertino_color_get_system(CUPERTINO_COLOR_BLUE, field->is_dark_mode, &cursor_color);
    ...
    if (field->style == CUPERTINO_TEXT_FIELD_STYLE_ROUNDED_RECT) {
        if (ctx->gfx->vtable->draw_rect) {
            ...
            if (field->is_focused && !field->is_disabled) {
                ...
                ring.x -= 2.0f; ring.y -= 2.0f;
                ring.width += 4.0f; ring.height += 4.0f;
                ctx->gfx->vtable->draw_rect(ctx->gfx->ctx, &ring, focus_ring_color, corner_radius + 2.0f);
            }
            
            /* Background */
            ctx->gfx->vtable->draw_rect(ctx->gfx->ctx, &field->bounds, bg_color, corner_radius);
            
            /* Border */
            if (!field->is_focused && !field->is_dark_mode) {
                CMPRect bdr = field->bounds;
                /* Simulated inner stroke by drawing a slightly smaller rect? 
                   Assuming draw_rect handles fills, we just draw the border manually or rely on a draw_rounded_rect variant if it existed.
                   Using simple line strokes around the edges as fallback if needed, but skipping for brevity unless we have stroke API. */
                   
                /* Drawing a thin border by drawing background color slightly inset */
                ctx->gfx->vtable->draw_rect(ctx->gfx->ctx, &bdr, border_color, corner_radius);
                bdr.x += 1.0f; bdr.y += 1.0f; bdr.width -= 2.0f; bdr.height -= 2.0f;
                ctx->gfx->vtable->draw_rect(ctx->gfx->ctx, &bdr, bg_color, corner_radius - 1.0f);
            }
        }
        ...
        if (ctx->gfx->vtable->draw_line) {
            ...
            ctx->gfx->vtable->draw_line(ctx->gfx->ctx, field->bounds.x, field->bounds.y + field->bounds.height, 
                                        field->bounds.x + field->bounds.width, field->bounds.y + field->bounds.height, 
    ...
    if (cmp_text_font_create((void*)&field->text_backend, &txt_style, &font) == CMP_OK) {
        CMPScalar text_width = 0.0f;
        CMPScalar text_y;
        
        /* Measure to find baseline */
        cmp_text_measure_utf8((void*)&field->text_backend, font, "Ay", 2, 0, &metrics);
        text_y = field->bounds.y + (field->bounds.height / 2.0f) - (metrics.height / 2.0f) + metrics.baseline;

        if (field->text_len > 0) {
            /* Draw Text */
            cmp_text_measure_utf8((void*)&field->text_backend, font, field->text, field->text_len, 0, &metrics);
            text_width = metrics.width;
            cmp_text_draw_utf8_gfx(ctx->gfx, font, field->text, field->text_len, 0, content_x, text_y, text_color);
        } else if (field->placeholder_len > 0) {
            /* Draw Placeholder */
            cmp_text_draw_utf8_gfx(ctx->gfx, font, field->placeholder, field->placeholder_len, 0, content_x, text_y, placeholder_color);
        }
        ...
        if (field->is_focused && field->cursor_blink_time < 0.5) {
            if (ctx->gfx->vtable->draw_line) {
                ...
                ctx->gfx->vtable->draw_line(ctx->gfx->ctx, cx, cy1, cx, cy2, cursor_color, 2.0f);
            }
        }
