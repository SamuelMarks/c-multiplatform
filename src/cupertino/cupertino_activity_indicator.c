...
CMP_API int CMP_CALL cupertino_activity_indicator_init(CupertinoActivityIndicator *indicator) {
    ...
    cupertino_color_get_system(CUPERTINO_COLOR_GRAY, CMP_FALSE, &indicator->color);
...
CMP_API int CMP_CALL cupertino_activity_indicator_paint(const CupertinoActivityIndicator *indicator, CMPPaintContext *ctx) {
    ...
    if (indicator == NULL || ctx == NULL || ctx->gfx == NULL || ctx->gfx->vtable == NULL) {
        return CMP_ERR_INVALID_ARGUMENT;
    }
    ...
    /* iOS spinner spins clockwise and has discrete "steps" (ticks), not perfectly smooth rotation */
    ...
    for (i = 0; i < num_spokes; i++) {
        ...
        angle = ((CMPScalar)i / (CMPScalar)num_spokes) * (CMPScalar)(2.0 * 3.1415926535);
        
        /* Calculate opacity for this spoke.
         * The "head" of the spinner is the current step. Opacity fades out counter-clockwise. */
        diff = i - current_step;
        if (diff < 0) {
            diff += num_spokes;
        }
        ...
        /* The tail shouldn't completely disappear in the default Apple design, base opacity ~0.2 */
        if (alpha < 0.2f) {
            alpha = 0.2f;
        }
        ...
        if (ctx->gfx->vtable->draw_rect) {
            ctx->gfx->vtable->draw_rect(ctx->gfx->ctx, &spoke_rect, spoke_color, spoke_width / 2.0f);
        }
    }
