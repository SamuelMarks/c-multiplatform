...
CMP_API int CMP_CALL cupertino_navigation_bar_style_init(CupertinoNavigationBarStyle *style) {
    ...
    cupertino_typography_get_style(&typo, CUPERTINO_TYPOGRAPHY_HEADLINE, &style->title_style);
    cupertino_typography_get_style(&typo, CUPERTINO_TYPOGRAPHY_LARGE_TITLE, &style->large_title_style);
    style->large_title_style.weight = 700; /* Bold large title */

    return CMP_OK;
}

CMP_API int CMP_CALL cupertino_navigation_bar_init(CupertinoNavigationBar *bar, const CMPTextBackend *text_backend) {
...
CMP_API int CMP_CALL cupertino_navigation_bar_set_title(CupertinoNavigationBar *bar, const char *title_utf8, cmp_usize title_len) {
    if (bar == NULL) {
        return CMP_ERR_INVALID_ARGUMENT;
    }
...
CMP_API int CMP_CALL cupertino_navigation_bar_layout(CupertinoNavigationBar *bar, CMPRect bounds, CMPRect safe_area) {
    ...
    if (bar->style.variant == CUPERTINO_NAV_BAR_LARGE_TITLE) {
        ...
        if (large_ext < 0.0f) large_ext = 0.0f;
    ...
    if (bar->left_item) {
        CMPRect btn_bounds = {
            16.0f, 
            top_inset + (44.0f - bar->left_item->bounds.height) / 2.0f,
            bar->left_item->bounds.width,
            bar->left_item->bounds.height
        };
    ...
    if (bar->right_item) {
        CMPRect btn_bounds = {
            ...
            bar->right_item->bounds.width,
            bar->right_item->bounds.height
        };
...
CMP_API int CMP_CALL cupertino_navigation_bar_paint(const CupertinoNavigationBar *bar, CMPPaintContext *ctx) {
    ...
    rc = cupertino_blur_get_effect(bar->style.background_blur, bar->style.is_dark_mode, &blur_effect);
    if (rc == CMP_OK && ctx->gfx->vtable->draw_rect) {
        /* Hack: Standard CMPGfx doesn't support blurs natively yet, so we use the tinted base color.
           In a real iOS backend, this rect would be drawn with UIVisualEffectView. */
        ctx->gfx->vtable->draw_rect(ctx->gfx->ctx, &bar->bounds, blur_effect.tint_color, 0.0f);
        ...
        if (ctx->gfx->vtable->draw_line) {
            ctx->gfx->vtable->draw_line(ctx->gfx->ctx, 0.0f, bar->bounds.height, bar->bounds.width, bar->bounds.height, border_color, 1.0f);
        }
    }
    ...
    if (bar->title_utf8 && bar->title_len > 0) {
        ...
        if (show_inline) {
            ...
            if (cmp_text_font_create(&bar->text_backend, &applied_style, &font) == CMP_OK) {
                cmp_text_measure_utf8(&bar->text_backend, font, bar->title_utf8, bar->title_len, 0, &metrics);
                
                text_x = bar->bounds.x + (bar->bounds.width - metrics.width) / 2.0f;
                text_y = top_inset + (44.0f - metrics.height) / 2.0f + metrics.baseline;
                
                cmp_text_draw_utf8_gfx(ctx->gfx, font, bar->title_utf8, bar->title_len, 0, text_x, text_y, text_color);
                cmp_text_font_destroy(&bar->text_backend, font);
            }
        }

        if (bar->style.variant == CUPERTINO_NAV_BAR_LARGE_TITLE && bar->scroll_offset < 52.0f) {
            CMPHandle large_font = {0};
            applied_style = bar->style.large_title_style;
            
            /* Fade out large title slightly as it collapses, though mostly it just shrinks/moves */
            text_color.a = 1.0f - (bar->scroll_offset / 52.0f);
            if (text_color.a < 0.0f) text_color.a = 0.0f;

            if (cmp_text_font_create(&bar->text_backend, &applied_style, &large_font) == CMP_OK) {
                cmp_text_measure_utf8(&bar->text_backend, large_font, bar->title_utf8, bar->title_len, 0, &metrics);
                
                text_x = 16.0f; /* iOS large titles are left-aligned */
                /* Base line calculation for the extended area */
                text_y = top_inset + 44.0f + (52.0f - bar->scroll_offset - metrics.height) / 2.0f + metrics.baseline;
                
                cmp_text_draw_utf8_gfx(ctx->gfx, large_font, bar->title_utf8, bar->title_len, 0, text_x, text_y, text_color);
