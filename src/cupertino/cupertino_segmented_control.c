...
CMP_API int CMP_CALL cupertino_segmented_control_set_selected(
  ...
  if (animated) {
    ...
    cmp_anim_controller_start_spring(&control->selection_anim,
...
CMP_API int CMP_CALL cupertino_segmented_control_update(
  ...
  cmp_anim_controller_step(&control->selection_anim, (CMPScalar)delta_time,
                           &out_val, &finished);
...
CMP_API int CMP_CALL cupertino_segmented_control_paint(
  ...
  if (control == NULL || ctx == NULL || ctx->gfx == NULL ||
      ctx->gfx->vtable == NULL) {
    return CMP_ERR_INVALID_ARGUMENT;
  }
  ...
  segment_w = (control->bounds.width - (padding * 2.0f)) /
              (CMPScalar)control->segment_count;
  ...
  if (control->is_dark_mode) {
    ...
    bg_color.g = 0.11f;
    bg_color.b = 0.118f;
    bg_color.a = 1.0f; /* iOS 13+ dark mode background */
    thumb_color.r = 0.38f;
    thumb_color.g = 0.38f;
    thumb_color.b = 0.4f;
    thumb_color.a = 1.0f;
    divider_color.r = 0.38f;
    divider_color.g = 0.38f;
    divider_color.b = 0.4f;
    divider_color.a = 0.5f;
    text_color_selected.r = 1.0f;
    text_color_selected.g = 1.0f;
    text_color_selected.b = 1.0f;
    text_color_selected.a = 1.0f;
    text_color_normal.r = 1.0f;
    text_color_normal.g = 1.0f;
    text_color_normal.b = 1.0f;
    text_color_normal.a = 1.0f;
  } else {
    bg_color.r = 0.93f;
    bg_color.g = 0.93f;
    bg_color.b = 0.937f;
    bg_color.a = 1.0f; /* iOS 13+ light mode background */
    thumb_color.r = 1.0f;
    thumb_color.g = 1.0f;
    thumb_color.b = 1.0f;
    thumb_color.a = 1.0f;
    divider_color.r = 0.55f;
    divider_color.g = 0.55f;
    divider_color.b = 0.55f;
    divider_color.a = 0.5f;
    text_color_selected.r = 0.0f;
    text_color_selected.g = 0.0f;
    text_color_selected.b = 0.0f;
    text_color_selected.a = 1.0f;
    text_color_normal.r = 0.0f;
    text_color_normal.g = 0.0f;
    text_color_normal.b = 0.0f;
    text_color_normal.a = 1.0f;
  }

  /* Track Background */
  if (ctx->gfx->vtable->draw_rect) {
    ctx->gfx->vtable->draw_rect(ctx->gfx->ctx, &control->bounds, bg_color,
                                corner_radius);
  }
  ...
  if (ctx->gfx->vtable->draw_line) {
    for (i = 1; i < control->segment_count; i++) {
      ...
      /* Don't draw divider if it's right next to the selected segment to mimic
       * iOS behavior closely */
      /* Simplified: just draw all dividers, the thumb will draw over them */
      ctx->gfx->vtable->draw_line(
          ctx->gfx->ctx, div_x, control->bounds.y + 6.0f, div_x,
          control->bounds.y + control->bounds.height - 6.0f, divider_color,
          1.0f);
    }
  }
  ...
  /* Thumb Position & Rect */
  thumb_x =
      control->bounds.x + padding + (control->selection_anim.value * segment_w);
  ...
  for (i = 0; i < control->segment_count; i++) {
    ...
    text_style.weight =
        500; /* Medium for selected, Regular for unselected usually, but using
                Medium for both for simplicity */
    text_style.color = (i == (cmp_usize)control->selected_index)
                           ? text_color_selected
                           : text_color_normal;
