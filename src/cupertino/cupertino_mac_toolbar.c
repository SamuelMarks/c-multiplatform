...
CMP_API int CMP_CALL cupertino_mac_toolbar_init(CupertinoMacToolbar *toolbar, const CMPTextBackend *text_backend) {
    ...
    toolbar->bounds.height = 52; /* Standard macOS unified toolbar height is often 52 or 54 */
...
CMP_API int CMP_CALL cupertino_mac_toolbar_add_item(CupertinoMacToolbar *toolbar, const char *title_utf8) {
    ...
    if (toolbar == NULL || title_utf8 == NULL) return CMP_ERR_INVALID_ARGUMENT;
    if (toolbar->item_count >= CUPERTINO_MAC_TOOLBAR_MAX_ITEMS) return CMP_ERR_OUT_OF_MEMORY;
...
CMP_API int CMP_CALL cupertino_mac_toolbar_add_flexible_space(CupertinoMacToolbar *toolbar) {
    CupertinoMacToolbarItem *item;
    if (toolbar == NULL) return CMP_ERR_INVALID_ARGUMENT;
    if (toolbar->item_count >= CUPERTINO_MAC_TOOLBAR_MAX_ITEMS) return CMP_ERR_OUT_OF_MEMORY;
...
CMP_API int CMP_CALL cupertino_mac_toolbar_paint(const CupertinoMacToolbar *toolbar, CMPPaintContext *ctx) {
    ...
    if (toolbar == NULL || ctx == NULL || ctx->gfx == NULL || ctx->gfx->vtable == NULL) {
        return CMP_ERR_INVALID_ARGUMENT;
    }
    ...
    if (flex_count > 0) {
        flex_width = (toolbar->bounds.width - total_fixed_width - 80.0f) / (CMPScalar)flex_count; /* 80 for traffic lights space */
        if (flex_width < 0.0f) flex_width = 0.0f;
    }

    /* Colors */
    if (toolbar->is_dark_mode) {
        bg_color.r = 0.15f; bg_color.g = 0.15f; bg_color.b = 0.15f; bg_color.a = 1.0f; 
        border_color.r = 0.0f; border_color.g = 0.0f; border_color.b = 0.0f; border_color.a = 1.0f;
        text_color.r = 0.9f; text_color.g = 0.9f; text_color.b = 0.9f; text_color.a = 1.0f;
        btn_bg_color.r = 0.3f; btn_bg_color.g = 0.3f; btn_bg_color.b = 0.3f; btn_bg_color.a = 1.0f;
    } else {
        bg_color.r = 0.95f; bg_color.g = 0.95f; bg_color.b = 0.95f; bg_color.a = 1.0f;
        border_color.r = 0.8f; border_color.g = 0.8f; border_color.b = 0.8f; border_color.a = 1.0f;
        text_color.r = 0.1f; text_color.g = 0.1f; text_color.b = 0.1f; text_color.a = 1.0f;
        btn_bg_color.r = 0.9f; btn_bg_color.g = 0.9f; btn_bg_color.b = 0.9f; btn_bg_color.a = 1.0f;
    }
    ...
    if (ctx->gfx->vtable->draw_rect) {
        ctx->gfx->vtable->draw_rect(ctx->gfx->ctx, &toolbar->bounds, bg_color, 0.0f);
    }
    
    /* Bottom border separator */
    if (ctx->gfx->vtable->draw_line) {
        ctx->gfx->vtable->draw_line(ctx->gfx->ctx, toolbar->bounds.x, toolbar->bounds.y + toolbar->bounds.height - 1.0f, 
                                    toolbar->bounds.x + toolbar->bounds.width, toolbar->bounds.y + toolbar->bounds.height - 1.0f, 
                                    border_color, 1.0f);
    }
    ...
    for (i = 0; i < toolbar->item_count; i++) {
        ...
        if (item->is_search) {
            ...
            if (ctx->gfx->vtable->draw_rect) {
                ...
                CMPColor search_bg = toolbar->is_dark_mode ? (CMPColor){0.0f, 0.0f, 0.0f, 0.2f} : (CMPColor){1.0f, 1.0f, 1.0f, 0.5f};
            ...
            /* Simple button */
            if (ctx->gfx->vtable->draw_rect) {
                ctx->gfx->vtable->draw_rect(ctx->gfx->ctx, &item_rect, btn_bg_color, 6.0f);
            }
        }
        ...
        if (item->title_len > 0) {
            ...
            if (cmp_text_font_create((void*)&toolbar->text_backend, &row_style, &font) == CMP_OK) {
                CMPScalar text_y;
                CMPScalar text_x;
                
                cmp_text_measure_utf8((void*)&toolbar->text_backend, font, item->title_utf8, item->title_len, 0, &metrics);
                text_y = item_rect.y + (item_rect.height / 2.0f) - (metrics.height / 2.0f) + metrics.baseline;
                
                if (item->is_search) {
                    text_x = item_rect.x + 8.0f; /* left aligned */
                } else {
                    text_x = item_rect.x + (item_rect.width / 2.0f) - (metrics.width / 2.0f); /* center */
                }

                cmp_text_draw_utf8_gfx(ctx->gfx, font, item->title_utf8, item->title_len, 0, text_x, text_y, text_color);
