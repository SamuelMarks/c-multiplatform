...
CMP_API int CMP_CALL cupertino_list_section_set_headers(CupertinoListSection *section, const char *header_utf8, const char *footer_utf8) {
    if (section == NULL) return CMP_ERR_INVALID_ARGUMENT;
    
    if (header_utf8) {
        cmp_usize len = strlen(header_utf8);
        if (len >= CUPERTINO_LIST_MAX_TEXT) len = CUPERTINO_LIST_MAX_TEXT - 1;
...
CMP_API int CMP_CALL cupertino_list_section_add_row(CupertinoListSection *section, const char *title_utf8, const char *value_utf8, CMPBool has_chevron) {
    ...
    if (section == NULL || title_utf8 == NULL) return CMP_ERR_INVALID_ARGUMENT;
    if (section->row_count >= CUPERTINO_LIST_MAX_ROWS) return CMP_ERR_OUT_OF_MEMORY;
...
CMP_API int CMP_CALL cupertino_list_section_paint(const CupertinoListSection *section, CMPPaintContext *ctx) {
    ...
    if (section == NULL || ctx == NULL || ctx->gfx == NULL || ctx->gfx->vtable == NULL) {
        return CMP_ERR_INVALID_ARGUMENT;
    }
    ...
    if (section->is_dark_mode) {
        ...
        pressed_color.r = 0.15f; pressed_color.g = 0.15f; pressed_color.b = 0.15f; pressed_color.a = 1.0f;
        divider_color.r = 0.2f; divider_color.g = 0.2f; divider_color.b = 0.2f; divider_color.a = 1.0f;
        title_color.r = 1.0f; title_color.g = 1.0f; title_color.b = 1.0f; title_color.a = 1.0f;
        value_color.r = 0.55f; value_color.g = 0.55f; value_color.b = 0.55f; value_color.a = 1.0f;
        header_color.r = 0.5f; header_color.g = 0.5f; header_color.b = 0.55f; header_color.a = 1.0f;
        chevron_color.r = 0.3f; chevron_color.g = 0.3f; chevron_color.b = 0.33f; chevron_color.a = 1.0f;
    } else {
        bg_color.r = 1.0f; bg_color.g = 1.0f; bg_color.b = 1.0f; bg_color.a = 1.0f;
        pressed_color.r = 0.9f; pressed_color.g = 0.9f; pressed_color.b = 0.9f; pressed_color.a = 1.0f;
        divider_color.r = 0.85f; divider_color.g = 0.85f; divider_color.b = 0.85f; divider_color.a = 1.0f;
        title_color.r = 0.0f; title_color.g = 0.0f; title_color.b = 0.0f; title_color.a = 1.0f;
        value_color.r = 0.55f; value_color.g = 0.55f; value_color.b = 0.57f; value_color.a = 1.0f;
        header_color.r = 0.45f; header_color.g = 0.45f; header_color.b = 0.47f; header_color.a = 1.0f;
        chevron_color.r = 0.78f; chevron_color.g = 0.78f; chevron_color.b = 0.8f; chevron_color.a = 1.0f;
    }
    ...
    if (section->header_len > 0) {
        ...
        if (cmp_text_font_create((void*)&section->text_backend, &h_style, &font) == CMP_OK) {
            cmp_text_measure_utf8((void*)&section->text_backend, font, "Ay", 2, 0, &metrics);
            cmp_text_draw_utf8_gfx(ctx->gfx, font, section->header_utf8, section->header_len, 0, section->bounds.x + padding_x, content_y + metrics.baseline, header_color);
    ...
    if (section->row_count > 0 && ctx->gfx->vtable->draw_rect) {
        ...
        ctx->gfx->vtable->draw_rect(ctx->gfx->ctx, &list_rect, bg_color, corner_radius);
    }
    ...
    for (i = 0; i < section->row_count; i++) {
        ...
        if (row->is_pressed && ctx->gfx->vtable->draw_rect) {
            ...
            ctx->gfx->vtable->draw_rect(ctx->gfx->ctx, &row_rect, pressed_color, 0.0f);
        }

        /* Divider (Not on the first row, drawn at top of current row) */
        if (i > 0 && ctx->gfx->vtable->draw_line) {
            ctx->gfx->vtable->draw_line(ctx->gfx->ctx, row_rect.x + padding_x, row_rect.y, row_rect.x + row_rect.width, row_rect.y, divider_color, 1.0f);
        }
        ...
        /* Text drawing */
        {
            ...
            if (cmp_text_font_create((void*)&section->text_backend, &row_style, &font) == CMP_OK) {
                CMPScalar text_y;
                cmp_text_measure_utf8((void*)&section->text_backend, font, "Ay", 2, 0, &metrics);
                text_y = row_rect.y + (row_height / 2.0f) - (metrics.height / 2.0f) + metrics.baseline;

                /* Title */
                cmp_text_draw_utf8_gfx(ctx->gfx, font, row->title_utf8, row->title_len, 0, row_rect.x + padding_x, text_y, title_color);
                ...
                if (row->has_chevron && ctx->gfx->vtable->draw_line) {
                    ...
                    CMPScalar size = 5.0f;
                    ctx->gfx->vtable->draw_line(ctx->gfx->ctx, cx, cy - size, cx + size, cy, chevron_color, 2.0f);
                    ctx->gfx->vtable->draw_line(ctx->gfx->ctx, cx + size, cy, cx, cy + size, chevron_color, 2.0f);
                ...
                if (row->value_len > 0) {
                    ...
                    cmp_text_measure_utf8((void*)&section->text_backend, font, row->value_utf8, row->value_len, 0, &v_metrics);
                    cmp_text_draw_utf8_gfx(ctx->gfx, font, row->value_utf8, row->value_len, 0, right_x - v_metrics.width, text_y, value_color);
                }
    ...
    if (section->footer_len > 0) {
        ...
        if (cmp_text_font_create((void*)&section->text_backend, &f_style, &font) == CMP_OK) {
            cmp_text_measure_utf8((void*)&section->text_backend, font, "Ay", 2, 0, &metrics);
            cmp_text_draw_utf8_gfx(ctx->gfx, font, section->footer_utf8, section->footer_len, 0, section->bounds.x + padding_x, content_y + metrics.baseline, header_color);
            cmp_text_font_destroy((void*)&section->text_backend, font);
        }
    }
