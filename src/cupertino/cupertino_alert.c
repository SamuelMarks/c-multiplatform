...
CMP_API int CMP_CALL cupertino_alert_init(CupertinoAlert *alert, const CMPTextBackend *text_backend) {
...
CMP_API int CMP_CALL cupertino_alert_set_content(CupertinoAlert *alert, const char *title_utf8, const char *message_utf8) {
    if (alert == NULL) {
        return CMP_ERR_INVALID_ARGUMENT;
    }
...
CMP_API int CMP_CALL cupertino_alert_add_action(CupertinoAlert *alert, const char *label_utf8, CMPBool is_destructive) {
    CupertinoButton *btn;
    ...
    if (is_destructive) {
        cupertino_color_get_system(CUPERTINO_COLOR_RED, alert->is_dark_mode, &style.tint_color);
    } else {
        cupertino_color_get_system(CUPERTINO_COLOR_BLUE, alert->is_dark_mode, &style.tint_color);
    }
...
CMP_API int CMP_CALL cupertino_alert_paint(const CupertinoAlert *alert, CMPPaintContext *ctx) {
    CMPRect alert_rect;
    CMPColor bg_color;
    CMPColor div_color;
    CMPColor text_color;
    ...
    if (alert == NULL || ctx == NULL || ctx->gfx == NULL || ctx->gfx->vtable == NULL) {
        return CMP_ERR_INVALID_ARGUMENT;
    }
    ...
    if (alert->is_dark_mode) {
        ...
        bg_color.r = 0.15f; bg_color.g = 0.15f; bg_color.b = 0.15f; bg_color.a = 0.95f;
        div_color.r = 0.3f; div_color.g = 0.3f; div_color.b = 0.3f; div_color.a = 0.5f;
        ...
        div_color.r = 0.8f; div_color.g = 0.8f; div_color.b = 0.8f; div_color.a = 0.5f;
        text_color.r = 0.0f; text_color.g = 0.0f; text_color.b = 0.0f; text_color.a = 1.0f;
    }

    /* Background */
    if (ctx->gfx->vtable->draw_rect) {
        ctx->gfx->vtable->draw_rect(ctx->gfx->ctx, &alert_rect, bg_color, corner_radius);
    }
    ...
    if (alert->title_len > 0) {
        ...
        if (cmp_text_font_create((void*)&alert->text_backend, &title_style, &font) == CMP_OK) {
            cmp_text_measure_utf8((void*)&alert->text_backend, font, alert->title_utf8, alert->title_len, 0, &metrics);
            
            CMPScalar title_x = alert_rect.x + (alert_rect.width - metrics.width) / 2.0f;
            cmp_text_draw_utf8_gfx(ctx->gfx, font, alert->title_utf8, alert->title_len, 0, title_x, content_y + metrics.baseline, text_color);
    ...
    if (alert->message_len > 0) {
        ...
        if (cmp_text_font_create((void*)&alert->text_backend, &msg_style, &font) == CMP_OK) {
            /* Basic multi-line measure would be here, assuming single line or simple layout for now */
            cmp_text_measure_utf8((void*)&alert->text_backend, font, alert->message_utf8, alert->message_len, 0, &metrics);
            
            CMPScalar msg_x = alert_rect.x + (alert_rect.width - metrics.width) / 2.0f;
            content_y += 4.0f;
            cmp_text_draw_utf8_gfx(ctx->gfx, font, alert->message_utf8, alert->message_len, 0, msg_x, content_y + metrics.baseline, text_color);
    ...
    if (ctx->gfx->vtable->draw_line) {
        ctx->gfx->vtable->draw_line(ctx->gfx->ctx, alert_rect.x, content_y, alert_rect.x + alert_rect.width, content_y, div_color, 1.0f);
    }
    ...
    if (alert->action_count == 2) {
        ...
        for (i = 0; i < 2; i++) {
            ...
            if (i == 0 && ctx->gfx->vtable->draw_line) {
                ctx->gfx->vtable->draw_line(ctx->gfx->ctx, alert_rect.x + btn_w, content_y, alert_rect.x + btn_w, content_y + btn_h, div_color, 1.0f);
            }
        }
