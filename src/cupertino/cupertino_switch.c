...
CMP_API int CMP_CALL cupertino_switch_init(CupertinoSwitch *switch_inst) {
    ...
    cupertino_color_get_system(CUPERTINO_COLOR_GREEN, CMP_FALSE, &switch_inst->on_tint_color);
...
CMP_API int CMP_CALL cupertino_switch_set_on(CupertinoSwitch *switch_inst, CMPBool is_on, CMPBool animated) {
    ...
    if (animated) {
        ...
        cmp_anim_controller_start_spring(&switch_inst->anim, switch_inst->anim.value, target, 300.0f, 25.0f, 1.0f);
    } else {
...
CMP_API int CMP_CALL cupertino_switch_update(CupertinoSwitch *switch_inst, double delta_time) {
    ...
    CMPScalar out_val;
    cmp_anim_controller_step(&switch_inst->anim, (CMPScalar)delta_time, &out_val, &finished);
...
CMP_API int CMP_CALL cupertino_switch_paint(const CupertinoSwitch *switch_inst, CMPPaintContext *ctx) {
    ...
    if (switch_inst == NULL || ctx == NULL || ctx->gfx == NULL || ctx->gfx->vtable == NULL) {
        return CMP_ERR_INVALID_ARGUMENT;
    }
    ...
    track_color.r = track_color.r + (switch_inst->on_tint_color.r - track_color.r) * progress;
    track_color.g = track_color.g + (switch_inst->on_tint_color.g - track_color.g) * progress;
    track_color.b = track_color.b + (switch_inst->on_tint_color.b - track_color.b) * progress;
    track_color.a = track_color.a + (switch_inst->on_tint_color.a - track_color.a) * progress;
    ...
    if (ctx->gfx->vtable->draw_rect) {
        ctx->gfx->vtable->draw_rect(ctx->gfx->ctx, &track_rect, track_color, track_radius);
    ...
    if (progress < 1.0f && !switch_inst->is_dark_mode) {
        ...
        if (ctx->gfx->vtable->draw_rect) {
            ctx->gfx->vtable->draw_rect(ctx->gfx->ctx, &track_rect, border_color, track_radius);
        }
    }
    ...
    /* Calculate knob X taking press state into account (knob expands slightly when pressed) */
    ...
    knob_offset_x = progress * (track_rect.width - knob_rect.width - (knob_padding * 2.0f));
    ...
    if (ctx->gfx->vtable->draw_rect) {
        ...
        shadow_rect.y += 2.0f;
        ctx->gfx->vtable->draw_rect(ctx->gfx->ctx, &shadow_rect, shadow_color, knob_radius);
        /* Draw knob */
        ctx->gfx->vtable->draw_rect(ctx->gfx->ctx, &knob_rect, switch_inst->thumb_tint_color, knob_radius);
    } else if (ctx->gfx->vtable->draw_rect) {
        ctx->gfx->vtable->draw_rect(ctx->gfx->ctx, &knob_rect, switch_inst->thumb_tint_color, 0.0f);
    }
