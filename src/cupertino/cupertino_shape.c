...
CMP_API int CMP_CALL cupertino_shape_append_squircle(CMPPath *path, CMPRect bounds,
    ...
    if (tr > 0.0f) {
        rc = cmp_path_cubic_to(path,
                               bounds.x + w - tr * (1.0f - CUPERTINO_SQUIRCLE_K), bounds.y,
    ...
    if (br > 0.0f) {
        rc = cmp_path_cubic_to(path,
                               bounds.x + w, bounds.y + h - br * (1.0f - CUPERTINO_SQUIRCLE_K),
                               bounds.x + w - br * (1.0f - CUPERTINO_SQUIRCLE_K), bounds.y + h,
                               bounds.x + w - br, bounds.y + h);
        if (rc != CMP_OK) return rc;
    }
    ...
    if (bl > 0.0f) {
        rc = cmp_path_cubic_to(path,
                               bounds.x + bl * (1.0f - CUPERTINO_SQUIRCLE_K), bounds.y + h,
                               bounds.x, bounds.y + h - bl * (1.0f - CUPERTINO_SQUIRCLE_K),
    ...
    if (tl > 0.0f) {
        rc = cmp_path_cubic_to(path,
                               bounds.x, bounds.y + tl * (1.0f - CUPERTINO_SQUIRCLE_K),
                               bounds.x + tl * (1.0f - CUPERTINO_SQUIRCLE_K), bounds.y,
                               bounds.x + tl, bounds.y);
        if (rc != CMP_OK) return rc;
    }
