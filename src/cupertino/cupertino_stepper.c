...
CMP_API int CMP_CALL cupertino_stepper_set_value(CupertinoStepper *stepper, double value) {
    ...
    if (value < stepper->min_value) value = stepper->min_value;
    if (value > stepper->max_value) value = stepper->max_value;
...
CMP_API int CMP_CALL cupertino_stepper_paint(const CupertinoStepper *stepper, CMPPaintContext *ctx) {
    ...
    if (stepper == NULL || ctx == NULL || ctx->gfx == NULL || ctx->gfx->vtable == NULL) {
        return CMP_ERR_INVALID_ARGUMENT;
    }
    ...
    if (stepper->is_dark_mode) {
        ...
        pressed_bg_color.r = 0.2f; pressed_bg_color.g = 0.2f; pressed_bg_color.b = 0.2f; pressed_bg_color.a = 1.0f;
        icon_color.r = 1.0f; icon_color.g = 1.0f; icon_color.b = 1.0f; icon_color.a = 1.0f;
        disabled_icon_color.r = 0.3f; disabled_icon_color.g = 0.3f; disabled_icon_color.b = 0.3f; disabled_icon_color.a = 1.0f;
        divider_color.r = 0.3f; divider_color.g = 0.3f; divider_color.b = 0.3f; divider_color.a = 1.0f;
    } else {
        bg_color.r = 0.93f; bg_color.g = 0.93f; bg_color.b = 0.937f; bg_color.a = 1.0f;
        pressed_bg_color.r = 0.8f; pressed_bg_color.g = 0.8f; pressed_bg_color.b = 0.8f; pressed_bg_color.a = 1.0f;
        icon_color.r = 0.0f; icon_color.g = 0.0f; icon_color.b = 0.0f; icon_color.a = 1.0f;
        disabled_icon_color.r = 0.7f; disabled_icon_color.g = 0.7f; disabled_icon_color.b = 0.7f; disabled_icon_color.a = 1.0f;
        divider_color.r = 0.7f; divider_color.g = 0.7f; divider_color.b = 0.7f; divider_color.a = 1.0f;
    }

    minus_disabled = stepper->is_disabled || (stepper->value <= stepper->min_value);
    plus_disabled = stepper->is_disabled || (stepper->value >= stepper->max_value);

    /* Background (Draw single rounded rect) */
    if (ctx->gfx->vtable->draw_rect) {
        ctx->gfx->vtable->draw_rect(ctx->gfx->ctx, &stepper->bounds, bg_color, corner_radius);
        
        /* Highlight pressed buttons */
        if (stepper->pressed_button == 0 && !minus_disabled) {
            /* Draw left side pressed */
            CMPRect pr = minus_rect;
            ctx->gfx->vtable->draw_rect(ctx->gfx->ctx, &pr, pressed_bg_color, corner_radius);
    ...
    if (ctx->gfx->vtable->draw_line) {
        ctx->gfx->vtable->draw_line(ctx->gfx->ctx, plus_rect.x, stepper->bounds.y + 4.0f, plus_rect.x, stepper->bounds.y + stepper->bounds.height - 4.0f, divider_color, 1.0f);
    }
    ...
    if (ctx->gfx->vtable->draw_line) {
        ...
        ctx->gfx->vtable->draw_line(ctx->gfx->ctx, cx - size, cy, cx + size, cy, c, 2.0f);
    }
    ...
    if (ctx->gfx->vtable->draw_line) {
        ...
        ctx->gfx->vtable->draw_line(ctx->gfx->ctx, cx - size, cy, cx + size, cy, c, 2.0f);
        ctx->gfx->vtable->draw_line(ctx->gfx->ctx, cx, cy - size, cx, cy + size, c, 2.0f);
    }
