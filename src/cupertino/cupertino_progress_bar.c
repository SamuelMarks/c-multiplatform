...
CMP_API int CMP_CALL cupertino_progress_bar_init(CupertinoProgressBar *bar) {
  ...
  bar->track_color.r = 0.9f;
  bar->track_color.g = 0.9f;
  bar->track_color.b = 0.9f;
  bar->track_color.a = 1.0f;
...
CMP_API int CMP_CALL cupertino_progress_bar_set_progress(
  ...
  if (animated) {
    ...
    cmp_anim_controller_start_spring(&bar->anim, bar->anim.value, progress,
                                     200.0f, 20.0f, 1.0f);
  } else {
...
CMP_API int CMP_CALL cupertino_progress_bar_update(CupertinoProgressBar *bar,
  ...
  cmp_anim_controller_step(&bar->anim, (CMPScalar)delta_time, &out_val,
                           &finished);
...
CMP_API int CMP_CALL cupertino_progress_bar_paint(
  ...
  if (bar->is_dark_mode) {
    ...
    draw_track_color.r = 0.2f;
    draw_track_color.g = 0.2f;
    draw_track_color.b = 0.2f;
  }
  ...
  if (ctx->gfx->vtable->draw_rect) {
    /* Draw Track */
    ctx->gfx->vtable->draw_rect(ctx->gfx->ctx, &track_rect, draw_track_color,
                                radius);

    /* Draw Fill (if visible) */
    if (fill_rect.width > 0.1f) {
      ctx->gfx->vtable->draw_rect(ctx->gfx->ctx, &fill_rect,
                                  bar->progress_color, radius);
    }
  }
