...
CMP_API int CMP_CALL cupertino_button_style_init(CupertinoButtonStyle *style) {
    ...
    /* Initialize typography */
    {
        ...
        cupertino_typography_get_style(&typo, CUPERTINO_TYPOGRAPHY_BODY, &style->text_style);
...
CMP_API int CMP_CALL cupertino_button_init(CupertinoButton *button, const CMPTextBackend *text_backend) {
    if (button == NULL || text_backend == NULL) {
        return CMP_ERR_INVALID_ARGUMENT;
    }

    if (text_backend->vtable == NULL || text_backend->vtable->measure_text == NULL) {
    ...
    /* Widget vtable setup will happen later if integrated fully into the widget tree */

    return CMP_OK;
}

CMP_API int CMP_CALL cupertino_button_set_label(CupertinoButton *button, const char *label_utf8, cmp_usize label_len) {
    if (button == NULL) {
        return CMP_ERR_INVALID_ARGUMENT;
    }
...
CMP_API int CMP_CALL cupertino_button_set_style(CupertinoButton *button, const CupertinoButtonStyle *style) {
    int rc;
...
CMP_API int CMP_CALL cupertino_button_measure(CupertinoButton *button, CMPMeasureSpec width_spec, CMPMeasureSpec height_spec, CMPSize *out_size) {
    ...
    if (button->label_utf8 && button->label_len > 0) {
        ...
        if (cmp_text_font_create(&button->text_backend, &button->style.text_style, &font) == CMP_OK) {
            rc = cmp_text_measure_utf8(&button->text_backend, font, button->label_utf8, button->label_len, 0, &text_metrics);
    ...
    if (button->style.size == CUPERTINO_BUTTON_SIZE_MEDIUM && out_size->height < 44.0f) {
    ...
    } else if (width_spec.mode == CMP_MEASURE_AT_MOST && out_size->width > width_spec.size) {
        out_size->width = width_spec.size;
    }

    if (height_spec.mode == CMP_MEASURE_EXACTLY) {
        out_size->height = height_spec.size;
    } else if (height_spec.mode == CMP_MEASURE_AT_MOST && out_size->height > height_spec.size) {
        out_size->height = height_spec.size;
    }
...
CMP_API int CMP_CALL cupertino_button_layout(CupertinoButton *button, CMPRect bounds) {
    if (button == NULL) {
        return CMP_ERR_INVALID_ARGUMENT;
    }
...
static int cupertino_button_resolve_colors(const CupertinoButton *button, CMPColor *bg, CMPColor *text) {
    CMPColor tint = button->style.tint_color;
    CMPBool is_dark = button->style.is_dark_mode;

    bg->r = 0; bg->g = 0; bg->b = 0; bg->a = 0;
    text->r = 0; text->g = 0; text->b = 0; text->a = 1;

    switch (button->style.variant) {
        case CUPERTINO_BUTTON_VARIANT_PLAIN:
            text->r = tint.r; text->g = tint.g; text->b = tint.b; text->a = tint.a;
            ...
            if (button->is_disabled) {
                ...
                text->r = sys_gray.r; text->g = sys_gray.g; text->b = sys_gray.b; text->a = sys_gray.a;
            }
            break;
        case CUPERTINO_BUTTON_VARIANT_TINTED:
            bg->r = tint.r; bg->g = tint.g; bg->b = tint.b; bg->a = is_dark ? 0.2f : 0.15f;
            text->r = tint.r; text->g = tint.g; text->b = tint.b; text->a = 1.0f;
            if (button->is_pressed) {
            ...
            if (button->is_disabled) {
                ...
                bg->r = sys_gray.r; bg->g = sys_gray.g; bg->b = sys_gray.b; bg->a = sys_gray.a;
                cupertino_color_get_system(CUPERTINO_COLOR_GRAY, is_dark, &sys_gray);
                text->r = sys_gray.r; text->g = sys_gray.g; text->b = sys_gray.b; text->a = sys_gray.a;
            }
            break;
        case CUPERTINO_BUTTON_VARIANT_FILLED:
            bg->r = tint.r; bg->g = tint.g; bg->b = tint.b; bg->a = 1.0f;
            text->r = 1.0f; text->g = 1.0f; text->b = 1.0f; text->a = 1.0f; /* Always white */
            if (button->is_pressed) {
                bg->r *= 0.8f; bg->g *= 0.8f; bg->b *= 0.8f; /* Darken */
            } else if (button->is_hovered) {
        ...
        case CUPERTINO_BUTTON_VARIANT_GRAY:
            {
                ...
                bg->r = sys_gray_bg.r; bg->g = sys_gray_bg.g; bg->b = sys_gray_bg.b; bg->a = sys_gray_bg.a;
                text->r = tint.r; text->g = tint.g; text->b = tint.b; text->a = 1.0f;

                if (button->is_pressed) {
                    CMPColor sys_gray4;
                    cupertino_color_get_system(CUPERTINO_COLOR_GRAY4, is_dark, &sys_gray4);
                    bg->r = sys_gray4.r; bg->g = sys_gray4.g; bg->b = sys_gray4.b; bg->a = sys_gray4.a;
                }
                if (button->is_disabled) {
                    CMPColor sys_gray;
                    cupertino_color_get_system(CUPERTINO_COLOR_GRAY, is_dark, &sys_gray);
...
CMP_API int CMP_CALL cupertino_button_paint(const CupertinoButton *button, CMPPaintContext *ctx) {
    CMPColor bg_color, text_color;
    CMPScalar corner_radius = 0.0f;
    int rc;

    if (button == NULL || ctx == NULL || ctx->gfx == NULL || ctx->gfx->vtable == NULL) {
        return CMP_ERR_INVALID_ARGUMENT;
    }

    if (ctx->gfx->vtable->draw_path == NULL || ctx->gfx->vtable->draw_rect == NULL) {
        return CMP_ERR_UNSUPPORTED;
    }
    ...
    if (button->style.shape == CUPERTINO_BUTTON_SHAPE_CAPSULE) {
        corner_radius = (button->bounds.height > button->bounds.width ? button->bounds.width : button->bounds.height) / 2.0f;
    ...
    if (bg_color.a > 0.0f) {
        if (button->style.shape == CUPERTINO_BUTTON_SHAPE_ROUNDED_RECTANGLE) {
            ...
            if (rc == CMP_OK) {
                ...
                if (rc == CMP_OK) {
                    cupertino_shape_append_squircle(&path, button->bounds, corner_radius, corner_radius, corner_radius, corner_radius);
            ...
            ctx->gfx->vtable->draw_rect(ctx->gfx->ctx, &button->bounds, bg_color, corner_radius);
        }
    }
    ...
    if (button->label_utf8 && button->label_len > 0) {
        ...
        if (cmp_text_font_create(&button->text_backend, &applied_style, &font) == CMP_OK) {
            cmp_text_measure_utf8(&button->text_backend, font, button->label_utf8, button->label_len, 0, &metrics);

            /* Center text */
            text_x = button->bounds.x + (button->bounds.width - metrics.width) / 2.0f;
            text_y = button->bounds.y + (button->bounds.height - metrics.height) / 2.0f + metrics.baseline;

            cmp_text_draw_utf8_gfx(ctx->gfx, font, button->label_utf8, button->label_len, 0, text_x, text_y, text_color);
