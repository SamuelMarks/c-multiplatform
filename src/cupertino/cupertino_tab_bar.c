...
CMP_API int CMP_CALL cupertino_tab_bar_style_init(CupertinoTabBarStyle *style) {
    ...
    cupertino_color_get_system(CUPERTINO_COLOR_GRAY, CMP_FALSE, &style->unselected_color);

    cupertino_typography_scale_init(&typo);
    cupertino_typography_get_style(&typo, CUPERTINO_TYPOGRAPHY_CAPTION_2, &style->label_style);
    style->label_style.weight = 500;

    return CMP_OK;
}

CMP_API int CMP_CALL cupertino_tab_bar_init(CupertinoTabBar *bar, const CMPTextBackend *text_backend) {
...
CMP_API int CMP_CALL cupertino_tab_bar_set_items(CupertinoTabBar *bar, const CupertinoTabBarItem *items, cmp_u32 count) {
    cmp_u32 i;
...
CMP_API int CMP_CALL cupertino_tab_bar_layout(CupertinoTabBar *bar, CMPRect bounds, CMPRect safe_area) {
...
CMP_API int CMP_CALL cupertino_tab_bar_paint(const CupertinoTabBar *bar, CMPPaintContext *ctx) {
    ...
    if (rc == CMP_OK && ctx->gfx->vtable->draw_rect) {
        ...
        if (ctx->gfx->vtable->draw_line) {
            ctx->gfx->vtable->draw_line(ctx->gfx->ctx, bar->bounds.x, bar->bounds.y, bar->bounds.x + bar->bounds.width, bar->bounds.y, border_color, 1.0f);
        }
    }
    ...
    for (i = 0; i < bar->item_count; i++) {
        CMPColor item_color = (i == bar->selected_index) ? bar->style.tint_color : bar->style.unselected_color;
        
        /* Draw icon placeholder (a simple rounded rect since we lack real SF Symbols here) */
        if (ctx->gfx->vtable->draw_rect) {
            ...
            if (bar->items[i].has_badge) {
                ...
                ctx->gfx->vtable->draw_rect(ctx->gfx->ctx, &badge_rect, bar->items[i].badge_color, 5.0f);
            }
        }
        ...
        if (bar->items[i].title_utf8 && bar->items[i].title_len > 0) {
            ...
            if (cmp_text_font_create(&bar->text_backend, &applied_style, &font) == CMP_OK) {
                cmp_text_measure_utf8(&bar->text_backend, font, bar->items[i].title_utf8, bar->items[i].title_len, 0, &metrics);
                
                text_x = current_x + (item_width - metrics.width) / 2.0f;
                text_y = bar->bounds.y + 34.0f + metrics.baseline;
                
                cmp_text_draw_utf8_gfx(ctx->gfx, font, bar->items[i].title_utf8, bar->items[i].title_len, 0, text_x, text_y, item_color);
