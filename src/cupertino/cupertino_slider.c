...
CMP_API int CMP_CALL cupertino_slider_init(CupertinoSlider *slider) {
    ...
    slider->bounds.height = 30; /* Height is the touch area; visual track is thinner */
    
    slider->min_value = 0.0f;
    slider->max_value = 1.0f;
    slider->value = 0.5f;

    cupertino_color_get_system(CUPERTINO_COLOR_BLUE, CMP_FALSE, &slider->min_track_color);
    cupertino_color_get_system(CUPERTINO_COLOR_GRAY5, CMP_FALSE, &slider->max_track_color);
...
CMP_API int CMP_CALL cupertino_slider_paint(const CupertinoSlider *slider, CMPPaintContext *ctx) {
    ...
    if (slider == NULL || ctx == NULL || ctx->gfx == NULL || ctx->gfx->vtable == NULL) {
        return CMP_ERR_INVALID_ARGUMENT;
    }
    ...
    if (slider->max_value > slider->min_value) {
        progress = (slider->value - slider->min_value) / (slider->max_value - slider->min_value);
    } else {
        progress = 0.0f;
    }
    ...
    track_rect.y = slider->bounds.y + (slider->bounds.height / 2.0f) - (track_height / 2.0f);
    ...
    thumb_x = track_rect.x + thumb_radius + progress * (track_rect.width - thumb_radius * 2.0f);
    ...
    thumb_rect.y = slider->bounds.y + (slider->bounds.height / 2.0f) - thumb_radius;
    ...
    if (slider->is_dark_mode) {
        cupertino_color_get_system(CUPERTINO_COLOR_GRAY4, CMP_TRUE, &current_max_track_color);
    }

    /* Draw Min Track */
    if (ctx->gfx->vtable->draw_rect) {
        ctx->gfx->vtable->draw_rect(ctx->gfx->ctx, &min_track_rect, slider->min_track_color, track_radius);
    } else if (ctx->gfx->vtable->draw_rect) {
        ctx->gfx->vtable->draw_rect(ctx->gfx->ctx, &min_track_rect, slider->min_track_color, 0.0f);
    }

    /* Draw Max Track */
    if (ctx->gfx->vtable->draw_rect) {
        ctx->gfx->vtable->draw_rect(ctx->gfx->ctx, &max_track_rect, current_max_track_color, track_radius);
    } else if (ctx->gfx->vtable->draw_rect) {
        ctx->gfx->vtable->draw_rect(ctx->gfx->ctx, &max_track_rect, current_max_track_color, 0.0f);
    }
    ...
    if (ctx->gfx->vtable->draw_rect) {
        ...
        /* Shadow */
        ctx->gfx->vtable->draw_rect(ctx->gfx->ctx, &shadow_rect, thumb_shadow_color, thumb_radius);
        /* Knob */
        ctx->gfx->vtable->draw_rect(ctx->gfx->ctx, &thumb_rect, slider->thumb_color, thumb_radius);
        
        /* Thin border for light mode thumb */
        if (!slider->is_dark_mode) {
            CMPColor border_color = {0.8f, 0.8f, 0.8f, 1.0f};
            if (ctx->gfx->vtable->draw_rect) {
                ctx->gfx->vtable->draw_rect(ctx->gfx->ctx, &thumb_rect, border_color, thumb_radius);
            }
        }
    } else if (ctx->gfx->vtable->draw_rect) {
        ctx->gfx->vtable->draw_rect(ctx->gfx->ctx, &thumb_rect, slider->thumb_color, 0.0f);
    }
