...
CMP_API int cmp_f2_menu_create(cmp_ui_node_t **out_node) {
  ...
  if (res == CMP_SUCCESS) {
    ...
    LOG_DEBUG("cmp_f2_menu_create: cmp_ui_box_create failed\n");
  }
  return res;
}

/**
 * @brief cmp_f2_menu_item_create
 *
 * @param out_node Parameter description.
...
CMP_API int cmp_f2_menu_item_create(cmp_ui_node_t **out_node,
  ...
  if (!out_node) {
    LOG_DEBUG("cmp_f2_menu_item_create: out_node is NULL\n");
    return CMP_ERROR_INVALID_ARG;
  }

  res = cmp_ui_box_create(out_node);
  if (res != CMP_SUCCESS) {
    LOG_DEBUG("cmp_f2_menu_item_create: cmp_ui_box_create failed\n");
    return res;
  }
  ...
  if (label) {
    ...
    if (res == CMP_SUCCESS) {
      ...
      if (res != CMP_SUCCESS)
        LOG_DEBUG("cmp_f2_menu_item_create: cmp_ui_node_add_child failed\n");
    } else {
      LOG_DEBUG("cmp_f2_menu_item_create: cmp_ui_text_create failed\n");
    }
  }
...
/**
 * @brief cmp_f2_menu_item_set_shortcut
 *
 * @param node Parameter description.
 * @param shortcut Parameter description.
 * @return Returns 0 on success, or an error code on failure.
 */
CMP_API int cmp_f2_menu_item_set_shortcut(cmp_ui_node_t *node,
  ...
  if (!node || !node->properties) {
    LOG_DEBUG("cmp_f2_menu_item_set_shortcut: Invalid arg\n");
    return CMP_ERROR_INVALID_ARG;
  }
  ...
  if (!data->shortcut_text_node && shortcut) {
    ...
    if (res == CMP_SUCCESS) {
      res = cmp_ui_node_add_child(node, data->shortcut_text_node);
      if (res != CMP_SUCCESS)
        LOG_DEBUG(
            "cmp_f2_menu_item_set_shortcut: cmp_ui_node_add_child failed\n");
    } else {
      LOG_DEBUG("cmp_f2_menu_item_set_shortcut: cmp_ui_text_create failed\n");
    }
  }
...
/**
 * @brief cmp_f2_menu_item_set_disabled
 *
 * @param node Parameter description.
 * @param is_disabled Parameter description.
 * @return Returns 0 on success, or an error code on failure.
 */
CMP_API int cmp_f2_menu_item_set_disabled(cmp_ui_node_t *node,
                                          int is_disabled) {
  cmp_f2_menu_item_t *data;
  if (!node || !node->properties) {
...
CMP_API int cmp_f2_nav_view_create(cmp_ui_node_t **out_node) {
  ...
  if (!out_node) {
    LOG_DEBUG("cmp_f2_nav_view_create: out_node is NULL\n");
    return CMP_ERROR_INVALID_ARG;
  }

  res = cmp_ui_box_create(out_node);
  if (res != CMP_SUCCESS) {
    LOG_DEBUG("cmp_f2_nav_view_create: cmp_ui_box_create failed\n");
    return res;
  }
...
/**
 * @brief cmp_f2_nav_view_set_state
 *
 * @param node Parameter description.
...
/**
 * @brief cmp_f2_nav_item_create
 *
 * @param out_node Parameter description.
 * @param label Parameter description.
 * @return Returns 0 on success, or an error code on failure.
 */
CMP_API int cmp_f2_nav_item_create(cmp_ui_node_t **out_node,
                                   const char *label) {
  cmp_f2_nav_item_t *data;
  int res;

  if (!out_node) {
    LOG_DEBUG("cmp_f2_nav_item_create: out_node is NULL\n");
    return CMP_ERROR_INVALID_ARG;
  }

  res = cmp_ui_box_create(out_node);
  if (res != CMP_SUCCESS) {
    LOG_DEBUG("cmp_f2_nav_item_create: cmp_ui_box_create failed\n");
    return res;
  }
  ...
  if (label) {
    ...
    if (res == CMP_SUCCESS) {
      ...
      if (res != CMP_SUCCESS)
        LOG_DEBUG("cmp_f2_nav_item_create: cmp_ui_node_add_child failed\n");
    } else {
      LOG_DEBUG("cmp_f2_nav_item_create: cmp_ui_text_create failed\n");
    }
  }
...
/**
 * @brief cmp_f2_nav_item_set_active
 *
 * @param node Parameter description.
...
/**
 * @brief cmp_f2_tab_container_create
 *
 * @param out_node Parameter description.
...
CMP_API int cmp_f2_tab_container_create(cmp_ui_node_t **out_node,
  ...
  if (!out_node) {
    LOG_DEBUG("cmp_f2_tab_container_create: out_node is NULL\n");
    return CMP_ERROR_INVALID_ARG;
  }

  res = cmp_ui_box_create(out_node);
  if (res != CMP_SUCCESS) {
    LOG_DEBUG("cmp_f2_tab_container_create: cmp_ui_box_create failed\n");
    return res;
  }
...
/**
 * @brief cmp_f2_tab_container_set_variant
 *
 * @param node Parameter description.
...
/**
 * @brief cmp_f2_tab_container_set_size
 *
 * @param node Parameter description.
...
/**
 * @brief cmp_f2_tab_item_create
 *
 * @param out_node Parameter description.
 * @param label Parameter description.
 * @return Returns 0 on success, or an error code on failure.
 */
CMP_API int cmp_f2_tab_item_create(cmp_ui_node_t **out_node,
                                   const char *label) {
  ...
  if (!out_node) {
    LOG_DEBUG("cmp_f2_tab_item_create: out_node is NULL\n");
    return CMP_ERROR_INVALID_ARG;
  }

  res = cmp_ui_box_create(out_node);
  if (res != CMP_SUCCESS) {
    LOG_DEBUG("cmp_f2_tab_item_create: cmp_ui_box_create failed\n");
    return res;
  }
  ...
  if (label) {
    ...
    if (res == CMP_SUCCESS) {
      res = cmp_ui_node_add_child(*out_node, label_node);
      if (res != CMP_SUCCESS)
        LOG_DEBUG("cmp_f2_tab_item_create: cmp_ui_node_add_child failed\n");
    } else {
      LOG_DEBUG("cmp_f2_tab_item_create: cmp_ui_text_create failed\n");
    }
  }
...
/**
 * @brief cmp_f2_breadcrumb_create
 *
 * @param out_node Parameter description.
 * @return Returns 0 on success, or an error code on failure.
 */
CMP_API int cmp_f2_breadcrumb_create(cmp_ui_node_t **out_node) {
  ...
  if (res == CMP_SUCCESS) {
    ...
    LOG_DEBUG("cmp_f2_breadcrumb_create: cmp_ui_box_create failed\n");
  }
  return res;
}

/**
 * @brief cmp_f2_breadcrumb_item_create
 *
 * @param out_node Parameter description.
 * @param label Parameter description.
 * @param is_last Parameter description.
 * @return Returns 0 on success, or an error code on failure.
 */
CMP_API int cmp_f2_breadcrumb_item_create(cmp_ui_node_t **out_node,
  ...
  if (!out_node) {
    LOG_DEBUG("cmp_f2_breadcrumb_item_create: out_node is NULL\n");
    return CMP_ERROR_INVALID_ARG;
  }

  res = cmp_ui_box_create(out_node);
  if (res != CMP_SUCCESS) {
    LOG_DEBUG("cmp_f2_breadcrumb_item_create: cmp_ui_box_create failed\n");
    return res;
  }
  ...
  if (label) {
    ...
    if (res == CMP_SUCCESS) {
      res = cmp_ui_node_add_child(*out_node, text_node);
      if (res != CMP_SUCCESS)
        LOG_DEBUG(
            "cmp_f2_breadcrumb_item_create: cmp_ui_node_add_child failed\n");
    } else {
      LOG_DEBUG("cmp_f2_breadcrumb_item_create: cmp_ui_text_create failed\n");
    }
  }
  ...
  if (!is_last) {
    ...
    if (res == CMP_SUCCESS) {
      res = cmp_ui_node_add_child(*out_node, chevron_node);
      if (res != CMP_SUCCESS)
        LOG_DEBUG("cmp_f2_breadcrumb_item_create: cmp_ui_node_add_child "
                  "(chevron) failed\n");
    } else {
      LOG_DEBUG("cmp_f2_breadcrumb_item_create: cmp_ui_text_create (chevron) "
                "failed\n");
    }
  }
...
CMP_API int cmp_f2_toolbar_create(cmp_ui_node_t **out_node) {
  ...
  if (!out_node) {
    LOG_DEBUG("cmp_f2_toolbar_create: out_node is NULL\n");
    return CMP_ERROR_INVALID_ARG;
  }

  res = cmp_ui_box_create(out_node);
  if (res != CMP_SUCCESS) {
    LOG_DEBUG("cmp_f2_toolbar_create: cmp_ui_box_create failed\n");
    return res;
  }
...
CMP_API int cmp_f2_link_create(cmp_ui_node_t **out_node, const char *label,
  ...
  if (!out_node) {
    LOG_DEBUG("cmp_f2_link_create: out_node is NULL\n");
    return CMP_ERROR_INVALID_ARG;
  }

  res = cmp_ui_box_create(out_node);
  if (res != CMP_SUCCESS) {
    LOG_DEBUG("cmp_f2_link_create: cmp_ui_box_create failed\n");
    return res;
  }
  ...
  if (label) {
    ...
    if (res == CMP_SUCCESS) {
      res = cmp_ui_node_add_child(*out_node, data->text_node);
      if (res != CMP_SUCCESS)
        LOG_DEBUG("cmp_f2_link_create: cmp_ui_node_add_child failed\n");
    } else {
      LOG_DEBUG("cmp_f2_link_create: cmp_ui_text_create failed\n");
    }
  }
...
/**
 * @brief cmp_f2_link_set_disabled
 *
 * @param node Parameter description.
 * @param is_disabled Parameter description.
 * @return Returns 0 on success, or an error code on failure.
...
CMP_API int cmp_f2_stepper_create(cmp_ui_node_t **out_node) {
  ...
  if (res == CMP_SUCCESS) {
    ...
    LOG_DEBUG("cmp_f2_stepper_create: cmp_ui_box_create failed\n");
  }
  return res;
}

/**
 * @brief cmp_f2_stepper_item_create
 *
 * @param out_node Parameter description.
 * @param label Parameter description.
 * @return Returns 0 on success, or an error code on failure.
...
CMP_API int cmp_f2_stepper_item_create(cmp_ui_node_t **out_node,
  ...
  if (!out_node) {
    LOG_DEBUG("cmp_f2_stepper_item_create: out_node is NULL\n");
    return CMP_ERROR_INVALID_ARG;
  }

  res = cmp_ui_box_create(out_node);
  if (res != CMP_SUCCESS) {
    LOG_DEBUG("cmp_f2_stepper_item_create: cmp_ui_box_create failed\n");
    return res;
  }
  ...
  if (label) {
    ...
    if (res == CMP_SUCCESS) {
      res = cmp_ui_node_add_child(*out_node, data->label_node);
      if (res != CMP_SUCCESS)
        LOG_DEBUG("cmp_f2_stepper_item_create: cmp_ui_node_add_child failed\n");
    } else {
      LOG_DEBUG("cmp_f2_stepper_item_create: cmp_ui_text_create failed\n");
    }
  }
...
/**
 * @brief cmp_f2_stepper_item_set_state
 *
 * @param node Parameter description.
 * @param state Parameter description.
 * @return Returns 0 on success, or an error code on failure.
