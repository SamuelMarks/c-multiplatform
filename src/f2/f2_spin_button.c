...
CMP_API int CMP_CALL f2_spin_button_init(F2SpinButton *spin_button,
    ...
    res = f2_text_field_init(&spin_button->text_field, backend, &style->text_field_style, allocator);
    if (res != CMP_OK) return res;

    res = f2_button_init(&spin_button->up_button, backend, &style->button_style, "^", 1);
    if (res != CMP_OK) return res;

    res = f2_button_init(&spin_button->down_button, backend, &style->button_style, "v", 1);
    if (res != CMP_OK) return res;
...
CMP_API int CMP_CALL f2_spin_button_set_value(F2SpinButton *spin_button, CMPScalar value) {
    if (!spin_button) return CMP_ERR_INVALID_ARGUMENT;
    if (value < spin_button->min_value) value = spin_button->min_value;
    if (value > spin_button->max_value) value = spin_button->max_value;
