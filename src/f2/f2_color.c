...
static int interpolate_rgb(cmp_u8 r1, cmp_u8 g1, cmp_u8 b1,
                           cmp_u8 r2, cmp_u8 g2, cmp_u8 b2,
                           CMPScalar t,
                           cmp_u8 *out_r, cmp_u8 *out_g, cmp_u8 *out_b) {
...
CMP_API int CMP_CALL f2_color_ramp_generate(cmp_u32 base_argb, F2ColorRamp *out_ramp) {
    ...
    interpolate_rgb(white_r, white_g, white_b, base_r, base_g, base_b, 0.1f, &r, &g, &b);
    out_ramp->shade10 = (base_a << 24) | (r << 16) | (g << 8) | b;

    /* Shade 20 */
    interpolate_rgb(white_r, white_g, white_b, base_r, base_g, base_b, 0.3f, &r, &g, &b);
    out_ramp->shade20 = (base_a << 24) | (r << 16) | (g << 8) | b;

    /* Shade 30 */
    interpolate_rgb(white_r, white_g, white_b, base_r, base_g, base_b, 0.5f, &r, &g, &b);
    out_ramp->shade30 = (base_a << 24) | (r << 16) | (g << 8) | b;

    /* Shade 40 */
    interpolate_rgb(white_r, white_g, white_b, base_r, base_g, base_b, 0.7f, &r, &g, &b);
    ...
    /* Shade 60 */
    interpolate_rgb(base_r, base_g, base_b, black_r, black_g, black_b, 0.2f, &r, &g, &b);
    out_ramp->shade60 = (base_a << 24) | (r << 16) | (g << 8) | b;

    /* Shade 70 */
    interpolate_rgb(base_r, base_g, base_b, black_r, black_g, black_b, 0.4f, &r, &g, &b);
    out_ramp->shade70 = (base_a << 24) | (r << 16) | (g << 8) | b;

    /* Shade 80 */
    interpolate_rgb(base_r, base_g, base_b, black_r, black_g, black_b, 0.6f, &r, &g, &b);
    out_ramp->shade80 = (base_a << 24) | (r << 16) | (g << 8) | b;

    /* Shade 90 */
    interpolate_rgb(base_r, base_g, base_b, black_r, black_g, black_b, 0.8f, &r, &g, &b);
    out_ramp->shade90 = (base_a << 24) | (r << 16) | (g << 8) | b;

    /* Shade 100 (darkest, near black) */
    interpolate_rgb(base_r, base_g, base_b, black_r, black_g, black_b, 0.95f, &r, &g, &b);
    out_ramp->shade100 = (base_a << 24) | (r << 16) | (g << 8) | b;

    return CMP_OK;
}

CMP_API int CMP_CALL f2_theme_generate(cmp_u32 brand_argb, CMPBool is_dark, F2Theme *out_theme) {
    int res;
...
CMP_API int CMP_CALL f2_theme_apply_high_contrast(F2Theme *theme, CMPBool is_dark_hcm) {
    if (!theme) return CMP_ERR_INVALID_ARGUMENT;
