...
CMP_API int CMP_CALL f2_toggle_button_init(F2ToggleButton *toggle_button,
  ...
  res = f2_button_init(&toggle_button->core, backend, style, utf8_label,
                       utf8_len);
  if (res != CMP_OK) {
    return res;
  }
...
CMP_API int CMP_CALL f2_split_button_init(F2SplitButton *split_button,
  ...
  res = f2_button_init(&split_button->primary_action, backend, style,
                       utf8_label, utf8_len);
  if (res != CMP_OK)
    return res;
...
CMP_API int CMP_CALL f2_split_button_set_dropdown_open(
    F2SplitButton *split_button, CMPBool is_open) {
  if (!split_button)
    return CMP_ERR_INVALID_ARGUMENT;
