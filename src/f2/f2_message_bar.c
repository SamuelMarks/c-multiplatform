...
static int f2_color_from_u32(cmp_u32 argb, CMPColor *out_color) {
    if (!out_color) return CMP_ERR_INVALID_ARGUMENT;
...
CMP_API int CMP_CALL f2_message_bar_style_init(F2MessageBarStyle *style, F2Intent intent, const F2Theme *theme) {
    if (!style || !theme) return CMP_ERR_INVALID_ARGUMENT;
    ...
    switch (intent) {
        case F2_INTENT_INFO:
            f2_color_from_u32(theme->semantics.info_background, &style->background_color);
            f2_color_from_u32(theme->semantics.info_text, &style->text_color);
            break;
        case F2_INTENT_SUCCESS:
            f2_color_from_u32(theme->semantics.success_background, &style->background_color);
            f2_color_from_u32(theme->semantics.success_text, &style->text_color);
            break;
        case F2_INTENT_WARNING:
            f2_color_from_u32(theme->semantics.warning_background, &style->background_color);
            f2_color_from_u32(theme->semantics.warning_text, &style->text_color);
            break;
        case F2_INTENT_ERROR:
            f2_color_from_u32(theme->semantics.danger_background, &style->background_color);
...
CMP_API int CMP_CALL f2_message_bar_init(F2MessageBar *bar,
                                         ...
                                         cmp_usize text_len) {
    if (!bar || !style) return CMP_ERR_INVALID_ARGUMENT;
...
CMP_API int CMP_CALL f2_message_bar_show(F2MessageBar *bar) {
    if (!bar) return CMP_ERR_INVALID_ARGUMENT;
