...
CMP_API int CMP_CALL f2_avatar_init_initials(F2Avatar *avatar,
    ...
    if (!avatar || !style) return CMP_ERR_INVALID_ARGUMENT;
...
CMP_API int CMP_CALL f2_avatar_set_presence(F2Avatar *avatar, F2PresenceState state, CMPBool show) {
    int res;
    if (!avatar) return CMP_ERR_INVALID_ARGUMENT;
    
    avatar->has_badge = show;
    if (show) {
        res = f2_presence_badge_init(&avatar->badge, &avatar->style.badge_style, state);
