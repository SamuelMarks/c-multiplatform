...
CMP_API int CMP_CALL f2_search_box_init(F2SearchBox *search_box,
    ...
    res = f2_text_field_init(&search_box->text_field, backend, &style->text_field_style, allocator);
    if (res != CMP_OK) return res;

    res = f2_button_init(&search_box->clear_button, backend, &style->clear_button_style, "X", 1);
    if (res != CMP_OK) return res;
...
CMP_API int CMP_CALL f2_search_box_set_text(F2SearchBox *search_box, const char *utf8_text) {
    if (!search_box) return CMP_ERR_INVALID_ARGUMENT;
