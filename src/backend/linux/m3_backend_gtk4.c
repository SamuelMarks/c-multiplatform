...
static int m3_gtk4_backend_log(struct M3GTK4Backend *backend, M3LogLevel level, const char *message)
{
...
static int m3_gtk4_queue_event(struct M3GTK4Backend *backend, const M3InputEvent *event)
{
    m3_usize tail;
    int rc;
...
static int m3_gtk4_transform_point_simple(const M3Mat3 *transform, M3Scalar x, M3Scalar y, M3Scalar *out_x, M3Scalar *out_y)
{
...
static int m3_gtk4_transform_rect_simple(const M3Mat3 *transform, const M3Rect *rect, M3Rect *out_rect)
{
    M3Scalar x0;
    M3Scalar y0;
    M3Scalar x1;
    ...
    M3_GTK4_RETURN_IF_ERROR(rc);
    rc = m3_gtk4_transform_point_simple(transform, rect->x + rect->width, rect->y + rect->height, &x1, &y1);
    M3_GTK4_RETURN_IF_ERROR(rc);
    ...
    for (i = 0; i < window->cmd_count; ++i) {
        if (window->cmds[i].type == M3_GTK4_CMD_TEXT) {
            if (window->cmds[i].data.text.utf8 != NULL) {
                rc = backend->allocator.free(backend->allocator.ctx, window->cmds[i].data.text.utf8);
...
static int m3_gtk4_cmd_list_push(M3GTK4Window *window, const M3GTK4Cmd *cmd)
{
    ...
    if (window->cmd_count >= capacity) {
        new_capacity = (capacity == 0) ? M3_GTK4_CMD_CAPACITY_INITIAL : capacity * 2u;
        ...
        if (window->cmds == NULL) {
            rc = backend->allocator.alloc(backend->allocator.ctx, new_capacity * sizeof(M3GTK4Cmd), &mem);
        } else {
            rc = backend->allocator.realloc(backend->allocator.ctx, window->cmds, new_capacity * sizeof(M3GTK4Cmd), &mem);
        }
        M3_GTK4_RETURN_IF_ERROR(rc);
...
static int m3_gtk4_ensure_surface(M3GTK4Window *window, m3_i32 width, m3_i32 height)
{
    cairo_surface_t *surface;
    ...
    if (window->surface != NULL && window->surface_width == width && window->surface_height == height) {
        return M3_OK;
    }
...
static void m3_gtk4_queue_window_resize(M3GTK4Window *window, m3_i32 width, m3_i32 height)
{
    struct M3GTK4Backend *backend;
    M3InputEvent event;
    int rc;
...
static void m3_gtk4_clipboard_read_cb(GObject *source, GAsyncResult *result, gpointer user_data)
{
    ...
    state->text = gdk_clipboard_read_text_finish(GDK_CLIPBOARD(source), result, NULL);
    state->done = TRUE;
}

static void m3_gtk4_on_snapshot(GtkWidget *widget, GtkSnapshot *snapshot, gpointer user_data)
{
    M3GTK4Window *window;
    cairo_t *cr;
...
static void m3_gtk4_on_size_allocate(GtkWidget *widget, int width, int height, int baseline, gpointer user_data)
{
    M3GTK4Window *window;
...
static gboolean m3_gtk4_on_close_request(GtkWindow *gtk_window, gpointer user_data)
{
    M3GTK4Window *window;
    struct M3GTK4Backend *backend;
    M3InputEvent event;
...
static void m3_gtk4_on_focus_enter(GtkEventControllerFocus *controller, gpointer user_data)
{
    M3GTK4Window *window;
    struct M3GTK4Backend *backend;
    M3InputEvent event;
...
static void m3_gtk4_on_focus_leave(GtkEventControllerFocus *controller, gpointer user_data)
{
    M3GTK4Window *window;
    struct M3GTK4Backend *backend;
    M3InputEvent event;
...
static void m3_gtk4_on_motion(GtkEventControllerMotion *controller, double x, double y, gpointer user_data)
{
    M3GTK4Window *window;
    struct M3GTK4Backend *backend;
    M3InputEvent event;
...
static void m3_gtk4_on_click_pressed(GtkGestureClick *gesture, int n_press, double x, double y, gpointer user_data)
{
    M3GTK4Window *window;
    struct M3GTK4Backend *backend;
    M3InputEvent event;
...
static void m3_gtk4_on_click_released(GtkGestureClick *gesture, int n_press, double x, double y, gpointer user_data)
{
    M3GTK4Window *window;
    struct M3GTK4Backend *backend;
    M3InputEvent event;
...
static gboolean m3_gtk4_on_scroll(GtkEventControllerScroll *controller, double dx, double dy, gpointer user_data)
{
    M3GTK4Window *window;
    struct M3GTK4Backend *backend;
    M3InputEvent event;
...
static gboolean m3_gtk4_on_key_pressed(GtkEventControllerKey *controller, guint keyval, guint keycode, GdkModifierType state,
    gpointer user_data)
{
    M3GTK4Window *window;
    struct M3GTK4Backend *backend;
...
static gboolean m3_gtk4_on_key_released(GtkEventControllerKey *controller, guint keyval, guint keycode, GdkModifierType state,
    gpointer user_data)
{
    M3GTK4Window *window;
    struct M3GTK4Backend *backend;
...
static int m3_gtk4_backend_resolve(struct M3GTK4Backend *backend, M3Handle handle, m3_u32 type_id, void **out_obj)
{
    void *resolved;
    m3_u32 actual_type;
    int rc;
...
static const M3ObjectVTable g_m3_gtk4_window_vtable = {
    m3_gtk4_object_retain,
    m3_gtk4_object_release,
    m3_gtk4_window_destroy,
...
static int m3_gtk4_ws_create_window(void *ws, const M3WSWindowConfig *config, M3Handle *out_window)
{
    struct M3GTK4Backend *backend;
    M3GTK4Window *window;
    ...
    rc = backend->allocator.alloc(backend->allocator.ctx, sizeof(M3GTK4Window), (void **)&window);
    M3_GTK4_RETURN_IF_ERROR(rc);
    ...
    gtk_window_set_resizable(gtk_window, (config->flags & M3_WS_WINDOW_RESIZABLE) != 0 ? TRUE : FALSE);
    ...
    rc = m3_object_header_init(&window->header, M3_GTK4_TYPE_WINDOW, 0, &g_m3_gtk4_window_vtable);
    if (rc != M3_OK) {
        if (window->surface != NULL) {
            cairo_surface_destroy(window->surface);
            window->surface = NULL;
    ...
    g_signal_connect(area, "size-allocate", G_CALLBACK(m3_gtk4_on_size_allocate), window);
    g_signal_connect(gtk_window, "close-request", G_CALLBACK(m3_gtk4_on_close_request), window);
    ...
    g_signal_connect(click, "pressed", G_CALLBACK(m3_gtk4_on_click_pressed), window);
    g_signal_connect(click, "released", G_CALLBACK(m3_gtk4_on_click_released), window);
    ...
    g_signal_connect(key, "key-pressed", G_CALLBACK(m3_gtk4_on_key_pressed), window);
    g_signal_connect(key, "key-released", G_CALLBACK(m3_gtk4_on_key_released), window);
...
static int m3_gtk4_ws_destroy_window(void *ws, M3Handle window)
{
    ...
    backend = (struct M3GTK4Backend *)ws;
    rc = m3_gtk4_backend_resolve(backend, window, M3_GTK4_TYPE_WINDOW, (void **)&resolved);
    M3_GTK4_RETURN_IF_ERROR(rc);
...
static int m3_gtk4_ws_set_window_title(void *ws, M3Handle window, const char *utf8_title)
{
    struct M3GTK4Backend *backend;
    M3GTK4Window *resolved;
    int rc;
...
static int m3_gtk4_ws_set_window_size(void *ws, M3Handle window, m3_i32 width, m3_i32 height)
{
    struct M3GTK4Backend *backend;
    M3GTK4Window *resolved;
    int rc;
...
static int m3_gtk4_ws_get_window_size(void *ws, M3Handle window, m3_i32 *out_width, m3_i32 *out_height)
{
    struct M3GTK4Backend *backend;
    M3GTK4Window *resolved;
    int width;
...
static int m3_gtk4_ws_set_window_dpi_scale(void *ws, M3Handle window, M3Scalar scale)
{
    struct M3GTK4Backend *backend;
    M3GTK4Window *resolved;
    int rc;
    ...
    rc = m3_gtk4_backend_log(backend, M3_LOG_LEVEL_INFO, "ws.set_window_dpi_scale");
    M3_GTK4_RETURN_IF_ERROR(rc);
...
static int m3_gtk4_ws_get_window_dpi_scale(void *ws, M3Handle window, M3Scalar *out_scale)
{
    struct M3GTK4Backend *backend;
    M3GTK4Window *resolved;
    int scale;
    ...
    rc = m3_gtk4_backend_log(backend, M3_LOG_LEVEL_INFO, "ws.get_window_dpi_scale");
    M3_GTK4_RETURN_IF_ERROR(rc);
...
static int m3_gtk4_ws_get_clipboard_text(void *ws, char *buffer, m3_usize buffer_size, m3_usize *out_length)
{
    struct M3GTK4Backend *backend;
    GdkDisplay *display;
    GdkClipboard *clipboard;
    ...
    state.done = FALSE;
    state.text = NULL;
    gdk_clipboard_read_text_async(clipboard, NULL, m3_gtk4_clipboard_read_cb, &state);
...
static int m3_gtk4_ws_poll_event(void *ws, M3InputEvent *out_event, M3Bool *out_has_event)
{
    struct M3GTK4Backend *backend;
    int rc;
...
static const M3WSVTable g_m3_gtk4_ws_vtable = {
    m3_gtk4_ws_init,
    m3_gtk4_ws_shutdown,
...
{
    cairo_set_source_rgba(cr, (double)color.r, (double)color.g, (double)color.b, (double)color.a);
}

static void m3_gtk4_path_rounded_rect(cairo_t *cr, const M3Rect *rect, M3Scalar radius)
{
    double x;
    double y;
    double w;
...
static void m3_gtk4_snapshot_draw_rect(GtkSnapshot *snapshot, const M3Rect *rect, M3Color color, M3Scalar radius)
{
...
static void m3_gtk4_snapshot_draw_line(GtkSnapshot *snapshot, const M3GTK4CmdLine *line)
{
    graphene_rect_t bounds;
    cairo_t *cr;
    M3Scalar min_x;
    ...
    cairo_move_to(cr, (double)(line->x0 - bounds.origin.x), (double)(line->y0 - bounds.origin.y));
    cairo_line_to(cr, (double)(line->x1 - bounds.origin.x), (double)(line->y1 - bounds.origin.y));
    cairo_stroke(cr);
    cairo_destroy(cr);
}

static void m3_gtk4_snapshot_draw_texture(GtkSnapshot *snapshot, M3GTK4Window *window, const M3GTK4CmdTexture *cmd)
{
    struct M3GTK4Backend *backend;
    ...
    rc = m3_gtk4_backend_resolve(backend, cmd->texture, M3_GTK4_TYPE_TEXTURE, (void **)&resolved);
    ...
    if (src_width <= 0.0f || src_height <= 0.0f || dst_width <= 0.0f || dst_height <= 0.0f) {
        return;
    }
    ...
    cairo_scale(cr, (double)dst_width / (double)src_width, (double)dst_height / (double)src_height);
    cairo_set_source_surface(cr, resolved->surface, (double)-cmd->src.x, (double)-cmd->src.y);
    cairo_paint_with_alpha(cr, (double)cmd->opacity);
    cairo_restore(cr);
    cairo_destroy(cr);
}

static void m3_gtk4_snapshot_draw_text(GtkSnapshot *snapshot, M3GTK4Window *window, const M3GTK4CmdText *cmd)
{
    struct M3GTK4Backend *backend;
    M3GTK4Font *resolved;
    graphene_rect_t bounds;
    ...
    rc = m3_gtk4_backend_resolve(backend, cmd->font, M3_GTK4_TYPE_FONT, (void **)&resolved);
    ...
    pango_layout_set_text(layout, (cmd->utf8 != NULL) ? cmd->utf8 : "", (int)cmd->len);
...
static void m3_gtk4_snapshot_cmds(M3GTK4Window *window, GtkSnapshot *snapshot)
{
    ...
    for (i = 0; i < window->cmd_count; ++i) {
        ...
        case M3_GTK4_CMD_CLEAR:
            m3_gtk4_snapshot_draw_rect(snapshot, &cmd->data.clear.rect, cmd->data.clear.color, 0.0f);
            break;
        case M3_GTK4_CMD_RECT:
            m3_gtk4_snapshot_draw_rect(snapshot, &cmd->data.rect.rect, cmd->data.rect.color, cmd->data.rect.radius);
            break;
...
static void m3_gtk4_write_argb32_pixel(unsigned char *dst, unsigned char r, unsigned char g, unsigned char b, unsigned char a)
{
    unsigned int alpha;
    unsigned char pr;
    unsigned char pg;
...
static int m3_gtk4_copy_texture_region(M3GTK4Texture *texture, const void *pixels, m3_usize size,
    m3_i32 x, m3_i32 y, m3_i32 width, m3_i32 height)
{
    ...
    if (texture->format == M3_TEX_FORMAT_A8) {
        ...
        for (row = 0; row < height; ++row) {
            dst = texture->pixels + (m3_usize)(y + row) * (m3_usize)texture->stride + (m3_usize)x;
    ...
    for (row = 0; row < height; ++row) {
        dst = texture->pixels + (m3_usize)(y + row) * (m3_usize)texture->stride + (m3_usize)x * 4u;
...
static int m3_gtk4_gfx_begin_frame(void *gfx, M3Handle window, m3_i32 width, m3_i32 height, M3Scalar dpi_scale)
{
    struct M3GTK4Backend *backend;
    M3GTK4Window *resolved;
    cairo_t *cr;
    ...
    if (resolved->use_gsk) {
        ...
        if (backend->has_transform) {
            ...
            backend->gsk_transform_simple = m3_gtk4_transform_is_simple(&backend->gsk_transform);
...
static int m3_gtk4_gfx_draw_rect(void *gfx, const M3Rect *rect, M3Color color, M3Scalar corner_radius)
{
    struct M3GTK4Backend *backend;
    M3GTK4Cmd cmd;
    M3Rect transformed;
    ...
    if (backend->frame_use_gsk) {
        ...
        rc = m3_gtk4_transform_rect_simple(&backend->gsk_transform, rect, &transformed);
        M3_GTK4_RETURN_IF_ERROR(rc);
...
static int m3_gtk4_gfx_draw_line(void *gfx, M3Scalar x0, M3Scalar y0, M3Scalar x1, M3Scalar y1, M3Color color, M3Scalar thickness)
{
    struct M3GTK4Backend *backend;
    M3GTK4Cmd cmd;
    M3Scalar tx0;
    ...
    if (backend->frame_use_gsk) {
        ...
        rc = m3_gtk4_transform_point_simple(&backend->gsk_transform, x0, y0, &tx0, &ty0);
        M3_GTK4_RETURN_IF_ERROR(rc);
        rc = m3_gtk4_transform_point_simple(&backend->gsk_transform, x1, y1, &tx1, &ty1);
        M3_GTK4_RETURN_IF_ERROR(rc);
...
static int m3_gtk4_gfx_push_clip(void *gfx, const M3Rect *rect)
{
    ...
    cairo_save(backend->frame_cr);
    cairo_rectangle(backend->frame_cr, rect->x, rect->y, rect->width, rect->height);
...
static int m3_gtk4_gfx_create_texture(void *gfx, m3_i32 width, m3_i32 height, m3_u32 format, const void *pixels, m3_usize size,
                                      M3Handle *out_texture)
{
    ...
    rc = backend->allocator.alloc(backend->allocator.ctx, sizeof(M3GTK4Texture), (void **)&texture);
    M3_GTK4_RETURN_IF_ERROR(rc);
    ...
    cairo_format = (format == M3_TEX_FORMAT_A8) ? CAIRO_FORMAT_A8 : CAIRO_FORMAT_ARGB32;
    ...
    rc = backend->allocator.alloc(backend->allocator.ctx, texture->size, (void **)&texture->pixels);
    M3_GTK4_RETURN_IF_ERROR_CLEANUP(rc, backend->allocator.free(backend->allocator.ctx, texture));
    memset(texture->pixels, 0, texture->size);

    surface = cairo_image_surface_create_for_data(texture->pixels, cairo_format, width, height, stride);
    ...
    if (pixels != NULL && size != 0) {
        rc = m3_gtk4_copy_texture_region(texture, pixels, size, 0, 0, width, height);
        if (rc != M3_OK) {
            cairo_surface_destroy(texture->surface);
            backend->allocator.free(backend->allocator.ctx, texture->pixels);
            backend->allocator.free(backend->allocator.ctx, texture);
    ...
    rc = m3_object_header_init(&texture->header, M3_GTK4_TYPE_TEXTURE, 0, &g_m3_gtk4_texture_vtable);
    if (rc != M3_OK) {
        cairo_surface_destroy(texture->surface);
        backend->allocator.free(backend->allocator.ctx, texture->pixels);
        backend->allocator.free(backend->allocator.ctx, texture);
...
static int m3_gtk4_gfx_update_texture(void *gfx, M3Handle texture, m3_i32 x, m3_i32 y, m3_i32 width, m3_i32 height, const void *pixels,
                                      m3_usize size)
{
    struct M3GTK4Backend *backend;
    M3GTK4Texture *resolved;
    ...
    backend = (struct M3GTK4Backend *)gfx;
    rc = m3_gtk4_backend_resolve(backend, texture, M3_GTK4_TYPE_TEXTURE, (void **)&resolved);
    M3_GTK4_RETURN_IF_ERROR(rc);
...
static int m3_gtk4_gfx_draw_texture(void *gfx, M3Handle texture, const M3Rect *src, const M3Rect *dst, M3Scalar opacity)
{
    struct M3GTK4Backend *backend;
    M3GTK4Texture *resolved;
    double scale_x;
    ...
    if (backend->frame_use_gsk) {
        ...
        rc = m3_gtk4_transform_rect_simple(&backend->gsk_transform, dst, &transformed);
        M3_GTK4_RETURN_IF_ERROR(rc);
    ...
    cairo_set_source_surface(backend->frame_cr, resolved->surface, -src->x, -src->y);
...
static const M3GfxVTable g_m3_gtk4_gfx_vtable = {
    m3_gtk4_gfx_begin_frame,
    m3_gtk4_gfx_end_frame,
    m3_gtk4_gfx_clear,
    m3_gtk4_gfx_draw_rect,
    m3_gtk4_gfx_draw_line,
    m3_gtk4_gfx_push_clip,
    m3_gtk4_gfx_pop_clip,
    m3_gtk4_gfx_set_transform,
    m3_gtk4_gfx_create_texture,
    m3_gtk4_gfx_update_texture,
    m3_gtk4_gfx_destroy_texture,
...
static int m3_gtk4_text_create_font(void *text, const char *utf8_family, m3_i32 size_px, m3_i32 weight, M3Bool italic, M3Handle *out_font)
{
    struct M3GTK4Backend *backend;
    M3GTK4Font *font;
    PangoFontDescription *desc;
    ...
    rc = backend->allocator.alloc(backend->allocator.ctx, sizeof(M3GTK4Font), (void **)&font);
    M3_GTK4_RETURN_IF_ERROR(rc);
    ...
    rc = m3_object_header_init(&font->header, M3_GTK4_TYPE_FONT, 0, &g_m3_gtk4_font_vtable);
    if (rc != M3_OK) {
...
static int m3_gtk4_text_destroy_font(void *text, M3Handle font)
{
    ...
    backend = (struct M3GTK4Backend *)text;
    rc = m3_gtk4_backend_resolve(backend, font, M3_GTK4_TYPE_FONT, (void **)&resolved);
    M3_GTK4_RETURN_IF_ERROR(rc);
...
static int m3_gtk4_text_measure_text(void *text, M3Handle font, const char *utf8, m3_usize utf8_len, M3Scalar *out_width, M3Scalar *out_height,
...
static int m3_gtk4_text_draw_text(void *text, M3Handle font, const char *utf8, m3_usize utf8_len, M3Scalar x, M3Scalar y, M3Color color)
{
    struct M3GTK4Backend *backend;
    M3GTK4Font *resolved;
    PangoLayout *layout;
    ...
    if (backend->frame_use_gsk) {
        ...
        rc = m3_gtk4_transform_point_simple(&backend->gsk_transform, x, y, &tx, &ty);
        M3_GTK4_RETURN_IF_ERROR(rc);
        ...
        if (utf8_len > 0) {
            rc = backend->allocator.alloc(backend->allocator.ctx, utf8_len + 1u, &mem);
            M3_GTK4_RETURN_IF_ERROR(rc);
...
static const M3TextVTable g_m3_gtk4_text_vtable = {
    m3_gtk4_text_create_font,
    m3_gtk4_text_destroy_font,
    m3_gtk4_text_measure_text,
    m3_gtk4_text_draw_text
};

static int m3_gtk4_io_read_file(void *io, const char *utf8_path, void *buffer, m3_usize buffer_size, m3_usize *out_read)
{
    struct M3GTK4Backend *backend;
    int rc;
...
static int m3_gtk4_io_read_file_alloc(void *io, const char *utf8_path, const M3Allocator *allocator, void **out_data, m3_usize *out_size)
{
    struct M3GTK4Backend *backend;
    int rc;
...
static int m3_gtk4_io_write_file(void *io, const char *utf8_path, const void *data, m3_usize size, M3Bool overwrite)
{
    struct M3GTK4Backend *backend;
    int rc;
...
static int m3_gtk4_io_file_exists(void *io, const char *utf8_path, M3Bool *out_exists)
{
    struct M3GTK4Backend *backend;
    int rc;
...
static int m3_gtk4_io_stat_file(void *io, const char *utf8_path, M3FileInfo *out_info)
{
    struct M3GTK4Backend *backend;
    int rc;
...
static const M3IOVTable g_m3_gtk4_io_vtable = {
    m3_gtk4_io_read_file,
    m3_gtk4_io_read_file_alloc,
    m3_gtk4_io_write_file,
    m3_gtk4_io_file_exists,
    m3_gtk4_io_delete_file,
    m3_gtk4_io_stat_file
};

static int m3_gtk4_sensors_is_available(void *sensors, m3_u32 type, M3Bool *out_available)
{
    struct M3GTK4Backend *backend;
    int rc;
...
static int m3_gtk4_sensors_read(void *sensors, m3_u32 type, M3SensorReading *out_reading, M3Bool *out_has_reading)
{
    struct M3GTK4Backend *backend;
    int rc;
...
static const M3SensorsVTable g_m3_gtk4_sensors_vtable = {
    m3_gtk4_sensors_is_available,
    m3_gtk4_sensors_start,
    m3_gtk4_sensors_stop,
    m3_gtk4_sensors_read
};

static int m3_gtk4_camera_path_from_id(m3_u32 camera_id, char *out_path, m3_usize path_capacity)
{
...
static int m3_gtk4_camera_select_id(const M3CameraConfig *config, m3_u32 *out_id)
{
...
static int m3_gtk4_camera_format_from_v4l2(m3_u32 v4l2_format, m3_u32 *out_format)
{
    if (out_format == NULL) {
        return M3_ERR_INVALID_ARGUMENT;
    }
...
static int m3_gtk4_camera_try_format(int fd, m3_u32 width, m3_u32 height, m3_u32 pixfmt, struct v4l2_format *out_fmt)
{
...
static int m3_gtk4_camera_set_format(int fd, m3_u32 width, m3_u32 height, m3_u32 format, struct v4l2_format *out_fmt)
{
    int rc;
    m3_u32 v4l2_format;

    if (format == M3_CAMERA_FORMAT_ANY) {
        rc = m3_gtk4_camera_try_format(fd, width, height, V4L2_PIX_FMT_NV12, out_fmt);
        if (rc == M3_OK) {
            return M3_OK;
        }

        rc = m3_gtk4_camera_try_format(fd, width, height, V4L2_PIX_FMT_BGR32, out_fmt);
        if (rc == M3_OK) {
            return M3_OK;
        }

        rc = m3_gtk4_camera_try_format(fd, width, height, V4L2_PIX_FMT_RGB32, out_fmt);
        if (rc == M3_OK) {
            return M3_OK;
        }
...
static int m3_gtk4_camera_release(struct M3GTK4Backend *backend)
{
    ...
    if (backend->camera_state.frame != NULL) {
        if (backend->allocator.free == NULL) {
            ...
            close_rc = backend->allocator.free(backend->allocator.ctx, backend->camera_state.frame);
...
static int m3_gtk4_camera_open_with_config(void *camera, const M3CameraConfig *config);
...
static int m3_gtk4_camera_open_with_config(void *camera, const M3CameraConfig *config)
{
...
#if M3_GTK4_V4L2_AVAILABLE
    {
        ...
        width = (config->width == 0u) ? M3_GTK4_CAMERA_DEFAULT_WIDTH : config->width;
        height = (config->height == 0u) ? M3_GTK4_CAMERA_DEFAULT_HEIGHT : config->height;

        rc = m3_gtk4_camera_path_from_id(camera_id, device_path, (m3_usize)sizeof(device_path));
        if (rc != M3_OK) {
            return rc;
        }
        ...
        rc = backend->allocator.alloc(backend->allocator.ctx, (m3_usize)fmt.fmt.pix.sizeimage, (void **)&frame);
        if (rc != M3_OK) {
            close(fd);
            return rc;
        }
...
static int m3_gtk4_camera_read_frame(void *camera, M3CameraFrame *out_frame, M3Bool *out_has_frame)
{
    struct M3GTK4Backend *backend;
    int rc;
...
static const M3CameraVTable g_m3_gtk4_camera_vtable = {
    m3_gtk4_camera_open,
    m3_gtk4_camera_open_with_config,
    m3_gtk4_camera_close,
    m3_gtk4_camera_start,
    m3_gtk4_camera_stop,
    m3_gtk4_camera_read_frame
};
...
static int m3_gtk4_network_add_usize(m3_usize a, m3_usize b, m3_usize *out_value)
{
    if (out_value == NULL) {
        return M3_ERR_INVALID_ARGUMENT;
    }
...
static int m3_gtk4_network_mul_usize(m3_usize a, m3_usize b, m3_usize *out_value)
{
    if (out_value == NULL) {
        return M3_ERR_INVALID_ARGUMENT;
    }
...
static size_t m3_gtk4_network_write_cb(char *ptr, size_t size, size_t nmemb, void *userdata)
{
...
static int m3_gtk4_network_append_headers(struct M3GTK4Backend *backend, const char *headers,
...
static int m3_gtk4_network_request(void *net, const M3NetworkRequest *request, const M3Allocator *allocator, M3NetworkResponse *out_response)
{
    struct M3GTK4Backend *backend;
#if defined(M3_LIBCURL_AVAILABLE)
    CURL *curl;
    ...
    curl_rc = curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, m3_gtk4_network_write_cb);
    if (curl_rc != CURLE_OK) {
        rc = m3_gtk4_network_error_from_curl(curl_rc);
        goto cleanup;
    }
    ...
    if (request->headers != NULL && request->headers[0] != '\0') {
        rc = m3_gtk4_network_append_headers(backend, request->headers, &header_list);
        if (rc != M3_OK) {
            goto cleanup;
        }
        if (header_list != NULL) {
...
static int m3_gtk4_network_free_response(void *net, const M3Allocator *allocator, M3NetworkResponse *response)
{
    struct M3GTK4Backend *backend;
    int rc;
    ...
    backend = (struct M3GTK4Backend *)net;
    rc = m3_gtk4_backend_log(backend, M3_LOG_LEVEL_DEBUG, "network.free_response");
    M3_GTK4_RETURN_IF_ERROR(rc);
#if defined(M3_LIBCURL_AVAILABLE)
...
static const M3NetworkVTable g_m3_gtk4_network_vtable = {
    m3_gtk4_network_request,
    m3_gtk4_network_free_response
};

static int m3_gtk4_tasks_thread_create(void *tasks, M3ThreadFn entry, void *user, M3Handle *out_thread)
{
    struct M3GTK4Backend *backend;
    int rc;
...
static int m3_gtk4_tasks_post_delayed(void *tasks, M3TaskFn fn, void *user, m3_u32 delay_ms)
{
    struct M3GTK4Backend *backend;
    int rc;
...
static const M3TasksVTable g_m3_gtk4_tasks_vtable = {
    m3_gtk4_tasks_thread_create,
    m3_gtk4_tasks_thread_join,
    m3_gtk4_tasks_mutex_create,
    m3_gtk4_tasks_mutex_destroy,
    m3_gtk4_tasks_mutex_lock,
    m3_gtk4_tasks_mutex_unlock,
    m3_gtk4_tasks_sleep_ms,
    m3_gtk4_tasks_post,
...
static const M3EnvVTable g_m3_gtk4_env_vtable = {
    m3_gtk4_env_get_io,
    m3_gtk4_env_get_sensors,
    m3_gtk4_env_get_camera,
    m3_gtk4_env_get_network,
    m3_gtk4_env_get_tasks,
    m3_gtk4_env_get_time_ms
};

int M3_CALL m3_gtk4_backend_create(const M3GTK4BackendConfig *config, M3GTK4Backend **out_backend)
{
...
#if M3_GTK4_V4L2_AVAILABLE
    if (backend->camera_state.opened == M3_TRUE || backend->camera_state.frame != NULL || backend->camera_state.fd >= 0) {
...
#ifdef M3_TESTING
int M3_CALL m3_gtk4_backend_test_set_initialized(M3GTK4Backend *backend, M3Bool initialized)
{
    if (backend == NULL) {
        return M3_ERR_INVALID_ARGUMENT;
    }
