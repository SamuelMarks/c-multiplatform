...
static int m3_null_backend_log(struct M3NullBackend *backend, M3LogLevel level,
                               const char *message) {
...
static int m3_null_backend_resolve(struct M3NullBackend *backend,
...
static const M3ObjectVTable g_m3_null_window_vtable = {
    m3_null_object_retain, m3_null_object_release, m3_null_window_destroy,
    m3_null_object_get_type_id};

static const M3ObjectVTable g_m3_null_texture_vtable = {
    m3_null_object_retain, m3_null_object_release, m3_null_texture_destroy,
    m3_null_object_get_type_id};

static const M3ObjectVTable g_m3_null_font_vtable = {
    m3_null_object_retain, m3_null_object_release, m3_null_font_destroy,
    m3_null_object_get_type_id};
...
static int m3_null_ws_create_window(void *ws, const M3WSWindowConfig *config,
                                    M3Handle *out_window) {
  struct M3NullBackend *backend;
  M3NullWindow *window;
  int rc;
  ...
  rc = backend->allocator.alloc(backend->allocator.ctx, sizeof(M3NullWindow),
                                (void **)&window);
  M3_NULL_RETURN_IF_ERROR(rc);
  ...
  rc = m3_object_header_init(&window->header, M3_NULL_TYPE_WINDOW, 0,
                             &g_m3_null_window_vtable);
...
static int m3_null_ws_destroy_window(void *ws, M3Handle window) {
  ...
  backend = (struct M3NullBackend *)ws;
  rc = m3_null_backend_resolve(backend, window, M3_NULL_TYPE_WINDOW,
                               (void **)&resolved);
  M3_NULL_RETURN_IF_ERROR(rc);
...
static int m3_null_ws_set_window_title(void *ws, M3Handle window,
...
static int m3_null_ws_set_window_size(void *ws, M3Handle window, m3_i32 width,
                                      m3_i32 height) {
  struct M3NullBackend *backend;
  M3NullWindow *resolved;
  int rc;
...
static int m3_null_ws_get_window_size(void *ws, M3Handle window,
...
static int m3_null_ws_set_window_dpi_scale(void *ws, M3Handle window,
                                           M3Scalar scale) {
  struct M3NullBackend *backend;
  M3NullWindow *resolved;
  int rc;
  ...
  rc = m3_null_backend_log(backend, M3_LOG_LEVEL_INFO,
...
static int m3_null_ws_get_window_dpi_scale(void *ws, M3Handle window,
                                           M3Scalar *out_scale) {
  struct M3NullBackend *backend;
  M3NullWindow *resolved;
  int rc;
...
static int m3_null_ws_get_clipboard_text(void *ws, char *buffer,
...
static int m3_null_ws_poll_event(void *ws, M3InputEvent *out_event,
...
static const M3WSVTable g_m3_null_ws_vtable = {m3_null_ws_init,
                                               m3_null_ws_shutdown,
...
static int m3_null_gfx_begin_frame(void *gfx, M3Handle window, m3_i32 width,
...
static int m3_null_gfx_draw_rect(void *gfx, const M3Rect *rect, M3Color color,
...
static int m3_null_gfx_draw_line(void *gfx, M3Scalar x0, M3Scalar y0,
...
static int m3_null_gfx_create_texture(void *gfx, m3_i32 width, m3_i32 height,
  ...
  rc = backend->allocator.alloc(backend->allocator.ctx, sizeof(M3NullTexture),
                                (void **)&texture);
  M3_NULL_RETURN_IF_ERROR(rc);
  ...
  rc = m3_object_header_init(&texture->header, M3_NULL_TYPE_TEXTURE, 0,
                             &g_m3_null_texture_vtable);
  M3_NULL_RETURN_IF_ERROR_CLEANUP(
      rc, backend->allocator.free(backend->allocator.ctx, texture));
...
static int m3_null_gfx_update_texture(void *gfx, M3Handle texture, m3_i32 x,
...
static int m3_null_gfx_draw_texture(void *gfx, M3Handle texture,
...
static int m3_null_text_create_font(void *text, const char *utf8_family,
  ...
  rc = backend->allocator.alloc(backend->allocator.ctx, sizeof(M3NullFont),
                                (void **)&font);
  M3_NULL_RETURN_IF_ERROR(rc);
...
static int m3_null_text_measure_text(void *text, M3Handle font,
...
static int m3_null_io_file_exists(void *io, const char *utf8_path,
                                  M3Bool *out_exists) {
  struct M3NullBackend *backend;
  int rc;
...
static const M3IOVTable g_m3_null_io_vtable = {
    m3_null_io_read_file,   m3_null_io_read_file_alloc, m3_null_io_write_file,
    m3_null_io_file_exists, m3_null_io_delete_file,     m3_null_io_stat_file};
...
static const M3SensorsVTable g_m3_null_sensors_vtable = {
    m3_null_sensors_is_available, m3_null_sensors_start, m3_null_sensors_stop,
...
static int m3_null_camera_open_with_config(void *camera,
...
static int m3_null_camera_read_frame(void *camera, M3CameraFrame *out_frame,
...
static int m3_null_network_request(void *net, const M3NetworkRequest *request,
...
static int m3_null_network_free_response(void *net,
                                         const M3Allocator *allocator,
  ...
  backend = (struct M3NullBackend *)net;
  rc =
      m3_null_backend_log(backend, M3_LOG_LEVEL_DEBUG, "network.free_response");
  M3_NULL_RETURN_IF_ERROR(rc);
  return M3_ERR_UNSUPPORTED;
}

static const M3NetworkVTable g_m3_null_network_vtable = {
    m3_null_network_request, m3_null_network_free_response};

static int m3_null_tasks_thread_create(void *tasks, M3ThreadFn entry,
...
static int m3_null_tasks_post_delayed(void *tasks, M3TaskFn fn, void *user,
                                      m3_u32 delay_ms) {
  struct M3NullBackend *backend;
  int rc;
...
static const M3EnvVTable g_m3_null_env_vtable = {
    m3_null_env_get_io,      m3_null_env_get_sensors, m3_null_env_get_camera,
    m3_null_env_get_network, m3_null_env_get_tasks,   m3_null_env_get_time_ms};
...
#ifdef M3_TESTING
int M3_CALL m3_null_backend_test_set_initialized(M3NullBackend *backend,
                                                 M3Bool initialized) {
  if (backend == NULL) {
    return M3_ERR_INVALID_ARGUMENT;
  }
...
int M3_CALL m3_null_backend_test_object_get_type_id(M3ObjectHeader *obj,
                                                    m3_u32 *out_type_id) {
