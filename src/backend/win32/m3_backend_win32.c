...
static int
m3_win32_backend_validate_config(const M3Win32BackendConfig *config) {
  if (config == NULL) {
...
#ifdef M3_TESTING
int M3_CALL
m3_win32_backend_test_validate_config(const M3Win32BackendConfig *config) {
...
typedef BOOL(WINAPI *M3Win32AlphaBlendFn)(HDC, int, int, int, int, HDC, int,
...
static int m3_win32_network_copy_wide_range(struct M3Win32Backend *backend,
...
static int m3_win32_backend_log(struct M3Win32Backend *backend,
                                M3LogLevel level, const char *message) {
...
static int m3_win32_backend_log_last_error(struct M3Win32Backend *backend,
                                           const char *message) {
  DWORD error_code;
  char buffer[256];
  int rc;
...
static int m3_win32_event_queue_push(M3Win32EventQueue *queue,
...
static int m3_win32_event_queue_pop(M3Win32EventQueue *queue,
...
static int m3_win32_backend_push_event(struct M3Win32Backend *backend,
                                       const M3InputEvent *event) {
  int rc;
...
static m3_u32 m3_win32_get_time_ms(void) { return (m3_u32)GetTickCount(); }

static int m3_win32_utf8_to_wide_alloc(struct M3Win32Backend *backend,
  ...
  *out_wide = NULL;
  ...
  if (utf8_len >= 0) {
    rc = backend->allocator.alloc(backend->allocator.ctx,
                                  (m3_usize)(wide_len + 1) * sizeof(wchar_t),
                                  (void **)&wide);
    M3_WIN32_RETURN_IF_ERROR(rc);
    ...
    rc = backend->allocator.alloc(backend->allocator.ctx,
                                  (m3_usize)wide_len * sizeof(wchar_t),
                                  (void **)&wide);
    M3_WIN32_RETURN_IF_ERROR(rc);
    if (MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, utf8, utf8_len, wide,
                            wide_len) != wide_len) {
...
static int m3_win32_wide_to_utf8(const wchar_t *wide, int wide_len,
                                 char *buffer, m3_usize buffer_size,
                                 m3_usize *out_length) {
  int required;
...
static int m3_win32_backend_resolve(struct M3Win32Backend *backend,
...
static void m3_win32_window_cleanup_unregistered(struct M3Win32Backend *backend,
                                                 M3Win32Window *window) {
...
static void
m3_win32_texture_cleanup_unregistered(struct M3Win32Backend *backend,
                                      M3Win32Texture *texture) {
...
static void m3_win32_font_cleanup_unregistered(struct M3Win32Backend *backend,
                                               M3Win32Font *font) {
...
static const M3ObjectVTable g_m3_win32_window_vtable = {
    m3_win32_object_retain, m3_win32_object_release, m3_win32_window_destroy,
    m3_win32_object_get_type_id};

static const M3ObjectVTable g_m3_win32_texture_vtable = {
    m3_win32_object_retain, m3_win32_object_release, m3_win32_texture_destroy,
    m3_win32_object_get_type_id};

static const M3ObjectVTable g_m3_win32_font_vtable = {
    m3_win32_object_retain, m3_win32_object_release, m3_win32_font_destroy,
    m3_win32_object_get_type_id};

static int m3_win32_window_flags_to_style(m3_u32 flags, DWORD *out_style,
                                          DWORD *out_ex_style) {
  DWORD style;
  DWORD ex_style;
...
static int m3_win32_window_ensure_backbuffer(M3Win32Window *window,
...
static int m3_win32_apply_transform(M3Win32Window *window,
                                    const M3Mat3 *transform,
                                    M3Bool has_transform) {
  XFORM xform;
...
static void m3_win32_texture_copy_pixels(m3_u8 *dst, m3_i32 dst_stride,
                                         const m3_u8 *src, m3_i32 src_stride,
                                         m3_i32 width, m3_i32 height,
                                         m3_u32 format) {
  m3_i32 y;
  m3_i32 x;
...
static LRESULT CALLBACK m3_win32_wndproc(HWND hwnd, UINT msg, WPARAM wparam,
                                         LPARAM lparam) {
  M3Win32Window *window;
  struct M3Win32Backend *backend;
  M3InputEvent event;
...
static int m3_win32_ws_create_window(void *ws, const M3WSWindowConfig *config,
                                     M3Handle *out_window) {
  struct M3Win32Backend *backend;
  M3Win32Window *window;
  DWORD style;
  ...
  rc = backend->allocator.alloc(backend->allocator.ctx, sizeof(M3Win32Window),
                                (void **)&window);
  M3_WIN32_RETURN_IF_ERROR(rc);
  ...
  if (has_title) {
    rc = m3_win32_utf8_to_wide_alloc(backend, config->utf8_title, -1,
                                     &title_wide, NULL);
    if (rc != M3_OK) {
      backend->allocator.free(backend->allocator.ctx, window);
      return rc;
  ...
  window->hwnd =
      CreateWindowExW(ex_style, g_m3_win32_window_class_name,
  ...
  rc = m3_object_header_init(&window->header, M3_WIN32_TYPE_WINDOW, 0,
                             &g_m3_win32_window_vtable);
  if (rc != M3_OK) {
...
static int m3_win32_ws_destroy_window(void *ws, M3Handle window) {
  ...
  backend = (struct M3Win32Backend *)ws;
  rc = m3_win32_backend_resolve(backend, window, M3_WIN32_TYPE_WINDOW,
                                (void **)&resolved);
  M3_WIN32_RETURN_IF_ERROR(rc);
...
static int m3_win32_ws_set_window_title(void *ws, M3Handle window,
...
static int m3_win32_ws_set_window_size(void *ws, M3Handle window, m3_i32 width,
                                       m3_i32 height) {
  struct M3Win32Backend *backend;
  M3Win32Window *resolved;
  RECT rect;
...
static int m3_win32_ws_get_window_size(void *ws, M3Handle window,
...
static int m3_win32_ws_set_window_dpi_scale(void *ws, M3Handle window,
                                            M3Scalar scale) {
  struct M3Win32Backend *backend;
  M3Win32Window *resolved;
  int rc;
  ...
  rc = m3_win32_backend_log(backend, M3_LOG_LEVEL_INFO,
...
static int m3_win32_ws_get_window_dpi_scale(void *ws, M3Handle window,
                                            M3Scalar *out_scale) {
  struct M3Win32Backend *backend;
  M3Win32Window *resolved;
  int rc;
...
static int m3_win32_ws_set_clipboard_text(void *ws, const char *utf8_text) {
  ...
  backend = (struct M3Win32Backend *)ws;
  rc =
      m3_win32_backend_log(backend, M3_LOG_LEVEL_INFO, "ws.set_clipboard_text");
  M3_WIN32_RETURN_IF_ERROR(rc);
...
static int m3_win32_ws_get_clipboard_text(void *ws, char *buffer,
  ...
  backend = (struct M3Win32Backend *)ws;
  rc =
      m3_win32_backend_log(backend, M3_LOG_LEVEL_INFO, "ws.get_clipboard_text");
  M3_WIN32_RETURN_IF_ERROR(rc);
...
static int m3_win32_ws_poll_event(void *ws, M3InputEvent *out_event,
                                  M3Bool *out_has_event) {
  struct M3Win32Backend *backend;
  int rc;
  ...
  return m3_win32_event_queue_pop(&backend->event_queue, out_event,
                                  out_has_event);
}
...
static int m3_win32_gfx_begin_frame(void *gfx, M3Handle window, m3_i32 width,
  ...
  rc = m3_win32_apply_transform(resolved, &backend->transform,
...
static int m3_win32_gfx_draw_rect(void *gfx, const M3Rect *rect, M3Color color,
...
static int m3_win32_gfx_draw_line(void *gfx, M3Scalar x0, M3Scalar y0,
...
static int m3_win32_gfx_pop_clip(void *gfx) {
  ...
  backend->clip_depth -= 1u;
  rc = m3_win32_apply_transform(window, &backend->transform,
                                backend->has_transform);
  M3_WIN32_RETURN_IF_ERROR(rc);
  return M3_OK;
}
...
static int m3_win32_gfx_create_texture(void *gfx, m3_i32 width, m3_i32 height,
  ...
  rc = backend->allocator.alloc(backend->allocator.ctx, sizeof(M3Win32Texture),
                                (void **)&texture);
  M3_WIN32_RETURN_IF_ERROR(rc);
  ...
  if (pixels != NULL && size != 0) {
    ...
    m3_win32_texture_copy_pixels((m3_u8 *)texture->pixels, texture->stride,
                                 (const m3_u8 *)pixels, (m3_i32)row_bytes,
                                 width, height, format);
  }

  rc = m3_object_header_init(&texture->header, M3_WIN32_TYPE_TEXTURE, 0,
                             &g_m3_win32_texture_vtable);
  if (rc != M3_OK) {
    m3_win32_texture_cleanup_unregistered(backend, texture);
    return rc;
  }
...
static int m3_win32_gfx_update_texture(void *gfx, M3Handle texture, m3_i32 x,
  ...
  backend = (struct M3Win32Backend *)gfx;
  rc = m3_win32_backend_resolve(backend, texture, M3_WIN32_TYPE_TEXTURE,
                                (void **)&resolved);
  M3_WIN32_RETURN_IF_ERROR(rc);
  ...
  rc = m3_win32_mul_usize((m3_usize)width, (m3_usize)height, &required);
  M3_WIN32_RETURN_IF_ERROR(rc);
  rc = m3_win32_mul_usize(required, (m3_usize)resolved->bytes_per_pixel,
                          &required);
  M3_WIN32_RETURN_IF_ERROR(rc);
  ...
  rc = m3_win32_mul_usize((m3_usize)width, (m3_usize)resolved->bytes_per_pixel,
                          &row_bytes);
  M3_WIN32_RETURN_IF_ERROR(rc);

  dst = (m3_u8 *)resolved->pixels + (m3_isize)y * (m3_isize)resolved->stride +
        (m3_isize)x * 4;
  m3_win32_texture_copy_pixels(dst, resolved->stride, (const m3_u8 *)pixels,
                               (m3_i32)row_bytes, width, height,
                               resolved->format);
  return M3_OK;
}
...
static int m3_win32_gfx_draw_texture(void *gfx, M3Handle texture,
...
static int m3_win32_text_create_font(void *text, const char *utf8_family,
  ...
  rc = backend->allocator.alloc(backend->allocator.ctx, sizeof(M3Win32Font),
                                (void **)&font);
  ...
  rc = m3_object_header_init(&font->header, M3_WIN32_TYPE_FONT, 0,
                             &g_m3_win32_font_vtable);
  if (rc != M3_OK) {
...
static int m3_win32_text_destroy_font(void *text, M3Handle font) {
  ...
  backend = (struct M3Win32Backend *)text;
  rc = m3_win32_backend_resolve(backend, font, M3_WIN32_TYPE_FONT,
                                (void **)&resolved);
  M3_WIN32_RETURN_IF_ERROR(rc);
...
static int m3_win32_text_measure_text(void *text, M3Handle font,
  ...
  if (utf8_len == 0) {
    ...
    wide = NULL;
    wide_len = 0;
    rc = m3_win32_utf8_to_wide_alloc(backend, utf8, (int)utf8_len, &wide,
                                     &wide_len);
    if (rc != M3_OK) {
      SelectObject(hdc, old_font);
      ReleaseDC(NULL, hdc);
...
static int m3_win32_text_draw_text(void *text, M3Handle font, const char *utf8,
...
static const M3TextVTable g_m3_win32_text_vtable = {
    m3_win32_text_create_font, m3_win32_text_destroy_font,
    m3_win32_text_measure_text, m3_win32_text_draw_text};

static int m3_win32_io_read_file(void *io, const char *utf8_path, void *buffer,
...
static int m3_win32_io_read_file_alloc(void *io, const char *utf8_path,
...
static int m3_win32_io_write_file(void *io, const char *utf8_path,
...
static int m3_win32_io_file_exists(void *io, const char *utf8_path,
                                   M3Bool *out_exists) {
  struct M3Win32Backend *backend;
  int rc;
...
static int m3_win32_io_stat_file(void *io, const char *utf8_path,
                                 M3FileInfo *out_info) {
  struct M3Win32Backend *backend;
  int rc;
...
static const M3IOVTable g_m3_win32_io_vtable = {
    m3_win32_io_read_file,   m3_win32_io_read_file_alloc,
    m3_win32_io_write_file,  m3_win32_io_file_exists,
    m3_win32_io_delete_file, m3_win32_io_stat_file};

static int m3_win32_sensors_is_available(void *sensors, m3_u32 type,
  ...
  rc =
      m3_win32_backend_log(backend, M3_LOG_LEVEL_DEBUG, "sensors.is_available");
  M3_WIN32_RETURN_IF_ERROR(rc);
...
static int m3_win32_sensors_read(void *sensors, m3_u32 type,
...
static const M3SensorsVTable g_m3_win32_sensors_vtable = {
    m3_win32_sensors_is_available, m3_win32_sensors_start,
    m3_win32_sensors_stop, m3_win32_sensors_read};
...
static int m3_win32_camera_open_with_config(void *camera,
...
static int m3_win32_camera_read_frame(void *camera, M3CameraFrame *out_frame,
...
static const M3CameraVTable g_m3_win32_camera_vtable = {
    m3_win32_camera_open,  m3_win32_camera_open_with_config,
    m3_win32_camera_close, m3_win32_camera_start,
    m3_win32_camera_stop,  m3_win32_camera_read_frame};

static int m3_win32_network_request(void *net, const M3NetworkRequest *request,
                                    const M3Allocator *allocator,
  ...
  rc =
      m3_win32_network_copy_wide_range(backend, components.lpszHostName,
  ...
  rc = m3_win32_utf8_to_wide_alloc(backend, request->method, -1, &method_wide,
                                   NULL);
  if (rc != M3_OK) {
    goto cleanup;
  }
  ...
  if (request->headers != NULL && request->headers[0] != '\0') {
    rc = m3_win32_utf8_to_wide_alloc(backend, request->headers, -1,
                                     &headers_wide, NULL);
    if (rc != M3_OK) {
      goto cleanup;
    }
  ...
  session = WinHttpOpen(L"LibM3C/1.0", WINHTTP_ACCESS_TYPE_DEFAULT_PROXY,
  ...
  if (!WinHttpReceiveResponse(request_handle, NULL)) {
    ...
    rc = m3_win32_network_error_from_winhttp(last_error);
    log_rc =
        m3_win32_backend_log_last_error(backend, "network.receive_response");
    if (log_rc != M3_OK && rc == M3_OK) {
      rc = log_rc;
    }
    goto cleanup;
  ...
  for (;;) {
    ...
    if (!WinHttpQueryDataAvailable(request_handle, &bytes_available)) {
      last_error = GetLastError();
      rc = m3_win32_network_error_from_winhttp(last_error);
      log_rc = m3_win32_backend_log_last_error(backend,
    ...
    bytes_read = 0;
    if (!WinHttpReadData(request_handle, (m3_u8 *)body + total_size,
...
static int m3_win32_network_free_response(void *net,
                                          const M3Allocator *allocator,
  ...
  backend = (struct M3Win32Backend *)net;
  rc = m3_win32_backend_log(backend, M3_LOG_LEVEL_DEBUG,
...
static const M3NetworkVTable g_m3_win32_network_vtable = {
    m3_win32_network_request, m3_win32_network_free_response};

static int m3_win32_tasks_thread_create(void *tasks, M3ThreadFn entry,
...
static int m3_win32_tasks_post_delayed(void *tasks, M3TaskFn fn, void *user,
                                       m3_u32 delay_ms) {
  struct M3Win32Backend *backend;
  int rc;
...
static const M3TasksVTable g_m3_win32_tasks_vtable = {
    m3_win32_tasks_thread_create, m3_win32_tasks_thread_join,
    m3_win32_tasks_mutex_create,  m3_win32_tasks_mutex_destroy,
    m3_win32_tasks_mutex_lock,    m3_win32_tasks_mutex_unlock,
    m3_win32_tasks_sleep_ms,      m3_win32_tasks_post,
...
static const M3EnvVTable g_m3_win32_env_vtable = {
    m3_win32_env_get_io,     m3_win32_env_get_sensors,
    m3_win32_env_get_camera, m3_win32_env_get_network,
    m3_win32_env_get_tasks,  m3_win32_env_get_time_ms};

int M3_CALL m3_win32_backend_create(const M3Win32BackendConfig *config,
                                    M3Win32Backend **out_backend) {
  ...
  *out_backend = NULL;
  ...
  rc = allocator.alloc(allocator.ctx, sizeof(struct M3Win32Backend),
                       (void **)&backend);
  M3_WIN32_RETURN_IF_ERROR(rc);
...
int M3_CALL m3_win32_backend_destroy(M3Win32Backend *backend) {
  ...
  if (backend->class_registered) {
    if (!UnregisterClassW(g_m3_win32_window_class_name, backend->instance) &&
        first_error == M3_OK) {
