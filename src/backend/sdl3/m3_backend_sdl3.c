...
#define M3_SDL3_DEFAULT_RENDERER_FLAGS (M3_SDL3_RENDERER_ACCELERATED | M3_SDL3_RENDERER_PRESENTVSYNC)

#define M3_SDL3_RENDERER_FLAG_MASK \
    (M3_SDL3_RENDERER_ACCELERATED | M3_SDL3_RENDERER_PRESENTVSYNC | M3_SDL3_RENDERER_TARGETTEXTURE)
...
#ifdef M3_TESTING
int M3_CALL m3_sdl3_backend_test_validate_config(const M3SDL3BackendConfig *config)
{
...
static int m3_sdl3_backend_log(struct M3SDL3Backend *backend, M3LogLevel level, const char *message)
{
...
static int m3_sdl3_backend_log_sdl_error(struct M3SDL3Backend *backend, const char *message)
{
...
#if defined(M3_SDL3_TTF_AVAILABLE)
static int m3_sdl3_backend_log_ttf_error(struct M3SDL3Backend *backend, const char *message)
{
...
static int m3_sdl3_window_flags_to_sdl(m3_u32 flags, Uint32 *out_flags)
{
    ...
    allowed = M3_WS_WINDOW_RESIZABLE | M3_WS_WINDOW_BORDERLESS | M3_WS_WINDOW_FULLSCREEN | M3_WS_WINDOW_HIGH_DPI;
    if ((flags & ~allowed) != 0u) {
        return M3_ERR_INVALID_ARGUMENT;
    }
...
static int m3_sdl3_backend_resolve(struct M3SDL3Backend *backend, M3Handle handle, m3_u32 type_id, void **out_obj)
{
    void *resolved;
    m3_u32 actual_type;
    int rc;
...
static const M3ObjectVTable g_m3_sdl3_window_vtable = {
    m3_sdl3_object_retain,
    m3_sdl3_object_release,
    m3_sdl3_window_destroy,
...
static int m3_sdl3_ws_create_window(void *ws, const M3WSWindowConfig *config, M3Handle *out_window)
{
    struct M3SDL3Backend *backend;
    M3SDL3Window *window;
    Uint32 sdl_flags;
    ...
    rc = backend->allocator.alloc(backend->allocator.ctx, sizeof(M3SDL3Window), (void **)&window);
    M3_SDL3_RETURN_IF_ERROR(rc);
    ...
    window->window = SDL_CreateWindow(config->utf8_title, config->width, config->height, sdl_flags);
#else
    window->window = SDL_CreateWindow(config->utf8_title, SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED,
                                       config->width, config->height, sdl_flags);
#endif
    if (window->window == NULL) {
    ...
    rc = m3_object_header_init(&window->header, M3_SDL3_TYPE_WINDOW, 0, &g_m3_sdl3_window_vtable);
    if (rc != M3_OK) {
        SDL_DestroyRenderer(window->renderer);
        SDL_DestroyWindow(window->window);
        backend->allocator.free(backend->allocator.ctx, window);
...
static int m3_sdl3_ws_destroy_window(void *ws, M3Handle window)
{
    ...
    backend = (struct M3SDL3Backend *)ws;
    rc = m3_sdl3_backend_resolve(backend, window, M3_SDL3_TYPE_WINDOW, (void **)&resolved);
    M3_SDL3_RETURN_IF_ERROR(rc);
...
static int m3_sdl3_ws_set_window_title(void *ws, M3Handle window, const char *utf8_title)
{
    struct M3SDL3Backend *backend;
    M3SDL3Window *resolved;
    int rc;
...
static int m3_sdl3_ws_set_window_size(void *ws, M3Handle window, m3_i32 width, m3_i32 height)
{
    struct M3SDL3Backend *backend;
    M3SDL3Window *resolved;
    int rc;
...
static int m3_sdl3_ws_get_window_size(void *ws, M3Handle window, m3_i32 *out_width, m3_i32 *out_height)
{
    struct M3SDL3Backend *backend;
    M3SDL3Window *resolved;
    int rc;
...
static int m3_sdl3_ws_set_window_dpi_scale(void *ws, M3Handle window, M3Scalar scale)
{
    struct M3SDL3Backend *backend;
    M3SDL3Window *resolved;
    int rc;
    ...
    rc = m3_sdl3_backend_log(backend, M3_LOG_LEVEL_INFO, "ws.set_window_dpi_scale");
    M3_SDL3_RETURN_IF_ERROR(rc);
...
static int m3_sdl3_ws_get_window_dpi_scale(void *ws, M3Handle window, M3Scalar *out_scale)
{
    struct M3SDL3Backend *backend;
    M3SDL3Window *resolved;
    int rc;
    ...
    rc = m3_sdl3_backend_log(backend, M3_LOG_LEVEL_INFO, "ws.get_window_dpi_scale");
    M3_SDL3_RETURN_IF_ERROR(rc);
...
static int m3_sdl3_ws_get_clipboard_text(void *ws, char *buffer, m3_usize buffer_size, m3_usize *out_length)
{
    struct M3SDL3Backend *backend;
    char *text;
    m3_usize length;
...
static int m3_sdl3_translate_event(struct M3SDL3Backend *backend, const SDL_Event *sdl_event, M3InputEvent *out_event, M3Bool *out_has_event)
{
    ...
    if (backend == NULL || sdl_event == NULL || out_event == NULL || out_has_event == NULL) {
        return M3_ERR_INVALID_ARGUMENT;
    }
    ...
    *out_has_event = M3_FALSE;
    ...
    if (sdl_event->type == SDL_EVENT_WINDOW_RESIZED) {
        ...
        out_event->modifiers = modifiers;
        out_event->window = window_handle;
        if (backend->active_window != NULL && backend->active_window->window != NULL) {
            int w = 0;
            int h = 0;
...
static int m3_sdl3_ws_poll_event(void *ws, M3InputEvent *out_event, M3Bool *out_has_event)
{
    struct M3SDL3Backend *backend;
    SDL_Event event;
    M3Bool has_event;
...
static const M3WSVTable g_m3_sdl3_ws_vtable = {
    m3_sdl3_ws_init,
    m3_sdl3_ws_shutdown,
...
static int m3_sdl3_get_active_renderer(struct M3SDL3Backend *backend, SDL_Renderer **out_renderer)
{
    if (backend == NULL || out_renderer == NULL) {
        return M3_ERR_INVALID_ARGUMENT;
    }
    if (backend->active_window == NULL || backend->active_window->renderer == NULL) {
        return M3_ERR_STATE;
    }
...
static int m3_sdl3_render_fill_rect(SDL_Renderer *renderer, const M3Rect *rect)
{
#if defined(SDL_MAJOR_VERSION) && (SDL_MAJOR_VERSION >= 3)
    SDL_FRect sdl_rect;
#else
...
static int m3_sdl3_render_line(SDL_Renderer *renderer, M3Scalar x0, M3Scalar y0, M3Scalar x1, M3Scalar y1)
{
    int rc;
...
static int m3_sdl3_gfx_begin_frame(void *gfx, M3Handle window, m3_i32 width, m3_i32 height, M3Scalar dpi_scale)
{
    struct M3SDL3Backend *backend;
    M3SDL3Window *resolved;
    int rc;
...
static int m3_sdl3_gfx_draw_rect(void *gfx, const M3Rect *rect, M3Color color, M3Scalar corner_radius)
{
    struct M3SDL3Backend *backend;
    SDL_Renderer *renderer;
    m3_u8 r;
...
static int m3_sdl3_gfx_draw_line(void *gfx, M3Scalar x0, M3Scalar y0, M3Scalar x1, M3Scalar y1, M3Color color, M3Scalar thickness)
{
    struct M3SDL3Backend *backend;
    SDL_Renderer *renderer;
    m3_u8 r;
...
static int m3_sdl3_gfx_create_texture(void *gfx, m3_i32 width, m3_i32 height, m3_u32 format, const void *pixels, m3_usize size,
...
static int m3_sdl3_gfx_update_texture(void *gfx, M3Handle texture, m3_i32 x, m3_i32 y, m3_i32 width, m3_i32 height, const void *pixels,
                                      m3_usize size)
{
    struct M3SDL3Backend *backend;
...
static const M3GfxVTable g_m3_sdl3_gfx_vtable = {
    m3_sdl3_gfx_begin_frame,
    m3_sdl3_gfx_end_frame,
    m3_sdl3_gfx_clear,
    m3_sdl3_gfx_draw_rect,
    m3_sdl3_gfx_draw_line,
    m3_sdl3_gfx_push_clip,
    m3_sdl3_gfx_pop_clip,
    m3_sdl3_gfx_set_transform,
    m3_sdl3_gfx_create_texture,
    m3_sdl3_gfx_update_texture,
    m3_sdl3_gfx_destroy_texture,
    m3_sdl3_gfx_draw_texture
};

static int m3_sdl3_text_make_cstr(struct M3SDL3Backend *backend, const char *utf8, m3_usize utf8_len, char **out_text)
{
    m3_usize max_value;
    m3_usize required;
    char *buffer;
    ...
    *out_text = NULL;
    ...
    rc = backend->allocator.alloc(backend->allocator.ctx, required, (void **)&buffer);
    M3_SDL3_RETURN_IF_ERROR(rc);
...
static int m3_sdl3_text_create_font(void *text, const char *utf8_family, m3_i32 size_px, m3_i32 weight, M3Bool italic, M3Handle *out_font)
{
    struct M3SDL3Backend *backend;
    M3SDL3Font *font;
#if defined(M3_SDL3_TTF_AVAILABLE)
    ...
    rc = backend->allocator.alloc(backend->allocator.ctx, sizeof(M3SDL3Font), (void **)&font);
    if (rc != M3_OK) {
    ...
    rc = m3_object_header_init(&font->header, M3_SDL3_TYPE_FONT, 0, &g_m3_sdl3_font_vtable);
    if (rc != M3_OK) {
        TTF_CloseFont(ttf_font);
...
static int m3_sdl3_text_destroy_font(void *text, M3Handle font)
{
    ...
    backend = (struct M3SDL3Backend *)text;
    rc = m3_sdl3_backend_resolve(backend, font, M3_SDL3_TYPE_FONT, (void **)&resolved);
    M3_SDL3_RETURN_IF_ERROR(rc);
...
static int m3_sdl3_text_measure_text(void *text, M3Handle font, const char *utf8, m3_usize utf8_len, M3Scalar *out_width, M3Scalar *out_height,
                                     M3Scalar *out_baseline)
{
    ...
    if (utf8_len == 0) {
        ...
        if (TTF_SizeUTF8((TTF_Font *)resolved->font, temp, &text_width, &text_height) != 0) {
...
static int m3_sdl3_text_draw_text(void *text, M3Handle font, const char *utf8, m3_usize utf8_len, M3Scalar x, M3Scalar y, M3Color color)
{
    struct M3SDL3Backend *backend;
#if defined(M3_SDL3_TTF_AVAILABLE)
    M3SDL3Font *resolved;
...
static const M3TextVTable g_m3_sdl3_text_vtable = {
    m3_sdl3_text_create_font,
    m3_sdl3_text_destroy_font,
    m3_sdl3_text_measure_text,
    m3_sdl3_text_draw_text
};

static int m3_sdl3_io_read_file(void *io, const char *utf8_path, void *buffer, m3_usize buffer_size, m3_usize *out_read)
{
    struct M3SDL3Backend *backend;
    int rc;
...
static int m3_sdl3_io_read_file_alloc(void *io, const char *utf8_path, const M3Allocator *allocator, void **out_data, m3_usize *out_size)
{
    struct M3SDL3Backend *backend;
    int rc;
...
static int m3_sdl3_io_write_file(void *io, const char *utf8_path, const void *data, m3_usize size, M3Bool overwrite)
{
    struct M3SDL3Backend *backend;
    int rc;
...
static int m3_sdl3_io_file_exists(void *io, const char *utf8_path, M3Bool *out_exists)
{
    struct M3SDL3Backend *backend;
    int rc;
...
static int m3_sdl3_io_stat_file(void *io, const char *utf8_path, M3FileInfo *out_info)
{
    struct M3SDL3Backend *backend;
    int rc;
...
static const M3IOVTable g_m3_sdl3_io_vtable = {
    m3_sdl3_io_read_file,
    m3_sdl3_io_read_file_alloc,
    m3_sdl3_io_write_file,
    m3_sdl3_io_file_exists,
    m3_sdl3_io_delete_file,
    m3_sdl3_io_stat_file
};

static int m3_sdl3_sensors_is_available(void *sensors, m3_u32 type, M3Bool *out_available)
{
    struct M3SDL3Backend *backend;
    int rc;
...
static int m3_sdl3_sensors_read(void *sensors, m3_u32 type, M3SensorReading *out_reading, M3Bool *out_has_reading)
{
    struct M3SDL3Backend *backend;
    int rc;
...
static const M3SensorsVTable g_m3_sdl3_sensors_vtable = {
    m3_sdl3_sensors_is_available,
    m3_sdl3_sensors_start,
    m3_sdl3_sensors_stop,
...
static int m3_sdl3_camera_read_frame(void *camera, M3CameraFrame *out_frame, M3Bool *out_has_frame)
{
    struct M3SDL3Backend *backend;
    int rc;
...
static const M3NetworkVTable g_m3_sdl3_network_vtable = {
    m3_sdl3_network_request,
    m3_sdl3_network_free_response
};

static int m3_sdl3_tasks_thread_create(void *tasks, M3ThreadFn entry, void *user, M3Handle *out_thread)
{
    struct M3SDL3Backend *backend;
    int rc;
...
static int m3_sdl3_tasks_post_delayed(void *tasks, M3TaskFn fn, void *user, m3_u32 delay_ms)
{
    struct M3SDL3Backend *backend;
    int rc;
...
static const M3TasksVTable g_m3_sdl3_tasks_vtable = {
    m3_sdl3_tasks_thread_create,
    m3_sdl3_tasks_thread_join,
    m3_sdl3_tasks_mutex_create,
    m3_sdl3_tasks_mutex_destroy,
    m3_sdl3_tasks_mutex_lock,
    m3_sdl3_tasks_mutex_unlock,
    m3_sdl3_tasks_sleep_ms,
    m3_sdl3_tasks_post,
...
static const M3EnvVTable g_m3_sdl3_env_vtable = {
    m3_sdl3_env_get_io,
    m3_sdl3_env_get_sensors,
    m3_sdl3_env_get_camera,
    m3_sdl3_env_get_network,
    m3_sdl3_env_get_tasks,
    m3_sdl3_env_get_time_ms
};

int M3_CALL m3_sdl3_backend_create(const M3SDL3BackendConfig *config, M3SDL3Backend **out_backend)
{
