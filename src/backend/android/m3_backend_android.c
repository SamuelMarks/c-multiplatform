...
static int m3_android_backend_validate_config(const M3AndroidBackendConfig *config)
{
    if (config == NULL) {
...
static int m3_android_camera_copy_image(M3AndroidCameraState *state, AImage *image)
{
    ...
    rc = m3_android_camera_mul_overflow((m3_usize)width, (m3_usize)height, &y_size);
    if (rc != M3_OK) {
        return rc;
    }
    uv_size = y_size / 2u;
    ...
    if (AImage_getPlaneData(image, 0, (uint8_t **)&y_plane, &y_len) != AMEDIA_OK) {
        return M3_ERR_IO;
    }
    if (AImage_getPlaneData(image, 1, (uint8_t **)&u_plane, &u_len) != AMEDIA_OK) {
        return M3_ERR_IO;
    }
    if (AImage_getPlaneData(image, 2, (uint8_t **)&v_plane, &v_len) != AMEDIA_OK) {
        return M3_ERR_IO;
    }
    ...
    if (state->backend->allocator.alloc == NULL || state->backend->allocator.realloc == NULL
    ...
    if (state->frame_capacity < needed) {
        m3_u8 *new_frame;
        rc = state->backend->allocator.realloc(state->backend->allocator.ctx, state->frame_data, needed,
            (void **)&new_frame);
        if (rc != M3_OK) {
            return rc;
        }
    ...
    for (row = 0u; row < (m3_usize)height; row += 1u) {
        memcpy(state->frame_data + row * row_bytes, y_plane + row * (m3_usize)y_row_stride, (size_t)row_bytes);
    }
    ...
    for (row = 0u; row < (m3_usize)(height / 2); row += 1u) {
        for (col = 0u; col < (m3_usize)(width / 2); col += 1u) {
            ...
            state->frame_data[uv_index] = u_plane[row * (m3_usize)u_row_stride + col * (m3_usize)u_pixel_stride];
            state->frame_data[uv_index + 1u] = v_plane[row * (m3_usize)v_row_stride + col * (m3_usize)v_pixel_stride];
        }
    }
...
static void m3_android_camera_on_disconnected(void *context, ACameraDevice *device)
{
    M3AndroidCameraState *state;
...
static void m3_android_camera_on_error(void *context, ACameraDevice *device, int error)
{
    M3AndroidCameraState *state;
...
#if M3_ANDROID_CAMERA2_AVAILABLE
static int m3_android_camera_select_index(ACameraManager *manager, const M3CameraConfig *config,
    const ACameraIdList *id_list, m3_u32 *out_index)
{
    m3_u32 desired;
    m3_u32 i;

    if (manager == NULL || config == NULL || id_list == NULL || out_index == NULL) {
        return M3_ERR_INVALID_ARGUMENT;
    }
    ...
    for (i = 0u; i < (m3_u32)id_list->numCameras; i += 1u) {
        ...
        metadata = NULL;
        status = ACameraManager_getCameraCharacteristics(manager, id_list->cameraIds[i], &metadata);
        ...
        if (ACameraMetadata_getConstEntry(metadata, ACAMERA_LENS_FACING, &entry) == ACAMERA_OK && entry.count > 0u) {
...
static int m3_android_camera_open_with_config(void *camera, const M3CameraConfig *config);
...
static int m3_android_camera_open_with_config(void *camera, const M3CameraConfig *config)
{
    ...
    if (config->format != M3_CAMERA_FORMAT_ANY && config->format != M3_CAMERA_FORMAT_NV12) {
        return M3_ERR_UNSUPPORTED;
    }

    state->backend = backend;
    state->width = (config->width == 0u) ? M3_ANDROID_CAMERA_DEFAULT_WIDTH : config->width;
    state->height = (config->height == 0u) ? M3_ANDROID_CAMERA_DEFAULT_HEIGHT : config->height;
    state->format = M3_CAMERA_FORMAT_NV12;
...

    {
        ...
        rc = m3_android_camera_select_index(state->manager, config, id_list, &selected_index);
        if (rc != M3_OK) {
        ...
        status = AImageReader_new((int)state->width, (int)state->height, AIMAGE_FORMAT_YUV_420_888, 2, &state->reader);
        if (status != ACAMERA_OK) {
            ACameraManager_deleteCameraIdList(id_list);
            ACameraManager_delete(state->manager);
            state->manager = NULL;
...

        {
            ...
            status = ACameraManager_openCamera(state->manager, id_list->cameraIds[selected_index], &callbacks,
                &state->device);
...
static int m3_android_camera_start(void *camera)
{
...

    {
        ...
        status = ACaptureSessionOutput_create(state->reader_window, &state->session_output);
        if (status != ACAMERA_OK) {
            ACaptureSessionOutputContainer_free(state->output_container);
            state->output_container = NULL;
            return m3_android_camera_status_to_error(status);
        ...
        status = ACaptureSessionOutputContainer_add(state->output_container, state->session_output);
        if (status != ACAMERA_OK) {
            ACaptureSessionOutput_free(state->session_output);
            state->session_output = NULL;
            ACaptureSessionOutputContainer_free(state->output_container);
        ...
        status = ACameraOutputTarget_create(state->reader_window, &state->output_target);
        if (status != ACAMERA_OK) {
            ACaptureSessionOutput_free(state->session_output);
            state->session_output = NULL;
            ACaptureSessionOutputContainer_free(state->output_container);
        ...
        status = ACameraDevice_createCaptureRequest(state->device, TEMPLATE_PREVIEW, &state->request);
        if (status != ACAMERA_OK) {
            ACameraOutputTarget_free(state->output_target);
            state->output_target = NULL;
            ACaptureSessionOutput_free(state->session_output);
        ...
        status = ACameraDevice_createCaptureSession(state->device, state->output_container, &callbacks, &state->session);
        if (status != ACAMERA_OK) {
            ACaptureRequest_free(state->request);
            state->request = NULL;
            ACameraOutputTarget_free(state->output_target);
        ...
        status = ACameraCaptureSession_setRepeatingRequest(state->session, NULL, 1, &state->request, NULL);
        if (status != ACAMERA_OK) {
...
static int m3_android_camera_read_frame(void *camera, M3CameraFrame *out_frame, M3Bool *out_has_frame)
{
    struct M3AndroidBackend *backend;
    M3AndroidCameraState *state;
    int rc;
...
static const M3CameraVTable g_m3_android_camera_vtable = {
    m3_android_camera_open,
    m3_android_camera_open_with_config,
    m3_android_camera_close,
    m3_android_camera_start,
    m3_android_camera_stop,
    m3_android_camera_read_frame
};
...
static int m3_android_env_get_network(void *env, M3Network *out_network)
{
    ...
    backend = (struct M3AndroidBackend *)env;
    if (backend->null_env.vtable == NULL || backend->null_env.vtable->get_network == NULL) {
        return M3_ERR_STATE;
    }
    return backend->null_env.vtable->get_network(backend->null_env.ctx, out_network);
}
...
static const M3EnvVTable g_m3_android_env_vtable = {
    m3_android_env_get_io,
    m3_android_env_get_sensors,
    m3_android_env_get_camera,
...
int M3_CALL m3_android_backend_get_gfx(M3AndroidBackend *backend, M3Gfx *out_gfx)
{
...
int M3_CALL m3_android_backend_get_env(M3AndroidBackend *backend, M3Env *out_env)
{
