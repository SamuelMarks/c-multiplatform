...
static int m3_web_backend_log(struct M3WebBackend *backend, M3LogLevel level, const char *message)
{
...
static int m3_web_backend_resolve(struct M3WebBackend *backend, M3Handle handle, m3_u32 type_id, void **out_obj)
{
    void *resolved;
    m3_u32 actual_type;
    int rc;
...
static int m3_web_backend_copy_string(const M3Allocator *allocator, const char *src, char **out_dst)
{
    m3_usize length;
    m3_usize max_value;
    char *mem;
...
static int m3_web_event_queue_push(M3WebEventQueue *queue, const M3InputEvent *event)
{
    m3_usize index;
...
static int m3_web_event_queue_pop(M3WebEventQueue *queue, M3InputEvent *out_event, M3Bool *out_has_event)
{
    m3_usize index;
...
static int m3_web_build_modifiers(m3_u32 *out_modifiers, int shift, int ctrl, int alt, int meta)
{
    m3_u32 mods;
...
static int m3_web_event_stamp(struct M3WebBackend *backend, M3InputEvent *event)
{
    int rc;
...
static int m3_web_gl_check_error(struct M3WebBackend *backend, const char *label)
{
    GLenum err;
    int rc;
...
static int m3_web_gl_compile_shader(struct M3WebBackend *backend, GLenum type, const char *source, GLuint *out_shader)
{
    GLuint shader;
    GLint status;
    char info[256];
...
static int m3_web_gl_link_program(struct M3WebBackend *backend, GLuint vert, GLuint frag, GLuint *out_program)
{
    GLuint program;
    GLint status;
    char info[256];
...
static int m3_web_gl_init(struct M3WebBackend *backend)
{
    ...
    rc = m3_web_gl_compile_shader(backend, GL_VERTEX_SHADER, vert_src, &vert_shader);
    if (rc != M3_OK) {
        emscripten_webgl_destroy_context(ctx);
        return rc;
    }

    rc = m3_web_gl_compile_shader(backend, GL_FRAGMENT_SHADER, frag_src, &frag_shader);
    if (rc != M3_OK) {
    ...
    if (gl->attr_pos < 0 || gl->attr_uv < 0 || gl->uni_color < 0 || gl->uni_use_tex < 0 || gl->uni_tex < 0 || gl->uni_opacity < 0 ||
        gl->uni_transform < 0 || gl->uni_viewport < 0 || gl->uni_rect < 0 || gl->uni_radius < 0) {
        glDeleteProgram(program);
        emscripten_webgl_destroy_context(ctx);
    ...
    glBufferData(GL_ARRAY_BUFFER, (GLsizeiptr)(sizeof(M3WebVertex) * 4u), NULL, GL_DYNAMIC_DRAW);
...
static int m3_web_gl_prepare(struct M3WebBackend *backend)
{
    ...
    glVertexAttribPointer((GLuint)backend->gl.attr_pos, 2, GL_FLOAT, GL_FALSE, (GLsizei)sizeof(M3WebVertex), (void *)0);
    glVertexAttribPointer((GLuint)backend->gl.attr_uv, 2, GL_FLOAT, GL_FALSE, (GLsizei)sizeof(M3WebVertex), (void *)(sizeof(float) * 2u));
    ...
    glUniform2f(backend->gl.uni_viewport, (float)backend->viewport_width, (float)backend->viewport_height);
...
static const char *g_m3_web_wgpu_shader =
    ...
    "  out.pos = p.xy;\n"
    "  out.uv = input.uv;\n"
    "  let clip = vec2<f32>((p.x / u.viewport.x) * 2.0 - 1.0, 1.0 - (p.y / u.viewport.y) * 2.0);\n"
...
static int m3_web_wgpu_align_size(m3_usize value, m3_usize alignment, m3_usize *out_value)
{
    m3_usize remainder;
    m3_usize add;
    m3_usize max_value;
...
static int m3_web_wgpu_init(struct M3WebBackend *backend)
{
    ...
    wgpu->bind_group_layout = wgpuDeviceCreateBindGroupLayout(device, &layout_desc);
    if (wgpu->bind_group_layout == NULL) {
    ...
    wgpuQueueWriteTexture(queue, &copy_dst, white_row, (m3_usize)M3_WEB_WGPU_ROW_ALIGN, &data_layout, &extent);
...
static int m3_web_wgpu_configure_surface(struct M3WebBackend *backend, m3_i32 width, m3_i32 height)
{
    M3WebGPUState *wgpu;
    ...
    if (wgpu->configured && wgpu->config_width == width && wgpu->config_height == height) {
        return M3_OK;
    }
...
static int m3_web_wgpu_apply_clip(struct M3WebBackend *backend)
{
    ...
    if (wgpu->pass_active && wgpu->pass != NULL) {
        wgpuRenderPassEncoderSetScissorRect(wgpu->pass, wgpu->scissor_x, wgpu->scissor_y, wgpu->scissor_w, wgpu->scissor_h);
        wgpu->scissor_dirty = M3_FALSE;
    }
...
static int m3_web_wgpu_prepare_uniforms(struct M3WebBackend *backend, const M3Rect *rect, M3Color color, M3Scalar opacity,
                                        M3Scalar radius, M3Bool use_texture, M3Bool alpha_only, M3WebGPUUniforms *out_uniforms)
{
    M3Mat3 transform;
    int rc;
...
static int m3_web_wgpu_draw_quad(struct M3WebBackend *backend, const M3WebVertex *verts, M3Color color, M3Bool use_texture,
                                 const M3WebTexture *texture, M3Scalar opacity, const M3Rect *rect, M3Scalar radius)
{
    M3WebGPUState *wgpu;
    ...
    wgpu = &backend->wgpu;
    if (!wgpu->initialized || wgpu->queue == NULL || wgpu->uniform_buffer == NULL || wgpu->vertex_buffer == NULL) {
        return M3_ERR_STATE;
    }
    ...
    bind_group = use_texture ? texture->wgpu_bind_group : wgpu->default_bind_group;
    if (bind_group == NULL) {
        return M3_ERR_STATE;
    }

    rc = m3_web_wgpu_prepare_uniforms(backend, rect, color, opacity, radius, use_texture,
        use_texture ? texture->wgpu_alpha_only : M3_FALSE, &uniforms);
    M3_WEB_RETURN_IF_ERROR(rc);

    wgpuQueueWriteBuffer(wgpu->queue, wgpu->uniform_buffer, 0u, &uniforms, (m3_usize)sizeof(uniforms));
    wgpuQueueWriteBuffer(wgpu->queue, wgpu->vertex_buffer, 0u, verts, (m3_usize)(sizeof(M3WebVertex) * 4u));
    ...
    wgpuRenderPassEncoderSetVertexBuffer(wgpu->pass, 0u, wgpu->vertex_buffer, 0u, (m3_usize)(sizeof(M3WebVertex) * 4u));
    wgpuRenderPassEncoderDraw(wgpu->pass, 4u, 1u, 0u, 0u);

    return M3_OK;
}

static int m3_web_wgpu_begin_frame(struct M3WebBackend *backend, M3WebWindow *resolved, m3_i32 width, m3_i32 height, M3Scalar dpi_scale)
{
    m3_i32 device_width;
    m3_i32 device_height;
    int rc;
    ...
    if (resolved->width != width || resolved->height != height || resolved->dpi_scale != dpi_scale) {
...
static int m3_web_gfx_draw_quad(struct M3WebBackend *backend, const M3WebVertex *verts, M3Color color, M3Bool use_texture, GLuint texture,
                                M3Scalar opacity, const M3Rect *rect, M3Scalar radius)
{
    int rc;
    ...
    glUniform4f(backend->gl.uni_rect, rect->x, rect->y, rect->width, rect->height);
    ...
    glBufferData(GL_ARRAY_BUFFER, (GLsizeiptr)(sizeof(M3WebVertex) * 4u), verts, GL_DYNAMIC_DRAW);
    glDrawArrays(GL_TRIANGLE_STRIP, 0, 4);

    return m3_web_gl_check_error(backend, "gfx.draw_quad");
}

static int m3_web_backend_set_canvas_size(struct M3WebBackend *backend, m3_i32 width, m3_i32 height, M3Scalar dpi_scale)
{
    m3_i32 device_width;
    m3_i32 device_height;
    int result;
    ...
    result = emscripten_set_canvas_element_size(backend->canvas_id, device_width, device_height);
    if (result != EMSCRIPTEN_RESULT_SUCCESS) {
        return m3_web_backend_map_result(result);
    }

    result = emscripten_set_element_css_size(backend->canvas_id, (double)width, (double)height);
    if (result != EMSCRIPTEN_RESULT_SUCCESS) {
        return m3_web_backend_map_result(result);
    }
...
static EM_BOOL m3_web_on_mouse_down(int event_type, const EmscriptenMouseEvent *event, void *user_data)
{
    struct M3WebBackend *backend;
    M3InputEvent input;
    m3_u32 mods;
    ...
    rc = m3_web_build_modifiers(&mods, event->shiftKey, event->ctrlKey, event->altKey, event->metaKey);
    if (rc != M3_OK) {
        return EM_FALSE;
    }
...
static EM_BOOL m3_web_on_mouse_up(int event_type, const EmscriptenMouseEvent *event, void *user_data)
{
    struct M3WebBackend *backend;
    M3InputEvent input;
    m3_u32 mods;
...
static EM_BOOL m3_web_on_mouse_move(int event_type, const EmscriptenMouseEvent *event, void *user_data)
{
    struct M3WebBackend *backend;
    M3InputEvent input;
    m3_u32 mods;
...
static EM_BOOL m3_web_on_wheel(int event_type, const EmscriptenWheelEvent *event, void *user_data)
{
    struct M3WebBackend *backend;
    M3InputEvent input;
    m3_u32 mods;
    ...
    rc = m3_web_build_modifiers(&mods, event->mouse.shiftKey, event->mouse.ctrlKey, event->mouse.altKey, event->mouse.metaKey);
    if (rc != M3_OK) {
        return EM_FALSE;
    }
...
static EM_BOOL m3_web_on_touch(int event_type, const EmscriptenTouchEvent *event, void *user_data)
{
    struct M3WebBackend *backend;
    m3_u32 mods;
    int rc;
    ...
    for (i = 0; i < event->numTouches; i += 1) {
        ...
        buttons = 1;
        if (event_type == EMSCRIPTEN_EVENT_TOUCHEND || event_type == EMSCRIPTEN_EVENT_TOUCHCANCEL) {
            buttons = 0;
        }
...
static EM_BOOL m3_web_on_key_down(int event_type, const EmscriptenKeyboardEvent *event, void *user_data)
{
    struct M3WebBackend *backend;
    M3InputEvent input;
    m3_u32 mods;
...
static EM_BOOL m3_web_on_key_up(int event_type, const EmscriptenKeyboardEvent *event, void *user_data)
{
    struct M3WebBackend *backend;
    M3InputEvent input;
    m3_u32 mods;
...
static EM_BOOL m3_web_on_key_press(int event_type, const EmscriptenKeyboardEvent *event, void *user_data)
{
    struct M3WebBackend *backend;
    M3InputEvent input;
    m3_u32 mods;
...
static EM_BOOL m3_web_on_focus(int event_type, const EmscriptenFocusEvent *event, void *user_data)
{
    struct M3WebBackend *backend;
    M3InputEvent input;
    int rc;
...
static EM_BOOL m3_web_on_blur(int event_type, const EmscriptenFocusEvent *event, void *user_data)
{
    struct M3WebBackend *backend;
    M3InputEvent input;
    int rc;
...
static EM_BOOL m3_web_on_resize(int event_type, const EmscriptenUiEvent *event, void *user_data)
{
    struct M3WebBackend *backend;
    M3WebWindow *window;
    M3InputEvent input;
    ...
    if (backend->event_window.id == 0u && backend->event_window.generation == 0u) {
        return EM_FALSE;
    }

    window = NULL;
    rc = m3_web_backend_resolve(backend, backend->event_window, M3_WEB_TYPE_WINDOW, (void **)&window);
    ...
    css_width = 0.0;
    css_height = 0.0;
    result = emscripten_get_element_css_size(backend->canvas_id, &css_width, &css_height);
    if (result != EMSCRIPTEN_RESULT_SUCCESS) {
        return EM_FALSE;
    }
    ...
    rc = m3_web_backend_set_canvas_size(backend, width, height, window->dpi_scale);
    if (rc != M3_OK) {
        return EM_FALSE;
    }
...
static int m3_web_register_callbacks(struct M3WebBackend *backend)
{
    ...
    result = emscripten_set_mousedown_callback(backend->canvas_id, backend, 1, m3_web_on_mouse_down);
    if (result != EMSCRIPTEN_RESULT_SUCCESS) {
        return m3_web_backend_map_result(result);
    }
    result = emscripten_set_mouseup_callback(backend->canvas_id, backend, 1, m3_web_on_mouse_up);
    if (result != EMSCRIPTEN_RESULT_SUCCESS) {
        return m3_web_backend_map_result(result);
    }
    result = emscripten_set_mousemove_callback(backend->canvas_id, backend, 1, m3_web_on_mouse_move);
    if (result != EMSCRIPTEN_RESULT_SUCCESS) {
        return m3_web_backend_map_result(result);
    }
    result = emscripten_set_wheel_callback(backend->canvas_id, backend, 1, m3_web_on_wheel);
    if (result != EMSCRIPTEN_RESULT_SUCCESS) {
        return m3_web_backend_map_result(result);
    }

    result = emscripten_set_touchstart_callback(backend->canvas_id, backend, 1, m3_web_on_touch);
    if (result != EMSCRIPTEN_RESULT_SUCCESS) {
        return m3_web_backend_map_result(result);
    }
    result = emscripten_set_touchend_callback(backend->canvas_id, backend, 1, m3_web_on_touch);
    if (result != EMSCRIPTEN_RESULT_SUCCESS) {
        return m3_web_backend_map_result(result);
    }
    result = emscripten_set_touchmove_callback(backend->canvas_id, backend, 1, m3_web_on_touch);
    if (result != EMSCRIPTEN_RESULT_SUCCESS) {
        return m3_web_backend_map_result(result);
    }
    result = emscripten_set_touchcancel_callback(backend->canvas_id, backend, 1, m3_web_on_touch);
    if (result != EMSCRIPTEN_RESULT_SUCCESS) {
        return m3_web_backend_map_result(result);
    }

    result = emscripten_set_keydown_callback(EMSCRIPTEN_EVENT_TARGET_WINDOW, backend, 1, m3_web_on_key_down);
    if (result != EMSCRIPTEN_RESULT_SUCCESS) {
        return m3_web_backend_map_result(result);
    }
    result = emscripten_set_keyup_callback(EMSCRIPTEN_EVENT_TARGET_WINDOW, backend, 1, m3_web_on_key_up);
    if (result != EMSCRIPTEN_RESULT_SUCCESS) {
        return m3_web_backend_map_result(result);
    }
    result = emscripten_set_keypress_callback(EMSCRIPTEN_EVENT_TARGET_WINDOW, backend, 1, m3_web_on_key_press);
    if (result != EMSCRIPTEN_RESULT_SUCCESS) {
        return m3_web_backend_map_result(result);
    }

    result = emscripten_set_focus_callback(EMSCRIPTEN_EVENT_TARGET_WINDOW, backend, 1, m3_web_on_focus);
    if (result != EMSCRIPTEN_RESULT_SUCCESS) {
        return m3_web_backend_map_result(result);
    }
    result = emscripten_set_blur_callback(EMSCRIPTEN_EVENT_TARGET_WINDOW, backend, 1, m3_web_on_blur);
    if (result != EMSCRIPTEN_RESULT_SUCCESS) {
        return m3_web_backend_map_result(result);
    }
    result = emscripten_set_resize_callback(EMSCRIPTEN_EVENT_TARGET_WINDOW, backend, 1, m3_web_on_resize);
    if (result != EMSCRIPTEN_RESULT_SUCCESS) {
        return m3_web_backend_map_result(result);
    }

    backend->callbacks_registered = M3_TRUE;
    rc = m3_web_backend_log(backend, M3_LOG_LEVEL_INFO, "ws.callbacks_registered");
    M3_WEB_RETURN_IF_ERROR(rc);
    return M3_OK;
}
...
static const M3ObjectVTable g_m3_web_window_vtable = {
    m3_web_object_retain,
    m3_web_object_release,
    m3_web_window_destroy,
...
static int m3_web_ws_create_window(void *ws, const M3WSWindowConfig *config, M3Handle *out_window)
{
    struct M3WebBackend *backend;
    M3WebWindow *window;
    double ratio;
    ...
    rc = backend->allocator.alloc(backend->allocator.ctx, sizeof(M3WebWindow), (void **)&window);
    M3_WEB_RETURN_IF_ERROR(rc);
    ...
    rc = m3_object_header_init(&window->header, M3_WEB_TYPE_WINDOW, 0, &g_m3_web_window_vtable);
    M3_WEB_RETURN_IF_ERROR_CLEANUP(rc, backend->allocator.free(backend->allocator.ctx, window));

    rc = backend->handles.vtable->register_object(backend->handles.ctx, &window->header);
    M3_WEB_RETURN_IF_ERROR_CLEANUP(rc, backend->allocator.free(backend->allocator.ctx, window));

    rc = m3_web_backend_set_canvas_size(backend, window->width, window->height, window->dpi_scale);
...
static int m3_web_ws_destroy_window(void *ws, M3Handle window)
{
    ...
    backend = (struct M3WebBackend *)ws;
    rc = m3_web_backend_resolve(backend, window, M3_WEB_TYPE_WINDOW, (void **)&resolved);
    M3_WEB_RETURN_IF_ERROR(rc);

    rc = m3_web_backend_log(backend, M3_LOG_LEVEL_INFO, "ws.destroy_window");
    M3_WEB_RETURN_IF_ERROR(rc);

    if (backend->event_window.id == window.id && backend->event_window.generation == window.generation) {
...
static int m3_web_ws_set_window_title(void *ws, M3Handle window, const char *utf8_title)
{
    struct M3WebBackend *backend;
    int rc;
...
static int m3_web_ws_set_window_size(void *ws, M3Handle window, m3_i32 width, m3_i32 height)
{
    struct M3WebBackend *backend;
    M3WebWindow *resolved;
    int rc;
    ...
    rc = m3_web_backend_set_canvas_size(backend, width, height, resolved->dpi_scale);
    M3_WEB_RETURN_IF_ERROR(rc);
...
static int m3_web_ws_get_window_size(void *ws, M3Handle window, m3_i32 *out_width, m3_i32 *out_height)
{
    struct M3WebBackend *backend;
    M3WebWindow *resolved;
    int rc;
...
static int m3_web_ws_set_window_dpi_scale(void *ws, M3Handle window, M3Scalar scale)
{
    struct M3WebBackend *backend;
    M3WebWindow *resolved;
    int rc;
    ...
    rc = m3_web_backend_log(backend, M3_LOG_LEVEL_INFO, "ws.set_window_dpi_scale");
    M3_WEB_RETURN_IF_ERROR(rc);

    resolved->dpi_scale = scale;
    rc = m3_web_backend_set_canvas_size(backend, resolved->width, resolved->height, resolved->dpi_scale);
    M3_WEB_RETURN_IF_ERROR(rc);

    return M3_OK;
}

static int m3_web_ws_get_window_dpi_scale(void *ws, M3Handle window, M3Scalar *out_scale)
{
    struct M3WebBackend *backend;
    M3WebWindow *resolved;
    int rc;
    ...
    rc = m3_web_backend_log(backend, M3_LOG_LEVEL_INFO, "ws.get_window_dpi_scale");
    M3_WEB_RETURN_IF_ERROR(rc);
...
static int m3_web_ws_get_clipboard_text(void *ws, char *buffer, m3_usize buffer_size, m3_usize *out_length)
{
    struct M3WebBackend *backend;
    m3_usize required;
    int rc;
...
static int m3_web_ws_poll_event(void *ws, M3InputEvent *out_event, M3Bool *out_has_event)
{
    struct M3WebBackend *backend;
    int rc;
...
static const M3WSVTable g_m3_web_ws_vtable = {
    m3_web_ws_init,
    m3_web_ws_shutdown,
...
static int m3_web_gfx_begin_frame(void *gfx, M3Handle window, m3_i32 width, m3_i32 height, M3Scalar dpi_scale)
{
    struct M3WebBackend *backend;
    M3WebWindow *resolved;
    m3_i32 device_width;
...
static int m3_web_gfx_draw_rect(void *gfx, const M3Rect *rect, M3Color color, M3Scalar corner_radius)
{
    struct M3WebBackend *backend;
    M3WebVertex verts[4];
    M3Rect rect_copy;
    ...
    rect_copy = *rect;
    max_radius = rect_copy.width < rect_copy.height ? rect_copy.width : rect_copy.height;
...
#if defined(M3_WEBGPU_AVAILABLE)
    if (backend->use_webgpu) {
        return m3_web_wgpu_draw_quad(backend, verts, color, M3_FALSE, NULL, 1.0f, &rect_copy, radius);
    }
#endif

    return m3_web_gfx_draw_quad(backend, verts, color, M3_FALSE, 0, 1.0f, &rect_copy, radius);
}

static int m3_web_gfx_draw_line(void *gfx, M3Scalar x0, M3Scalar y0, M3Scalar x1, M3Scalar y1, M3Color color, M3Scalar thickness)
{
    struct M3WebBackend *backend;
    M3WebVertex verts[4];
    M3Rect rect;
...
#if defined(M3_WEBGPU_AVAILABLE)
    if (backend->use_webgpu) {
        return m3_web_wgpu_draw_quad(backend, verts, color, M3_FALSE, NULL, 1.0f, &rect, 0.0f);
    }
#endif

    return m3_web_gfx_draw_quad(backend, verts, color, M3_FALSE, 0, 1.0f, &rect, 0.0f);
}
...
static int m3_web_gfx_push_clip(void *gfx, const M3Rect *rect)
{
    ...
    if (backend->clip_depth == 0) {
        ...
        rc = m3_rect_intersect(&backend->clip_stack[backend->clip_depth - 1u], rect, &clipped, &has_intersection);
        M3_WEB_RETURN_IF_ERROR(rc);
        if (!has_intersection) {
            clipped.x = rect->x;
            clipped.y = rect->y;
...
static int m3_web_convert_bgra(const M3Allocator *allocator, const void *pixels, m3_usize size, void **out_pixels)
{
    unsigned char *dst;
...
#if defined(M3_WEBGPU_AVAILABLE)
static int m3_web_wgpu_write_texture(struct M3WebBackend *backend, M3WebTexture *texture, m3_i32 x, m3_i32 y, m3_i32 width, m3_i32 height,
                                     const void *pixels, m3_usize size)
{
    ...
    rc = m3_web_mul_size((m3_usize)width, (m3_usize)texture->bytes_per_pixel, &row_bytes);
    M3_WEB_RETURN_IF_ERROR(rc);
    ...
    rc = m3_web_wgpu_align_size(row_bytes, (m3_usize)M3_WEB_WGPU_ROW_ALIGN, &padded_row_bytes);
    M3_WEB_RETURN_IF_ERROR(rc);
    ...
    if (padded_row_bytes != row_bytes) {
        rc = backend->allocator.alloc(backend->allocator.ctx, padded_size, (void **)&staging);
        M3_WEB_RETURN_IF_ERROR(rc);

        src = (const unsigned char *)pixels;
        for (row = 0; row < height; ++row) {
            memcpy(staging + (m3_usize)row * padded_row_bytes, src + (m3_usize)row * row_bytes, row_bytes);
            if (padded_row_bytes > row_bytes) {
                memset(staging + (m3_usize)row * padded_row_bytes + row_bytes, 0, padded_row_bytes - row_bytes);
            }
        }
    }
    ...
    wgpuQueueWriteTexture(backend->wgpu.queue, &dst, staging, padded_size, &layout, &extent);
...
static int m3_web_wgpu_create_texture(struct M3WebBackend *backend, m3_i32 width, m3_i32 height, m3_u32 format, const void *pixels,
                                      m3_usize size, M3Handle *out_texture)
{
    ...
    if (backend->wgpu.device == NULL || backend->wgpu.bind_group_layout == NULL || backend->wgpu.uniform_buffer == NULL ||
        backend->wgpu.sampler == NULL) {
        return M3_ERR_STATE;
    }
    ...
    rc = backend->allocator.alloc(backend->allocator.ctx, sizeof(M3WebTexture), (void **)&texture);
    M3_WEB_RETURN_IF_ERROR(rc);
    ...
    texture->wgpu_texture = wgpuDeviceCreateTexture(backend->wgpu.device, &texture_desc);
    if (texture->wgpu_texture == NULL) {
    ...
    texture->wgpu_bind_group = wgpuDeviceCreateBindGroup(backend->wgpu.device, &bind_desc);
    if (texture->wgpu_bind_group == NULL) {
        wgpuTextureViewRelease(texture->wgpu_view);
        wgpuTextureRelease(texture->wgpu_texture);
        backend->allocator.free(backend->allocator.ctx, texture);
    ...
    if (pixels != NULL && size != 0) {
        rc = m3_web_wgpu_write_texture(backend, texture, 0, 0, width, height, pixels, size);
        if (rc != M3_OK) {
            wgpuBindGroupRelease(texture->wgpu_bind_group);
            wgpuTextureViewRelease(texture->wgpu_view);
            wgpuTextureRelease(texture->wgpu_texture);
    ...
    rc = m3_object_header_init(&texture->header, M3_WEB_TYPE_TEXTURE, 0, &g_m3_web_texture_vtable);
    if (rc != M3_OK) {
        wgpuBindGroupRelease(texture->wgpu_bind_group);
        wgpuTextureViewRelease(texture->wgpu_view);
        wgpuTextureRelease(texture->wgpu_texture);
...
static int m3_web_wgpu_update_texture(struct M3WebBackend *backend, M3WebTexture *resolved, m3_i32 x, m3_i32 y, m3_i32 width,
                                      m3_i32 height, const void *pixels, m3_usize size)
{
    int rc;
    ...
    rc = m3_web_wgpu_write_texture(backend, resolved, x, y, width, height, pixels, size);
    M3_WEB_RETURN_IF_ERROR(rc);
...
static int m3_web_gfx_create_texture(void *gfx, m3_i32 width, m3_i32 height, m3_u32 format, const void *pixels, m3_usize size,
                                     M3Handle *out_texture)
{
...
#if defined(M3_WEBGPU_AVAILABLE)
    if (backend->use_webgpu) {
        return m3_web_wgpu_create_texture(backend, width, height, format, pixels, size, out_texture);
    }
#endif
    ...
    if (format == M3_TEX_FORMAT_BGRA8 && pixels != NULL && size != 0) {
        rc = m3_web_convert_bgra(&backend->allocator, pixels, expected_size, &upload_pixels);
        if (rc != M3_OK) {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, gl_format, width, height, 0, gl_format, GL_UNSIGNED_BYTE, upload_pixels);
...
static int m3_web_gfx_update_texture(void *gfx, M3Handle texture, m3_i32 x, m3_i32 y, m3_i32 width, m3_i32 height, const void *pixels,
                                     m3_usize size)
{
    struct M3WebBackend *backend;
    ...
    backend = (struct M3WebBackend *)gfx;
    rc = m3_web_backend_resolve(backend, texture, M3_WEB_TYPE_TEXTURE, (void **)&resolved);
    M3_WEB_RETURN_IF_ERROR(rc);
...
#if defined(M3_WEBGPU_AVAILABLE)
    if (backend->use_webgpu) {
        return m3_web_wgpu_update_texture(backend, resolved, x, y, width, height, pixels, size);
    }
#endif
    ...
    rc = m3_web_mul_size((m3_usize)width, (m3_usize)height, &row_size);
    M3_WEB_RETURN_IF_ERROR(rc);
    rc = m3_web_mul_size(row_size, (m3_usize)resolved->bytes_per_pixel, &expected_size);
    M3_WEB_RETURN_IF_ERROR(rc);
    ...
    glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
    glTexSubImage2D(GL_TEXTURE_2D, 0, x, y, width, height, gl_format, GL_UNSIGNED_BYTE, upload_pixels);
...
static int m3_web_gfx_draw_texture(void *gfx, M3Handle texture, const M3Rect *src, const M3Rect *dst, M3Scalar opacity)
{
    struct M3WebBackend *backend;
    M3WebTexture *resolved;
    M3WebVertex verts[4];
    ...
    if (src->x < 0.0f || src->y < 0.0f || src->width < 0.0f || src->height < 0.0f) {
        return M3_ERR_RANGE;
    }
    if (src->x + src->width > (M3Scalar)resolved->width || src->y + src->height > (M3Scalar)resolved->height) {
        return M3_ERR_RANGE;
    }
...
#if defined(M3_WEBGPU_AVAILABLE)
    if (backend->use_webgpu) {
        return m3_web_wgpu_draw_quad(backend, verts, color, M3_TRUE, resolved, opacity, &rect_copy, 0.0f);
    }
#endif

    return m3_web_gfx_draw_quad(backend, verts, color, M3_TRUE, resolved->gl_texture, opacity, &rect_copy, 0.0f);
}

static const M3GfxVTable g_m3_web_gfx_vtable = {
    m3_web_gfx_begin_frame,
    m3_web_gfx_end_frame,
    m3_web_gfx_clear,
    m3_web_gfx_draw_rect,
    m3_web_gfx_draw_line,
    m3_web_gfx_push_clip,
    m3_web_gfx_pop_clip,
    m3_web_gfx_set_transform,
    m3_web_gfx_create_texture,
    m3_web_gfx_update_texture,
    m3_web_gfx_destroy_texture,
    m3_web_gfx_draw_texture
};

static int m3_web_text_create_font(void *text, const char *utf8_family, m3_i32 size_px, m3_i32 weight, M3Bool italic, M3Handle *out_font)
{
    struct M3WebBackend *backend;
    M3WebFont *font;
    int rc;
    ...
    rc = backend->allocator.alloc(backend->allocator.ctx, sizeof(M3WebFont), (void **)&font);
    M3_WEB_RETURN_IF_ERROR(rc);
    ...
    rc = m3_object_header_init(&font->header, M3_WEB_TYPE_FONT, 0, &g_m3_web_font_vtable);
    M3_WEB_RETURN_IF_ERROR_CLEANUP(rc, backend->allocator.free(backend->allocator.ctx, font));
...
static int m3_web_text_destroy_font(void *text, M3Handle font)
{
    ...
    backend = (struct M3WebBackend *)text;
    rc = m3_web_backend_resolve(backend, font, M3_WEB_TYPE_FONT, (void **)&resolved);
    M3_WEB_RETURN_IF_ERROR(rc);
...
static int m3_web_text_measure_text(void *text, M3Handle font, const char *utf8, m3_usize utf8_len, M3Scalar *out_width, M3Scalar *out_height,
...
static int m3_web_text_draw_text(void *text, M3Handle font, const char *utf8, m3_usize utf8_len, M3Scalar x, M3Scalar y, M3Color color)
{
    struct M3WebBackend *backend;
    int rc;
...
static const M3TextVTable g_m3_web_text_vtable = {
    m3_web_text_create_font,
    m3_web_text_destroy_font,
    m3_web_text_measure_text,
    m3_web_text_draw_text
};

static int m3_web_io_read_file(void *io, const char *utf8_path, void *buffer, m3_usize buffer_size, m3_usize *out_read)
{
    struct M3WebBackend *backend;
    int rc;
...
static int m3_web_io_read_file_alloc(void *io, const char *utf8_path, const M3Allocator *allocator, void **out_data, m3_usize *out_size)
{
    struct M3WebBackend *backend;
    int rc;
...
static int m3_web_io_write_file(void *io, const char *utf8_path, const void *data, m3_usize size, M3Bool overwrite)
{
    struct M3WebBackend *backend;
    int rc;
...
static int m3_web_io_file_exists(void *io, const char *utf8_path, M3Bool *out_exists)
{
    struct M3WebBackend *backend;
    int rc;
...
static int m3_web_io_stat_file(void *io, const char *utf8_path, M3FileInfo *out_info)
{
    struct M3WebBackend *backend;
    int rc;
...
static const M3IOVTable g_m3_web_io_vtable = {
    m3_web_io_read_file,
    m3_web_io_read_file_alloc,
    m3_web_io_write_file,
    m3_web_io_file_exists,
    m3_web_io_delete_file,
    m3_web_io_stat_file
};

static int m3_web_sensors_is_available(void *sensors, m3_u32 type, M3Bool *out_available)
{
    struct M3WebBackend *backend;
    int rc;
...
static int m3_web_sensors_read(void *sensors, m3_u32 type, M3SensorReading *out_reading, M3Bool *out_has_reading)
{
    struct M3WebBackend *backend;
    int rc;
...
static const M3SensorsVTable g_m3_web_sensors_vtable = {
    m3_web_sensors_is_available,
    m3_web_sensors_start,
    m3_web_sensors_stop,
    m3_web_sensors_read
};
...
static int m3_web_camera_open_with_config(void *camera, const M3CameraConfig *config)
{
    if (config == NULL) {
        return M3_ERR_INVALID_ARGUMENT;
    }
...
static int m3_web_camera_read_frame(void *camera, M3CameraFrame *out_frame, M3Bool *out_has_frame)
{
    struct M3WebBackend *backend;
    int rc;
...
static const M3CameraVTable g_m3_web_camera_vtable = {
    m3_web_camera_open,
    m3_web_camera_open_with_config,
    m3_web_camera_close,
    m3_web_camera_start,
    m3_web_camera_stop,
    m3_web_camera_read_frame
};

static int m3_web_network_request(void *net, const M3NetworkRequest *request, const M3Allocator *allocator, M3NetworkResponse *out_response)
{
    struct M3WebBackend *backend;
    int rc;
...
static int m3_web_network_free_response(void *net, const M3Allocator *allocator, M3NetworkResponse *response)
{
    struct M3WebBackend *backend;
    int rc;
...
static const M3NetworkVTable g_m3_web_network_vtable = {
    m3_web_network_request,
    m3_web_network_free_response
};

static int m3_web_tasks_thread_create(void *tasks, M3ThreadFn entry, void *user, M3Handle *out_thread)
{
    struct M3WebBackend *backend;
    int rc;
...
static int m3_web_tasks_post_delayed(void *tasks, M3TaskFn fn, void *user, m3_u32 delay_ms)
{
    struct M3WebBackend *backend;
    int rc;
...
static const M3TasksVTable g_m3_web_tasks_vtable = {
    m3_web_tasks_thread_create,
    m3_web_tasks_thread_join,
    m3_web_tasks_mutex_create,
    m3_web_tasks_mutex_destroy,
    m3_web_tasks_mutex_lock,
    m3_web_tasks_mutex_unlock,
    m3_web_tasks_sleep_ms,
    m3_web_tasks_post,
...
static const M3EnvVTable g_m3_web_env_vtable = {
    m3_web_env_get_io,
    m3_web_env_get_sensors,
    m3_web_env_get_camera,
    m3_web_env_get_network,
    m3_web_env_get_tasks,
    m3_web_env_get_time_ms
};

int M3_CALL m3_web_backend_create(const M3WebBackendConfig *config, M3WebBackend **out_backend)
{
    ...
    *out_backend = NULL;
    ...
    rc = m3_web_backend_copy_string(&allocator, config->utf8_canvas_id, &backend->canvas_id);
    if (rc != M3_OK) {
